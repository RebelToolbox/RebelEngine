...
void GridMapEditor::_menu_option(int p_option) {
    switch (p_option) {
        ...
        case MENU_OPTION_LOCK_VIEW: {
            int index =
                options->get_popup()->get_item_index(MENU_OPTION_LOCK_VIEW);
        ...
        case MENU_OPTION_CLIP_BELOW: {
            ...
            for (int i = 0; i < 3; i++) {
                int index = options->get_popup()->get_item_index(
                    MENU_OPTION_CLIP_DISABLED + i
                );
        ...
        case MENU_OPTION_Z_AXIS: {
            ...
            for (int i = 0; i < 3; i++) {
                int idx = options->get_popup()->get_item_index(
                    MENU_OPTION_X_AXIS + i
                );
            ...
            if (edit_axis != new_axis) {
                int item1 =
                    options->get_popup()->get_item_index(MENU_OPTION_NEXT_LEVEL
                    );
                int item2 =
                    options->get_popup()->get_item_index(MENU_OPTION_PREV_LEVEL
                    );
                if (edit_axis == Vector3::AXIS_Y) {
                    options->get_popup()->set_item_text(
                        item1,
                        TTR("Next Plane")
                    );
                    options->get_popup()->set_item_text(
        ...
        case MENU_OPTION_CURSOR_ROTATE_Y: {
            ...
            if (input_action == INPUT_PASTE) {
                ...
                r.rotate(Vector3(0, 1, 0), -static_cast<real_t>(Math_PI) / 2);
        ...
        case MENU_OPTION_CURSOR_ROTATE_X: {
            ...
            if (input_action == INPUT_PASTE) {
                r.set_orthogonal_index(paste_indicator.orientation);
                r.rotate(Vector3(1, 0, 0), -static_cast<real_t>(Math_PI) / 2);
                paste_indicator.orientation = r.get_orthogonal_index();
                _update_paste_indicator();
                break;
            }
        ...
        case MENU_OPTION_CURSOR_ROTATE_Z: {
            ...
            if (input_action == INPUT_PASTE) {
                r.set_orthogonal_index(paste_indicator.orientation);
                r.rotate(Vector3(0, 0, 1), -static_cast<real_t>(Math_PI) / 2);
                paste_indicator.orientation = r.get_orthogonal_index();
                _update_paste_indicator();
                break;
            }
        ...
        case MENU_OPTION_PASTE_SELECTS: {
            int idx =
                options->get_popup()->get_item_index(MENU_OPTION_PASTE_SELECTS);
            options->get_popup()->set_item_checked(
                idx,
                !options->get_popup()->is_item_checked(idx)
            );
        } break;
        ...
        case MENU_OPTION_GRIDMAP_SETTINGS: {
            settings_dialog->popup_centered(
                settings_vbc->get_combined_minimum_size()
                + Size2(50, 50) * EDSCALE
            );
        } break;
    }
...
void GridMapEditor::_update_cursor_transform() {
    ...
    if (selected_palette >= 0) {
        if (node && !node->get_mesh_library().is_null()) {
            cursor_transform *=
                node->get_mesh_library()->get_item_mesh_transform(
                    selected_palette
                );
        }
    }
...
void GridMapEditor::_update_selection_transform() {
    ...
    Transform xf;
    xf.scale(
        (Vector3(1, 1, 1) + (selection.end - selection.begin))
        * node->get_cell_size()
    );
    ...
    VisualServer::get_singleton()->instance_set_transform(
        selection_instance,
        node->get_global_transform() * xf
    );

    for (int i = 0; i < 3; i++) {
        if (i != edit_axis
            || (edit_floor[edit_axis] < selection.begin[edit_axis])
...
void GridMapEditor::_set_selection(
    bool p_active,
    ...
    options->get_popup()->set_item_disabled(
        options->get_popup()->get_item_index(MENU_OPTION_SELECTION_CLEAR),
        !selection.active
    );
    options->get_popup()->set_item_disabled(
        options->get_popup()->get_item_index(MENU_OPTION_SELECTION_CUT),
        !selection.active
    );
    options->get_popup()->set_item_disabled(
        options->get_popup()->get_item_index(MENU_OPTION_SELECTION_DUPLICATE),
        !selection.active
    );
    options->get_popup()->set_item_disabled(
        options->get_popup()->get_item_index(MENU_OPTION_SELECTION_FILL),
        !selection.active
    );
}

bool GridMapEditor::do_input_action(
    Camera* p_camera,
    ...
    if (selected_palette < 0 && input_action != INPUT_PICK
        && input_action != INPUT_SELECT && input_action != INPUT_PASTE) {
        return false;
    }
    ...
    if (input_action != INPUT_PICK && input_action != INPUT_SELECT
        && input_action != INPUT_PASTE
        && !mesh_library->has_item(selected_palette)) {
        return false;
    }
    ...
    Vector3 inters;
    if (!p.intersects_segment(
            from,
            from + normal * settings_pick_distance->get_value(),
            &inters
        )) {
        return false;
    }
    ...
    int cell[3];
    float cell_size[3] = {
        node->get_cell_size().x,
        node->get_cell_size().y,
        node->get_cell_size().z
    };
    ...
    VS::get_singleton()->instance_set_transform(
        grid_instance[edit_axis],
        node->get_global_transform() * edit_grid_xform
    );

    if (cursor_instance.is_valid()) {
        cursor_origin = (Vector3(cell[0], cell[1], cell[2])
                         + Vector3(
                             0.5 * node->get_center_x(),
                             0.5 * node->get_center_y(),
                             0.5 * node->get_center_z()
                         ))
                      * node->get_cell_size();
        cursor_visible = true;
        ...
        for (int j = selection.begin.y; j <= selection.end.y; j++) {
            for (int k = selection.begin.z; k <= selection.end.z; k++) {
                ...
                undo_redo->add_undo_method(
                    ...
                    j,
                    k,
                    node->get_cell_item(i, j, k),
                    node->get_cell_item_orientation(i, j, k)
                );
            }
        }
    }
...
void GridMapEditor::_update_paste_indicator() {
    ...
    Vector3 center = 0.5
                   * Vector3(
                         float(node->get_center_x()),
                         float(node->get_center_y()),
                         float(node->get_center_z())
                   );
    Vector3 scale =
        (Vector3(1, 1, 1) + (paste_indicator.end - paste_indicator.begin))
        * node->get_cell_size();
    Transform xf;
    xf.scale(scale);
    xf.origin = (paste_indicator.begin
                 + (paste_indicator.current - paste_indicator.click) + center)
              * node->get_cell_size();
    Basis rot;
...
void GridMapEditor::_do_paste() {
    ...
    for (List<ClipboardItem>::Element* E = clipboard_items.front(); E;
        ...
        undo_redo->add_undo_method(
            ...
            pos.y,
            pos.z,
            node->get_cell_item(pos.x, pos.y, pos.z),
            node->get_cell_item_orientation(pos.x, pos.y, pos.z)
        );
    }
    ...
    if (reselect) {
        undo_redo->add_do_method(
            ...
            "_set_selection",
            true,
            paste_indicator.begin + ofs,
            paste_indicator.end + ofs
        );
        undo_redo->add_undo_method(
            this,
            "_set_selection",
...
bool GridMapEditor::forward_spatial_input_event(
    Camera* p_camera,
    ...
    if (mb.is_valid()) {
        ...
        if (mb->is_pressed()) {
            SpatialEditorViewport::NavigationScheme nav_scheme =
            ...
            if (mb->get_button_index() == BUTTON_LEFT
                ...
                undo_redo->add_undo_method(
                    this,
                    "_set_selection",
                    last_selection.active,
                    last_selection.begin,
                    last_selection.end
                );
                undo_redo->commit_action();
            }
            ...
            if (mb->get_button_index() == BUTTON_RIGHT
                && (input_action == INPUT_ERASE || input_action == INPUT_PASTE
                )) {
                input_action = INPUT_NONE;
                return true;
            }
    ...
    if (k.is_valid()) {
        if (k->is_pressed()) {
            ...
            if (k->get_shift() && selection.active
                && input_action != INPUT_PASTE) {
                if (k->get_scancode()
                    == options->get_popup()->get_item_accelerator(
                        options->get_popup()->get_item_index(
...
void GridMapEditor::update_palette() {
    ...
    if (display_mode == DISPLAY_THUMBNAIL) {
        ...
        mesh_library_palette->set_fixed_column_width(
            min_size * MAX(size_slider->get_value(), 1.5)
        );
...
void GridMapEditor::edit(GridMap* p_gridmap) {
    ...
    clip_mode =
        p_gridmap->has_meta("_editor_clip_")
            ? ClipMode(p_gridmap->get_meta("_editor_clip_").operator int())
            : CLIP_DISABLED;
...
void GridMapEditor::_update_clip() {
    ...
    if (clip_mode == CLIP_DISABLED) {
        ...
        node->set_clip(
            true,
            clip_mode == CLIP_ABOVE,
            edit_floor[edit_axis],
            edit_axis
        );
    }
}
...
void GridMapEditor::_draw_grids(const Vector3& cell_size) {
    Vector3 edited_floor = node->has_meta("_editor_floor_")
                             ? node->get_meta("_editor_floor_")
                             : Variant();
    ...
    for (int i = 0; i < 3; i++) {
        ...
        VisualServer::get_singleton()
            ->mesh_surface_set_material(grid[i], 0, indicator_mat->get_rid());
    }
}
...
void GridMapEditor::_notification(int p_what) {
    switch (p_what) {
        case NOTIFICATION_ENTER_TREE: {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                VS::get_singleton()->instance_set_layer_mask(
                    grid_instance[i],
                    1 << SpatialEditorViewport::MISC_TOOL_LAYER
                );
                selection_level_instance[i] =
            ...
            paste_instance = VisualServer::get_singleton()->instance_create2(
                paste_mesh,
                get_tree()->get_root()->get_world()->get_scenario()
            );
            VS::get_singleton()->instance_set_layer_mask(
        ...
        case NOTIFICATION_PROCESS: {
            ...
            if (lock_view) {
                EditorNode* editor = Object::cast_to<EditorNode>(
        ...
        case NOTIFICATION_WM_FOCUS_OUT: {
            if (input_action == INPUT_PAINT) {
                // Simulate mouse released event to stop drawing when editor
                // focus exists.
...
void GridMapEditor::_update_cursor_instance() {
    ...
    if (selected_palette >= 0) {
        if (node && !node->get_mesh_library().is_null()) {
            Ref<Mesh> mesh =
                node->get_mesh_library()->get_item_mesh(selected_palette);
...
void GridMapEditor::_floor_changed(float p_value) {
    ...
    node->set_meta(
        "_editor_floor_",
        Vector3(edit_floor[0], edit_floor[1], edit_floor[2])
    );
    update_grid();
    _update_clip();
    _update_selection_transform();
...
void GridMapEditor::_bind_methods() {
    ...
    ClassDB::bind_method(
        ...
        &GridMapEditor::_mesh_library_palette_input
    );
    ClassDB::bind_method(
        "_icon_size_changed",
        &GridMapEditor::_icon_size_changed
    );
    ClassDB::bind_method("_menu_option", &GridMapEditor::_menu_option);
    ClassDB::bind_method("_configure", &GridMapEditor::_configure);
    ClassDB::bind_method(
        "_item_selected_cbk",
        &GridMapEditor::_item_selected_cbk
    );
    ...
    ClassDB::bind_method(
        D_METHOD("_set_display_mode", "mode"),
        &GridMapEditor::_set_display_mode
    );
...
GridMapEditor::GridMapEditor(EditorNode* p_editor) {
    ...
    floor->get_line_edit()
        ->connect("mouse_exited", this, "_floor_mouse_exited");
    ...
    options->get_popup()
        ->add_item(TTR("Previous Floor"), MENU_OPTION_PREV_LEVEL, KEY_Q);
    options->get_popup()
        ->add_item(TTR("Next Floor"), MENU_OPTION_NEXT_LEVEL, KEY_E);
    options->get_popup()->add_separator();
    options->get_popup()->add_radio_check_item(
        TTR("Clip Disabled"),
        MENU_OPTION_CLIP_DISABLED
    );
    options->get_popup()->set_item_checked(
        options->get_popup()->get_item_index(MENU_OPTION_CLIP_DISABLED),
        true
    );
    options->get_popup()->add_radio_check_item(
        TTR("Clip Above"),
    ...
    options->get_popup()->add_separator();
    options->get_popup()
        ->add_radio_check_item(TTR("Edit X Axis"), MENU_OPTION_X_AXIS, KEY_Z);
    options->get_popup()
        ->add_radio_check_item(TTR("Edit Y Axis"), MENU_OPTION_Y_AXIS, KEY_X);
    options->get_popup()
        ->add_radio_check_item(TTR("Edit Z Axis"), MENU_OPTION_Z_AXIS, KEY_C);
    options->get_popup()->set_item_checked(
        options->get_popup()->get_item_index(MENU_OPTION_Y_AXIS),
        true
    );
    options->get_popup()->add_separator();
    options->get_popup()
        ->add_item(TTR("Cursor Rotate X"), MENU_OPTION_CURSOR_ROTATE_X, KEY_A);
    options->get_popup()
        ->add_item(TTR("Cursor Rotate Y"), MENU_OPTION_CURSOR_ROTATE_Y, KEY_S);
    options->get_popup()
        ->add_item(TTR("Cursor Rotate Z"), MENU_OPTION_CURSOR_ROTATE_Z, KEY_D);
    options->get_popup()->add_item(
        TTR("Cursor Back Rotate X"),
        MENU_OPTION_CURSOR_BACK_ROTATE_X,
        KEY_MASK_SHIFT + KEY_A
    );
    options->get_popup()->add_item(
        TTR("Cursor Back Rotate Y"),
        MENU_OPTION_CURSOR_BACK_ROTATE_Y,
        KEY_MASK_SHIFT + KEY_S
    );
    options->get_popup()->add_item(
        TTR("Cursor Back Rotate Z"),
        MENU_OPTION_CURSOR_BACK_ROTATE_Z,
        KEY_MASK_SHIFT + KEY_D
    );
    options->get_popup()->add_item(
        TTR("Cursor Clear Rotation"),
    ...
    mode_thumbnail->set_pressed(true);
    mode_thumbnail->set_icon(
        p_editor->get_gui_base()->get_icon("FileThumbnail", "EditorIcons")
    );
    hb->add_child(mode_thumbnail);
    mode_thumbnail->connect(
        "pressed",
        this,
        "_set_display_mode",
        varray(DISPLAY_THUMBNAIL)
    );
    ...
    mode_list->set_pressed(false);
    mode_list->set_icon(
        p_editor->get_gui_base()->get_icon("FileList", "EditorIcons")
    );
    hb->add_child(mode_list);
    mode_list
        ->connect("pressed", this, "_set_display_mode", varray(DISPLAY_LIST));
    ...
    mesh_library_palette
        ->connect("gui_input", this, "_mesh_library_palette_input");

    info_message = memnew(Label);
    info_message->set_text(
        TTR("Give a MeshLibrary resource to this GridMap to use its meshes.")
    );
...

    {
        ...
        selection_floor_mat->set_albedo(Color(0.8f, 0.8f, 1, 1));
        ...
        VisualServer::get_singleton()
            ->mesh_surface_set_material(paste_mesh, 0, inner_mat->get_rid());

        d[VS::ARRAY_VERTEX] = lines;
        VisualServer::get_singleton()
            ->mesh_add_surface_from_arrays(paste_mesh, VS::PRIMITIVE_LINES, d);
        VisualServer::get_singleton()
