...
#ifdef DEBUG_ENABLED
#define __ASSERT_SCRIPT_REASON                                                 \
    "Cannot retrieve PluginScript class for this script, is your code "        \
    "correct?"
...
PluginScriptInstance* PluginScript::_create_instance(
    ...
    // There is currently no way to get the constructor function name of the
    // script. instance->call("__init__", p_args, p_argcount, r_error);
    if (p_argcount > 0) {
...
Variant PluginScript::_new(
    ...
    PluginScriptInstance* instance =
        _create_instance(p_args, p_argcount, owner, r_error);
...
void PluginScript::update_exports() {
    ...
    if (placeholders.size()) {
        ...
        for (Set<PlaceHolderScriptInstance*>::Element* E = placeholders.front();
             E;
             E = E->next()) {
...
ScriptInstance* PluginScript::instance_create(Object* p_this) {
    ...
    if (!_tool && !ScriptServer::is_scripting_enabled()) {
        ...
        PlaceHolderScriptInstance* si = memnew(
            PlaceHolderScriptInstance(get_language(), Ref<Script>(this), p_this)
        );
    ...
    if (base_type) {
        if (!ClassDB::is_parent_class(p_this->get_class_name(), base_type)) {
            String msg = "Script inherits from native type '"
                       + String(base_type)
...
#define FREE_SCRIPT_MANIFEST(manifest)                                         \
    ...
    if (err) {
        ...
        // TODO: GDscript uses `ScriptDebugger` here to jump into the parsing
        // error
        return err;
    }
    ...
    if (*base_name) {
        if (ClassDB::class_exists(*base_name)) {
            ...
            if (res.is_valid()) {
                ...
                ERR_FAIL_V_MSG(
                    ERR_PARSE_ERROR,
                    _path + ": Script '" + name + "' has an invalid parent '"
                        + *base_name + "'."
                );
            }
        }
...
void PluginScript::get_script_method_list(List<MethodInfo>* r_methods) const {
    ASSERT_SCRIPT_VALID();
    for (Map<StringName, MethodInfo>::Element* e = _methods_info.front();
         e != nullptr;
         e = e->next()) {
...
void PluginScript::get_script_property_list(List<PropertyInfo>* r_properties
) const {
    ASSERT_SCRIPT_VALID();
    for (Map<StringName, PropertyInfo>::Element* e = _properties_info.front();
         e != nullptr;
         e = e->next()) {
...
MethodInfo PluginScript::get_method_info(const StringName& p_method) const {
    ...
    const Map<StringName, MethodInfo>::Element* e =
        _methods_info.find(p_method);
    if (e != nullptr) {
        return e->get();
    } else {
        return MethodInfo();
...
PropertyInfo PluginScript::get_property_info(const StringName& p_property
) const {
    ASSERT_SCRIPT_VALID_V(PropertyInfo());
    const Map<StringName, PropertyInfo>::Element* e =
        _properties_info.find(p_property);
    if (e != nullptr) {
        return e->get();
    } else {
        return PropertyInfo();
...
bool PluginScript::get_property_default_value(
    ...
    const Map<StringName, Variant>::Element* e =
        _properties_default_values.find(p_property);
    if (e != nullptr) {
        r_value = e->get();
        return true;
    } else {
...
void PluginScript::get_script_signal_list(List<MethodInfo>* r_signals) const {
    ASSERT_SCRIPT_VALID();
    for (Map<StringName, MethodInfo>::Element* e = _signals_info.front();
         e != nullptr;
         e = e->next()) {
...
MultiplayerAPI::RPCMode PluginScript::get_rpc_mode(const StringName& p_method
) const {
    ASSERT_SCRIPT_VALID_V(MultiplayerAPI::RPC_MODE_DISABLED);
    const Map<StringName, MultiplayerAPI::RPCMode>::Element* e =
        _methods_rpc_mode.find(p_method);
    if (e != nullptr) {
        return e->get();
    } else {
...
MultiplayerAPI::RPCMode PluginScript::get_rset_mode(const StringName& p_variable
) const {
    ASSERT_SCRIPT_VALID_V(MultiplayerAPI::RPC_MODE_DISABLED);
    const Map<StringName, MultiplayerAPI::RPCMode>::Element* e =
        _variables_rset_mode.find(p_variable);
    if (e != nullptr) {
        return e->get();
    } else {
        return MultiplayerAPI::RPC_MODE_DISABLED;
