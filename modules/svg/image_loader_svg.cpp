...
inline void change_nsvg_paint_color(
    NSVGpaint* p_paint,
    const uint32_t p_old,
    const uint32_t p_new
) {
    if (p_paint->type == NSVG_PAINT_COLOR) {
        if (p_paint->color << 8 == p_old << 8) {
            p_paint->color =
                (p_paint->color & 0xFF000000) | (p_new & 0x00FFFFFF);
        }
    }

    if (p_paint->type == NSVG_PAINT_LINEAR_GRADIENT
        || p_paint->type == NSVG_PAINT_RADIAL_GRADIENT) {
        for (int stop_index = 0; stop_index < p_paint->gradient->nstops;
             stop_index++) {
    ...
    for (NSVGshape* shape = p_svg_image->shapes; shape != nullptr;
        ...
        for (int i = 0; i < replace_colors.old_colors.size(); i++) {
            change_nsvg_paint_color(
                &(shape->stroke),
                replace_colors.old_colors[i],
                replace_colors.new_colors[i]
            );
            change_nsvg_paint_color(
                &(shape->fill),
                replace_colors.old_colors[i],
...
Error ImageLoaderSVG::_create_image(
    Ref<Image> p_image,
    ...
    ERR_FAIL_COND_V_MSG(
        ...
        vformat(
            "Can't create image from SVG with scale %s, the resulting image "
            "size exceeds max width.",
            rtos(p_scale)
        )
    );
...
Error ImageLoaderSVG::create_image_from_string(
    Ref<Image> p_image,
...
Error ImageLoaderSVG::load_image(
    Ref<Image> p_image,
    FileAccess* f,
    bool p_force_linear,
    float p_scale
...
void ImageLoaderSVG::get_recognized_extensions(List<String>* p_extensions
) const {
