...
void GDScriptTextDocument::_bind_methods() {
    ...
    ClassDB::bind_method(
        ...
        &GDScriptTextDocument::nativeSymbol
    );
    ClassDB::bind_method(
        D_METHOD("documentSymbol"),
        &GDScriptTextDocument::documentSymbol
    );
    ClassDB::bind_method(
        D_METHOD("completion"),
        &GDScriptTextDocument::completion
    );
    ...
    ClassDB::bind_method(
        ...
        &GDScriptTextDocument::documentLink
    );
    ClassDB::bind_method(
    ...
    ClassDB::bind_method(
        D_METHOD("definition"),
        &GDScriptTextDocument::definition
    );
    ClassDB::bind_method(
        D_METHOD("declaration"),
        &GDScriptTextDocument::declaration
    );
    ClassDB::bind_method(
        D_METHOD("signatureHelp"),
        &GDScriptTextDocument::signatureHelp
    );
    ClassDB::bind_method(
        D_METHOD("show_native_symbol_in_editor"),
        &GDScriptTextDocument::show_native_symbol_in_editor
    );
}
...
lsp::TextDocumentItem GDScriptTextDocument::load_document_item(
...
Array GDScriptTextDocument::documentSymbol(const Dictionary& p_params) {
    ...
    String path       = GDScriptLanguageProtocol::get_singleton()
                      ->get_workspace()
                      ->get_file_path(uri);
    Array arr;
    if (const Map<String, ExtendGDScriptParser*>::Element* parser =
            GDScriptLanguageProtocol::get_singleton()
                ->get_workspace()
                ->scripts.find(path)) {
...
Array GDScriptTextDocument::completion(const Dictionary& p_params) {
    ...
    GDScriptLanguageProtocol::get_singleton()->get_workspace()->completion(
        params,
        &options
    );
    ...
    if (!options.empty()) {
        ...
        for (Map<String, ExtendGDScriptParser*>::Element* E =
                 GDScriptLanguageProtocol::get_singleton()
                     ->get_workspace()
                     ->scripts.front();
             E;
...
Dictionary GDScriptTextDocument::rename(const Dictionary& p_params) {
    ...
    return GDScriptLanguageProtocol::get_singleton()->get_workspace()->rename(
        params,
        new_name
    );
}
...
Dictionary GDScriptTextDocument::resolve(const Dictionary& p_params) {
    ...
    if (data.get_type() == Variant::DICTIONARY) {
        ...
        symbol = GDScriptLanguageProtocol::get_singleton()
                     ->get_workspace()
                     ->resolve_symbol(
                         params,
                         item.label,
        ...
        if (param_symbols.size() >= 2) {
            ...
            if (!symbol) {
                if (const Map<String, ExtendGDScriptParser*>::Element* E =
                        GDScriptLanguageProtocol::get_singleton()
                            ->get_workspace()
...
Variant GDScriptTextDocument::declaration(const Dictionary& p_params) {
    ...
    if (arr.empty() && !symbols.empty()
        ...
        if (GDScriptLanguageProtocol::get_singleton()
...
void GDScriptTextDocument::sync_script_content(
    const String& p_path,
    ...
    GDScriptLanguageProtocol::get_singleton()->get_workspace()->parse_script(
        path,
        p_content
    );
...
void GDScriptTextDocument::show_native_symbol_in_editor(
...
Array GDScriptTextDocument::find_symbols(
    ...
    if (symbol) {
        ...
        const String& path = GDScriptLanguageProtocol::get_singleton()
                                 ->get_workspace()
