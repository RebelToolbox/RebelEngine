...
btTransform CollisionObjectBullet::ShapeWrapper::get_adjusted_transform(
) const {
    if (shape->get_type() == PhysicsServer::SHAPE_HEIGHTMAP) {
        const HeightMapShapeBullet* hm_shape =
            (const HeightMapShapeBullet*)shape; // should be safe to cast now
        ...
        adjusted_transform.setOrigin(btVector3(
            0.0,
            hm_shape->min_height
                + ((hm_shape->max_height - hm_shape->min_height) * 0.5),
            0.0
        ));
...
void CollisionObjectBullet::ShapeWrapper::claim_bt_shape(
    const btVector3& body_scale
) {
    if (!bt_shape) {
        if (active) {
...
void CollisionObjectBullet::set_body_scale(const Vector3& p_new_scale) {
    if (!equal(p_new_scale[0], body_scale[0])
        || !equal(p_new_scale[1], body_scale[1])
        || !equal(p_new_scale[2], body_scale[2])) {
...
void CollisionObjectBullet::setupBulletCollisionObject(
    btCollisionObject* p_collisionObject
) {
    ...
    p_collisionObject->setCollisionFlags(
        p_collisionObject->getCollisionFlags()
        | btCollisionObject::CF_CUSTOM_MATERIAL_CALLBACK
    );
}

void CollisionObjectBullet::add_collision_exception(
    ...
    bt_collision_object->setIgnoreCollisionCheck(
        p_ignoreCollisionObject->bt_collision_object,
        true
    );
    if (space) {
        space->get_broadphase()->getOverlappingPairCache()->cleanProxyFromPairs(
            bt_collision_object->getBroadphaseHandle(),
            space->get_dispatcher()
        );
    }
}

void CollisionObjectBullet::remove_collision_exception(
    const CollisionObjectBullet* p_ignoreCollisionObject
) {
...
bool CollisionObjectBullet::has_collision_exception(
...
void CollisionObjectBullet::set_collision_enabled(bool p_enabled) {
    ...
    if (collisionsEnabled) {
        bt_collision_object->setCollisionFlags(
            bt_collision_object->getCollisionFlags()
...
void CollisionObjectBullet::set_transform__bullet(
...
void RigidCollisionObjectBullet::add_shape(
    ShapeBullet* p_shape,
    const Transform& p_transform,
    bool p_disabled
) {
...
void RigidCollisionObjectBullet::remove_all_shapes(
    bool p_permanentlyFromThisBody,
    bool p_force_not_reload
) {
    // Reverse order required for delete.
    for (int i = shapes.size() - 1; 0 <= i; --i) {
...
void RigidCollisionObjectBullet::set_shape_transform(
    int p_index,
    const Transform& p_transform
) {
    ERR_FAIL_INDEX(p_index, get_shape_count());
...
const btTransform& RigidCollisionObjectBullet::get_bt_shape_transform(
    int p_index
) const {
...
void RigidCollisionObjectBullet::set_shape_disabled(
    int p_index,
    bool p_disabled
) {
...
void RigidCollisionObjectBullet::reload_shapes() {
    ...
    btCompoundShape* compoundShape =
        bulletnew(btCompoundShape(enableDynamicAabbTree, shape_count));
...
void RigidCollisionObjectBullet::internal_shape_destroy(
    int p_index,
    bool p_permanentlyFromThisBody
) {
    ShapeWrapper& shp = shapes.write[p_index];
