...
void ShapeBullet::remove_owner(
    ShapeOwnerBullet* p_owner,
    bool p_permanentlyFromThisBody
) {
    Map<ShapeOwnerBullet*, int>::Element* E = owners.find(p_owner);
    if (!E) {
        return;
...
btStaticPlaneShape* ShapeBullet::create_shape_plane(
...
btCapsuleShapeZ* ShapeBullet::create_shape_capsule(
    btScalar radius,
    btScalar height
) {
...
btCylinderShape* ShapeBullet::create_shape_cylinder(
    btScalar radius,
    btScalar height
) {
...
btConvexPointCloudShape* ShapeBullet::create_shape_convex(
    btAlignedObjectArray<btVector3>& p_vertices,
    const btVector3& p_local_scaling
) {
    return bulletnew(btConvexPointCloudShape(
        &p_vertices[0],
        p_vertices.size(),
        p_local_scaling
    ));
...
btScaledBvhTriangleMeshShape* ShapeBullet::create_shape_concave(
    btBvhTriangleMeshShape* p_mesh_shape,
    const btVector3& p_local_scaling
) {
    if (p_mesh_shape) {
        return bulletnew(
            btScaledBvhTriangleMeshShape(p_mesh_shape, p_local_scaling)
        );
    } else {
        return nullptr;
    }
...
btHeightfieldTerrainShape* ShapeBullet::create_shape_height_field(
    ...
    // The shape can be created without params when you do
    // PhysicsServer.shape_create(PhysicsServer.SHAPE_HEIGHTMAP)
    if (heightsPtr) {
...
btRayShape* ShapeBullet::create_shape_ray(
    real_t p_length,
...
PlaneShapeBullet::PlaneShapeBullet() : ShapeBullet() {}
...
SphereShapeBullet::SphereShapeBullet() : ShapeBullet() {}
...
btCollisionShape* SphereShapeBullet::create_bt_shape(
    ...
    return prepare(ShapeBullet::create_shape_sphere(
        radius * p_implicit_scale[0] + p_extra_edge
    ));
}

/* Box */
BoxShapeBullet::BoxShapeBullet() : ShapeBullet() {}
...
btCollisionShape* BoxShapeBullet::create_bt_shape(
    ...
    return prepare(ShapeBullet::create_shape_box(
        (half_extents * p_implicit_scale)
        + btVector3(p_extra_edge, p_extra_edge, p_extra_edge)
    ));
}

/* Capsule */

CapsuleShapeBullet::CapsuleShapeBullet() : ShapeBullet() {}
...
btCollisionShape* CapsuleShapeBullet::create_bt_shape(
    ...
    return prepare(ShapeBullet::create_shape_capsule(
        radius * p_implicit_scale[0] + p_extra_edge,
        height * p_implicit_scale[1] + p_extra_edge
    ));
}

/* Cylinder */

CylinderShapeBullet::CylinderShapeBullet() : ShapeBullet() {}
...
btCollisionShape* CylinderShapeBullet::create_bt_shape(
    const btVector3& p_implicit_scale,
    real_t p_margin
) {
    return prepare(ShapeBullet::create_shape_cylinder(
        radius * p_implicit_scale[0] + p_margin,
        height * p_implicit_scale[1] + p_margin
    ));
}

/* Convex polygon */

ConvexPolygonShapeBullet::ConvexPolygonShapeBullet() : ShapeBullet() {}
...
Variant ConvexPolygonShapeBullet::get_data() const {
    ConvexPolygonShapeBullet* variable_self =
        const_cast<ConvexPolygonShapeBullet*>(this);
...
void ConcavePolygonShapeBullet::setup(PoolVector<Vector3> p_faces) {
    ...
    if (0 < src_face_count) {
        // It counts the faces and assert the array contains the correct number
        // of vertices.
        ...
        meshShape = bulletnew(
            btBvhTriangleMeshShape(shapeInterface, useQuantizedAabbCompression)
        );
...
btCollisionShape* ConcavePolygonShapeBullet::create_bt_shape(
    ...
    if (!cs) {
        // This is necessary since if 0 faces the creation of concave return
        // NULL
...
HeightMapShapeBullet::HeightMapShapeBullet() : ShapeBullet() {}
...
void HeightMapShapeBullet::set_data(const Variant& p_data) {
    ...
    // because that precision is unnecessary for a heightmap and Bullet doesn't
    // support it...
    ...
    if (l_heights_v.get_type() == Variant::POOL_REAL_ARRAY) {
        ...
        // If an image is passed, we have to convert it to a format Bullet
        // supports. this would be expensive to do with a script, so it's nice
        // to have it here.
...
void HeightMapShapeBullet::setup(
    PoolVector<real_t>& p_heights,
    int p_width,
    int p_depth,
    real_t p_min_height,
    ...
    // TODO cell size must be tweaked using localScaling, which is a shared
    // property for all Bullet shapes

    // If this array is resized outside of here, it should be preserved due to
    // CoW
    heights = p_heights;
...
btCollisionShape* HeightMapShapeBullet::create_bt_shape(
    ...
    btCollisionShape* cs(ShapeBullet::create_shape_height_field(
        heights,
        width,
        depth,
        min_height,
...
btCollisionShape* RayShapeBullet::create_bt_shape(
    ...
    return prepare(ShapeBullet::create_shape_ray(
        length * p_implicit_scale[1] + p_extra_edge,
        slips_on_slope
    ));
}
