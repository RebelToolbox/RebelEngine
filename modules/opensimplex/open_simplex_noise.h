...
class OpenSimplexNoise : public Resource {
    ...
    // The maximum number of octaves allowed. Note that these are statically
    // allocated. Higher values become exponentially slower, so this shouldn't
    // be set too high to avoid freezing the editor for long periods of time.
    static const int MAX_OCTAVES = 9;

    osn_context contexts[MAX_OCTAVES];

    int seed;
    float persistence; // Controls details, value in [0,1]. Higher increases
                       // grain, lower increases smoothness.
    int octaves;       // Number of noise layers
    float period;      // Distance above which we start to see similarities. The
                       // higher, the longer "hills" will be on a terrain.
    float lacunarity;  // Controls period change across octaves. 2 is usually a
                       // good value to address all detail levels.
...
public:
    ...
    Ref<Image> get_image(
        int p_width,
        int p_height,
        const Vector2& p_noise_offset = Vector2()
    ) const;
    ...
    _FORCE_INLINE_ float _get_octave_noise_3d(
        int octave,
        float x,
        float y,
        float z
    ...
    _FORCE_INLINE_ float _get_octave_noise_4d(
        int octave,
        float x,
        float y,
        float z,
    ...
    _FORCE_INLINE_ float get_noise_3dv(const Vector3& v) const {
