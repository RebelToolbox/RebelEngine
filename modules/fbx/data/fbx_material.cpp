...
void FBXMaterial::add_search_string(
    String p_filename,
    String p_current_directory,
    String search_directory,
    Vector<String>& texture_search_paths
) {
    if (search_directory.empty()) {
        texture_search_paths.push_back(
            p_current_directory.get_base_dir().plus_file(p_filename)
        );
    } else {
        texture_search_paths.push_back(
            p_current_directory.get_base_dir().plus_file(
                search_directory + "/" + p_filename
            )
        );
        texture_search_paths.push_back(
            p_current_directory.get_base_dir().plus_file(
...
// fbx will not give us good path information and let's not regex them to fix
// them no relative paths are in fbx generally they have a rel field but it's
// populated incorrectly by the SDK.
String FBXMaterial::find_texture_path_by_filename(
    const String p_filename,
    const String p_current_directory
) {
    _Directory dir;
    Vector<String> paths;
...
FBXMaterial::MaterialInfo FBXMaterial::extract_material_info(
    const FBXDocParser::Material* material
) const {
    ...
    for (std::pair<std::string, const FBXDocParser::Texture*> texture :
         material->Textures()) {
        ...
        if (fbx_feature_mapping_desc.count(fbx_mapping_name) > 0) {
            ...
            mat_info.features.push_back(
                fbx_feature_mapping_desc.at(fbx_mapping_name)
            );
            continue;
        }
        ...
        const String absoulte_fbx_file_path =
            texture.second->FileName().c_str();
        const String file_extension =
            absoulte_fbx_file_path.get_extension().to_upper();
        ...
        ERR_CONTINUE_MSG(
            ...
            "The FBX file contains a texture with an unrecognized extension: "
                + file_extension_uppercase
        );

        const String texture_name = absoulte_fbx_file_path.get_file();
        print_verbose(
            "Getting FBX mapping mode for " + String(fbx_mapping_name.c_str())
        );
        const SpatialMaterial::TextureParam mapping_mode =
            fbx_texture_mapping_desc.at(fbx_mapping_name);
        print_verbose(
            "Set FBX mapping mode to " + get_texture_param_name(mapping_mode)
        );
        ...
        switch (mapping_mode) {
            ...
            case SpatialMaterial::TextureParam::TEXTURE_DETAIL_NORMAL:
                mat_info.features.push_back(
                    SpatialMaterial::Feature::FEATURE_DETAIL
                );
                break;
        }
    }
...
T extract_from_prop(
    ...
    ERR_FAIL_COND_V_MSG(
        prop == nullptr,
        p_default,
        "invalid property passed to extractor"
    );
    const FBXDocParser::TypedProperty<T>* val =
        dynamic_cast<const FBXDocParser::TypedProperty<T>*>(prop);

    ERR_FAIL_COND_V_MSG(
        val == nullptr,
        p_default,
        "The FBX is corrupted, the property `" + String(p_name.c_str())
            + "` is a `" + String(typeid(*prop).name()) + "` but should be a "
            + p_type
    );
...
Ref<SpatialMaterial> FBXMaterial::import_material(ImportState& state) {
    ...
    print_verbose(
        ...
        + ImportUtils::FBXNodeToName(material->Name())
    );
    ...
    for (FBXDocParser::LazyPropertyMap::value_type iter :
         material->Props()->GetLazyProperties()) {
        ...
        if (desc == PROPERTY_DESC_NOT_FOUND || desc == PROPERTY_DESC_IGNORE) {
            // count the texture mapping references. Skip this one if it's found
            // and we can't look up a property value.
        ...
        if (desc == PROPERTY_DESC_IGNORE) {
            // WARN_PRINT("[Ignored] The FBX material parameter: `" +
            // String(name.c_str()) + "` is ignored.");
            continue;
        } else {
            print_verbose("FBX Material parameter: " + String(name.c_str()));

            // Check for Diffuse material system / lambert materials / legacy
            // basically
            if (name == "Diffuse" && !warning_non_pbr_material) {
                ValidationTracker::get_singleton()->add_validation_error(
                    state.path,
                    "Invalid material settings change to Ai Standard Surface "
                    "shader, mat name: "
                        + material_name.c_escape()
                );
                warning_non_pbr_material = true;
            }
        }

        // DISABLE when adding support for all weird and wonderful material
        // formats
        ...
        ERR_CONTINUE_MSG(
            prop == nullptr,
            "This file may be corrupted because is not possible to extract the "
            "material parameter: "
                + String(name.c_str())
        );
        ...
        const FBXDocParser::TypedProperty<real_t>* real_value =
            dynamic_cast<const FBXDocParser::TypedProperty<real_t>*>(prop);
        const FBXDocParser::TypedProperty<Vector3>* vector_value =
            dynamic_cast<const FBXDocParser::TypedProperty<Vector3>*>(prop);

        if (!real_value && !vector_value) {
            // WARN_PRINT("unsupported datatype in property: " +
            // String(name.c_str()));
            continue;
        }
        ...
        // TODO: implement fields correctly tomorrow so we check 'has x mapping'
        // before 'read x mapping' etc.
        ...
        switch (desc) {
            ...
            case PROPERTY_DESC_TRANSPARENT: {
                if (real_value) {
                    ...
                    if (opacity < (1.0 - CMP_EPSILON)) {
                        ...
                        spatial_material->set_depth_draw_mode(
            ...
            case PROPERTY_DESC_SPECULAR: {
                if (real_value) {
                    print_verbose(
                        "specular real value: " + rtos(real_value->Value())
                    );
                ...
                if (vector_value) {
                    print_error(
                        "unsupported specular vector value: "
                        + vector_value->Value()
                    );
                }
            } break;
            ...
            case PROPERTY_DESC_SPECULAR_COLOR: {
                if (vector_value) {
                    print_error(
                        "unsupported specular color: " + vector_value->Value()
                    );
                }
            } break;
            case PROPERTY_DESC_SHINYNESS: {
                if (real_value) {
                    print_error(
                        "unsupported shinyness:" + rtos(real_value->Value())
                    );
                }
            } break;
            case PROPERTY_DESC_METALLIC: {
                if (real_value) {
                    print_verbose(
                        "metallic real value: " + rtos(real_value->Value())
                    );
                    spatial_material->set_metallic(
                        MIN(1.0f, real_value->Value())
                    );
                } else {
            ...
            case PROPERTY_DESC_ROUGHNESS: {
                if (real_value) {
                    print_verbose(
                        "roughness real value: " + rtos(real_value->Value())
                    );
                    spatial_material->set_roughness(
                        MIN(1.0f, real_value->Value())
                    );
                } else {
            ...
            case PROPERTY_DESC_COAT: {
                if (real_value) {
                    ...
                    print_verbose(
                        "clearcoat real value: " + rtos(real_value->Value())
                    );
                    spatial_material->set_clearcoat(
                        MIN(1.0f, real_value->Value())
                    );
                } else {
            ...
            case PROPERTY_DESC_COAT_ROUGHNESS: {
                // meaning is that approx equal to zero is disabled not actually
                // zero. ;)
                if (real_value
                    && Math::is_equal_approx(real_value->Value(), 0.0f)) {
                    print_verbose(
                        "clearcoat real value: " + rtos(real_value->Value())
                    );
                    spatial_material->set_clearcoat_gloss(
                        1.0 - real_value->Value()
                    );
            ...
            case PROPERTY_DESC_EMISSIVE: {
                if (real_value
                    && Math::is_equal_approx(real_value->Value(), 0.0f)) {
                    print_verbose(
                        "Emissive real value: " + rtos(real_value->Value())
                    );
            ...
            case PROPERTY_DESC_EMISSIVE_COLOR: {
                if (vector_value
                    && !vector_value->Value().is_equal_approx(Vector3(0, 0, 0)
                    )) {
                    const Vector3& color = vector_value->Value();
                    Color c;
                    c[0] = color[0];
    ...
    for (int x = 0; x < material_info.textures.size(); x++) {
        ...
        if (state.cached_image_searches.has(mapping.name)) {
            ...
            String path = find_texture_path_by_filename(
                mapping.name,
                p_fbx_current_directory
            );
            if (!path.empty()) {
                Error err;
                Ref<Texture> image_texture =
                    ResourceLoader::load(path, "Texture", false, &err);

                ERR_CONTINUE_MSG(
                    err != OK,
                    "unable to import image file not loaded yet: " + path
                );
                ERR_CONTINUE(
                    image_texture == nullptr || image_texture.is_null()
                );
                ...
                const String extension =
                    mapping.name.get_extension().to_upper();
                if (extension == "PNG") {
                    // The stored file is a PNG.
                    image = Image::_png_mem_loader_func(
                        mapping.texture->Media()->Content(),
                        mapping.texture->Media()->ContentLength()
                    );
                    ERR_CONTINUE_MSG(
                        image.is_valid() == false,
                        "FBX Embedded PNG image load fail."
                    );

                } else if (extension == "JPEG" || extension == "JPG") {
                    // The stored file is a JPEG.
                    image = Image::_jpg_mem_loader_func(
                        mapping.texture->Media()->Content(),
                        mapping.texture->Media()->ContentLength()
                    );
                    ERR_CONTINUE_MSG(
                        image.is_valid() == false,
                        "FBX Embedded JPEG image load fail."
                    );

                } else if (extension == "TGA") {
                    // The stored file is a TGA.
                    image = Image::_tga_mem_loader_func(
                        mapping.texture->Media()->Content(),
                        mapping.texture->Media()->ContentLength()
                    );
                    ERR_CONTINUE_MSG(
                        image.is_valid() == false,
                        "FBX Embedded TGA image load fail."
                    );

                } else if (extension == "WEBP") {
                    // The stored file is a WEBP.
                    image = Image::_webp_mem_loader_func(
                        mapping.texture->Media()->Content(),
                        mapping.texture->Media()->ContentLength()
                    );
                    ERR_CONTINUE_MSG(
                    ...
                    ERR_CONTINUE_MSG(
                        true,
                        "The embedded image with extension: " + extension
                            + " is not yet supported. Open an issue please."
                    );
                }
        ...
        switch (mapping.map_mode) {
            case SpatialMaterial::TextureParam::TEXTURE_METALLIC:
                if (mapping.name.to_lower().find("ser") >= 0) {
                    // SER shader.
                    spatial_material->set_metallic_texture_channel(
                        SpatialMaterial::TextureChannel::TEXTURE_CHANNEL_RED
                    );
                } else {
            ...
            case SpatialMaterial::TextureParam::TEXTURE_ROUGHNESS:
                if (mapping.name.to_lower().find("ser") >= 0) {
                    // SER shader.
                    spatial_material->set_roughness_texture_channel(
                        SpatialMaterial::TextureChannel::TEXTURE_CHANNEL_BLUE
                    );
                } else {
                    // Use grayscale as default.
                    spatial_material->set_roughness_texture_channel(
            ...
            case SpatialMaterial::TextureParam::TEXTURE_AMBIENT_OCCLUSION:
                // Use grayscale as default.
                spatial_material->set_ao_texture_channel(
                    SpatialMaterial::TextureChannel::TEXTURE_CHANNEL_GRAYSCALE
                );
                break;
            case SpatialMaterial::TextureParam::TEXTURE_REFRACTION:
                // Use grayscale as default.
                spatial_material->set_refraction_texture_channel(
                    SpatialMaterial::TextureChannel::TEXTURE_CHANNEL_GRAYSCALE
                );
                break;
            default:
        ...
        print_verbose(
            "Texture mapping mode: " + itos(mapping.map_mode)
            + "  name: " + get_texture_param_name(mapping.map_mode)
        );
