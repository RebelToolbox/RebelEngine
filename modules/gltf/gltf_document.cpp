...
static Transform _arr_to_xform(const Array& p_array) {
    ...
    Transform xform;
    xform.basis.set_axis(
        Vector3::AXIS_X,
        Vector3(p_array[0], p_array[1], p_array[2])
    );
    xform.basis.set_axis(
        Vector3::AXIS_Y,
        Vector3(p_array[4], p_array[5], p_array[6])
    );
    xform.basis.set_axis(
        Vector3::AXIS_Z,
        Vector3(p_array[8], p_array[9], p_array[10])
    );
...
String GLTFDocument::_sanitize_scene_name(
    Ref<GLTFState> state,
    const String& p_name
) {
...
#else
        WARN_PRINT(
            "GLTF: Legacy scene names are not supported without the RegEx "
            "module. Falling back to new names."
        );
#endif // MODULE_REGEX_ENABLED
    }
...
String GLTFDocument::_gen_unique_name(
    Ref<GLTFState> state,
    const String& p_name
) {
...
String GLTFDocument::_sanitize_animation_name(const String& p_name) {
    // Animations disallow the normal node invalid characters as well as  ","
    // and "[" (See animation/animation_player.cpp::add_animation)
...
String GLTFDocument::_gen_unique_animation_name(
    Ref<GLTFState> state,
    const String& p_name
) {
...
String GLTFDocument::_sanitize_bone_name(
    Ref<GLTFState> state,
    const String& p_name
) {
    if (state->use_legacy_names) {
...
#else
        WARN_PRINT(
            "GLTF: Legacy bone names are not supported without the RegEx "
            "module. Falling back to new names."
        );
#endif // MODULE_REGEX_ENABLED
    }
...
Error GLTFDocument::_parse_scenes(Ref<GLTFState> state) {
    ...
    if (state->json.has("scene")) {
        ...
        WARN_PRINT(
            "The load-time scene is not defined in the glTF2 file. Picking the "
            "first scene."
        );
    }
    ...
    if (scenes.size()) {
        ...
        if (s.has("name") && !String(s["name"]).empty()
            && !((String)s["name"]).begins_with("Scene")) {
...
Error GLTFDocument::_parse_nodes(Ref<GLTFState> state) {
    ...
    for (GLTFNodeIndex node_i = 0; node_i < state->nodes.size(); node_i++) {
        for (int j = 0; j < state->nodes[node_i]->children.size(); j++) {
            ...
            ERR_CONTINUE(
                state->nodes[child_i]->parent != -1
            ); // node already has a parent, wtf.
...
Error GLTFDocument::_encode_buffer_glb(
    Ref<GLTFState> state,
    const String& p_path
) {
...
Error GLTFDocument::_encode_buffer_bins(
    Ref<GLTFState> state,
    const String& p_path
) {
    print_verbose("glTF: Total buffers: " + itos(state->buffers.size()));
...
Error GLTFDocument::_parse_buffers(
    Ref<GLTFState> state,
        ...
        if (i == 0 && state->glb_data.size()) {
            ...
            if (buffer.has("uri")) {
                ...
                if (uri.begins_with("data:")) { // Embedded data using base64.
                    // Validate data MIME types and throw an error if it's one
                    // we don't know/support.
                    if (!uri.begins_with("data:application/octet-stream;base64")
                        && !uri.begins_with(
                            "data:application/gltf-buffer;base64"
                        )) {
                        ERR_PRINT(
                            "glTF: Got buffer with an unknown URI data type: "
                            + uri
                        );
                    }
                    ...
                    uri = p_base_path.plus_file(uri).replace(
                        "\\",
                        "/"
                    ); // Fix for Windows.
                    ...
                    ERR_FAIL_COND_V_MSG(
                        buffer.size() == 0,
                        ERR_PARSE_ERROR,
                        "glTF: Couldn't load binary file as an array: " + uri
                    );
                }
...
Error GLTFDocument::_encode_buffer_views(Ref<GLTFState> state) {
    ...
    print_verbose(
        "glTF: Total buffer views: " + itos(state->buffer_views.size())
    );
    if (!buffers.size()) {
        return OK;
    }
...
Error GLTFDocument::_encode_accessors(Ref<GLTFState> state) {
    ...
    for (GLTFAccessorIndex i = 0; i < state->accessors.size(); i++) {
        ...
        d["min"] = min;
        d["bufferView"] =
            accessor->buffer_view; // optional because it may be sparse...
    ...
    for (GLTFAccessorIndex i = 0; i < accessors.size(); i++) {
        ...
        if (d.has("bufferView")) {
            accessor->buffer_view =
                d["bufferView"]; // optional because it may be sparse...
        }
...
Error GLTFDocument::_encode_buffer_view(
    Ref<GLTFState> state,
    const double* src,
    const int count,
    const GLTFType type,
    ...
    // use to debug
    print_verbose(
        "glTF: encoding type " + _get_type_name(type)
        + " component type: " + _get_component_type_name(component_type)
        + " stride: " + itos(stride) + " amount " + itos(count)
    );

    print_verbose(
        "glTF: encoding accessor offset " + itos(byte_offset)
        + " view offset: " + itos(bv->byte_offset) + " total buffer len: "
        + itos(gltf_buffer.size()) + " view len " + itos(bv->byte_length)
    );

    const int buffer_end =
        (stride * (count - 1)) + _get_component_type_size(component_type);
    ...
    ERR_FAIL_COND_V(
        (int)(offset + buffer_end) > gltf_buffer.size(),
        ERR_INVALID_DATA
    );
...
Error GLTFDocument::_decode_buffer_view(
    Ref<GLTFState> state,
    double* dst,
    ...
    const uint32_t offset = bv->byte_offset + byte_offset;
    Vector<uint8_t> buffer =
        state->buffers[bv->buffer]; // copy on write, so no performance hit
    const uint8_t* bufptr = buffer.ptr();

    // use to debug
    print_verbose(
        "glTF: type " + _get_type_name(type)
        + " component type: " + _get_component_type_name(component_type)
        + " stride: " + itos(stride) + " amount " + itos(count)
    );
    print_verbose(
        "glTF: accessor offset " + itos(byte_offset) + " view offset: "
        + itos(bv->byte_offset) + " total buffer len: " + itos(buffer.size())
        + " view len " + itos(bv->byte_length)
    );
    ...
    ERR_FAIL_COND_V(
        (int)(offset + buffer_end) > buffer.size(),
        ERR_PARSE_ERROR
    );
...
Vector<double> GLTFDocument::_decode_accessor(
    Ref<GLTFState> state,
    ...
    if (a->buffer_view >= 0) {
        ERR_FAIL_INDEX_V(
            a->buffer_view,
            state->buffer_views.size(),
            Vector<double>()
        );

        const Error err = _decode_buffer_view(
            state,
            dst,
            a->buffer_view,
            skip_every,
    ...
    if (a->sparse_count > 0) {
        ...
        const int indices_component_size =
            _get_component_type_size(a->sparse_indices_component_type);

        Error err = _decode_buffer_view(
            state,
            indices.ptrw(),
            a->sparse_indices_buffer_view,
            0,
            0,
            ...
            TYPE_SCALAR,
            1,
            a->sparse_indices_component_type,
            indices_component_size,
            false,
            a->sparse_indices_byte_offset,
            false
        );
        if (err != OK) {
            return Vector<double>();
        ...
        err = _decode_buffer_view(
            state,
            data.ptrw(),
            a->sparse_values_buffer_view,
            skip_every,
            skip_bytes,
            element_size,
            a->sparse_count,
...
Vector<int> GLTFDocument::_decode_accessor_as_ints(
    Ref<GLTFState> state,
    const GLTFAccessorIndex p_accessor,
    const bool p_for_vertex
) {
    const Vector<double> attribs =
        _decode_accessor(state, p_accessor, p_for_vertex);
    Vector<int> ret;
...
Vector<float> GLTFDocument::_decode_accessor_as_floats(
    Ref<GLTFState> state,
    const GLTFAccessorIndex p_accessor,
    const bool p_for_vertex
) {
...
GLTFAccessorIndex GLTFDocument::_encode_accessor_as_vec2(
    Ref<GLTFState> state,
    ...
    for (int i = 0; i < p_attribs.size(); i++) {
        ...
        attribs.write[(i * element_count) + 0] =
            Math::stepify(attrib.x, CMP_NORMALIZE_TOLERANCE);
        attribs.write[(i * element_count) + 1] =
            Math::stepify(attrib.y, CMP_NORMALIZE_TOLERANCE);
...
GLTFAccessorIndex GLTFDocument::_encode_accessor_as_color(
    Ref<GLTFState> state,
    ...
    for (int i = 0; i < p_attribs.size(); i++) {
        ...
        attribs.write[(i * element_count) + 0] =
            Math::stepify(attrib.r, CMP_NORMALIZE_TOLERANCE);
        attribs.write[(i * element_count) + 1] =
            Math::stepify(attrib.g, CMP_NORMALIZE_TOLERANCE);
        attribs.write[(i * element_count) + 2] =
            Math::stepify(attrib.b, CMP_NORMALIZE_TOLERANCE);
        attribs.write[(i * element_count) + 3] =
            Math::stepify(attrib.a, CMP_NORMALIZE_TOLERANCE);
...
GLTFAccessorIndex GLTFDocument::_encode_accessor_as_weights(
    Ref<GLTFState> state,
    const Vector<Color> p_attribs,
    const bool p_for_vertex
) {
...
GLTFAccessorIndex GLTFDocument::_encode_accessor_as_joints(
    Ref<GLTFState> state,
    const Vector<Color> p_attribs,
    const bool p_for_vertex
) {
...
GLTFAccessorIndex GLTFDocument::_encode_accessor_as_quats(
    Ref<GLTFState> state,
    ...
    for (int i = 0; i < p_attribs.size(); i++) {
        ...
        attribs.write[(i * element_count) + 0] =
            Math::stepify(quat.x, CMP_NORMALIZE_TOLERANCE);
        attribs.write[(i * element_count) + 1] =
            Math::stepify(quat.y, CMP_NORMALIZE_TOLERANCE);
        attribs.write[(i * element_count) + 2] =
            Math::stepify(quat.z, CMP_NORMALIZE_TOLERANCE);
        attribs.write[(i * element_count) + 3] =
            Math::stepify(quat.w, CMP_NORMALIZE_TOLERANCE);
...
Vector<Vector2> GLTFDocument::_decode_accessor_as_vec2(
    Ref<GLTFState> state,
    const GLTFAccessorIndex p_accessor,
    const bool p_for_vertex
) {
    ...
    ret.resize(ret_size);
    {
        for (int i = 0; i < ret_size; i++) {
            ret.write[i] =
                Vector2(attribs_ptr[i * 2 + 0], attribs_ptr[i * 2 + 1]);
        }
    }
    return ret;
}

GLTFAccessorIndex GLTFDocument::_encode_accessor_as_floats(
    Ref<GLTFState> state,
...
GLTFAccessorIndex GLTFDocument::_encode_accessor_as_vec3(
    Ref<GLTFState> state,
    ...
    for (int i = 0; i < p_attribs.size(); i++) {
        ...
        attribs.write[(i * element_count) + 1] =
            Math::stepify(attrib.y, CMP_NORMALIZE_TOLERANCE);
        attribs.write[(i * element_count) + 2] =
            Math::stepify(attrib.z, CMP_NORMALIZE_TOLERANCE);
...
GLTFAccessorIndex GLTFDocument::_encode_accessor_as_xform(
    Ref<GLTFState> state,
    ...
    for (int i = 0; i < p_attribs.size(); i++) {
        ...
        attribs.write[i * element_count + 1] =
            Math::stepify(axis_0.y, CMP_NORMALIZE_TOLERANCE);
        attribs.write[i * element_count + 2] =
            Math::stepify(axis_0.z, CMP_NORMALIZE_TOLERANCE);
        attribs.write[i * element_count + 3] = 0.0;

        Vector3 axis_1 = basis.get_axis(Vector3::AXIS_Y);
        attribs.write[i * element_count + 4] =
            Math::stepify(axis_1.x, CMP_NORMALIZE_TOLERANCE);
        attribs.write[i * element_count + 5] =
            Math::stepify(axis_1.y, CMP_NORMALIZE_TOLERANCE);
        attribs.write[i * element_count + 6] =
            Math::stepify(axis_1.z, CMP_NORMALIZE_TOLERANCE);
        attribs.write[i * element_count + 7] = 0.0;

        Vector3 axis_2 = basis.get_axis(Vector3::AXIS_Z);
        attribs.write[i * element_count + 8] =
            Math::stepify(axis_2.x, CMP_NORMALIZE_TOLERANCE);
        attribs.write[i * element_count + 9] =
            Math::stepify(axis_2.y, CMP_NORMALIZE_TOLERANCE);
        attribs.write[i * element_count + 10] =
            Math::stepify(axis_2.z, CMP_NORMALIZE_TOLERANCE);
        attribs.write[i * element_count + 11] = 0.0;

        Vector3 origin = attrib.get_origin();
        attribs.write[i * element_count + 12] =
            Math::stepify(origin.x, CMP_NORMALIZE_TOLERANCE);
        attribs.write[i * element_count + 13] =
            Math::stepify(origin.y, CMP_NORMALIZE_TOLERANCE);
        attribs.write[i * element_count + 14] =
            Math::stepify(origin.z, CMP_NORMALIZE_TOLERANCE);
...
Vector<Vector3> GLTFDocument::_decode_accessor_as_vec3(
    Ref<GLTFState> state,
    const GLTFAccessorIndex p_accessor,
    const bool p_for_vertex
) {
...
Vector<Color> GLTFDocument::_decode_accessor_as_color(
    Ref<GLTFState> state,
    const GLTFAccessorIndex p_accessor,
    const bool p_for_vertex
) {
    ...
    ret.resize(ret_size);
    {
        for (int i = 0; i < ret_size; i++) {
            ret.write[i] = Color(
                attribs_ptr[i * vec_len + 0],
                attribs_ptr[i * vec_len + 1],
                attribs_ptr[i * vec_len + 2],
                vec_len == 4 ? attribs_ptr[i * 4 + 3] : 1.0
            );
        }
    }
    return ret;
...
Vector<Quat> GLTFDocument::_decode_accessor_as_quat(
    Ref<GLTFState> state,
    const GLTFAccessorIndex p_accessor,
    const bool p_for_vertex
) {
...
Vector<Transform2D> GLTFDocument::_decode_accessor_as_xform2d(
    Ref<GLTFState> state,
    const GLTFAccessorIndex p_accessor,
    const bool p_for_vertex
) {
...
Vector<Basis> GLTFDocument::_decode_accessor_as_basis(
    Ref<GLTFState> state,
    const GLTFAccessorIndex p_accessor,
    const bool p_for_vertex
) {
    ...
    for (int i = 0; i < ret.size(); i++) {
        ret.write[i].set_axis(
            0,
            Vector3(attribs[i * 9 + 0], attribs[i * 9 + 1], attribs[i * 9 + 2])
        );
        ret.write[i].set_axis(
            1,
            Vector3(attribs[i * 9 + 3], attribs[i * 9 + 4], attribs[i * 9 + 5])
        );
        ret.write[i].set_axis(
            2,
            Vector3(attribs[i * 9 + 6], attribs[i * 9 + 7], attribs[i * 9 + 8])
        );
    }
    return ret;
}

Vector<Transform> GLTFDocument::_decode_accessor_as_xform(
    Ref<GLTFState> state,
    const GLTFAccessorIndex p_accessor,
    const bool p_for_vertex
) {
    ...
    for (int i = 0; i < ret.size(); i++) {
        ...
        ret.write[i].set_origin(Vector3(
            attribs[i * 16 + 12],
            attribs[i * 16 + 13],
            attribs[i * 16 + 14]
        ));
...
Error GLTFDocument::_serialize_meshes(Ref<GLTFState> state) {
    Array meshes;
    for (GLTFMeshIndex gltf_mesh_i = 0; gltf_mesh_i < state->meshes.size();
         gltf_mesh_i++) {
        print_verbose("glTF: Serializing mesh: " + itos(gltf_mesh_i));
        Ref<ArrayMesh> import_mesh =
            state->meshes.write[gltf_mesh_i]->get_mesh();
        if (import_mesh.is_null()) {
            continue;
        }
        Array instance_materials =
            state->meshes.write[gltf_mesh_i]->get_instance_materials();
        Array primitives;
        Dictionary gltf_mesh;
        Array target_names;
        Array weights;
        for (int morph_i = 0; morph_i < import_mesh->get_blend_shape_count();
             morph_i++) {
            target_names.push_back(import_mesh->get_blend_shape_name(morph_i));
        }
        for (int surface_i = 0; surface_i < import_mesh->get_surface_count();
             surface_i++) {
            Array targets;
            Dictionary primitive;
            Mesh::PrimitiveType primitive_type =
                import_mesh->surface_get_primitive_type(surface_i);
            ...
            Dictionary attributes;
            {
                ...
                attributes["POSITION"] =
                    _encode_accessor_as_vec3(state, a, true);
            }
            {
                ...
                if (a.size()) {
                    ...
                    attributes["TANGENT"] =
                        _encode_accessor_as_color(state, attribs, true);
                }
            }
            {
                ...
                if (a.size()) {
                    ...
                    attributes["NORMAL"] =
                        _encode_accessor_as_vec3(state, attribs, true);
                }
            }
            {
                ...
                if (a.size()) {
                    attributes["TEXCOORD_0"] =
                        _encode_accessor_as_vec2(state, a, true);
                }
            }
            {
            ...
            }
            {
                ...
                if (a.size()) {
                    attributes["COLOR_0"] =
                        _encode_accessor_as_color(state, a, true);
                }
            }
            Map<int, int> joint_i_to_bone_i;
            for (GLTFNodeIndex node_i = 0; node_i < state->nodes.size();
                 node_i++) {
            ...
            }
            {
                ...
                if ((a.size() / JOINT_GROUP_SIZE) == vertex_array.size()) {
                    ...
                    attribs.resize(ret_size);
                    {
                        for (int array_i = 0; array_i < attribs.size();
                             array_i++) {
                            int32_t joint_0 =
                                a[(array_i * JOINT_GROUP_SIZE) + 0];
                            int32_t joint_1 =
                                a[(array_i * JOINT_GROUP_SIZE) + 1];
                            int32_t joint_2 =
                                a[(array_i * JOINT_GROUP_SIZE) + 2];
                            int32_t joint_3 =
                                a[(array_i * JOINT_GROUP_SIZE) + 3];
                            attribs.write[array_i] =
                                Color(joint_0, joint_1, joint_2, joint_3);
                        }
                    }
                    attributes["JOINTS_0"] =
                        _encode_accessor_as_joints(state, attribs, true);
                }
                ERR_FAIL_COND_V(
                    (a.size() / (JOINT_GROUP_SIZE * 2)) >= vertex_array.size(),
                    FAILED
                );
            }
            {
                ...
                if ((a.size() / JOINT_GROUP_SIZE) == vertex_array.size()) {
                    ...
                    for (int i = 0; i < vertex_count; i++) {
                        attribs.write[i] = Color(
                            a[(i * JOINT_GROUP_SIZE) + 0],
                            a[(i * JOINT_GROUP_SIZE) + 1],
                            a[(i * JOINT_GROUP_SIZE) + 2],
                            a[(i * JOINT_GROUP_SIZE) + 3]
                        );
                    }
                    ...
                    for (int32_t vertex_i = 0; vertex_i < vertex_count;
                         vertex_i++) {
                        Color weight_0;
                    ...
                    attributes["WEIGHTS_0"] =
                        _encode_accessor_as_weights(state, weights_0, true);
                    attributes["WEIGHTS_1"] =
                        _encode_accessor_as_weights(state, weights_1, true);
                }
            }
            {
                ...
                if (mesh_indices.size()) {
                    if (primitive_type == Mesh::PRIMITIVE_TRIANGLES) {
                        ...
                        for (int k = 0; k < is; k += 3) {
                            SWAP(
                                mesh_indices.write[k + 0],
                                mesh_indices.write[k + 2]
                            );
                        }
                    }
                    primitive["indices"] =
                        _encode_accessor_as_ints(state, mesh_indices, true);
                } else {
                    if (primitive_type == Mesh::PRIMITIVE_TRIANGLES) {
                        // generate indices because they need to be swapped for
                        // CW/CCW
                        ...
                        Vector<int32_t> generated_indices =
                            st->commit_to_arrays()[Mesh::ARRAY_INDEX];
                        ...
                        primitive["indices"] = _encode_accessor_as_ints(
                            state,
                            generated_indices,
                            true
                        );
            ...
            if (import_mesh->get_blend_shape_count()) {
                ArrayMesh::BlendShapeMode shape_mode =
                    import_mesh->get_blend_shape_mode();
                Array array_morphs =
                    import_mesh->surface_get_blend_shape_arrays(surface_i);
                for (int morph_i = 0; morph_i < array_morphs.size();
                     morph_i++) {
                    Array array_morph = array_morphs[morph_i];
                    Dictionary t;
                    Vector<Vector3> varr = array_morph[Mesh::ARRAY_VERTEX];
                    Array mesh_arrays =
                        import_mesh->surface_get_arrays(surface_i);
                    if (varr.size()) {
                        ...
                        if (shape_mode
                            ...
                            for (int blend_i = 0; blend_i < max_idx;
                                 blend_i++) {
                                varr.write[blend_i] =
                                    Vector3(varr[blend_i]) - src_varr[blend_i];
                            }
                        }

                        t["POSITION"] =
                            _encode_accessor_as_vec3(state, varr, true);
                    }

                    Vector<Vector3> narr = array_morph[Mesh::ARRAY_NORMAL];
                    if (narr.size()) {
                        t["NORMAL"] =
                            _encode_accessor_as_vec3(state, narr, true);
                    }
            ...
            if (mat.is_valid()) {
                Map<Ref<Material>, GLTFMaterialIndex>::Element*
                    material_cache_i = state->material_cache.find(mat);
        ...
        for (int name_i = 0; name_i < target_names.size(); name_i++) {
            real_t weight = 0.0;
            if (name_i
                < state->meshes.write[gltf_mesh_i]->get_blend_weights().size(
                )) {
                weight = state->meshes.write[gltf_mesh_i]->get_blend_weights(
                )[name_i];
            }
...
Error GLTFDocument::_parse_meshes(Ref<GLTFState> state) {
    ...
    for (GLTFMeshIndex i = 0; i < meshes.size(); i++) {
        ...
        const Dictionary& extras =
            d.has("extras") ? (Dictionary)d["extras"] : Dictionary();
        Ref<ArrayMesh> import_mesh;
        ...
        import_mesh->set_name(_gen_unique_name(
            state,
            vformat("%s_%s", state->scene_name, mesh_name)
        ));
        ...
        for (int j = 0; j < primitives.size(); j++) {
            ...
            if (p.has("mode")) {
                ...
                static const Mesh::PrimitiveType primitives2[7] = {
                    ...
                    Mesh::PRIMITIVE_LINES, // loop not supported, should ce
                                           // converted
                    Mesh::PRIMITIVE_LINES,
                    Mesh::PRIMITIVE_TRIANGLES,
                    Mesh::PRIMITIVE_TRIANGLE_STRIP,
                    Mesh::PRIMITIVE_TRIANGLES, // fan not supported, should be
                                               // converted
#ifndef _MSC_VER
// #warning line loop and triangle fan are not supported and need to be
// converted to lines and triangles
#endif
            ...
            if (a.has("POSITION")) {
                array[Mesh::ARRAY_VERTEX] =
                    _decode_accessor_as_vec3(state, a["POSITION"], true);
            }
            if (a.has("NORMAL")) {
                array[Mesh::ARRAY_NORMAL] =
                    _decode_accessor_as_vec3(state, a["NORMAL"], true);
            }
            if (a.has("TANGENT")) {
                array[Mesh::ARRAY_TANGENT] =
                    _decode_accessor_as_floats(state, a["TANGENT"], true);
            }
            if (a.has("TEXCOORD_0")) {
                array[Mesh::ARRAY_TEX_UV] =
                    _decode_accessor_as_vec2(state, a["TEXCOORD_0"], true);
            }
            if (a.has("TEXCOORD_1")) {
                array[Mesh::ARRAY_TEX_UV2] =
                    _decode_accessor_as_vec2(state, a["TEXCOORD_1"], true);
            }
            if (a.has("COLOR_0")) {
                array[Mesh::ARRAY_COLOR] =
                    _decode_accessor_as_color(state, a["COLOR_0"], true);
                has_vertex_color = true;
            }
            if (a.has("JOINTS_0") && !a.has("JOINTS_1")) {
                array[Mesh::ARRAY_BONES] =
                    _decode_accessor_as_ints(state, a["JOINTS_0"], true);
            }
            ...
            if (p.has("indices")) {
                Vector<int> indices =
                    _decode_accessor_as_ints(state, p["indices"], false);
            ...
            bool generate_tangents =
                (primitive == Mesh::PRIMITIVE_TRIANGLES && !a.has("TANGENT")
                 && a.has("TEXCOORD_0") && a.has("NORMAL"));
            ...
            if (p.has("targets")) {
                ...
                // ideally BLEND_SHAPE_MODE_RELATIVE since gltf2 stores in
                // displacement but it could require a larger refactor?
                import_mesh->set_blend_shape_mode(
                    Mesh::BLEND_SHAPE_MODE_NORMALIZED
                );

                if (j == 0) {
                    const Array& target_names = extras.has("targetNames")
                                                  ? (Array)extras["targetNames"]
                                                  : Array();
                    for (int k = 0; k < targets.size(); k++) {
                        const String name = k < target_names.size()
                                              ? (String)target_names[k]
                                              : String("morph_") + itos(k);
                ...
                for (int k = 0; k < targets.size(); k++) {
                    ...
                    if (t.has("POSITION")) {
                        Vector<Vector3> varr = _decode_accessor_as_vec3(
                            state,
                            t["POSITION"],
                            true
                        );
                        const Vector<Vector3> src_varr =
                            array[Mesh::ARRAY_VERTEX];
                    ...
                    if (t.has("NORMAL")) {
                        Vector<Vector3> narr =
                            _decode_accessor_as_vec3(state, t["NORMAL"], true);
                        const Vector<Vector3> src_narr =
                            array[Mesh::ARRAY_NORMAL];
                    ...
                    if (t.has("TANGENT")) {
                        const Vector<Vector3> tangents_v3 =
                            _decode_accessor_as_vec3(state, t["TANGENT"], true);
                        const Vector<float> src_tangents =
                            array[Mesh::ARRAY_TANGENT];
                        ERR_FAIL_COND_V(
                            src_tangents.size() == 0,
                            ERR_PARSE_ERROR
            ...
            import_mesh->add_surface_from_arrays(
                primitive,
                array,
                morphs,
                state->compress_flags
            );
        ...
        for (int32_t weight_i = 0; weight_i < blend_weights.size();
             weight_i++) {
...
Error GLTFDocument::_serialize_images(
    Ref<GLTFState> state,
    const String& p_path
) {
    Array images;
    for (int i = 0; i < state->images.size(); i++) {
        ...
        if (p_path.to_lower().ends_with("glb")) {
            ...
            state->buffers.write[bi].resize(
                state->buffers[bi].size() + bv->byte_length
            );
            memcpy(
                &state->buffers.write[bi].write[bv->byte_offset],
                buffer.read().ptr(),
                buffer.size()
            );
            ERR_FAIL_COND_V(
                bv->byte_offset + bv->byte_length > state->buffers[bi].size(),
                ERR_FILE_CORRUPT
            );
...
Error GLTFDocument::_parse_images(
    Ref<GLTFState> state,
    const String& p_base_path
) {
    ...
    for (int i = 0; i < images.size(); i++) {
        ...
        // glTF 2.0 supports PNG and JPEG types, which can be specified as (from
        // spec):
        // "- a URI to an external file in one of the supported images formats,
        // or
        //  - a URI with embedded base64-encoded data, or
        //  - a reference to a bufferView; in that case mimeType must be
        //  defined."
        // Since the mimeType is optional for external files and base64 data,
        // we parse the data to determine if it's a PNG or JPEG image.

        // We'll assume that we use either URI or bufferView, so let's warn the
        // user if their image somehow uses both. And fail if it has neither.
        ERR_CONTINUE_MSG(
            !d.has("uri") && !d.has("bufferView"),
            "Invalid image definition in glTF file, it should specify an 'uri' "
            "or 'bufferView'."
        );
        if (d.has("uri") && d.has("bufferView")) {
            WARN_PRINT(
                "Invalid image definition in glTF file using both 'uri' and "
                "'bufferView'. 'uri' will take precedence."
            );
        }
        ...
        if (d.has("uri")) {
            ...
            if (uri.begins_with("data:")) { // Embedded data using base64.
                // Validate data MIME types and throw a warning if it's one we
                // don't know/support.
                if (!uri.begins_with("data:application/octet-stream;base64")
                    ...
                    WARN_PRINT(vformat(
                        "glTF: Image index '%d' uses an unsupported URI data "
                        "type: %s. Skipping it.",
                        i,
                        uri
                    ));
                ...
                // ResourceLoader will rely on the file extension to use the
                // relevant loader. The spec says that if mimeType is defined,
                // it should take precedence (e.g. there could be a `.png` image
                // which is actually JPEG), but we don't have an API for that,
                // so we load it into the buffer embedded in the material.
                ...
                if (texture.is_valid()) {
                    ...
                    if (data.size() == 0) {
                        WARN_PRINT(vformat(
                            "glTF: Image index '%d' couldn't be loaded as a "
                            "buffer of MIME type '%s' from URI: %s. Skipping "
                            "it.",
                            i,
                            mimetype,
                            uri
                    ...
                    WARN_PRINT(vformat(
                        "glTF: Image index '%d' couldn't be loaded from URI: "
                        "%s. Skipping it.",
                        i,
                        uri
                    ));
            ...
            ERR_FAIL_COND_V_MSG(
                ...
                ERR_FILE_CORRUPT,
                vformat(
                    "glTF: Image index '%d' specifies 'bufferView' but no "
                    "'mimeType', which is invalid.",
                    i
                )
            );
        ...
        if (img.is_null()) {
            ERR_PRINT(vformat(
                "glTF: Couldn't load image index '%d' with its given mimetype: "
                "%s.",
                i,
                mimetype
            ));
...
GLTFTextureIndex GLTFDocument::_set_texture(
    Ref<GLTFState> state,
...
Ref<Texture> GLTFDocument::_get_texture(
    Ref<GLTFState> state,
    ...
    for (int32_t i = 0; i < state->materials.size(); i++) {
        ...
        }
        {
            ...
            }
            {
                ...
                Ref<Texture> albedo_texture =
                    material->get_texture(SpatialMaterial::TEXTURE_ALBEDO);
                ...
                if (gltf_texture_index != -1) {
                    ...
                    bct["extensions"] =
                        _serialize_texture_transform_uv1(material);
            ...
            bool has_roughness =
                ...
                && material->get_texture(SpatialMaterial::TEXTURE_ROUGHNESS)
                       ...
                       .is_valid();
            bool has_ao =
                material->get_feature(SpatialMaterial::FEATURE_AMBIENT_OCCLUSION
                )
                && material
                       ->get_texture(SpatialMaterial::TEXTURE_AMBIENT_OCCLUSION)
                       .is_valid();
            bool has_metalness =
                material->get_texture(SpatialMaterial::TEXTURE_METALLIC)
                    .is_valid()
                && material->get_texture(SpatialMaterial::TEXTURE_METALLIC)
                       ->get_data()
                       .is_valid();
            if (has_ao || has_roughness || has_metalness) {
                Dictionary mrt;
                Ref<Texture> roughness_texture =
                    material->get_texture(SpatialMaterial::TEXTURE_ROUGHNESS);
                SpatialMaterial::TextureChannel roughness_channel =
                    material->get_roughness_texture_channel();
                Ref<Texture> metallic_texture =
                    material->get_texture(SpatialMaterial::TEXTURE_METALLIC);
                SpatialMaterial::TextureChannel metalness_channel =
                    material->get_metallic_texture_channel();
                Ref<Texture> ao_texture = material->get_texture(
                    SpatialMaterial::TEXTURE_AMBIENT_OCCLUSION
                );
                SpatialMaterial::TextureChannel ao_channel =
                    material->get_ao_texture_channel();
                ...
                if (ao_image.is_valid()
                    && ao_image->get_size() != Vector2(width, height)) {
                    ao_image->resize(width, height, Image::INTERPOLATE_LANCZOS);
                }
                if (roughness_image.is_valid()
                    && roughness_image->get_size() != Vector2(width, height)) {
                    roughness_image
                        ->resize(width, height, Image::INTERPOLATE_LANCZOS);
                }
                if (metallness_image.is_valid()
                    && metallness_image->get_size() != Vector2(width, height)) {
                    metallness_image
                        ->resize(width, height, Image::INTERPOLATE_LANCZOS);
                }
                orm_image->lock();
                for (int32_t h = 0; h < height; h++) {
                    for (int32_t w = 0; w < width; w++) {
                        ...
                        if (has_ao) {
                            ao_image->lock();
                            if (SpatialMaterial::TextureChannel::
                                    TEXTURE_CHANNEL_RED
                                == ao_channel) {
        ...
        if (material->get_feature(SpatialMaterial::FEATURE_NORMAL_MAPPING)) {
            ...
            tex.instance();
            {
                Ref<Texture> normal_texture =
                    material->get_texture(SpatialMaterial::TEXTURE_NORMAL);
        ...
        if (material->get_feature(SpatialMaterial::FEATURE_EMISSION)) {
            ...
            Ref<Texture> emission_texture =
                material->get_texture(SpatialMaterial::TEXTURE_EMISSION);
            GLTFTextureIndex gltf_texture_index = -1;
            if (emission_texture.is_valid()
                && emission_texture->get_data().is_valid()) {
        ...
        const bool ds =
            material->get_cull_mode() == SpatialMaterial::CULL_DISABLED;
        if (ds) {
            d["doubleSided"] = ds;
        }
...
Error GLTFDocument::_parse_materials(Ref<GLTFState> state) {
    ...
    for (GLTFMaterialIndex i = 0; i < materials.size(); i++) {
        ...
        if (pbr_spec_gloss_extensions.has("KHR_materials_pbrSpecularGlossiness"
            ...
            WARN_PRINT(
                "Material uses a specular and glossiness workflow. Textures "
                "will be converted to roughness and metallic workflow, which "
                "may not be 100% accurate."
            );
            Dictionary sgm = pbr_spec_gloss_extensions
            ...
            if (sgm.has("diffuseTexture")) {
                ...
                if (diffuse_texture_dict.has("index")) {
                    Ref<Texture> diffuse_texture =
                        _get_texture(state, diffuse_texture_dict["index"]);
            ...
            if (sgm.has("glossinessFactor")) {
                ...
                material->set_roughness(
                    1.0f - CLAMP(spec_gloss->gloss_factor, 0.0f, 1.0f)
                );
            }
            if (sgm.has("specularGlossinessTexture")) {
                const Dictionary& spec_gloss_texture =
                    sgm["specularGlossinessTexture"];
                if (spec_gloss_texture.has("index")) {
                    const Ref<Texture> orig_texture =
                        _get_texture(state, spec_gloss_texture["index"]);
            ...
            if (mr.has("metallicRoughnessTexture")) {
                ...
                if (bct.has("index")) {
                    ...
                    material->set_metallic_texture_channel(
                        SpatialMaterial::TEXTURE_CHANNEL_BLUE
                    );
                    material->set_texture(
                        SpatialMaterial::TEXTURE_ROUGHNESS,
                        t
                    );
                    material->set_roughness_texture_channel(
                        SpatialMaterial::TEXTURE_CHANNEL_GREEN
                    );
                    if (!mr.has("metallicFactor")) {
        ...
        if (d.has("normalTexture")) {
            ...
            if (bct.has("index")) {
                material->set_texture(
                    SpatialMaterial::TEXTURE_NORMAL,
                    _get_texture(state, bct["index"])
                );
                material->set_feature(
                    SpatialMaterial::FEATURE_NORMAL_MAPPING,
                    true
                );
            }
            if (bct.has("scale")) {
        ...
        if (d.has("occlusionTexture")) {
            ...
            if (bct.has("index")) {
                ...
                material->set_ao_texture_channel(
                    SpatialMaterial::TEXTURE_CHANNEL_RED
                );
                material->set_feature(
                    SpatialMaterial::FEATURE_AMBIENT_OCCLUSION,
                    true
                );
            }
        }
        ...
        if (d.has("emissiveTexture")) {
            ...
            if (bct.has("index")) {
                material->set_texture(
                    SpatialMaterial::TEXTURE_EMISSION,
                    _get_texture(state, bct["index"])
                );
                material->set_feature(SpatialMaterial::FEATURE_EMISSION, true);
        ...
        if (d.has("alphaMode")) {
            ...
            if (am == "BLEND") {
                ...
                material->set_flag(
                    SpatialMaterial::FLAG_USE_ALPHA_SCISSOR,
                    true
                );
...
void GLTFDocument::_set_texture_transform_uv1(
    const Dictionary& d,
    Ref<SpatialMaterial> material
) {
    if (d.has("extensions")) {
        ...
        if (extensions.has("KHR_texture_transform")) {
            ...
            if (offset_arr.size() == 2) {
                const Vector3 offset_vector3 =
                    Vector3(offset_arr[0], offset_arr[1], 0.0f);
                material->set_uv1_offset(offset_vector3);
            }

            const Array& scale_arr = texture_transform["scale"];
            if (scale_arr.size() == 2) {
                const Vector3 scale_vector3 =
                    Vector3(scale_arr[0], scale_arr[1], 1.0f);
...
void GLTFDocument::spec_gloss_to_rough_metal(
    ...
    rm_img->create(
        r_spec_gloss->spec_gloss_img->get_width(),
        r_spec_gloss->spec_gloss_img->get_height(),
        false,
        Image::FORMAT_RGBA8
    );
    rm_img->lock();
    ...
    if (r_spec_gloss->diffuse_img.is_valid()) {
        ...
        r_spec_gloss->diffuse_img->resize(
            r_spec_gloss->spec_gloss_img->get_width(),
            r_spec_gloss->spec_gloss_img->get_height(),
            Image::INTERPOLATE_LANCZOS
        );
        r_spec_gloss->spec_gloss_img->resize(
            r_spec_gloss->diffuse_img->get_width(),
            r_spec_gloss->diffuse_img->get_height(),
            Image::INTERPOLATE_LANCZOS
        );
    }
    for (int32_t y = 0; y < r_spec_gloss->spec_gloss_img->get_height(); y++) {
        for (int32_t x = 0; x < r_spec_gloss->spec_gloss_img->get_width();
             x++) {
            ...
            float metallic = 0.0f;
            Color base_color;
            spec_gloss_to_metal_base_color(
                specular,
                diffuse,
                base_color,
                metallic
    ...
    if (has_roughness) {
        ...
        p_material->set_roughness_texture_channel(
            SpatialMaterial::TEXTURE_CHANNEL_GREEN
        );
    }
    ...
    if (has_metal) {
        p_material->set_texture(
            SpatialMaterial::TEXTURE_METALLIC,
            rm_image_texture
        );
        p_material->set_metallic_texture_channel(
            SpatialMaterial::TEXTURE_CHANNEL_BLUE
        );
    }
}
...
void GLTFDocument::spec_gloss_to_metal_base_color(
    ...
    Color specular =
        Color(p_specular_factor.r, p_specular_factor.g, p_specular_factor.b);
    ...
    const Color base_color_from_diffuse =
        p_diffuse
        * (one_minus_specular_strength / (1.0f - dielectric_specular_red)
    ...
    r_base_color.r = Math::lerp(
        base_color_from_diffuse.r,
        base_color_from_specular.r,
        r_metallic * r_metallic
    );
    r_base_color.g = Math::lerp(
        base_color_from_diffuse.g,
        base_color_from_specular.g,
        r_metallic * r_metallic
    );
    r_base_color.b = Math::lerp(
        base_color_from_diffuse.b,
        base_color_from_specular.b,
        r_metallic * r_metallic
    );
...
GLTFNodeIndex GLTFDocument::_find_highest_node(
    Ref<GLTFState> state,
...
bool GLTFDocument::_capture_nodes_in_skin(
    Ref<GLTFState> state,
    Ref<GLTFSkin> skin,
    ...
    for (int i = 0; i < state->nodes[node_index]->children.size(); ++i) {
        found_joint |= _capture_nodes_in_skin(
            state,
            skin,
            state->nodes[node_index]->children[i]
        );
    }

    if (found_joint) {
        // Mark it if we happen to find another skins joint...
        if (state->nodes[node_index]->joint
            && skin->joints.find(node_index) < 0) {
...
void GLTFDocument::_capture_nodes_for_multirooted_skin(
    Ref<GLTFState> state,
    Ref<GLTFSkin> skin
) {
    ...
    // Go up the tree till all of the multiple roots of the skin are at the same
    // hierarchy level.
    for (int i = 0; i < roots.size(); ++i) {
    ...
    do {
        ...
        if (!all_same) {
            for (int i = 0; i < roots.size(); ++i) {
                ...
                if (state->nodes[parent]->joint
                    && skin->joints.find(parent) < 0) {
                    skin->joints.push_back(parent);
                } else if (skin->non_joints.find(parent) < 0) {
                    skin->non_joints.push_back(parent);
                }
...
Error GLTFDocument::_verify_skin(Ref<GLTFState> state, Ref<GLTFSkin> skin) {
    // This may seem duplicated from expand_skins, but this is really a sanity
    // check! (so it kinda is) In case additional interpolating logic is added
    // to the skins, this will help ensure that you do not cause it to self
    // implode into a fiery blaze

    // We are going to re-calculate the root nodes and compare them to the ones
    // saved in the skin, then ensure the multiple trees (if they exist) are on
    // the same sublevel
...
Error GLTFDocument::_parse_skins(Ref<GLTFState> state) {
    ...
    for (int i = 0; i < skins.size(); i++) {
        ...
        if (d.has("inverseBindMatrices")) {
            skin->inverse_binds = _decode_accessor_as_xform(
                state,
                d["inverseBindMatrices"],
                false
            );
            ERR_FAIL_COND_V(
                skin->inverse_binds.size() != joints.size(),
                ERR_PARSE_ERROR
            );
        }
    ...
    for (GLTFSkinIndex i = 0; i < state->skins.size(); ++i) {
        ...
        // Expand the skin to capture all the extra non-joints that lie in
        // between the actual joints, and expand the hierarchy to ensure
        // multi-rooted trees lie on the same height level
...
Error GLTFDocument::_determine_skeletons(Ref<GLTFState> state) {
    // Using a disjoint set, we are going to potentially combine all skins that
    // are actually branches of a main skeleton, or treat skins defining the
    // same set of nodes as ONE skeleton. This is another unclear issue caused
    // by the current glTF specification.
    ...
    for (GLTFSkinIndex skin_i = 0; skin_i < state->skins.size(); ++skin_i) {
        ...
        // We are going to connect the separate skin subtrees in each skin
        // together so that the final roots are entire sets of valid skin trees
        for (int i = 1; i < skin->roots.size(); ++i) {
            skeleton_sets.create_union(skin->roots[0], skin->roots[i]);
        }
    }

    { // attempt to joint all touching subsets (siblings/parent are part of
      // another skin)
        ...
        for (int i = 0; i < highest_group_members.size(); ++i) {
            ...
            for (int j = i + 1; j < highest_group_members.size(); ++j) {
                ...
                if (state->nodes[node_i]->parent
                    == state->nodes[node_j]->parent) {
                    skeleton_sets.create_union(node_i, node_j);
                }
            }

            // Attach any parenting going on together (we need to do this n^2
            // times)
    ...
    for (GLTFSkeletonIndex skel_i = 0; skel_i < skeleton_owners.size();
         ++skel_i) {
        ...
        for (GLTFSkinIndex skin_i = 0; skin_i < state->skins.size(); ++skin_i) {
            ...
            // If any of the the skeletons nodes exist in a skin, that skin now
            // maps to the skeleton
            for (int i = 0; i < skeleton_nodes.size(); ++i) {
                GLTFNodeIndex skel_node_i = skeleton_nodes[i];
                if (skin->joints.find(skel_node_i) >= 0
                    || skin->non_joints.find(skel_node_i) >= 0) {
        ...
        _reparent_non_joint_skeleton_subtrees(
            state,
            state->skeletons.write[skel_i],
            non_joints
        );
    }

    for (GLTFSkeletonIndex skel_i = 0; skel_i < state->skeletons.size();
         ++skel_i) {
        ...
        ERR_FAIL_COND_V(
            _determine_skeleton_roots(state, skel_i),
            ERR_PARSE_ERROR
        );
    }
...
Error GLTFDocument::_reparent_non_joint_skeleton_subtrees(
    Ref<GLTFState> state,
    Ref<GLTFSkeleton> skeleton,
    ...
    // Populate the disjoint set with ONLY non joints that are in the skeleton
    // hierarchy (non_joints vector) This way we can find any joints that lie in
    // between joints, as the current glTF specification mentions nothing about
    // non-joints being in between joints of the same skin. Hopefully one day we
    ...
    for (int i = 0; i < non_joints.size(); ++i) {
        ...
        if (parent_i >= 0 && non_joints.find(parent_i) >= 0
            && !state->nodes[parent_i]->joint) {
...
Error GLTFDocument::_determine_skeleton_roots(
    Ref<GLTFState> state,
    const GLTFSkeletonIndex skel_i
) {
    DisjointSet<GLTFNodeIndex> disjoint_set;
    ...
    for (GLTFNodeIndex i = 0; i < state->nodes.size(); ++i) {
        ...
        if (node->parent >= 0
            && state->nodes[node->parent]->skeleton == skel_i) {
...
Error GLTFDocument::_create_skeletons(Ref<GLTFState> state) {
    for (GLTFSkeletonIndex skel_i = 0; skel_i < state->skeletons.size();
        ...
        // Make the skeleton creation deterministic by going through the roots
        // in a sorted order, and DEPTH FIRST
        bones.sort();
        ...
        while (!bones.empty()) {
            ...
            node->set_name(
                _gen_unique_bone_name(state, skel_i, node->get_name())
            );
            ...
            if (node->parent >= 0
                && state->nodes[node->parent]->skeleton == skel_i) {
                const int bone_parent =
                    skeleton->find_bone(state->nodes[node->parent]->get_name());
...
Error GLTFDocument::_map_skin_joints_indices_to_skeleton_bone_indices(
    Ref<GLTFState> state
) {
    for (GLTFSkinIndex skin_i = 0; skin_i < state->skins.size(); ++skin_i) {
        Ref<GLTFSkin> skin = state->skins.write[skin_i];
        ...
        for (int joint_index = 0; joint_index < skin->joints_original.size();
            ...
            const int bone_index =
                skeleton->skeleton->find_bone(node->get_name());
...
Error GLTFDocument::_create_skins(Ref<GLTFState> state) {
    ...
    for (GLTFSkinIndex skin_i = 0; skin_i < state->skins.size(); ++skin_i) {
        Ref<Skin> skin = state->skins.write[skin_i]->skin;
...
bool GLTFDocument::_skins_are_same(
    const Ref<Skin> skin_a,
    const Ref<Skin> skin_b
) {
...
void GLTFDocument::_remove_duplicate_skins(Ref<GLTFState> state) {
    for (int i = 0; i < state->skins.size(); ++i) {
        for (int j = i + 1; j < state->skins.size(); ++j) {
            const Ref<Skin> skin_i = state->skins[i]->skin;
            const Ref<Skin> skin_j = state->skins[j]->skin;

            if (_skins_are_same(skin_i, skin_j)) {
                // replace it and delete the old
                state->skins.write[j]->skin = skin_i;
            }
        }
    }
}
...
Error GLTFDocument::_serialize_cameras(Ref<GLTFState> state) {
    ...
    for (GLTFCameraIndex i = 0; i < state->cameras.size(); i++) {
        ...
        if (camera->get_perspective() == false) {
            ...
            Dictionary ppt;
            // GLTF spec is in radians, Rebel's camera is in degrees.
...
Error GLTFDocument::_parse_cameras(Ref<GLTFState> state) {
    ...
    for (GLTFCameraIndex i = 0; i < cameras.size(); i++) {
        ...
        if (type == "orthographic") {
            ...
            ERR_FAIL_V_MSG(
                ERR_PARSE_ERROR,
                "Camera should be in 'orthographic' or 'perspective'"
            );
        }
...
String GLTFDocument::interpolation_to_string(
...
Error GLTFDocument::_serialize_animations(Ref<GLTFState> state) {
    ...
    for (int32_t player_i = 0; player_i < state->animation_players.size();
         player_i++) {
        ...
        if (animation_names.size()) {
            for (int animation_name_i = 0;
                 animation_name_i < animation_names.size();
                 animation_name_i++) {
                _convert_animation(
                    state,
                    animation_player,
                    animation_names[animation_name_i]
                );
            }
        }
    }
    Array animations;
    for (GLTFAnimationIndex animation_i = 0;
         animation_i < state->animations.size();
         animation_i++) {
        Dictionary d;
        ...
        for (Map<int, GLTFAnimation::Track>::Element* track_i =
                 gltf_animation->get_tracks().front();
             track_i;
             track_i = track_i->next()) {
            ...
            if (track.translation_track.times.size()) {
                ...
                s["interpolation"] = interpolation_to_string(
                    track.translation_track.interpolation
                );
                ...
                Vector<Vector3> values =
                    Variant(track.translation_track.values);
            ...
            if (track.rotation_track.times.size()) {
                ...
                s["interpolation"] =
                    interpolation_to_string(track.rotation_track.interpolation);
            ...
            if (track.scale_track.times.size()) {
                ...
                s["interpolation"] =
                    interpolation_to_string(track.scale_track.interpolation);
            ...
            if (track.weight_tracks.size()) {
                ...
                for (int32_t track_idx = 0;
                     track_idx < track.weight_tracks.size();
                     track_idx++) {
                    int32_t last_time_index =
                        track.weight_tracks[track_idx].times.size() - 1;
                    length = MAX(
                        length,
                        track.weight_tracks[track_idx].times[last_time_index]
                    );
                }
                ...
                for (int32_t track_idx = 0;
                    ...
                    while (true) {
                        float weight = _interpolate_track<float>(
                            track.weight_tracks[track_idx].times,
                ...
                s["interpolation"] = interpolation_to_string(
                    track.weight_tracks[track.weight_tracks.size() - 1]
                        .interpolation
                );
                s["input"] =
                    _encode_accessor_as_floats(state, all_track_times, false);
                s["output"] =
                    _encode_accessor_as_floats(state, all_track_values, false);
    ...
    print_verbose(
        "glTF: Total animations '" + itos(state->animations.size()) + "'."
    );
...
Error GLTFDocument::_parse_animations(Ref<GLTFState> state) {
    ...
    for (GLTFAnimationIndex i = 0; i < animations.size(); i++) {
        ...
        if (d.has("name")) {
            ...
            if (name.begins_with("loop") || name.ends_with("loop")
                || name.begins_with("cycle") || name.ends_with("cycle")) {
        ...
        for (int j = 0; j < channels.size(); j++) {
            ...
            const Vector<float> times =
                _decode_accessor_as_floats(state, input, false);
            if (path == "translation") {
                const Vector<Vector3> translations =
                    _decode_accessor_as_vec3(state, output, false);
                track->translation_track.interpolation = interp;
                track->translation_track.times =
                    Variant(times); // convert via variant
                track->translation_track.values =
                    Variant(translations); // convert via variant
            } else if (path == "rotation") {
                const Vector<Quat> rotations =
                    _decode_accessor_as_quat(state, output, false);
                track->rotation_track.interpolation = interp;
                track->rotation_track.times =
                    Variant(times); // convert via variant
                track->rotation_track.values = rotations;
            } else if (path == "scale") {
                const Vector<Vector3> scales =
                    _decode_accessor_as_vec3(state, output, false);
                track->scale_track.interpolation = interp;
                track->scale_track.times = Variant(times); // convert via
                                                           // variant
                track->scale_track.values =
                    Variant(scales); // convert via variant
            } else if (path == "weights") {
                const Vector<float> weights =
                    _decode_accessor_as_floats(state, output, false);

                ERR_FAIL_INDEX_V(
                    state->nodes[node]->mesh,
                    state->meshes.size(),
                    ERR_PARSE_ERROR
                );
                ...
                const int expected_value_count =
                    times.size() * output_count * wc;
                ERR_CONTINUE_MSG(
                    weights.size() != expected_value_count,
                    "Invalid weight data, expected "
                        + itos(expected_value_count) + " weight values, got "
                        + itos(weights.size()) + " instead."
                );

                const int wlen = weights.size() / wc;
                for (int k = 0; k < wc;
                     k++) { // separate tracks, having them together is not such
                            // a good idea
...
void GLTFDocument::_assign_scene_names(Ref<GLTFState> state) {
    for (int i = 0; i < state->nodes.size(); i++) {
        ...
        // Any joints get unique names generated when the skeleton is made,
        // unique to the skeleton
        if (n->skeleton >= 0) {
            continue;
        }
    ...
    // Assign a unique name to the scene last to avoid naming conflicts with the
    // root
    state->scene_name = _gen_unique_name(state, state->scene_name);
}

BoneAttachment* GLTFDocument::_generate_bone_attachment(
    Ref<GLTFState> state,
    Skeleton* skeleton,
    const GLTFNodeIndex node_index,
    ...
    print_verbose(
        "glTF: Creating bone attachment for: " + gltf_node->get_name()
    );
...
GLTFMeshIndex GLTFDocument::_convert_mesh_to_gltf(
    Ref<GLTFState> state,
    ...
    int32_t blend_count = mesh->get_blend_shape_count();
    Vector<float> blend_weights;
    ...
    if (am != nullptr) {
        ...
        for (int32_t surface_i = 0; surface_i < mesh->get_surface_count();
             surface_i++) {
            Mesh::PrimitiveType primitive_type =
                mesh->surface_get_primitive_type(surface_i);
            Array arrays              = mesh->surface_get_arrays(surface_i);
    ...
    gltf_mesh.instance();
    Array instance_materials;
    for (int32_t surface_i = 0; surface_i < import_mesh->get_surface_count();
         surface_i++) {
...
Spatial* GLTFDocument::_generate_mesh_instance(
    Ref<GLTFState> state,
    Node* scene_parent,
    const GLTFNodeIndex node_index
) {
    ...
    for (int i = 0; i < mesh->get_blend_weights().size(); i++) {
        mi->set(
            "blend_shapes/" + mesh->get_mesh()->get_blend_shape_name(i),
            mesh->get_blend_weights()[i]
        );
    }
    return mi;
}

Spatial* GLTFDocument::_generate_light(
    Ref<GLTFState> state,
    Node* scene_parent,
    const GLTFNodeIndex node_index
) {
    ...
    if (intensity > 10) {
        // GLTF spec has the default around 1, but Blender defaults lights to
        // 100. The only sane way to handle this is to check where it came from
        // and handle it accordingly. If it's over 10, it probably came from
        // Blender.
        intensity /= 100;
    }
    ...
    if (l->type == "spot") {
        ...
        light->set_param(
            SpotLight::PARAM_SPOT_ANGLE,
            Math::rad2deg(l->outer_cone_angle)
        );
        light->set_color(l->color);
...
Camera* GLTFDocument::_generate_camera(
    Ref<GLTFState> state,
    Node* scene_parent,
    const GLTFNodeIndex node_index
) {
    ...
    if (c->get_perspective()) {
        camera
            ->set_perspective(c->get_fov_size(), c->get_znear(), c->get_zfar());
    } else {
        camera
            ->set_orthogonal(c->get_fov_size(), c->get_znear(), c->get_zfar());
    }

    return camera;
}

GLTFCameraIndex GLTFDocument::_convert_camera(
    Ref<GLTFState> state,
    Camera* p_camera
) {
...
GLTFLightIndex GLTFDocument::_convert_light(
    Ref<GLTFState> state,
    Light* p_light
) {
    ...
    if (cast_to<DirectionalLight>(p_light)) {
        ...
        // Range for directional lights is infinite in Rebel Engine.
        l->range     = FLT_MAX;
        ...
        // This equation is the inverse of the import equation (which has a
        // desmos link).
        float angle_ratio =
            1
...
void GLTFDocument::_convert_spatial(
    Ref<GLTFState> state,
    Spatial* p_spatial,
    Ref<GLTFNode> p_node
) {
...
Spatial* GLTFDocument::_generate_spatial(
    Ref<GLTFState> state,
    Node* scene_parent,
    const GLTFNodeIndex node_index
) {
...
void GLTFDocument::_convert_scene_node(
    Ref<GLTFState> state,
    Node* p_current,
    const GLTFNodeIndex p_gltf_parent,
    ...
    if (cast_to<MeshInstance>(p_current)) {
        ...
        _convert_bone_attachment_to_gltf(
            bone,
            state,
            p_gltf_parent,
            p_gltf_root,
        ...
        AnimationPlayer* animation_player =
            Object::cast_to<AnimationPlayer>(p_current);
        _convert_animation_player_to_gltf(
            animation_player,
            state,
            p_gltf_parent,
            p_gltf_root,
...
#ifdef MODULE_CSG_ENABLED
void GLTFDocument::_convert_csg_shape_to_gltf(
    CSGShape* p_current,
    ...
    for (int32_t surface_i = 0; surface_i < array_mesh->get_surface_count();
         surface_i++) {
        import_mesh->add_surface_from_arrays(
            Mesh::PRIMITIVE_TRIANGLES,
            array_mesh->surface_get_arrays(surface_i)
        );
    }
    gltf_mesh->set_mesh(import_mesh);
    GLTFMeshIndex mesh_i = state->meshes.size();
...
void GLTFDocument::_create_gltf_node(
    Ref<GLTFState> state,
    Node* p_scene_parent,
...
void GLTFDocument::_convert_animation_player_to_gltf(
    AnimationPlayer* animation_player,
    Ref<GLTFState> state,
    ...
    print_verbose(
        String("glTF: Converting animation player: ")
        + animation_player->get_name()
    );
}
...
#ifdef MODULE_GRIDMAP_ENABLED
void GLTFDocument::_convert_grid_map_to_gltf(
    GridMap* p_grid_map,
    GLTFNodeIndex p_parent_node_index,
    ...
    for (int32_t k = 0; k < cells.size(); k++) {
        ...
        import_mesh_node->set_mesh(
            p_grid_map->get_mesh_library()->get_item_mesh(cell)
        );
        ...
        cell_xform.basis.scale(Vector3(
        ...
        new_gltf_node->set_name(_gen_unique_name(
            state,
            p_grid_map->get_mesh_library()->get_item_name(cell)
        ));
    }
}
#endif // MODULE_GRIDMAP_ENABLED

void GLTFDocument::_convert_mult_mesh_instance_to_gltf(
    ...
    if (multi_mesh.is_valid()) {
        for (int32_t instance_i = 0;
             instance_i < multi_mesh->get_instance_count();
             instance_i++) {
            GLTFNode* new_gltf_node = memnew(GLTFNode);
            Transform transform;
            if (multi_mesh->get_transform_format() == MultiMesh::TRANSFORM_2D) {
                Transform2D xform_2d =
                    multi_mesh->get_instance_transform_2d(instance_i);
                transform.origin = Vector3(
                    xform_2d.get_origin().x,
                    0,
                    xform_2d.get_origin().y
                );
                real_t rotation = xform_2d.get_rotation();
                Quat quat(Vector3(0, 1, 0), rotation);
                Size2 scale = xform_2d.get_scale();
                transform.basis.set_quat_scale(
                    quat,
            ...
            if (mm.is_valid()) {
                Ref<ArrayMesh> mesh;
                mesh.instance();
                for (int32_t surface_i = 0; surface_i < mm->get_surface_count();
                     surface_i++) {
                    Array surface = mm->surface_get_arrays(surface_i);
                    mesh->add_surface_from_arrays(
                        mm->surface_get_primitive_type(surface_i),
                        surface
                    );
                }
                Ref<GLTFMesh> gltf_mesh;
            ...
            new_gltf_node->set_name(
                _gen_unique_name(state, p_multi_mesh_instance->get_name())
            );
            gltf_node->children.push_back(state->nodes.size());
            state->nodes.push_back(new_gltf_node);
        }
...
void GLTFDocument::_convert_skeleton_to_gltf(
    Skeleton* p_skeleton3d,
    Ref<GLTFState> state,
    GLTFNodeIndex p_parent_node_index,
    GLTFNodeIndex p_root_node_index,
    Ref<GLTFNode> gltf_node
    ...
    // GLTFSkeleton is only used to hold internal state data. It will not be
    ...
    for (BoneId bone_i = 0; bone_i < bone_count; bone_i++) {
        ...
        joint_node.instance();
        // Note that we cannot use _gen_unique_bone_name here, because glTF spec
        // requires all node names to be unique regardless of whether or not
        // they are used as joints.
        joint_node->set_name(
            _gen_unique_name(state, skeleton->get_bone_name(bone_i))
        );
        Transform xform =
        ...
        gltf_skeleton->bone_node.insert(bone_i, current_node_i);
    }
    for (BoneId bone_i = 0; bone_i < bone_count; bone_i++) {
        GLTFNodeIndex current_node_i = gltf_skeleton->bone_node[bone_i];
        BoneId parent_bone_id        = skeleton->get_bone_parent(bone_i);
        if (parent_bone_id == -1) {
            if (p_parent_node_index != -1) {
                state->nodes.write[current_node_i]->parent =
                    p_parent_node_index;
                state->nodes.write[p_parent_node_index]->children.push_back(
                    current_node_i
                );
            }
        } else {
            GLTFNodeIndex parent_node_i =
                gltf_skeleton->bone_node[parent_bone_id];
...
void GLTFDocument::_convert_bone_attachment_to_gltf(
    ...
    Skeleton* skeleton;
    // Note that relative transforms to external skeletons and pose overrides
    // are not supported.
    skeleton = cast_to<Skeleton>(p_bone_attachment->get_parent());
    GLTFSkeletonIndex skel_gltf_i = -1;
    if (skeleton != nullptr
        && state->skeleton3d_to_gltf_skeleton.has(skeleton->get_instance_id()
        )) {
        skel_gltf_i =
            state->skeleton3d_to_gltf_skeleton[skeleton->get_instance_id()];
    }
    ...
    for (int node_i = 0; node_i < p_bone_attachment->get_child_count();
         node_i++) {
        _convert_scene_node(
            state,
            p_bone_attachment->get_child(node_i),
            par_node_index,
            p_root_node_index
        );
    }
...
void GLTFDocument::_convert_mesh_instance_to_gltf(
    MeshInstance* p_scene_parent,
    Ref<GLTFState> state,
    Ref<GLTFNode> gltf_node
) {
    GLTFMeshIndex gltf_mesh_index =
        _convert_mesh_to_gltf(state, p_scene_parent);
    if (gltf_mesh_index != -1) {
        gltf_node->mesh = gltf_mesh_index;
    }
}

void GLTFDocument::_generate_scene_node(
    Ref<GLTFState> state,
    Node* scene_parent,
    Spatial* scene_root,
    const GLTFNodeIndex node_index
    ...
    // If we have an active skeleton, and the node is node skinned, we need to
    // create a bone attachment
    if (non_bone_parented_to_skeleton && gltf_node->skin < 0) {
        // Bone Attachment - Parent Case
        BoneAttachment* bone_attachment = _generate_bone_attachment(
            state,
            active_skeleton,
            node_index,
            gltf_node->parent
        ...
        // There is no gltf_node that represent this, so just directly create a
        // unique name
        bone_attachment->set_name(_gen_unique_name(state, "BoneAttachment"));

        // We change the scene_parent to our bone attachment now. We do not set
        // current_node because we want to make the node and attach it to the
        // bone_attachment
...
void GLTFDocument::_generate_skeleton_bone_node(
    Ref<GLTFState> state,
    Node* scene_parent,
    Spatial* scene_root,
    const GLTFNodeIndex node_index
    ...
    Skeleton* skeleton = state->skeletons[gltf_node->skeleton]->skeleton;
    // In this case, this node is already a bone in skeleton.
    const bool is_skinned_mesh = (gltf_node->skin >= 0 && gltf_node->mesh >= 0);
    const bool requires_extra_node =
        (gltf_node->mesh >= 0 || gltf_node->camera >= 0 || gltf_node->light >= 0
        );
    ...
    if (active_skeleton != skeleton) {
        if (active_skeleton) {
            ...
            // There is no gltf_node that represent this, so just directly
            // create a unique name
            bone_attachment->set_name(_gen_unique_name(state, "BoneAttachment")
            );

            // We change the scene_parent to our bone attachment now. We do not
            // set current_node because we want to make the node and attach it
            // to the bone_attachment
            scene_parent = bone_attachment;
            WARN_PRINT(vformat(
                "glTF: Generating scene detected direct parented Skeletons at "
                "node %d",
                node_index
            ));
        }
    ...
    if (requires_extra_node) {
        ...
        if (gltf_node->mesh >= 0) {
            current_node =
                _generate_mesh_instance(state, scene_parent, node_index);
        } else if (gltf_node->camera >= 0) {
            current_node = _generate_camera(state, scene_parent, node_index);
        } else if (gltf_node->light >= 0) {
            current_node = _generate_light(state, scene_parent, node_index);
        ...
        if (state->use_legacy_names) {
            current_node->set_name(
                _legacy_validate_node_name(gltf_node->get_name())
            );
        } else {
            current_node->set_name(gltf_node->get_name());
        }
...
struct EditorSceneImporterGLTFInterpolate<Quat> {
    Quat lerp(const Quat& a, const Quat& b, const float c) const {
        ERR_FAIL_COND_V_MSG(
            !a.is_normalized(),
            Quat(),
            "The quaternion \"a\" must be normalized."
        );
        ERR_FAIL_COND_V_MSG(
            !b.is_normalized(),
            Quat(),
            "The quaternion \"b\" must be normalized."
        );
    ...
    Quat catmull_rom(
        ...
        ERR_FAIL_COND_V_MSG(
            !p1.is_normalized(),
            Quat(),
            "The quaternion \"p1\" must be normalized."
        );
        ERR_FAIL_COND_V_MSG(
            !p2.is_normalized(),
            Quat(),
            "The quaternion \"p2\" must be normalized."
        );
    ...
    Quat bezier(
        const Quat start,
        const Quat control_1,
        const Quat control_2,
        const Quat end,
        const float t
    ) {
        ERR_FAIL_COND_V_MSG(
...
template <class T>
T GLTFDocument::_interpolate_track(
    ...
    if (p_times.size()
        != (p_values.size()
            / (p_interp == GLTFAnimation::INTERP_CUBIC_SPLINE ? 3 : 1))) {
        ERR_PRINT_ONCE(
            "The interpolated values are not corresponding to its times."
        );
        return p_values[0];
    }
...
void GLTFDocument::_import_animation(
    Ref<GLTFState> state,
    AnimationPlayer* ap,
    ...
    if (name.empty()) {
        // No node represent these, and they are not in the hierarchy, so just
        // make a unique name
    ...
    for (Map<int, GLTFAnimation::Track>::Element* track_i =
             anim->get_tracks().front();
         track_i;
         track_i = track_i->next()) {
        const GLTFAnimation::Track& track = track_i->get();
        // need to find the path: for skeletons, weight tracks will affect the
        // mesh
        NodePath node_path;
        ...
        Map<GLTFNodeIndex, Node*>::Element* node_element =
            state->scene_nodes.find(node_index);
        ERR_CONTINUE_MSG(
            node_element == nullptr,
            vformat("Unable to find node %d for animation", node_index)
        );
        node_path = root->get_path_to(node_element->get());

        if (gltf_node->skeleton >= 0) {
            const Skeleton* sk =
                state->skeletons[gltf_node->skeleton]->skeleton;
        ...
        const bool transform_affects_skinned_mesh_instance =
            gltf_node->skeleton < 0 && gltf_node->skin >= 0;
        if ((track.rotation_track.values.size()
             || track.translation_track.values.size()
             || track.scale_track.values.size())
            && !transform_affects_skinned_mesh_instance) {
            ...
            while (true) {
                ...
                if (track.rotation_track.times.size()) {
                    rot = _interpolate_track<Quat>(
                        track.rotation_track.times,
                        track.rotation_track.values,
                        time,
                        track.rotation_track.interpolation
                    );
                }

                if (track.scale_track.times.size()) {
                    scale = _interpolate_track<Vector3>(
                        track.scale_track.times,
                        track.scale_track.values,
                        time,
                        track.scale_track.interpolation
                    );
                }
                ...
                if (gltf_node->skeleton >= 0) {
                    ...
                    const Skeleton* skeleton =
                        state->skeletons[gltf_node->skeleton]->skeleton;
                    const int bone_idx =
                        skeleton->find_bone(gltf_node->get_name());
                    xform = skeleton->get_bone_rest(bone_idx).affine_inverse()
                          * xform;
        ...
        for (int i = 0; i < track.weight_tracks.size(); i++) {
            ERR_CONTINUE(
                gltf_node->mesh < 0 || gltf_node->mesh >= state->meshes.size()
            );
            ...
            // Only LINEAR and STEP (NEAREST) can be supported out of the box by
            // Rebel's Animation, the other modes have to be baked.
            GLTFAnimation::Interpolation gltf_interp =
                track.weight_tracks[i].interpolation;
            if (gltf_interp == GLTFAnimation::INTERP_LINEAR
                || gltf_interp == GLTFAnimation::INTERP_STEP) {
...
void GLTFDocument::_convert_mesh_instances(Ref<GLTFState> state) {
    for (GLTFNodeIndex mi_node_i = 0; mi_node_i < state->nodes.size();
         ++mi_node_i) {
        ...
        Map<GLTFNodeIndex, Node*>::Element* mi_element =
            state->scene_nodes.find(mi_node_i);
        if (!mi_element) {
            continue;
        }
        ...
        if (state->skeleton3d_to_gltf_skeleton.has(skeleton->get_instance_id()
            )) {
            // This is a skinned mesh. If the mesh has no ARRAY_WEIGHTS or
            // ARRAY_BONES, it will be invisible.
            const GLTFSkeletonIndex skeleton_gltf_i =
                state->skeleton3d_to_gltf_skeleton[skeleton->get_instance_id()];
            ...
            ObjectID gltf_skel_key    = skeleton->get_instance_id();
            GLTFSkinIndex skin_gltf_i = -1;
            GLTFNodeIndex root_gltf_i = -1;
            if (!gltf_skeleton->roots.empty()) {
                root_gltf_i = gltf_skeleton->roots[0];
            }
            if (state->skin_and_skeleton3d_to_gltf_skin.has(gltf_skin_key)
                && state->skin_and_skeleton3d_to_gltf_skin[gltf_skin_key].has(
                    gltf_skel_key
                )) {
                skin_gltf_i =
                    state->skin_and_skeleton3d_to_gltf_skin[gltf_skin_key]
                                                           [gltf_skel_key];
            } else {
                if (skin.is_null()) {
                    // Note that gltf_skin_key should remain null, so these can
                    // share a reference.
                ...
                for (int bind_i = 0, cnt = skin->get_bind_count(); bind_i < cnt;
                    ...
                    GLTFNodeIndex skeleton_bone_i =
                        gltf_skeleton->joints[bone_i];
...
float GLTFDocument::solve_metallic(
    ...
    const float b =
        diffuse * p_one_minus_specular_strength / (1.0f - p_dielectric_specular)
        + specular - 2.0f * p_dielectric_specular;
...
float GLTFDocument::get_perceived_brightness(const Color p_color) {
    const Color coeff =
        Color(R_BRIGHTNESS_COEFF, G_BRIGHTNESS_COEFF, B_BRIGHTNESS_COEFF);
...
void GLTFDocument::_process_mesh_instances(
    ...
    for (GLTFNodeIndex node_i = 0; node_i < state->nodes.size(); ++node_i) {
        ...
        if (node->skin >= 0 && node->mesh >= 0) {
            ...
            Map<GLTFNodeIndex, Node*>::Element* mi_element =
                state->scene_nodes.find(node_i);
            ERR_CONTINUE_MSG(
                mi_element == nullptr,
                vformat("Unable to find node %d", node_i)
            );
            ...
            ERR_CONTINUE_MSG(
                mi == nullptr,
                vformat(
                    "Unable to cast node %d of type %s to MeshInstance",
                    node_i,
                    mi_element->get()->get_class_name()
                )
            );
            ...
            Ref<GLTFSkeleton> gltf_skeleton = state->skeletons.write[skel_i];
            Skeleton* skeleton              = gltf_skeleton->skeleton;
            ERR_CONTINUE_MSG(
                skeleton == nullptr,
                vformat(
                    "Unable to find Skeleton for node %d skin %d",
                    node_i,
                    skin_i
                )
            );
            ...
            mi->set_skin(state->skins.write[skin_i]->skin);
...
GLTFAnimation::Track GLTFDocument::_convert_animation_track(
    Ref<GLTFState> state,
    ...
    Animation::InterpolationType interpolation =
        p_animation->track_get_interpolation_type(p_track_i);

    GLTFAnimation::Interpolation gltf_interpolation =
    ...
    if (track_type == Animation::TYPE_TRANSFORM) {
        ...
        for (int32_t key_i = 0; key_i < key_count; key_i++) {
            ...
            Quat rotation;
            Vector3 scale;
            Error err = p_animation->transform_track_get_key(
                p_track_i,
                key_i,
                &translation,
                &rotation,
        ...
        for (int32_t key_i = 0; key_i < key_count; key_i++) {
            Transform xform =
                p_animation->track_get_key_value(p_track_i, key_i);
            p_track.translation_track.values.write[key_i] = xform.get_origin();
            p_track.rotation_track.values.write[key_i] =
                xform.basis.get_rotation_quat();
            p_track.scale_track.values.write[key_i] = xform.basis.get_scale();
        ...
        if (path.find("/scale") != -1) {
            const int32_t keys =
                p_animation->track_get_key_time(p_track_i, key_count - 1)
                * BAKE_FPS;
            if (!p_track.scale_track.times.size()) {
                ...
                for (int32_t key_i = 0; key_i < keys; key_i++) {
                    p_track.scale_track.values.write[key_i] =
            ...
            for (int32_t key_i = 0; key_i < keys; key_i++) {
                ...
                if (path.find("/scale:x") != -1) {
                    bezier_track.x = p_animation->bezier_track_interpolate(
                        p_track_i,
                        key_i / BAKE_FPS
                    );
                    bezier_track.x =
                        p_bone_rest.affine_inverse().basis.get_scale().x
                        * bezier_track.x;
                } else if (path.find("/scale:y") != -1) {
                    bezier_track.y = p_animation->bezier_track_interpolate(
                        p_track_i,
                        key_i / BAKE_FPS
                    );
                    bezier_track.y =
                        p_bone_rest.affine_inverse().basis.get_scale().y
                        * bezier_track.y;
                } else if (path.find("/scale:z") != -1) {
                    bezier_track.z = p_animation->bezier_track_interpolate(
                        p_track_i,
                        key_i / BAKE_FPS
                    );
                    bezier_track.z =
                        p_bone_rest.affine_inverse().basis.get_scale().z
                        * bezier_track.z;
                }
            ...
            for (int32_t key_i = 0; key_i < keys; key_i++) {
                ...
                if (path.find("/translation:x") != -1) {
                    ...
                    bezier_track.x =
                        p_bone_rest.affine_inverse().origin.x * bezier_track.x;
                    ...
                    bezier_track.y =
                        p_bone_rest.affine_inverse().origin.y * bezier_track.y;
                    ...
                    bezier_track.z =
                        p_bone_rest.affine_inverse().origin.z * bezier_track.z;
                }
...
void GLTFDocument::_convert_animation(
    Ref<GLTFState> state,
    AnimationPlayer* ap,
    ...
    for (int32_t track_i = 0; track_i < animation->get_track_count();
         track_i++) {
        ...
        if (String(orig_track_path).find(":translation") != -1) {
            ...
            for (Map<GLTFNodeIndex, Node*>::Element* translation_scene_node_i =
                 ...
                 translation_scene_node_i = translation_scene_node_i->next()) {
                if (translation_scene_node_i->get() == node) {
                    GLTFNodeIndex node_index = translation_scene_node_i->key();
                    Map<int, GLTFAnimation::Track>::Element*
                        translation_track_i =
                            gltf_animation->get_tracks().find(node_index);
                    GLTFAnimation::Track track;
                    if (translation_track_i) {
                        track = translation_track_i->get();
                    }
                    track = _convert_animation_track(
                        state,
                        track,
                        animation,
                        Transform(),
            ...
            for (Map<GLTFNodeIndex, Node*>::Element*
                 ...
                 rotation_degree_scene_node_i =
                     rotation_degree_scene_node_i->next()) {
                if (rotation_degree_scene_node_i->get() == node) {
                    GLTFNodeIndex node_index =
                        rotation_degree_scene_node_i->key();
                    Map<int, GLTFAnimation::Track>::Element*
                        rotation_degree_track_i =
                            gltf_animation->get_tracks().find(node_index);
                    GLTFAnimation::Track track;
            ...
            for (Map<GLTFNodeIndex, Node*>::Element* scale_scene_node_i =
                     state->scene_nodes.front();
                 scale_scene_node_i;
                 scale_scene_node_i = scale_scene_node_i->next()) {
                if (scale_scene_node_i->get() == node) {
                    GLTFNodeIndex node_index = scale_scene_node_i->key();
                    Map<int, GLTFAnimation::Track>::Element* scale_track_i =
                        gltf_animation->get_tracks().find(node_index);
                    GLTFAnimation::Track track;
                    if (scale_track_i) {
            ...
            for (Map<GLTFNodeIndex, Node*>::Element* transform_track_i =
                     state->scene_nodes.front();
                 transform_track_i;
                 transform_track_i = transform_track_i->next()) {
                if (transform_track_i->get() == node) {
                    ...
                    track = _convert_animation_track(
                        ...
                        Transform(),
                        track_i,
                        transform_track_i->key()
                    );
                    gltf_animation->get_tracks().insert(
                        transform_track_i->key(),
                        track
                    );
                }
            ...
            for (Map<GLTFNodeIndex, Node*>::Element* mesh_track_i =
                     state->scene_nodes.front();
                 mesh_track_i;
                 mesh_track_i = mesh_track_i->next()) {
            ...
            Map<int, GLTFAnimation::Track>& tracks =
                gltf_animation->get_tracks();
            GLTFAnimation::Track track =
                gltf_animation->get_tracks().has(mesh_index)
                    ? gltf_animation->get_tracks()[mesh_index]
                    : GLTFAnimation::Track();
            if (!tracks.has(mesh_index)) {
                for (int32_t shape_i = 0;
                     shape_i < mesh->get_blend_shape_count();
                     shape_i++) {
                    String shape_name = mesh->get_blend_shape_name(shape_i);
                    NodePath shape_path =
                        String(path) + ":blend_shapes/" + shape_name;
                    ...
                    Animation::InterpolationType interpolation =
                        animation->track_get_interpolation_type(track_i);
                    GLTFAnimation::Interpolation gltf_interpolation =
                        GLTFAnimation::INTERP_LINEAR;
                    if (interpolation
            ...
            const NodePath node_path = node_suffix[0];
            const String suffix      = node_suffix[1];
            Node* node         = ap->get_parent()->get_node_or_null(node_path);
            Skeleton* skeleton = nullptr;
            GLTFSkeletonIndex skeleton_gltf_i = -1;
            for (GLTFSkeletonIndex skeleton_i = 0;
                 skeleton_i < state->skeletons.size();
                 skeleton_i++) {
                if (state->skeletons[skeleton_i]->skeleton
                    == cast_to<Skeleton>(node)) {
                    skeleton        = state->skeletons[skeleton_i]->skeleton;
                    skeleton_gltf_i = skeleton_i;
                    ERR_CONTINUE(!skeleton);
                    Ref<GLTFSkeleton> skeleton_gltf =
                        state->skeletons[skeleton_gltf_i];
                    int32_t bone = skeleton->find_bone(suffix);
                    ERR_CONTINUE(bone == -1);
                    Transform xform = skeleton->get_bone_rest(bone);
                    if (!skeleton_gltf->bone_node.has(bone)) {
                        continue;
                    }
                    GLTFNodeIndex node_i = skeleton_gltf->bone_node[bone];
                    Map<int, GLTFAnimation::Track>::Element* property_track_i =
                        gltf_animation->get_tracks().find(node_i);
                    GLTFAnimation::Track track;
                    if (property_track_i) {
            ...
            for (int32_t node_i = 0;
                ...
                for (Map<GLTFNodeIndex, Node*>::Element* scene_node_i =
                         state->scene_nodes.front();
                     scene_node_i;
                     scene_node_i = scene_node_i->next()) {
                    if (scene_node_i->get() == node) {
                        GLTFNodeIndex node_index = scene_node_i->key();
                        Map<int, GLTFAnimation::Track>::Element* node_track_i =
                            gltf_animation->get_tracks().find(node_index);
                        GLTFAnimation::Track track;
                        if (node_track_i) {
...
Dictionary GLTFDocument::_serialize_texture_transform_uv2(
    Ref<SpatialMaterial> p_material
) {
    ...
    if (mat.is_valid()) {
        ...
        // Rebel doesn't support texture rotation
        extension["KHR_texture_transform"] = texture_transform;
    }
    return extension;
}

Dictionary GLTFDocument::_serialize_texture_transform_uv1(
    Ref<SpatialMaterial> p_material
) {
    Dictionary extension;
...
Error GLTFDocument::_serialize_file(Ref<GLTFState> state, const String p_path) {
    ...
    if (p_path.to_lower().ends_with("glb")) {
        ...
        f->store_32(
            header_size + chunk_header_size + text_chunk_length
            + chunk_header_size + binary_chunk_length
        ); // length
        f->store_32(text_chunk_length);
        f->store_32(text_chunk_type);
        f->store_buffer((uint8_t*)&cs[0], cs.length());
        for (uint32_t pad_i = text_data_length; pad_i < text_chunk_length;
             pad_i++) {
            f->store_8(' ');
        }
        ...
        for (uint32_t pad_i = binary_data_length; pad_i < binary_chunk_length;
             pad_i++) {
            f->store_8(0);
        }
...
Error GLTFDocument::_parse_gltf_extensions(Ref<GLTFState> state) {
    ...
    if (state->json.has("extensionsRequired")
        && state->json["extensionsRequired"].get_type() == Variant::ARRAY) {
        Array extensions_required = state->json["extensionsRequired"];
        if (extensions_required.find("KHR_draco_mesh_compression") != -1) {
            ERR_PRINT(
                "glTF2 extension KHR_draco_mesh_compression is not supported."
            );
