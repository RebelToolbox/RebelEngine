...
class BindingsGenerator {
    ...
    struct MethodInterface {
        ...
        /**
         * Determines if the method visibility is 'internal' (visible only to
         * files in the same assembly). Currently, we only use this for methods
         * that are not meant to be exposed, but are required by properties as
         * getters or setters. Methods that are not meant to be exposed are
         * those that begin with underscore and are not virtual.
         */
        bool is_internal;
    ...
    struct TypeInterface {
        ...
        /**
         * Used only by Object-derived types.
         * Determines if the C# class owns the native handle and must free it
         * somehow when disposed. e.g.: Reference types must notify when the C#
         * instance is disposed, for proper refcounting.
         */
        bool memory_own;

        /**
         * This must be set to true for any struct bigger than 32-bits. Those
         * cannot be passed/returned by value with internal calls, so we must
         * use pointers instead. Returns must be replace with out parameters. In
         * this case, [c_out] and [cs_out] must have a different format,
         * explained below. The Mono IL interpreter icall trampolines don't
         * support passing structs bigger than 32-bits by value (at least not on
         * WASM).
         */
        bool ret_as_byref_arg;

        // !! The comments of the following fields make reference to other
        // fields via square brackets, e.g.: [field_name]
        // !! When renaming those fields, make sure to rename their references
        // in the comments
        ...
        /**
         * One or more statements that manipulate the parameter before being
         * passed as argument of a ptrcall. If the statement adds a local that
         * must be passed as the argument instead of the parameter, the name of
         * that local must be specified with [c_arg_in]. For variadic methods,
         * this field is required and, if empty, [DEFAULT_VARARG_C_IN] is used
         * instead. Formatting elements: %0: [c_type] of the parameter %1: name
         * of the parameter
         */
        String c_in;
        ...
        /**
         * One or more statements that determine how a variable of this type is
         * returned from a function. It must contain the return statement(s).
         * Formatting elements:
         * %0: [c_type_out] of the return type
         * %1: name of the variable to be returned
         * %2: [name] of the return type
         * ---------------------------------------
         * If [ret_as_byref_arg] is true, the format is different. Instead of
         * using a return statement, the value must be assigned to a parameter.
         * This type of this parameter is a pointer to [c_type_out]. Formatting
         * elements: %0: [c_type_out] of the return type %1: name of the
         * variable to be returned %2: [name] of the return type %3: name of the
         * parameter that must be assigned the return value
         */
        String c_out;

        /**
         * The actual expected type, as seen (in most cases) in Variant copy
         * constructors Used for the type of the return variable and to format
         * [c_in]. The value must be the following depending of the type:
         ...
         * Reference types override this for the type of the return variable:
         * Ref<Reference>
         */
        String c_type;
        ...
        /**
         * An expression that overrides the way the parameter is passed to the
         * internal call. If empty, the parameter is passed as is. Formatting
         * elements: %0 or %s: name of the parameter
         */
        String cs_in;

        /**
         * One or more statements that determine how a variable of this type is
         * returned from a method. It must contain the return statement(s).
         * Formatting elements:
        ...
        /**
         * Type used for method signatures, both for parameters and the return
         * type. Same as [proxy_name] except for variable arguments (VarArg) and
         * collections (which include the namespace).
         */
        String cs_type;
        ...
        const PropertyInterface* find_property_by_name(const StringName& p_cname
        ) const {
            for (const List<PropertyInterface>::Element* E = properties.front();
                 E;
                 E = E->next()) {
                if (E->get().cname == p_cname) {
                    return &E->get();
        ...
        const PropertyInterface* find_property_by_proxy_name(
            const String& p_proxy_name
        ) const {
            for (const List<PropertyInterface>::Element* E = properties.front();
                 E;
    ...
    public:
        ...
        static TypeInterface create_object_type(
            const StringName& p_cname,
            ClassDB::APIType p_api_type
        ) {
            TypeInterface itype;
        ...
        static void create_placeholder_type(
        ...
        static void postsetup_enum_type(TypeInterface& r_enum_itype) {
            ...
            // to apply any of the changes done here to the 'uint32_t' type
            // interface as well.
    ...
    struct InternalCall {
        String name;
        String im_type_out; // Return type for the C# method declaration. Also
                            // used as companion of [unique_siq]
    ...
    const List<InternalCall>::Element* find_icall_by_name(
        const String& p_name,
        const List<InternalCall>& p_list
    ) {
    ...
    const ConstantInterface* find_constant_by_name(
        const String& p_name,
    ...
    void _apply_prefix_to_enum_constants(
        EnumInterface& p_ienum,
        int p_prefix_length
    );
...
public:
    ...
    BindingsGenerator() : log_print_enabled(true), initialized(false) {
        _initialize();
    }
};
