...
namespace RebelTools.ProjectEditor
{
    ...
    public static class ProjectUtils
    {
        ...
        public static void MigrateToProjectSdksStyle(MSBuildProject project, string projectName)
        {
            ...
            root.Sdk = $"{ProjectGenerator.RebelSdkNameToUse}/{ProjectGenerator.RebelSdkVersionToUse}";
            ...
            // only remove elements that could potentially cause issues with the Rebel.NET.Sdk.
        ...
        public static void EnsureRebelSdkIsUpToDate(MSBuildProject project)
        {
            string rebelSdkAttrValue = $"{ProjectGenerator.RebelSdkNameToUse}/{ProjectGenerator.RebelSdkVersionToUse}";
            ...
            if (!string.IsNullOrEmpty(rootSdk))
            {
                ...
                if (rootSdk.Equals(rebelSdkAttrValue, StringComparison.OrdinalIgnoreCase))
                    return;

                // We also allow higher versions as long as the major and minor are the same.
                var semVerToUse = SemVersion.Parse(ProjectGenerator.RebelSdkVersionToUse);
                var rebelSdkAttrLaxValueRegex = new Regex($@"^{ProjectGenerator.RebelSdkNameToUse}/(?<ver>.*)$");

                var match = rebelSdkAttrLaxValueRegex.Match(rootSdk);
