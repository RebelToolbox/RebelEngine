...
namespace RebelTools.IdeMessaging.CLI
{
    internal static class Program
    {
        ...
        private static async Task<int> StartAsync(string[] args, Stream inputStream, Stream outputStream)
        {
            ...
            try
            {
                ...
                string rebelProjectDir = args[0];

                if (!Directory.Exists(rebelProjectDir))
                {
                    Logger.LogError($"The specified Rebel project directory does not exist: {rebelProjectDir}");
                    return 1;
                }

                var forwarder = new ForwarderMessageHandler(outputWriter);

                using (var fwdClient = new Client("VisualStudioCode", rebelProjectDir, forwarder, Logger))
                {
                    fwdClient.Start();
