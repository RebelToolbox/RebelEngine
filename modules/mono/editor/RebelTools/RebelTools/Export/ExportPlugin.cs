...
namespace RebelTools.Export
{
    public class ExportPlugin : EditorExportPlugin
    {
        ...
        private void AddI18NAssemblies(Rebel.Collections.Dictionary<string, string> assemblies, string bclDir)
        {
        ...
        public void RegisterExportSettings()
        {
            ...
            ProjectSettings.AddPropertyInfo(new Rebel.Collections.Dictionary
            {
        ...
        public override void _ExportFile(string path, string type, string[] features)
        {
            ...
            if (!includeScriptsContent)
            {
                ...
                // Sadly, Rebel prints errors when adding an empty file (nothing goes wrong, it's just noise).
                // Because of this, we add a file which contains a line break.
                AddFile(path, System.Text.Encoding.UTF8.GetBytes("\n"), remap: false);

                // Tell the Rebel exporter that we already took care of the file
                Skip();
            }
        }
        ...
        private void _ExportBeginImpl(string[] features, bool isDebug, string path, int flags)
        {
            ...
            var assemblies = new Rebel.Collections.Dictionary<string, string>();

            string projectDllName = RebelSharpEditor.ProjectAssemblyName;
            string projectDllSrcDir = Path.Combine(RebelSharpDirs.ResTempAssembliesBaseDir, buildConfig);
            ...
            {
                string rebelAndroidExtProfileDir = GetBclProfileDir("rebel_android_ext");
                string monoAndroidAssemblyPath = Path.Combine(rebelAndroidExtProfileDir, "Mono.Android.dll");
            ...
            string resAssembliesDir = Path.Combine(RebelSharpDirs.ResAssembliesBaseDir, apiConfig);
        ...
        public override void _ExportEnd()
        {
            ...
            if (!string.IsNullOrEmpty(_maybeLastExportError)) // Check empty as well, because it's set to empty after hot-reloading
            {
                ...
                RebelSharpEditor.Instance.ShowErrorDialog(lastExportError, "Failed to export C# project");
            }
        }
        ...
        {
            // MacOS export templates are contained in a zip, so we place our custom template inside it and let Rebel do the rest.
        ...
        /// <summary>
        /// Determines whether the BCL bundled with the Rebel editor can be used for the target platform,
        ...
        private static extern void internal_GetExportedAssemblyDependencies(Rebel.Collections.Dictionary<string, string> initialAssemblies,
            string buildConfig, string customBclDir, Rebel.Collections.Dictionary<string, string> dependencyAssemblies);
    }
}
