...
using RebelTools.Export;
using RebelTools.Utils;
using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;
using RebelTools.Build;
using RebelTools.Ides;
using RebelTools.Ides.Rider;
using RebelTools.Internals;
using RebelTools.ProjectEditor;
...
{
    public class RebelSharpEditor : EditorPlugin, ISerializationListener
    {
        ...
        public RebelIdeManager RebelIdeManager { get; private set; }
        ...
        private bool CreateProjectSolution()
        {
            using (var pr = new EditorProgress("create_csharp_solution", "Generating solution...".TTR(), 3))
            {
                ...
                if (guid.Length > 0)
                {
                    ...
                    pr.Step("Updating Rebel API assemblies...".TTR());
                    ...
                    {
                        ShowErrorDialog("Failed to update the Rebel API assemblies: " + debugApiAssembliesError);
                        return false;
                    }
                    ...
                    {
                        ShowErrorDialog("Failed to update the Rebel API assemblies: " + releaseApiAssembliesError);
                        return false;
                    }
        ...
        private void _FileSystemDockFileMoved(string file, string newFile)
        {
            ...
            {
                ProjectUtils.RenameItemInProjectChecked(RebelSharpDirs.ProjectCsProjPath, "Compile",
        ...
        private void _FileSystemDockFileRemoved(string file)
        {
            if (Path.GetExtension(file) == Internal.CSharpLanguageExtension)
            {
                ProjectUtils.RemoveItemFromProjectChecked(RebelSharpDirs.ProjectCsProjPath, "Compile",
        ...
        {
            if (File.Exists(RebelSharpDirs.ProjectCsProjPath))
            {
                ProjectUtils.RenameItemsToNewFolderInProjectChecked(RebelSharpDirs.ProjectCsProjPath, "Compile",
        ...
        private void _FileSystemDockFolderRemoved(string oldFolder)
        {
            if (File.Exists(RebelSharpDirs.ProjectCsProjPath))
            {
                ProjectUtils.RemoveItemsInFolderFromProjectChecked(RebelSharpDirs.ProjectCsProjPath, "Compile",
        ...
        public Error OpenInExternalEditor(Script script, int line, int col)
        {
            ...
            switch (editorId)
            {
                ...
                case ExternalEditorId.VisualStudio:
                {
                    ...
                    {
                        RebelSharpDirs.ProjectSlnPath,
                        line >= 0 ? $"{scriptPath};{line + 1};{col + 1}" : scriptPath
                    };

                    string command = Path.Combine(RebelSharpDirs.DataEditorToolsDir, "RebelTools.OpenVisualStudio.exe");
                ...
                {
                    string scriptPath = ProjectSettings.GlobalizePath(script.ResourcePath);
                    RiderPathManager.OpenFile(RebelSharpDirs.ProjectSlnPath, scriptPath, line);
                    return Error.Ok;
                }
                case ExternalEditorId.MonoDevelop:
                {
                    ...
                    RebelIdeManager.LaunchIdeAsync().ContinueWith(launchTask =>
                    {
        ...
        private void ApplyNecessaryChangesToSolution()
        {
            try
            {
                ...
                DotNetSolution.MigrateFromOldConfigNames(RebelSharpDirs.ProjectSlnPath);

                var msbuildProject = ProjectUtils.Open(RebelSharpDirs.ProjectCsProjPath)
                ...
                ProjectUtils.EnsureRebelSdkIsUpToDate(msbuildProject);

                if (msbuildProject.HasUnsavedChanges)
                {
                    // Save a copy of the project before replacing it
                    FileUtils.SaveBackupCopy(RebelSharpDirs.ProjectCsProjPath);
        ...
        public override void EnablePlugin()
        {
            ...
            if (File.Exists(RebelSharpDirs.ProjectSlnPath) && File.Exists(RebelSharpDirs.ProjectCsProjPath))
            {
            ...
            _editorSettings.AddPropertyInfo(new Rebel.Collections.Dictionary
            {
            ...
            RebelIdeManager = new RebelIdeManager();
            AddChild(RebelIdeManager);
        }
        ...
        public static RebelSharpEditor Instance { get; private set; }

        [UsedImplicitly]
        private RebelSharpEditor()
        {
        }
    }
}
