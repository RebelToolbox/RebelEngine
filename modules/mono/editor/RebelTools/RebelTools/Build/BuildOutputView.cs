...
namespace RebelTools.Build
{
    public class BuildOutputView : VBoxContainer, ISerializationListener
    {
        ...
        {
            using (var file = new Rebel.File())
            {
                try
                {
                    Error openError = file.Open(csvFile, Rebel.File.ModeFlags.Read);
        ...
        private void IssueActivated(int idx)
        {
            ...
            string file = Path.Combine(projectDir.SimplifyRebelPath(), issue.File.SimplifyRebelPath());
