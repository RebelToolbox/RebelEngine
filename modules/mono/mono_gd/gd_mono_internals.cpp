...
void tie_managed_to_unmanaged(MonoObject* managed, Object* unmanaged) {
    ...
    // need to have a CSharpScript in order for their methods to be callable
    ...
    if (native == klass) {
        ...
        // doing this is that this instance could outlive CSharpLanguage, which
        // would be problematic when using a script. See:
        // https://github.com/godotengine/godot/issues/25621
        ...
        // The object was just created, no script instance binding should have
        // been attached
        CRASH_COND(unmanaged->has_script_instance_binding(
        ...
        void* data;
        {
            ...
            data = (void*)CSharpLanguage::get_singleton()
                       ->insert_script_binding(unmanaged, script_binding);
        }

        // Should be thread safe because the object was just created and nothing
        // else should be referencing it
        unmanaged->set_script_instance_binding(
            CSharpLanguage::get_singleton()->get_language_index(),
            data
        );
    ...
    Ref<CSharpScript> script =
        CSharpScript::create_for_managed_type(klass, native);

    CRASH_COND(script.is_null());

    ScriptInstance* si = CSharpInstance::create_for_managed_type(
        unmanaged,
        script.ptr(),
        gchandle
    );
...
void unhandled_exception(MonoException* p_exc) {
    ...
    if (GDMono::get_singleton()->get_unhandled_exception_policy()
        == GDMono::POLICY_TERMINATE_APP) {
        // Too bad 'mono_invoke_unhandled_exception_hook' is not exposed to
        // embedders
