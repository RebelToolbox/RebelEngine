...
#define BOX_DOUBLE(x)                                                          \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(double), &x)
#define BOX_FLOAT(x)                                                           \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(float), &x)
#define BOX_INT64(x)                                                           \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(int64_t), &x)
#define BOX_INT32(x)                                                           \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(int32_t), &x)
#define BOX_INT16(x)                                                           \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(int16_t), &x)
#define BOX_INT8(x)                                                            \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(int8_t), &x)
#define BOX_UINT64(x)                                                          \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(uint64_t), &x)
#define BOX_UINT32(x)                                                          \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(uint32_t), &x)
#define BOX_UINT16(x)                                                          \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(uint16_t), &x)
#define BOX_UINT8(x)                                                           \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(uint8_t), &x)
#define BOX_BOOLEAN(x)                                                         \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(bool), &x)
#define BOX_PTR(x)                                                             \
    mono_value_box(mono_domain_get(), CACHED_CLASS_RAW(IntPtr), x)
#define BOX_ENUM(m_enum_class, x)                                              \
    mono_value_box(mono_domain_get(), m_enum_class, &x)
...
_FORCE_INLINE_ MonoObject* variant_to_mono_object(
    const Variant& p_var,
    const ManagedType& p_type
) {
...
/// Tries to convert the MonoObject* to Variant and then convert the Variant to
/// String. If the MonoObject* cannot be converted to Variant, then 'ToString()'
/// is called instead.
...
enum {
    ...
    MATCHES_Transform =
        (MATCHES_Basis && MATCHES_Vector3
         && (sizeof(Transform) == (sizeof(Basis) + sizeof(Vector3)))
         && offsetof(Transform, basis) == 0
         && offsetof(Transform, origin) == sizeof(Basis)),

    MATCHES_AABB =
        (MATCHES_Vector3 && (sizeof(AABB) == (sizeof(Vector3) * 2))
         && offsetof(AABB, position) == (sizeof(Vector3) * 0)
         && offsetof(AABB, size) == (sizeof(Vector3) * 1)),

    MATCHES_Color =
        (MATCHES_float && (sizeof(Color) == (sizeof(float) * 4))
    ...
    MATCHES_Plane =
        (MATCHES_Vector3 && MATCHES_real_t
         && (sizeof(Plane) == (sizeof(Vector3) + sizeof(real_t)))
...
GD_STATIC_ASSERT(
    MATCHES_Vector2&& MATCHES_Rect2&& MATCHES_Transform2D&& MATCHES_Vector3&&
        MATCHES_Basis&& MATCHES_Quat&& MATCHES_Transform&& MATCHES_AABB&&
            MATCHES_Color&& MATCHES_Plane
);
#endif
...
struct M_Rect2 {
    ...
    static _FORCE_INLINE_ Rect2 convert_to(const M_Rect2& p_from) {
        return Rect2(
            M_Vector2::convert_to(p_from.position),
    ...
    static _FORCE_INLINE_ M_Rect2 convert_from(const Rect2& p_from) {
        M_Rect2 ret = {
            M_Vector2::convert_from(p_from.position),
            M_Vector2::convert_from(p_from.size)
        };
        return ret;
    }
};
...
struct M_Basis {
    ...
    static _FORCE_INLINE_ Basis convert_to(const M_Basis& p_from) {
        return Basis(
            M_Vector3::convert_to(p_from.elements[0]),
...
struct M_Transform {
    ...
    static _FORCE_INLINE_ Transform convert_to(const M_Transform& p_from) {
        return Transform(
            M_Basis::convert_to(p_from.basis),
            M_Vector3::convert_to(p_from.origin)
        );
    }

    static _FORCE_INLINE_ M_Transform convert_from(const Transform& p_from) {
        M_Transform ret = {
            M_Basis::convert_from(p_from.basis),
            M_Vector3::convert_from(p_from.origin)
        };
        return ret;
    }
};
...
struct M_AABB {
    ...
    static _FORCE_INLINE_ AABB convert_to(const M_AABB& p_from) {
        return AABB(
            M_Vector3::convert_to(p_from.position),
            M_Vector3::convert_to(p_from.size)
        );
    }

    static _FORCE_INLINE_ M_AABB convert_from(const AABB& p_from) {
        M_AABB ret = {
            M_Vector3::convert_from(p_from.position),
            M_Vector3::convert_from(p_from.size)
        };
        return ret;
    }
};
...
#define DECL_TYPE_MARSHAL_TEMPLATES(m_type)                                    \
    template <int>                                                             \
    _FORCE_INLINE_ m_type marshalled_in_##m_type##_impl(                       \
        const M_##m_type* p_from                                               \
    );                                                                         \
                                                                               \
    template <>                                                                \
    _FORCE_INLINE_ m_type marshalled_in_##m_type##_impl<0>(                    \
        const M_##m_type* p_from                                               \
    ) {                                                                        \
        return M_##m_type::convert_to(*p_from);                                \
                                                                               ...
                                                                               \
    template <>                                                                \
    _FORCE_INLINE_ m_type marshalled_in_##m_type##_impl<1>(                    \
        const M_##m_type* p_from                                               \
    ) {                                                                        \
    ...
    _FORCE_INLINE_ m_type marshalled_in_##m_type(const M_##m_type* p_from) {   \
        return marshalled_in_##m_type##_impl<InteropLayout::MATCHES_##m_type>( \
            p_from                                                             \
        );                                                                     \
    }                                                                          \
                                                                               \
    template <int>                                                             \
    _FORCE_INLINE_ M_##m_type marshalled_out_##m_type##_impl(                  \
        const m_type& p_from                                                   \
    );                                                                         \
                                                                               \
    template <>                                                                \
    _FORCE_INLINE_ M_##m_type marshalled_out_##m_type##_impl<0>(               \
        const m_type& p_from                                                   \
    ) {                                                                        \
        return M_##m_type::convert_from(p_from);                               \
                                                                               ...
                                                                               \
    template <>                                                                \
    _FORCE_INLINE_ M_##m_type marshalled_out_##m_type##_impl<1>(               \
        const m_type& p_from                                                   \
    ) {                                                                        \
    ...
    _FORCE_INLINE_ M_##m_type marshalled_out_##m_type(const m_type& p_from) {  \
        return marshalled_out_##m_type##_impl<                                 \
            InteropLayout::MATCHES_##m_type>(p_from);                          \
    }
...
#define MARSHALLED_IN(m_type, m_from_ptr)                                      \
    (GDMonoMarshal::marshalled_in_##m_type(m_from_ptr))
#define MARSHALLED_OUT(m_type, m_from)                                         \
