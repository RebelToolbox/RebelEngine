...
String GDMonoClass::get_full_name(MonoClass* p_mono_class) {
    ...
    // the job
    MonoReflectionType* type_obj =
        mono_type_get_object(mono_domain_get(), get_mono_type(p_mono_class));
...
MonoType* GDMonoClass::get_mono_type() const {
    ...
    // There is mono_metadata_type_equal, how is this different from comparing
    // two MonoClass*?
...
bool GDMonoClass::is_static() const {
    uint32_t static_class_flags =
        MONO_TYPE_ATTR_ABSTRACT | MONO_TYPE_ATTR_SEALED;
...
GDMonoClass* GDMonoClass::get_parent_class() const {
    ...
    return parent_mono_class
             ? GDMono::get_singleton()->get_class(parent_mono_class)
             : NULL;
}

GDMonoClass* GDMonoClass::get_nesting_class() const {
    MonoClass* nesting_type = mono_class_get_nesting_type(mono_class);
    return nesting_type ? GDMono::get_singleton()->get_class(nesting_type)
                        : NULL;
}
...
Vector<MonoClassField*> GDMonoClass::get_enum_fields() {
    ...
    while ((raw_field = mono_class_get_fields(get_mono_ptr(), &iter)) != NULL) {
        ...
        // Enums have an instance field named value__ which holds the value of
        // the enum. Enum constants are static, so we will use this to ignore
        // the value__ field.
        if (field_flags & MONO_FIELD_ATTR_PUBLIC
            && field_flags & MONO_FIELD_ATTR_STATIC) {
...
#ifdef DEBUG_ENABLED
            String fullname = method->get_ret_type_full_name() + " " + name
...
#ifdef DEBUG_ENABLED
        // For debug builds, we also fetched from native base classes as well
        // before if this is not a native base class. This allows us to warn the
        // user here if they are using snake_case by mistake.

        if (p_native_base != this) {
            GDMonoClass* native_top = p_native_base;
            while (native_top) {
                GDMonoMethod* m = native_top->get_method(
                    name,
                ...
                if (m && m->get_name() != name) {
                    // found
                    String fullname = m->get_ret_type_full_name() + " " + name
                                    + "(" + m->get_signature_desc(true) + ")";
                    WARN_PRINT(
                        "Method '" + fullname + "' should be '"
                        + m->get_full_name_no_class() + "'. In class '"
                        + namespace_name + "." + class_name + "'."
                    );
                    break;
                }
...
GDMonoMethod* GDMonoClass::get_fetched_method_unknown_params(
...
bool GDMonoClass::implements_interface(GDMonoClass* p_interface) {
    return mono_class_implements_interface(
        mono_class,
        p_interface->get_mono_ptr()
    );
}
...
GDMonoMethod* GDMonoClass::get_method(
    const StringName& p_name,
    int p_params_count
) {
    ...
    MonoMethod* raw_method = mono_class_get_method_from_name(
        mono_class,
...
GDMonoMethod* GDMonoClass::get_method_with_desc(
    const String& p_description,
    bool p_include_namespace
) {
...
GDMonoField* GDMonoClass::get_field(const StringName& p_name) {
    ...
    MonoClassField* raw_field = mono_class_get_field_from_name(
        mono_class,
        String(p_name).utf8().get_data()
    );
...
GDMonoProperty* GDMonoClass::get_property(const StringName& p_name) {
    ...
    MonoProperty* raw_property = mono_class_get_property_from_name(
        mono_class,
        String(p_name).utf8().get_data()
    );
...
const Vector<GDMonoProperty*>& GDMonoClass::get_all_properties() {
    ...
    while ((raw_property = mono_class_get_properties(mono_class, &iter)) != NULL
        ...
        Map<StringName, GDMonoProperty*>::Element* match =
            properties.find(name);

        if (match) {
            properties_list.push_back(match->get());
        } else {
            GDMonoProperty* property =
                memnew(GDMonoProperty(raw_property, this));
...
const Vector<GDMonoClass*>& GDMonoClass::get_all_delegates() {
    ...
    while ((raw_class = mono_class_get_nested_types(mono_class, &iter)) != NULL
        ...
        if (mono_class_is_delegate(raw_class)) {
            ...
            Map<StringName, GDMonoClass*>::Element* match =
                delegates.find(name);

            if (match) {
                delegates_list.push_back(match->get());
            } else {
                GDMonoClass* delegate = memnew(GDMonoClass(
                    String::utf8(mono_class_get_namespace(raw_class)),
                    String::utf8(mono_class_get_name(raw_class)),
                    raw_class,
                    assembly
                ));
...
const Vector<GDMonoMethod*>& GDMonoClass::get_all_methods() {
    if (!method_list_fetched) {
        ...
        while ((raw_method = mono_class_get_methods(get_mono_ptr(), &iter))
               != NULL) {
            method_list.push_back(memnew(GDMonoMethod(
                String::utf8(mono_method_get_name(raw_method)),
                raw_method
            )));
        }
