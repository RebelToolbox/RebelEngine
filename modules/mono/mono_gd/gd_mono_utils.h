...
_FORCE_INLINE_ void hash_combine(
    uint32_t& p_hash,
    const uint32_t& p_with_hash
) {
    p_hash ^= p_with_hash + 0x9e3779b9 + (p_hash << 6) + (p_hash >> 2);
}

/**
 * If the object has a csharp script, returns the target of the gchandle stored
 * in the script instance Otherwise returns a newly constructed MonoObject*
 * which is attached to the object Returns NULL on error
 */
...
_FORCE_INLINE_ bool is_main_thread() {
    return mono_domain_get() != NULL
        && mono_thread_get_main() == mono_thread_current();
}
...
/**
 * Sets the exception as pending. The exception will be thrown when returning to
 * managed code. If no managed method is being invoked by the runtime, the
 * exception will be treated as an unhandled exception and the method will not
 * return.
 */
...
#define GD_MONO_BEGIN_RUNTIME_INVOKE                                           \
    int& _runtime_invoke_count_ref =                                           \
        GDMonoUtils::get_runtime_invoke_count_ref();                           \
    _runtime_invoke_count_ref += 1;

#define GD_MONO_END_RUNTIME_INVOKE _runtime_invoke_count_ref -= 1;
