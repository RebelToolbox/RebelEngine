...
// Implemented mostly following:
// https://github.com/mono/mono/blob/master/sdks/ios/app/runtime.m

// Definition generated by the Rebel exporter
...
// The following are P/Invoke functions required by the monotouch profile of the
...
GD_PINVOKE_EXPORT void xamarin_log(const uint16_t* p_unicode_message) {
    ...
    NSString* msg =
        [[NSString alloc] initWithBytes:p_unicode_message
                                 length:length
...
GD_PINVOKE_EXPORT void* xamarin_timezone_get_data(
    const char* p_name,
    uint32_t* p_size
) { // FIXME: uint32_t since Dec 2019, unsigned long before
    NSTimeZone* tz = nil;
    if (p_name) {
