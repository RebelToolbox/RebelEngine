...
Vector3 MobileVRInterface::scale_magneto(const Vector3& p_magnetometer) {
    ...
    // Well it may on Mac OS X because we're getting a calibrated value in the
    // current implementation but Android we're getting raw data. This is a
...
Basis MobileVRInterface::combine_acc_mag(
    const Vector3& p_grav,
    const Vector3& p_magneto
) {
    // yup, stock standard cross product solution...
    Vector3 up = -p_grav.normalized();

    Vector3 magneto_east = up.cross(p_magneto.normalized()
    ); // or is this west?, but should be horizon aligned now
    magneto_east.normalize();

    Vector3 magneto =
        up.cross(magneto_east); // and now we have a horizon aligned north
    magneto.normalize();
...
void MobileVRInterface::set_position_from_sensors() {
    ...
    // this is a helper function that attempts to adjust our transform using our
    // 9dof sensors 9dof is a misleading marketing term coming from 3
    ...
    if (grav.length() < 0.1) {
        // not ideal but use our accelerometer, this will contain shakey shakey
        // user behaviour maybe look into some math but I'm guessing that if
        // this isn't available, its because we lack the gyro sensor to actually
        // work out what a stable gravity vector is
        grav = acc;
        if (grav.length() > 0.1) {
            has_grav = true;
        };
    ...
    if (gyro.length() > 0.1) {
        /* this can return to 0.0 if the user doesn't move the phone, so once
         * on, it's on */
        has_gyro = true;
    };
    ...
    ///@TODO improve this, the magnetometer is very fidgity sometimes flipping
    /// the axis for no apparent reason (probably a bug on my part)
    // if you have a gyro + accelerometer that combo tends to be better then
    // combining all three but without a gyro you need the magnetometer..
...
void MobileVRInterface::_bind_methods() {
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "iod", PROPERTY_HINT_RANGE, "4.0,10.0,0.1"),
        "set_iod",
        "get_iod"
    );
    ADD_PROPERTY(
...
Transform MobileVRInterface::get_transform_for_eye(
    ...
    if (initialized) {
        ...
        // we don't need to check for the existence of our HMD, doesn't effect
        // our values... note * 0.01 to convert cm to m and * 0.5 as we're
        // moving half in each direction...
        if (p_eye == ARVRInterface::EYE_LEFT) {
            transform_for_eye.origin.x =
                -(intraocular_dist * 0.01 * 0.5 * world_scale);
        ...
        transform_for_eye = p_cam_transform
                          * (arvr_server->get_reference_frame()) * hmd_transform
                          * transform_for_eye;
    } else {
...
CameraMatrix MobileVRInterface::get_projection_for_eye(
    ARVRInterface::Eyes p_eye,
    real_t p_aspect,
    real_t p_z_near,
    real_t p_z_far
    ...
    if (p_eye == ARVRInterface::EYE_MONO) {
        ///@TODO for now hardcode some of this, what is really needed here is
        /// that this needs to be in sync with the real cameras properties
        ...
        eye.set_for_hmd(
            p_eye == ARVRInterface::EYE_LEFT ? 1 : 2,
            p_aspect,
            intraocular_dist,
            display_width,
            display_to_lens,
...
void MobileVRInterface::commit_for_eye(
    ARVRInterface::Eyes p_eye,
    RID p_render_target,
    ...
    // unset our render target so we are outputting to our main screen by making
    // RasterizerStorageGLES3::system_fbo our current FBO
    VSG::rasterizer->set_current_render_target(RID());

    // and output
    VSG::rasterizer->output_lens_distorted_to_screen(
        p_render_target,
        dest,
        k1,
        k2,
