...
// Cylinder SAT analytic methods and circle-face contact points for
// cylinder-trimesh and cylinder-box collision are based on ODE colliders.
...
static void _generate_contacts_point_edge(
    const Vector3* p_points_A,
    int p_point_count_A,
    const Vector3* p_points_B,
    int p_point_count_B,
    ...
    Vector3 closest_B = Geometry::get_closest_point_to_segment_uncapped(
        *p_points_A,
        p_points_B
    );
...
static void _generate_contacts_point_face(
    const Vector3* p_points_A,
    int p_point_count_A,
    const Vector3* p_points_B,
    int p_point_count_B,
...
static void _generate_contacts_point_circle(
    const Vector3* p_points_A,
    int p_point_count_A,
    const Vector3* p_points_B,
    int p_point_count_B,
...
static void _generate_contacts_edge_edge(
    const Vector3* p_points_A,
    int p_point_count_A,
    const Vector3* p_points_B,
    int p_point_count_B,
    ...
    if (Math::is_zero_approx(rel_A.dot(c))) {
        ...
        real_t dvec[4] = {
            axis.dot(p_points_A[0]),
            axis.dot(p_points_A[1]),
            axis.dot(p_points_B[0]),
            axis.dot(p_points_B[1])
        };
...
static void _generate_contacts_edge_circle(
    const Vector3* p_points_A,
    int p_point_count_A,
    const Vector3* p_points_B,
    int p_point_count_B,
    ...
    Vector3 circle_B_normal =
        circle_B_line_1.cross(circle_B_line_2).normalized();
    ...
    if (num_points < 2) {
        ...
        // If the term we intend to square root is less than 0 then the answer
        // won't be real, so the line doesn't intersect.
        if (sqrtterm >= 0) {
            ...
            if ((fraction_2 > 0.0) && (fraction_2 < 1.0)
                && !Math::is_equal_approx(fraction_1, fraction_2)) {
    ...
    for (int i = 0; i < num_points; i++) {
        ...
        if (p_callback->normal.dot(contact_point_A)
...
static void _generate_contacts_face_face(
    const Vector3* p_points_A,
    int p_point_count_A,
    const Vector3* p_points_B,
    int p_point_count_B,
...
static void _generate_contacts_face_circle(
    const Vector3* p_points_A,
    int p_point_count_A,
    const Vector3* p_points_B,
    int p_point_count_B,
    ...
    for (int i = 0; i < p_point_count_A; i++) {
        ...
        if (dist0 * dist1 < 0) {
            ...
            real_t dist =
                -(circle_plane.normal.dot(edge0_A) - circle_plane.d) / den;
            Vector3 inters = edge0_A + rel * dist;
...
static void _generate_contacts_circle_circle(
    const Vector3* p_points_A,
    int p_point_count_A,
    const Vector3* p_points_B,
    int p_point_count_B,
    ...
    Vector3 circle_A_normal =
        circle_A_line_1.cross(circle_A_line_2).normalized();
    ...
    if (!Math::is_zero_approx(proj_dist)) {
        comp_proj /= proj_dist;
        if ((proj_dist > circle_A_radius - circle_B_radius)
            && (proj_dist > circle_B_radius - circle_A_radius)) {
            // Circles are overlapping, use the 2 points of intersection as
            // contacts.
            ...
            // Add 2 points from circle A and B along the line between the
            // centers.
            ...
            contact_points[num_points] = point_A;
            ++num_points;
        } // Otherwise one circle is inside the other one, use 3 arbitrary
          // equidistant points.
    ...
    if (num_points == 0) {
        ...
        if (circle_A_radius < circle_B_radius) {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                circle_A_point +=
                    circle_A_line_1 * Math::cos(2.0 * Math_PI * i / 3.0);
                circle_A_point +=
                    circle_A_line_2 * Math::sin(2.0 * Math_PI * i / 3.0);
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                circle_B_point +=
                    circle_B_line_1 * Math::cos(2.0 * Math_PI * i / 3.0);
                circle_B_point +=
                    circle_B_line_2 * Math::sin(2.0 * Math_PI * i / 3.0);
...
static void _generate_contacts_from_supports(
    const Vector3* p_points_A,
    int p_point_count_A,
    ShapeSW::FeatureType p_feature_type_A,
    const Vector3* p_points_B,
    int p_point_count_B,
    ShapeSW::FeatureType p_feature_type_B,
    _CollectorCallback* p_callback
) {
#ifdef DEBUG_ENABLED
    ERR_FAIL_COND(p_point_count_A < 1);
    ...
    static const GenerateContactsFunc generate_contacts_func_table[4][4] = {
        {
         ...
         _generate_contacts_point_face, _generate_contacts_point_circle,
         },
        {
         nullptr,                              _generate_contacts_edge_edge,
         _generate_contacts_face_face,                       _generate_contacts_edge_circle,
         },
        {
         nullptr,   nullptr,
         _generate_contacts_face_face,                             _generate_contacts_face_circle,
         },
        {
         nullptr,                        nullptr,
         nullptr,  _generate_contacts_circle_circle,
         },
    };
...
public:
    _FORCE_INLINE_ bool test_previous_axis() {
        if (callback && callback->prev_axis
            && *callback->prev_axis != Vector3()) {
    ...
    static _FORCE_INLINE_ void test_contact_points(
        const Vector3& p_point_A,
        const Vector3& p_point_B,
        void* p_userdata
    ) {
        SeparatorAxisTest<ShapeA, ShapeB, withMargin>* separator =
            (SeparatorAxisTest<ShapeA, ShapeB, withMargin>*)p_userdata;
    ...
    _FORCE_INLINE_ void generate_contacts() {
        ...
        shape_A->get_supports(
            transform_A->basis.xform_inv(-best_axis).normalized(),
            max_supports,
            supports_A,
            support_count_A,
            support_type_A
        ...
        shape_B->get_supports(
            transform_B->basis.xform_inv(best_axis).normalized(),
            max_supports,
            supports_B,
            support_count_B,
            support_type_B
        ...
        _generate_contacts_from_supports(
            supports_A,
            support_count_A,
            support_type_A,
            supports_B,
    ...
    _FORCE_INLINE_ SeparatorAxisTest(
        const ShapeA* p_shape_A,
        const Transform& p_transform_A,
        const ShapeB* p_shape_B,
...
static void _collision_sphere_sphere(
    const ShapeSW* p_a,
    const Transform& p_transform_a,
    const ShapeSW* p_b,
    const Transform& p_transform_b,
    _CollectorCallback* p_collector,
    real_t p_margin_a,
    real_t p_margin_b
) {
...
template <bool withMargin>
static void _collision_sphere_box(
    const ShapeSW* p_a,
    const Transform& p_transform_a,
    const ShapeSW* p_b,
    const Transform& p_transform_b,
...
template <bool withMargin>
static void _collision_sphere_capsule(
    const ShapeSW* p_a,
    const Transform& p_transform_a,
    const ShapeSW* p_b,
    const Transform& p_transform_b,
...
template <bool withMargin>
static void _collision_sphere_cylinder(
    const ShapeSW* p_a,
    const Transform& p_transform_a,
    const ShapeSW* p_b,
    const Transform& p_transform_b,
    ...
    const SphereShapeSW* sphere_A = static_cast<const SphereShapeSW*>(p_a);
    const CylinderShapeSW* cylinder_B =
        static_cast<const CylinderShapeSW*>(p_b);

    SeparatorAxisTest<SphereShapeSW, CylinderShapeSW, withMargin> separator(
        sphere_A,
        p_transform_a,
        cylinder_B,
        p_transform_b,
    ...
    if (!separator.test_axis(cylinder_B_axis.cross(cylinder_diff)
                                 .cross(cylinder_B_axis)
                                 .normalized())) {
        return;
    }
...
static void _collision_sphere_convex_polygon(
    ...
    SeparatorAxisTest<SphereShapeSW, ConvexPolygonShapeSW, withMargin>
        separator(
            sphere_A,
            p_transform_a,
            convex_polygon_B,
...
template <bool withMargin>
static void _collision_box_box(
    const ShapeSW* p_a,
    const Transform& p_transform_a,
    const ShapeSW* p_b,
    const Transform& p_transform_b,
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            for (int k = 0; k < 2; k++) {
                ...
                // Vector3 axis = (point - cyl_axis *
                // cyl_axis.dot(point)).normalized();
...
template <bool withMargin>
static void _collision_box_cylinder(
    const ShapeSW* p_a,
    const Transform& p_transform_a,
    const ShapeSW* p_b,
    const Transform& p_transform_b,
    ...
    SeparatorAxisTest<BoxShapeSW, CylinderShapeSW, withMargin> separator(
        box_A,
        p_transform_a,
        cylinder_B,
        p_transform_b,
    ...
    for (int i = 0; i < 2; i++) {
        Vector3 cap_pos =
            p_transform_b.origin + ((i == 0) ? cap_axis : -cap_axis);
...
template <bool withMargin>
static void _collision_box_face(
    const ShapeSW* p_a,
    const Transform& p_transform_a,
    const ShapeSW* p_b,
    const Transform& p_transform_b,
    ...
    if (withMargin) {
        ...
        // convex edges and box points, there has to be a way to speed up this
        // (get closest point?)
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++) {
                for (int k = 0; k < 2; k++) {
...
static void _collision_capsule_cylinder(
    ...
    SeparatorAxisTest<CapsuleShapeSW, CylinderShapeSW, withMargin> separator(
        capsule_A,
        p_transform_a,
        cylinder_B,
        p_transform_b,
    ...
    Vector3 capsule_A_ball_1 =
        p_transform_a.origin + capsule_A_axis * (capsule_A->get_height() * 0.5);
    Vector3 capsule_A_ball_2 =
        p_transform_a.origin - capsule_A_axis * (capsule_A->get_height() * 0.5);

    if (!separator.test_axis((p_transform_b.origin - capsule_A_ball_1)
                                 .cross(cylinder_B_axis)
                                 .cross(cylinder_B_axis)
                                 .normalized())) {
        return;
    ...
    if (!separator.test_axis((p_transform_b.origin - capsule_A_ball_2)
                                 .cross(cylinder_B_axis)
                                 .cross(cylinder_B_axis)
                                 .normalized())) {
        return;
    ...
    if (!separator.test_axis(
            capsule_A_axis.cross(center_diff).cross(capsule_A_axis).normalized()
        )) {
        return;
    }

    if (!separator.test_axis(cylinder_B_axis.cross(center_diff)
                                 .cross(cylinder_B_axis)
                                 .normalized())) {
        return;
    }

    real_t proj = capsule_A_axis.cross(cylinder_B_axis)
                      .cross(cylinder_B_axis)
                      .dot(capsule_A_axis);
    if (Math::is_zero_approx(proj)) {
        // Parallel capsule and cylinder axes, handle with specific axes only.
        // Note: GJKEPA with no margin can lead to degenerate cases in this
        // situation.
        separator.generate_contacts();
        return;
    }
...
static void _collision_capsule_convex_polygon(
    ...
    SeparatorAxisTest<CapsuleShapeSW, ConvexPolygonShapeSW, withMargin>
        separator(
            capsule_A,
            p_transform_a,
            convex_polygon_B,
...
template <bool withMargin>
static void _collision_capsule_face(
    const ShapeSW* p_a,
    const Transform& p_transform_a,
    const ShapeSW* p_b,
    const Transform& p_transform_b,
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int j = 0; j < 2; j++) {
            // point-spheres
            Vector3 sphere_pos = p_transform_a.origin
                               + ((j == 0) ? capsule_axis : -capsule_axis);
...
template <bool withMargin>
static void _collision_cylinder_cylinder(
    const ShapeSW* p_a,
    const Transform& p_transform_a,
    const ShapeSW* p_b,
    const Transform& p_transform_b,
    ...
    const CylinderShapeSW* cylinder_A =
        static_cast<const CylinderShapeSW*>(p_a);
    const CylinderShapeSW* cylinder_B =
        static_cast<const CylinderShapeSW*>(p_b);

    SeparatorAxisTest<CylinderShapeSW, CylinderShapeSW, withMargin> separator(
        cylinder_A,
        p_transform_a,
        cylinder_B,
        p_transform_b,
    ...
    if (!separator.test_axis(cylinder_A_axis.cross(cylinder_diff)
                                 .cross(cylinder_A_axis)
                                 .normalized())) {
        return;
    }
    ...
    real_t proj = cylinder_A_axis.cross(cylinder_B_axis)
                      .cross(cylinder_B_axis)
                      .dot(cylinder_A_axis);
    if (Math::is_zero_approx(proj)) {
    ...
    CollisionSolverSW::CallbackResult callback =
        SeparatorAxisTest<CylinderShapeSW, CylinderShapeSW, withMargin>::
            test_contact_points;
...
static void _collision_cylinder_convex_polygon(
    ...
    SeparatorAxisTest<CylinderShapeSW, ConvexPolygonShapeSW, withMargin>
        separator(
            cylinder_A,
            p_transform_a,
            convex_polygon_B,
...
template <bool withMargin>
static void _collision_cylinder_face(
    const ShapeSW* p_a,
    const Transform& p_transform_a,
    const ShapeSW* p_b,
    const Transform& p_transform_b,
    ...
    SeparatorAxisTest<CylinderShapeSW, FaceShapeSW, withMargin> separator(
        cylinder_A,
        p_transform_a,
        face_B,
        p_transform_b,
    ...
    for (int i = 0; i < 2; i++) {
        Vector3 cap_pos =
            p_transform_a.origin + ((i == 0) ? cap_axis : -cap_axis);
...
static void _collision_convex_polygon_convex_polygon(
    ...
    for (int i = 0; i < face_count_A; i++) {
        Vector3 axis =
            a_xform_normal.xform(faces_A[i].plane.normal).normalized();
    ...
    for (int i = 0; i < face_count_B; i++) {
        Vector3 axis =
            b_xform_normal.xform(faces_B[i].plane.normal).normalized();
...
bool sat_calculate_penetration(
    ...
    static const CollisionFunc collision_table[6][6] = {
        ...
        {nullptr,
         nullptr,                                  _collision_capsule_capsule<false>,
        ...
        {nullptr,
         nullptr,                                  nullptr,
         nullptr,                                                                              _collision_convex_polygon_convex_polygon<false>,
    ...
    static const CollisionFunc collision_table_margin[6][6] = {
        ...
        {nullptr,
         nullptr,                                 _collision_capsule_capsule<true>,
        ...
        {nullptr,
         nullptr,                                 nullptr,
         nullptr,                                                                            _collision_convex_polygon_convex_polygon<true>,
