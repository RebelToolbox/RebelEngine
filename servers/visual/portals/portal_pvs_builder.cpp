...
void PVSBuilder::create_secondary_pvs(
    int p_room_id,
    ...
    for (int n = 0; n < num_rooms; n++) {
        ...
        if (_log_active) {
            ...
            for (int i = 0; i < room._secondary_pvs_size; i++) {
                int visible_room = _pvs->get_secondary_pvs_room_id(
                    room._secondary_pvs_first + i
                );
                string += itos(visible_room);
                string += ", ";
            }
...
// The full routine deals with re-entrant rooms. I.e. more than one portal path
// can lead into a room. This makes the logic more complex, because we cannot
// terminate on the second entry to a room, and have to account for internal
// rooms, and the possibility of portal paths going back on themselves.
void PVSBuilder::trace_rooms_recursive(
    int p_depth,
    ...
    if (p_depth > _depth_limit) {
        WARN_PRINT_ONCE(
            "PVS Depth Limit reached (seeing through too many portals)"
        );
        return;
    }
    ...
    for (int p = 0; p < num_portals; p++) {
        ...
        // For pvs there is no real start point, but we will use the centre of
        // the first portal. This is used for checking portals are pointing
        // outward from start point.
        if (p_source_room_id == p_room_id) {
            ...
            // to coplanar portals for the first to second portals, before
            // planes have been added. So we will place the trace start point
            // slightly behind the first portal plane (e.g. slightly in the
            // source room). The epsilon must balance being enough in not to
            // cause numerical error at large distances from the origin, but too
            // large and this will also prevent the PVS entering portals that
            // are very closely aligned to the portal in. Closely aligned
            // portals should not happen in normal level design, and will
            // usually be a design error. Watch for bugs here though, caused by
            // closely aligned portals.
            ...
            // instead of using dot product with a varying view direction, we
            // simply find which side of the portal plane the camera is on! If
            // it is behind, the portal can be seen through, if in front, it
            // can't
        ...
        if (p_first_portal_id != -1) {
            // add new planes
            const VSPortal& first_portal =
                _portal_renderer->get_portal(p_first_portal_id);
            portal.add_pvs_planes(
                first_portal,
                p_first_portal_outgoing,
                planes,
                outgoing != 0
            );
        }
        ...
        // i.e. we can now cull using the portal and forget about the rest of
        // the frustum (yay)
        if (overall_res != VSPortal::ClipResult::CLIP_INSIDE) {
            // if it WASN'T totally inside the existing frustum, we also need to
            // add any existing planes that cut the portal.
...
// This simpler routine was the first used. It is reliable and no epsilons, and
// fast. But it will not create the correct result where there are multiple
// portal paths through a room when building the PVS.
void PVSBuilder::trace_rooms_recursive_simple(
    int p_depth,
    int p_source_room_id,
    int p_room_id,
    int p_first_portal_id,
