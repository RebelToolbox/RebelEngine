...
void PortalTracer::trace_debug_sprawl(
...
void PortalTracer::trace(
    ...
    // immediately in front of the camera. as a result we want to use the near
    // clipping plane for objects, but construct a fake near plane at exactly
    // the position of the camera, to clip out portals that are behind us.
    ...
    // if the camera is not in a room do nothing
    // (this will return no hits, but is unlikely because the find_rooms lookup
    // will return the nearest room even if not inside)
    if (p_start_room_id == -1) {
        return;
    }
    ...
    if (p_portal_renderer.get_cull_using_pvs()) {
        ...
        // alternative : instead of copying straight, we create the first (near)
        // clipping plane manually, at 0 distance from the camera. This ensures
        // that portals will not be missed, while still culling portals and
        // objects behind us. If we use the actual near clipping plane then a
        // portal in front of the camera may not be seen through, giving
        // glitches
        ...
        if (params.use_pvs) {
            ...
            if (!pvs.get_pvs_size()) {
                ...
                // decompress a simple to read roomlist bitfield (could use bits
                // maybe but bytes ok for now)
                params.decompressed_room_pvs = nullptr;
                params.decompressed_room_pvs =
                    (uint8_t*)alloca(sizeof(uint8_t) * pvs.get_pvs_size());
                memset(
                    params.decompressed_room_pvs,
                    0,
                    sizeof(uint8_t) * pvs.get_pvs_size()
                );
                const VSRoom& source_room =
                    _portal_renderer->get_room(p_start_room_id);

                for (int n = 0; n < source_room._pvs_size; n++) {
                    int room_id =
                        pvs.get_pvs_room_id(source_room._pvs_first + n);
...
void PortalTracer::cull_roamers(
    const VSRoom& p_room,
    const LocalVector<Plane>& p_planes
) {
    ...
    for (int n = 0; n < num_roamers; n++) {
        ...
        PortalRenderer::Moving& moving =
            _portal_renderer->get_pool_moving(pool_id);
...
void PortalTracer::cull_statics(
    const VSRoom& p_room,
    const LocalVector<Plane>& p_planes
) {
    int num_statics = p_room._static_ids.size();
    ...
    for (int n = 0; n < num_statics; n++) {
        ...
        // deal with dynamic stats
        if (stat.dynamic) {
            VSG::scene->_instance_get_transformed_aabb(
                stat.instance,
                stat.aabb
            );
        }
...
int PortalTracer::trace_globals(
    const LocalVector<Plane>& p_planes,
    ...
    if (!p_override_camera) {
        for (uint32_t n = 0; n < num_globals; n++) {
            const PortalRenderer::Moving& moving =
                _portal_renderer->get_moving_global(n);

#ifdef PORTAL_RENDERER_STORE_MOVING_RIDS
            // debug check the instance is valid
            void* vss_instance =
                VSG::scene->_instance_get_from_rid(moving.instance_rid);

            if (vss_instance) {
#endif
                if (test_cull_inside(moving.exact_aabb, p_planes, false)) {
                    if (VSG::scene
                            ->_instance_cull_check(moving.instance, p_mask)) {
                ...
                WARN_PRINT(
                    "vss instance is null "
                    + PortalRenderer::_addr_to_string(moving.instance)
                );
            }
#endif
        }
        ...
        // If we are overriding the camera there is a potential problem in the
        // editor: gizmos BEHIND the override camera will not be drawn. As this
        // should be editor only and performance is not critical, we will just
        // disable frustum culling for global objects when the camera is
        // overridden.
        for (uint32_t n = 0; n < num_globals; n++) {
            const PortalRenderer::Moving& moving =
                _portal_renderer->get_moving_global(n);
...
void PortalTracer::trace_debug_sprawl_recursive(int p_depth, int p_room_id) {
    ...
    for (int p = 0; p < num_portals; p++) {
        const VSPortal& portal =
            _portal_renderer->get_portal(room._portal_ids[p]);
...
void PortalTracer::trace_recursive(
    ...
    int p_room_id,
    const LocalVector<Plane>& p_planes,
    int p_from_external_room_id
) {
    // prevent too much depth
    if (p_depth > _depth_limit) {
        WARN_PRINT_ONCE(
            "Portal Depth Limit reached (seeing through too many portals)"
        );
        return;
    }
    ...
    _occlusion_culler.prepare(
        *_portal_renderer,
        room,
        _trace_start_point,
        p_planes,
        &_near_and_far_planes[0]
    );
    ...
    for (int p = 0; p < num_portals; p++) {
        ...
        // portals can be switched on and off at runtime, like opening and
        // closing a door
        if (!portal._active) {
            continue;
        }
        ...
        // cull by PVS
        if (p_params.use_pvs
            && (!p_params.decompressed_room_pvs[linked_room_id])) {
            continue;
        }
        ...
        // instead of using dot product with a varying view direction, we simply
        // find which side of the portal plane the camera is on! If it is
        // behind, the portal can be seen through, if in front, it can't
        ...
        // while clipping to the planes we maintain a list of partial planes, so
        // we can add them to the recursive next iteration of planes to check
        static LocalVector<int> partial_planes;
        partial_planes.clear();

        // for portals, we want to ignore the near clipping plane, as we might
        // be right on the edge of a doorway and still want to look through the
        // portal. so earlier we have set it that the first plane (ASSUMING that
        // plane zero is the near clipping plane) starts from the camera
        // position, and NOT the actual near clipping plane. if we need quite a
        // distant near plane, we may need a different strategy.
        for (uint32_t l = 0; l < p_planes.size(); l++) {
            ...
            switch (res) {
                ...
                case VSPortal::ClipResult::CLIP_PARTIAL: {
                    // if the portal intersects one of the planes, we should
                    // take this plane into account in the next call of this
                    // recursive trace, because it can be used to cull out more
                    // objects
                    overall_res = res;
            ...
            // if the portal was totally outside the 'frustum' then we can
            // ignore it
        ...
        // if the external room has already been processed in this trace stack.
        // This prevents unneeded processing, and also prevents recursive
        // feedback where you see into internal room -> external room and back
        // into the same internal room via the same portal.
        if (portal._internal && (linked_room_id != -1)) {
            if (outgoing) {
                ...
                // set the external room id, so we can recognise this when we
                // are later exiting the internal rooms. Note that as we can
                // only store 1 previous external room, this system won't work
                // completely correctly when you have 2 levels of internal room
                // and you can see from roomgroup a -> b -> c. However this
                // should just result in a little slower culling for that
                // particular view, and hopefully will not break with recursive
                // loop looking through the same portal multiple times. (don't
                // think this is possible in this scenario).
                p_from_external_room_id = p_room_id;
            }
        }

        // occlusion culling of portals
        if (_occlusion_culler.cull_sphere(
                portal._pt_center,
                portal._bounding_sphere_radius
            )) {
            continue;
        }
        ...
        if (linked_room_id != -1) {
            ...
            // if the planes pool is not empty, we got some planes, and can
            // recurse
            if (pool_mem != (unsigned int)-1) {
                // get a new vector of planes from the pool
                LocalVector<Plane>& new_planes = _planes_pool.get(pool_mem);

                // makes sure there are none left over (as the pool may not
                // clear them)
                new_planes.clear();

                // if portal is totally inside the planes, don't copy the old
                // planes .. i.e. we can now cull using the portal and forget
                // about the rest of the frustum (yay) note that this loses the
                // far clipping plane .. but that shouldn't be important
                // usually? (maybe we might need to account for this in future
                // .. look for issues)
                if (overall_res != VSPortal::ClipResult::CLIP_INSIDE) {
                    // if it WASN'T totally inside the existing frustum, we also
                    // need to add any existing planes that cut the portal.
                    for (uint32_t n = 0; n < partial_planes.size(); n++) {
                        new_planes.push_back(p_planes[partial_planes[n]]);
                    }
                }

                // we will always add the portals planes. This could probably be
                // optimized, as some portal planes may be culled out by partial
                // planes... NYI
                portal
                    .add_planes(_trace_start_point, new_planes, outgoing != 0);

                // always add the far plane. It is likely the portal is inside
                // the far plane, but it is still needed in future for culling
                // portals and objects. note that there is a small possibility
                // of far plane being added twice here in some situations, but I
                // don't think it should be a problem. The fake near plane BTW
                // is almost never added (otherwise it would prematurely break
                // traversal through the portals), so near clipping must be done
                ...
                // This will happen if the view goes through shedloads of
                // portals The solution is either to increase the plane pool
                // size, or not build levels with views through multiple
                // portals. Looking through multiple portals is likely to be
                // slow anyway because of the number of planes to test.
                WARN_PRINT_ONCE("planes pool is empty");
                // note we also have a depth check at the top of this function.
                // Which will probably get hit before the pool gets empty.
            }
...
int PortalTracer::occlusion_cull(
    ...
    // pretty cheap anyway as it will just copy 6 planes, max a few times per
    // frame...
    ...
    _occlusion_culler.prepare_generic(
        p_portal_renderer,
        p_portal_renderer.get_occluders_active_list(),
        p_point,
        local_planes
    );
    ...
    for (int n = 0; n < count; n++) {
        ...
        // this will return false for GLOBAL instances, so we don't occlusion
        // cull gizmos
        if (VSG::scene
                ->_instance_get_transformed_aabb_for_occlusion(instance, bb)) {
            if (_occlusion_culler.cull_aabb(bb)) {
