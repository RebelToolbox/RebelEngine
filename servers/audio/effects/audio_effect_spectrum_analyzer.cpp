...
void AudioEffectSpectrumAnalyzerInstance::process(
    ...
    while (p_frame_count) {
        ...
        for (int i = 0; i < to_fill; i++) { // left and right buffers
            float window = -0.5
                             * Math::cos(
                                 2.0 * Math_PI * (double)temporal_fft_pos
                                 / (double)fft_size
                             )
                         + 0.5;
        ...
        if (temporal_fft_pos == fft_size * 2) {
            ...
            AudioFrame* hw = (AudioFrame*)fft_history[next].ptr(
            ); // do not use write, avoid cow

            for (int i = 0; i < fft_size; i++) {
                // abs(vec)/fft_size normalizes each frequency
                hw[i].l = Vector2(fftw[i * 2], fftw[i * 2 + 1]).length()
                        / float(fft_size);
                hw[i].r = Vector2(
                              fftw[fft_size * 4 + i * 2],
                              fftw[fft_size * 4 + i * 2 + 1]
                          )
                              .length()
                        / float(fft_size);
            }
...
Ref<AudioEffectInstance> AudioEffectSpectrumAnalyzer::instance() {
    ...
    ins->temporal_fft.resize(
        ins->fft_size * 8
    ); // x2 stereo, x2 amount of samples for freqs, x2 for input
    ins->temporal_fft_pos = 0;
    for (int i = 0; i < ins->fft_count; i++) {
        ins->fft_history.write[i].resize(ins->fft_size
...
void AudioEffectSpectrumAnalyzer::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set_tap_back_pos", "seconds"),
        &AudioEffectSpectrumAnalyzer::set_tap_back_pos
    );
    ClassDB::bind_method(
        D_METHOD("get_tap_back_pos"),
