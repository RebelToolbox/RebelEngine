...
_FORCE_INLINE_ static void _generate_contacts_point_point(
    const Vector2* p_points_A,
    int p_point_count_A,
    const Vector2* p_points_B,
    int p_point_count_B,
...
_FORCE_INLINE_ static void _generate_contacts_point_edge(
    const Vector2* p_points_A,
    int p_point_count_A,
    const Vector2* p_points_B,
    int p_point_count_B,
    ...
    Vector2 closest_B = Geometry::get_closest_point_to_segment_uncapped_2d(
        *p_points_A,
        p_points_B
    );
...
_FORCE_INLINE_ static void _generate_contacts_edge_edge(
    const Vector2* p_points_A,
    int p_point_count_A,
    const Vector2* p_points_B,
    int p_point_count_B,
...
public:
    ...
    _FORCE_INLINE_ void generate_contacts() {
        ...
        int support_count_A;
        if (castA) {
            shape_A->get_supports_transformed_cast(
                motion_A,
                -best_axis,
                *transform_A,
                supports_A,
            ...
            shape_A->get_supports(
                transform_A->basis_xform_inv(-best_axis).normalized(),
                supports_A,
                support_count_A
            );
        ...
        int support_count_B;
        if (castB) {
            shape_B->get_supports_transformed_cast(
                motion_B,
                best_axis,
                *transform_B,
                supports_B,
            ...
            shape_B->get_supports(
                transform_B->basis_xform_inv(best_axis).normalized(),
                supports_B,
                support_count_B
            );
        ...
        if (callback) {
            ...
            if (callback->sep_axis && *callback->sep_axis != Vector2()) {
                *callback->sep_axis =
                    Vector2(); // invalidate previous axis (no test)
            }
        }
    }

    _FORCE_INLINE_ SeparatorAxisTest2D(
...
#define TEST_POINT(m_a, m_b)                                                   \
    ((!separator.test_axis(((m_a) - (m_b)).normalized()))                      \
     || (castA                                                                 \
         && !separator.test_axis(((m_a) + p_motion_a - (m_b)).normalized()))   \
     || (castB                                                                 \
         && !separator.test_axis(((m_a) - ((m_b) + p_motion_b)).normalized())) \
     || (castA && castB                                                        \
         && !separator.test_axis(                                              \
             ((m_a) + p_motion_a - ((m_b) + p_motion_b)).normalized()          \
         )))
...
static void _collision_segment_segment(
    const Shape2DSW* p_a,
    const Transform2D& p_transform_a,
    const Shape2DSW* p_b,
    const Transform2D& p_transform_b,
    ...
    const SegmentShape2DSW* segment_A =
        static_cast<const SegmentShape2DSW*>(p_a);
    const SegmentShape2DSW* segment_B =
        static_cast<const SegmentShape2DSW*>(p_b);
    ...
    if (withMargin) {
        ...
        if (TEST_POINT(
                p_transform_a.xform(segment_A->get_a()),
                p_transform_b.xform(segment_B->get_a())
            )) {
            return;
        }
        if (TEST_POINT(
                p_transform_a.xform(segment_A->get_a()),
                p_transform_b.xform(segment_B->get_b())
            )) {
            return;
        }
        if (TEST_POINT(
                p_transform_a.xform(segment_A->get_b()),
                p_transform_b.xform(segment_B->get_a())
            )) {
            return;
        }
...
template <bool castA, bool castB, bool withMargin>
static void _collision_segment_circle(
    const Shape2DSW* p_a,
    const Transform2D& p_transform_a,
    const Shape2DSW* p_b,
    const Transform2D& p_transform_b,
    ...
    // segment normal
    if (!separator.test_axis((p_transform_a.xform(segment_A->get_b())
                              - p_transform_a.xform(segment_A->get_a()))
                                 .normalized()
                                 .tangent())) {
        return;
...
template <bool castA, bool castB, bool withMargin>
static void _collision_segment_rectangle(
    const Shape2DSW* p_a,
    const Transform2D& p_transform_a,
    const Shape2DSW* p_b,
    const Transform2D& p_transform_b,
    ...
    const SegmentShape2DSW* segment_A =
        static_cast<const SegmentShape2DSW*>(p_a);
    const RectangleShape2DSW* rectangle_B =
        static_cast<const RectangleShape2DSW*>(p_b);
    ...
    if (withMargin) {
        ...
        if (!separator.test_axis(
                rectangle_B->get_circle_axis(p_transform_b, inv, a)
            )) {
            return;
        }
        if (!separator.test_axis(
                rectangle_B->get_circle_axis(p_transform_b, inv, b)
            )) {
            return;
        }

        if (castA) {
            if (!separator.test_axis(rectangle_B->get_circle_axis(
                    p_transform_b,
                    inv,
                    a + p_motion_a
                ))) {
...
template <bool castA, bool castB, bool withMargin>
static void _collision_segment_capsule(
    const Shape2DSW* p_a,
    const Transform2D& p_transform_a,
    const Shape2DSW* p_b,
    const Transform2D& p_transform_b,
    ...
    const SegmentShape2DSW* segment_A =
        static_cast<const SegmentShape2DSW*>(p_a);
    const CapsuleShape2DSW* capsule_B =
        static_cast<const CapsuleShape2DSW*>(p_b);
    ...
    if (TEST_POINT(
            p_transform_a.xform(segment_A->get_a()),
            (p_transform_b.get_origin()
             + p_transform_b.elements[1] * capsule_B->get_height() * 0.5)
        )) {
        return;
    }
    if (TEST_POINT(
...
template <bool castA, bool castB, bool withMargin>
static void _collision_segment_convex_polygon(
    const Shape2DSW* p_a,
    const Transform2D& p_transform_a,
    const Shape2DSW* p_b,
    const Transform2D& p_transform_b,
    ...
    const SegmentShape2DSW* segment_A =
        static_cast<const SegmentShape2DSW*>(p_a);
    const ConvexPolygonShape2DSW* convex_B =
        static_cast<const ConvexPolygonShape2DSW*>(p_b);
    ...
    for (int i = 0; i < convex_B->get_point_count(); i++) {
        if (!separator.test_axis(
                convex_B->get_xformed_segment_normal(p_transform_b, i)
            )) {
            return;
        }
        ...
        if (withMargin) {
            if (TEST_POINT(
                    p_transform_a.xform(segment_A->get_a()),
                    p_transform_b.xform(convex_B->get_point(i))
                )) {
                return;
            }
            if (TEST_POINT(
...
template <bool castA, bool castB, bool withMargin>
static void _collision_circle_circle(
    const Shape2DSW* p_a,
    const Transform2D& p_transform_a,
    const Shape2DSW* p_b,
    const Transform2D& p_transform_b,
...
static void _collision_circle_rectangle(
    ...
    {
        if (!separator.test_axis(
                rectangle_B->get_circle_axis(p_transform_b, binv, sphere)
            )) {
            return;
        }
    }
    ...
    if (castA) {
        ...
        if (!separator.test_axis(
                rectangle_B->get_circle_axis(p_transform_b, binv, sphereofs)
            )) {
            return;
        }
    }
...
template <bool castA, bool castB, bool withMargin>
static void _collision_circle_convex_polygon(
    const Shape2DSW* p_a,
    const Transform2D& p_transform_a,
    const Shape2DSW* p_b,
    const Transform2D& p_transform_b,
...
template <bool castA, bool castB, bool withMargin>
static void _collision_rectangle_rectangle(
    const Shape2DSW* p_a,
    const Transform2D& p_transform_a,
    const Shape2DSW* p_b,
    const Transform2D& p_transform_b,
    ...
    const RectangleShape2DSW* rectangle_A =
        static_cast<const RectangleShape2DSW*>(p_a);
    const RectangleShape2DSW* rectangle_B =
        static_cast<const RectangleShape2DSW*>(p_b);
    ...
    if (withMargin) {
        ...
        if (!separator.test_axis(rectangle_A->get_box_axis(
                p_transform_a,
                invA,
                rectangle_B,
                p_transform_b,
...
template <bool castA, bool castB, bool withMargin>
static void _collision_rectangle_capsule(
    const Shape2DSW* p_a,
    const Transform2D& p_transform_a,
    const Shape2DSW* p_b,
    const Transform2D& p_transform_b,
    ...
    for (int i = 0; i < 2; i++) {
        {
            Vector2 capsule_endpoint = p_transform_b.get_origin()
                                     + p_transform_b.elements[1]
                                           * capsule_B->get_height()
                                           * (i == 0 ? 0.5 : -0.5);
...
template <bool castA, bool castB, bool withMargin>
static void _collision_rectangle_convex_polygon(
    const Shape2DSW* p_a,
    const Transform2D& p_transform_a,
    const Shape2DSW* p_b,
    const Transform2D& p_transform_b,
        ...
        if (withMargin) {
            ...
            if (castA) {
                if (!separator.test_axis(rectangle_A->get_circle_axis(
                        p_transform_a,
                        boxinv,
                        p_transform_b.xform(convex_B->get_point(i)) - p_motion_a
                    ))) {
                    return;
                }
            }
            if (castB) {
                if (!separator.test_axis(rectangle_A->get_circle_axis(
                        p_transform_a,
                        boxinv,
                        p_transform_b.xform(convex_B->get_point(i)) + p_motion_b
                    ))) {
                    return;
                }
            }
...
static void _collision_capsule_capsule(
    ...
    const CapsuleShape2DSW* capsule_A =
        static_cast<const CapsuleShape2DSW*>(p_a);
    const CapsuleShape2DSW* capsule_B =
        static_cast<const CapsuleShape2DSW*>(p_b);
    ...
    for (int i = 0; i < 2; i++) {
        Vector2 capsule_endpoint_A = p_transform_a.get_origin()
                                   + p_transform_a.elements[1]
                                         * capsule_A->get_height()
                                         * (i == 0 ? 0.5 : -0.5);
...
static void _collision_convex_polygon_convex_polygon(
    ...
    const ConvexPolygonShape2DSW* convex_A =
        static_cast<const ConvexPolygonShape2DSW*>(p_a);
    const ConvexPolygonShape2DSW* convex_B =
        static_cast<const ConvexPolygonShape2DSW*>(p_b);
    ...
    for (int i = 0; i < convex_A->get_point_count(); i++) {
        if (!separator.test_axis(
                convex_A->get_xformed_segment_normal(p_transform_a, i)
            )) {
            return;
        }
    }
    ...
    if (withMargin) {
        for (int i = 0; i < convex_A->get_point_count(); i++) {
            for (int j = 0; j < convex_B->get_point_count(); j++) {
                if (TEST_POINT(
                        p_transform_a.xform(convex_A->get_point(i)),
                        p_transform_b.xform(convex_B->get_point(j))
                    )) {
                    return;
                }
            }
...
bool sat_2d_calculate_penetration(
    ...
    if (p_margin_A || p_margin_B) {
        if (*motion_A == Vector2() && *motion_B == Vector2()) {
            ...
            collision_func =
                collision_table_castA_margin[type_A - 2][type_B - 2];
        } else if (*motion_A == Vector2() && *motion_B != Vector2()) {
            collision_func =
                collision_table_castB_margin[type_A - 2][type_B - 2];
        } else {
            collision_func =
                collision_table_castA_castB_margin[type_A - 2][type_B - 2];
        }
    } else {
        if (*motion_A == Vector2() && *motion_B == Vector2()) {
            ...
            collision_func =
                collision_table_castA_castB[type_A - 2][type_B - 2];
        }
    }
