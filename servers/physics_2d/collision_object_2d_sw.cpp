...
void CollisionObject2DSW::add_shape(
    Shape2DSW* p_shape,
...
void CollisionObject2DSW::set_shape_metadata(
    int p_index,
...
void CollisionObject2DSW::set_shape_transform(
    int p_index,
    const Transform2D& p_transform
) {
    ERR_FAIL_INDEX(p_index, shapes.size());
...
void CollisionObject2DSW::remove_shape(int p_index) {
    // remove anything from shape to be erased to end, so subindices don't
    // change
    ERR_FAIL_INDEX(p_index, shapes.size());
...
void CollisionObject2DSW::_update_shapes() {
    ...
    for (int i = 0; i < shapes.size(); i++) {
        ...
        if (s.bpid == 0) {
            s.bpid =
                space->get_broadphase()->create(this, i, shape_aabb, _static);
            space->get_broadphase()->set_static(s.bpid, _static);
        }
