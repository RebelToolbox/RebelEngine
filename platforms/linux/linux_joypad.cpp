...
#define test_bit(nr, addr)                                                     \
    (((1UL << ((nr) % LONG_BITS)) & ((addr)[(nr) / LONG_BITS])) != 0)
...
LinuxJoypad::LinuxJoypad(InputDefault* in) {
#ifdef UDEV_ENABLED
    use_udev = initialize_libudev() == 0;
    if (use_udev) {
        print_verbose("LinuxJoypad: udev enabled and loaded successfully.");
    } else {
        print_verbose(
            "LinuxJoypad: udev enabled, but couldn't be loaded. Falling back "
            "to /dev/input to detect joypads."
        );
    }
#else
    print_verbose(
        "LinuxJoypad: udev disabled, parsing /dev/input to detect joypads."
    );
#endif
    input = in;
...
LinuxJoypad::~LinuxJoypad() {
    exit_monitor.set();
    joy_thread.wait_to_finish();
    close_joypad();
}

void LinuxJoypad::joy_thread_func(void* p_user) {
    if (p_user) {
        LinuxJoypad* joy = (LinuxJoypad*)p_user;
        joy->run_joypad_thread();
    }
}

void LinuxJoypad::run_joypad_thread() {
#ifdef UDEV_ENABLED
    if (use_udev) {
        udev* _udev = udev_new();
        if (!_udev) {
            use_udev = false;
            ERR_PRINT(
                "Failed getting an udev context, falling back to parsing "
                "/dev/input."
            );
            monitor_joypads();
        } else {
...
#ifdef UDEV_ENABLED
void LinuxJoypad::enumerate_joypads(udev* p_udev) {
...
void LinuxJoypad::monitor_joypads(udev* p_udev) {
...
void LinuxJoypad::monitor_joypads() {
    while (!exit_monitor.is_set()) {
        joy_mutex.lock();
...
int LinuxJoypad::get_joy_from_path(String p_path) const {
...
void LinuxJoypad::close_joypad(int p_id) {
    if (p_id == -1) {
        for (int i = 0; i < JOYPADS_MAX; i++) {
            close_joypad(i);
        };
...
void LinuxJoypad::setup_joypad_properties(int p_id) {
    Joypad* joy = &joypads[p_id];
...
void LinuxJoypad::open_joypad(const char* p_path) {
    ...
    if (fd != -1 && joy_num != -1) {
        ...
        // check if the device supports basic gamepad events, prevents certain
        // keyboards from being detected as joypads
        if (!(test_bit(EV_KEY, evbit) && test_bit(EV_ABS, evbit)
              && (test_bit(ABS_X, absbit) || test_bit(ABS_Y, absbit)
                  || test_bit(ABS_HAT0X, absbit) || test_bit(ABS_GAS, absbit)
            ...
            && !(
                test_bit(EV_ABS, evbit) && test_bit(ABS_X, absbit)
                && test_bit(ABS_Y, absbit) && test_bit(ABS_RX, absbit)
...
void LinuxJoypad::joypad_vibration_start(
    int p_id,
    ...
    Joypad& joy = joypads[p_id];
    if (!joy.force_feedback || joy.fd == -1 || p_weak_magnitude < 0.f
        || p_weak_magnitude > 1.f || p_strong_magnitude < 0.f
        || p_strong_magnitude > 1.f) {
        return;
    }
    ...
    effect.u.rumble.strong_magnitude =
        floor(p_strong_magnitude * (float)0xffff);
...
void LinuxJoypad::joypad_vibration_stop(int p_id, uint64_t p_timestamp) {
    Joypad& joy = joypads[p_id];
...
InputDefault::JoyAxis LinuxJoypad::axis_correct(
    const input_absinfo* p_abs,
    int p_value
) const {
...
void LinuxJoypad::process_joypads() {
                        ...
                        switch (ev.code) {
                            case ABS_HAT0X:
                                if (ev.value != 0) {
                                    if (ev.value < 0) {
                                        joy->dpad =
                                            (joy->dpad
                                             | InputDefault::HAT_MASK_LEFT)
                                            & ~InputDefault::HAT_MASK_RIGHT;
                                    } else {
                                        joy->dpad =
                                            (joy->dpad
                                             | InputDefault::HAT_MASK_RIGHT)
                            ...
                            case ABS_HAT0Y:
                                if (ev.value != 0) {
                                    if (ev.value < 0) {
                                        joy->dpad =
                                            (joy->dpad
                                             | InputDefault::HAT_MASK_UP)
                                            & ~InputDefault::HAT_MASK_DOWN;
                                    } else {
                                        joy->dpad =
                                            (joy->dpad
                                             | InputDefault::HAT_MASK_DOWN)
                            ...
                            default:
                                ...
                                if (joy->abs_map[ev.code] != -1
                                    && joy->abs_info[ev.code]) {
                                    InputDefault::JoyAxis value = axis_correct(
                                        joy->abs_info[ev.code],
                                        ev.value
                                    );
                                    joy->curr_axis[joy->abs_map[ev.code]] =
                                        value;
                                }
                                break;
                        }
