...
static Ref<ImageTexture> editor_generate_icon(
    ...
    // Upsample icon generation only if the editor scale isn't an integer
    // multiplier. Generating upsampled icons is slower, and the benefit is
    // hardly visible with integer editor scales.
    const bool upsample = !Math::is_equal_approx(Math::round(p_scale), p_scale);
    ImageLoaderSVG::create_image_from_string(
        img,
        editor_icons_sources[p_index],
        p_scale,
        upsample,
        p_convert_color
    );
...
void editor_register_and_generate_icons(
    ...
    if (!p_only_thumbs) {
        for (int i = 0; i < editor_icons_count; i++) {
            ...
            const Ref<ImageTexture> icon =
                editor_generate_icon(i, !is_exception);
    ...
    // We don't need filtering when generating at one of the default
    // resolutions.
...
Ref<Theme> create_editor_theme(const Ref<Theme> p_theme) {
    ...
    // Please, use alphabet order if you've added new theme here(After "Default"
    // and "Custom")
    ...
    if (preset == "Default") {
        ...
        preset_accent_color = Color(0.11f, 1, 0.6f);
        ...
        preset_accent_color = Color(0.72f, 0.89f, 1);
        preset_base_color   = Color(0.24f, 0.24f, 0.24f);
        preset_contrast     = 0.2f;
    } else if (preset == "Light") {
        preset_accent_color = Color(0.13f, 0.44f, 1);
    ...
    const Color font_color_hl =
        mono_color.linear_interpolate(base_color, 0.15f);
    const Color font_color_focus =
        mono_color.linear_interpolate(base_color, 0.15f);
    const Color font_color_disabled =
        Color(mono_color.r, mono_color.g, mono_color.b, 0.3f);
    const Color font_color_readonly =
        Color(mono_color.r, mono_color.g, mono_color.b, 0.65f);
    const Color font_color_selection = accent_color * Color(1, 1, 1, 0.4f);
    const Color color_disabled =
        mono_color.inverted().linear_interpolate(base_color, 0.7f);
    const Color color_disabled_bg =
        mono_color.inverted().linear_interpolate(base_color, 0.9f);

    Color icon_color_hover   = Color(1, 1, 1) * (dark_theme ? 1.15f : 1.45f);
    icon_color_hover.a       = 1;
    ...
    icon_color_pressed.a     = 1;

    const Color separator_color =
        Color(mono_color.r, mono_color.g, mono_color.b, 0.1f);

    const Color highlight_color =
        Color(mono_color.r, mono_color.g, mono_color.b, 0.2f);
    ...
    theme->set_color(
        ...
        "Editor",
        accent_color * Color(1, 1, 1, 0.3f)
    );
    theme->set_color(
        "box_selection_stroke_color",
        "Editor",
        accent_color * Color(1, 1, 1, 0.8f)
    );
    ...
    Color property_color =
        font_color.linear_interpolate(Color(0.5f, 0.5f, 0.5f), 0.5f);
    ...
    // the resolution and the icon color (dark_theme bool) has not changed, so
    // we do not regenerate the icons
    if (p_theme != nullptr
    ...
    if (p_theme != nullptr
        && fabs(
               (double)p_theme->get_constant("thumb_size", "Editor")
               - thumb_size
           ) > 0.00001) {
        editor_register_and_generate_icons(
            p_theme,
            dark_theme,
            thumb_size,
            true
    ...
    const int margin_size_extra =
        default_margin_size + CLAMP(border_size, 0, 3);

    // styleboxes
    // this is the most commonly used stylebox, variations should be made as
    // duplicate of this
    Ref<StyleBoxFlat> style_default = make_flat_stylebox(
        base_color,
        default_margin_size,
        default_margin_size,
        default_margin_size,
    ...
    // Button and widgets
    const float extra_spacing =
        EDITOR_GET("interface/theme/additional_spacing");

    Ref<StyleBoxFlat> style_widget = style_default->duplicate();
    style_widget->set_default_margin(
        MARGIN_LEFT,
        (extra_spacing + 6) * EDSCALE
    );
    style_widget->set_default_margin(
        MARGIN_TOP,
    ...
    style_popup_separator->set_grow_begin(
        popup_margin_size - MAX(EDSCALE, border_width)
    );
    style_popup_separator->set_grow_end(
        popup_margin_size - MAX(EDSCALE, border_width)
    );
    ...
    style_popup_labeled_separator_left->set_grow_begin(
        popup_margin_size - MAX(EDSCALE, border_width)
    );
    ...
    style_popup_labeled_separator_right->set_grow_end(
        popup_margin_size - MAX(EDSCALE, border_width)
    );
    style_popup_labeled_separator_right->set_color(separator_color);
    style_popup_labeled_separator_right->set_thickness(
        MAX(EDSCALE, border_width)
    );

    Ref<StyleBoxEmpty> style_empty = make_empty_stylebox(
        default_margin_size,
        default_margin_size,
        default_margin_size,
        default_margin_size
    ...
    const int tab_default_margin_vertical =
        5 * EDSCALE + extra_spacing * EDSCALE;
    ...
    style_tab_selected->set_default_margin(
        MARGIN_LEFT,
        tab_default_margin_side
    );
    style_tab_selected->set_default_margin(
        MARGIN_RIGHT,
        tab_default_margin_side
    ...
    // CanvasItem Editor
    Ref<StyleBoxFlat> style_canvas_editor_info =
        make_flat_stylebox(Color(0, 0, 0, 0.2f));
    style_canvas_editor_info->set_expand_margin_size_all(4 * EDSCALE);
    theme->set_stylebox(
        "CanvasItemInfoOverlay",
        "EditorStyles",
        style_canvas_editor_info
    );
    ...
    theme
        ->set_color("font_color_disabled", "OptionButton", font_color_disabled);
    ...
    theme->set_icon(
        "on_disabled",
        "CheckButton",
        theme->get_icon("GuiToggleOnDisabled", "EditorIcons")
    );
    theme->set_icon(
        "off",
        "CheckButton",
    ...
    theme->set_icon(
        "off_disabled",
        "CheckButton",
        theme->get_icon("GuiToggleOffDisabled", "EditorIcons")
    );
    ...
    sb_checkbox->set_default_margin(
        MARGIN_RIGHT,
        default_margin_size * EDSCALE
    );
    ...
    theme->set_stylebox(
        ...
        "PopupMenu",
        style_popup_labeled_separator_left
    );
    theme->set_stylebox(
        "labeled_separator_right",
        "PopupMenu",
        style_popup_labeled_separator_right
    );
    ...
    theme->set_constant(
        "vseparation",
        "PopupMenu",
        (extra_spacing + default_margin_size + 1) * EDSCALE
    );
    ...
    for (int i = 0; i < 16; i++) {
        ...
        si_base_color.set_hsv(
            Math::fmod(float(si_base_color.get_h() + hue_rotate), float(1.0)),
            si_base_color.get_s(),
            si_base_color.get_v()
        );
        si_base_color = accent_color.linear_interpolate(
            si_base_color,
            float(EDITOR_GET("docks/property_editor/subresource_hue_tint"))
        );

        Ref<StyleBoxFlat> sub_inspector_bg;

        sub_inspector_bg = make_flat_stylebox(
            dark_color_1.linear_interpolate(si_base_color, 0.08f),
            2,
            0,
            2,
            2
        ...
        sub_inspector_bg->set_border_color(
            si_base_color * Color(0.7f, 0.7f, 0.7f, 0.8f)
        );
        sub_inspector_bg->set_draw_center(true);

        theme->set_stylebox(
            "sub_inspector_bg" + itos(i),
            "Editor",
            sub_inspector_bg
        );
        ...
        bg_color.instance();
        bg_color->set_bg_color(si_base_color * Color(0.7f, 0.7f, 0.7f, 0.8f));
        ...
        bg_color_selected->set_bg_color(
            si_base_color * Color(0.8f, 0.8f, 0.8f, 0.8f)
        );

        theme->set_stylebox(
            "sub_inspector_property_bg" + itos(i),
            "Editor",
            bg_color
        );
        theme->set_stylebox(
            "sub_inspector_property_bg_selected" + itos(i),
            "Editor",
            bg_color_selected
        );
    }
    ...
    theme->set_color(
        "sub_inspector_property_color",
        "Editor",
        dark_theme ? Color(1, 1, 1, 1) : Color(0, 0, 0, 1)
    );
    ...
    theme->set_stylebox(
        "bg",
        "EditorProperty",
        Ref<StyleBoxEmpty>(memnew(StyleBoxEmpty))
    );
    theme->set_constant(
        "vseparation",
        "EditorProperty",
    ...
    const Color guide_color =
        Color(mono_color.r, mono_color.g, mono_color.b, 0.05f);
    Color relationship_line_color = Color(
        mono_color.r,
        mono_color.g,
        mono_color.b,
    ...
    Color prop_category_color =
        dark_color_1.linear_interpolate(mono_color, 0.12f);
    Color prop_section_color =
        dark_color_1.linear_interpolate(mono_color, 0.09f);
    Color prop_subsection_color =
        dark_color_1.linear_interpolate(mono_color, 0.06f);
    ...
    theme->set_icon(
        "menu_highlight",
        "TabContainer",
        theme->get_icon("GuiTabMenuHl", "EditorIcons")
    );
    theme->set_stylebox("SceneTabFG", "EditorStyles", style_tab_selected);
    theme->set_stylebox("SceneTabBG", "EditorStyles", style_tab_unselected);
    theme
        ->set_icon("close", "Tabs", theme->get_icon("GuiClose", "EditorIcons"));
    ...
    theme->set_icon(
        "increment_highlight",
        "Tabs",
        theme->get_icon("GuiScrollArrowRightHl", "EditorIcons")
    );
    theme->set_icon(
        "decrement_highlight",
        "Tabs",
        theme->get_icon("GuiScrollArrowLeftHl", "EditorIcons")
    );
    theme->set_icon(
        "increment_highlight",
        "TabContainer",
    ...
    style_content_panel->set_default_margin(
        MARGIN_TOP,
    ...
    // These styleboxes can be used on tabs against the base color background
    // (e.g. nested tabs).
    ...
    theme->set_stylebox(
        "tab_selected_odd",
        "TabContainer",
        style_tab_selected_odd
    );

    Ref<StyleBoxFlat> style_content_panel_odd =
    ...
    style_content_panel_vp->set_default_margin(
        MARGIN_TOP,
        default_margin_size * EDSCALE
    );
    ...
    Ref<StyleBoxFlat> style_theme_preview_tab =
        style_tab_selected_odd->duplicate();
    style_theme_preview_tab->set_expand_margin_size(MARGIN_BOTTOM, 3 * EDSCALE);
    theme->set_stylebox(
        "ThemeEditorPreviewFG",
        "EditorStyles",
        style_theme_preview_tab
    );
    Ref<StyleBoxFlat> style_theme_preview_bg_tab =
        style_tab_unselected->duplicate();
    style_theme_preview_bg_tab->set_expand_margin_size(
        MARGIN_BOTTOM,
        2 * EDSCALE
    );
    theme->set_stylebox(
        "ThemeEditorPreviewBG",
        "EditorStyles",
        style_theme_preview_bg_tab
    );
    ...
    theme->set_stylebox(
        "separator",
        "VSeparator",
        make_line_stylebox(separator_color, border_width, 0, 0, true)
    );
    ...
    style_panel_invisible_top->set_expand_margin_size(
        MARGIN_TOP,
        -stylebox_offset
    );
    ...
    theme
        ->set_icon("tab", "TextEdit", theme->get_icon("GuiTab", "EditorIcons"));
    theme->set_icon(
        "space",
        "TextEdit",
        theme->get_icon("GuiSpace", "EditorIcons")
    );
    theme->set_icon(
        "folded",
        "TextEdit",
    ...
    style_complex_window->set_border_color(
        highlight_tabs ? tab_color : dark_color_2
    );
    ...
    theme->set_stylebox(
        ...
        "HScrollBar",
        make_stylebox(
            theme->get_icon("GuiScrollBg", "EditorIcons"),
            5,
            5,
            5,
            5,
    ...
    theme->set_stylebox(
        ...
        "HScrollBar",
        make_stylebox(
            theme->get_icon("GuiScrollGrabber", "EditorIcons"),
            6,
            6,
            6,
            6,
    ...
    theme->set_stylebox(
        ...
        "HScrollBar",
        make_stylebox(
            theme->get_icon("GuiScrollGrabberHl", "EditorIcons"),
            5,
            5,
            5,
            5,
    ...
    theme->set_stylebox(
        ...
        "HScrollBar",
        make_stylebox(
            theme->get_icon("GuiScrollGrabberPressed", "EditorIcons"),
            6,
            6,
            6,
            6,
    ...
    style_tooltip->set_bg_color(
        Color(mono_color.r, mono_color.g, mono_color.b, 0.9f)
    );
    ...
    theme->set_color(
        "font_color_shadow",
        "TooltipLabel",
        mono_color.inverted() * Color(1, 1, 1, 0.1f)
    );
    ...
    theme->set_stylebox(
        ...
        "ProgressBar",
        make_stylebox(
            theme->get_icon("GuiProgressBar", "EditorIcons"),
            4,
            4,
            4,
            4,
    ...
    theme->set_stylebox(
        ...
        "ProgressBar",
        make_stylebox(
            theme->get_icon("GuiProgressFill", "EditorIcons"),
            6,
            6,
            6,
            6,
    ...
    if (dark_theme) {
        theme->set_color("grid_major", "GraphEdit", Color(1, 1, 1, 0.15f));
        theme->set_color("grid_minor", "GraphEdit", Color(1, 1, 1, 0.07f));
    } else {
        theme->set_color("grid_major", "GraphEdit", Color(0, 0, 0, 0.15f));
        theme->set_color("grid_minor", "GraphEdit", Color(0, 0, 0, 0.07f));
    }
    theme->set_color(
        "selection_fill",
        "GraphEdit",
        theme->get_color("box_selection_fill_color", "Editor")
    );
    theme->set_color(
        "selection_stroke",
        "GraphEdit",
        theme->get_color("box_selection_stroke_color", "Editor")
    );
    ...
    theme->set_icon(
        "minimap",
        "GraphEdit",
        theme->get_icon("GridMinimap", "EditorIcons")
    );
    theme->set_constant("bezier_len_pos", "GraphEdit", 80 * EDSCALE);
    theme->set_constant("bezier_len_neg", "GraphEdit", 160 * EDSCALE);

    // GraphEditMinimap
    Ref<StyleBoxFlat> style_minimap_bg =
        make_flat_stylebox(dark_color_1, 0, 0, 0, 0);
    ...
    if (dark_theme) {
        ...
        style_minimap_camera =
            make_flat_stylebox(Color(0.38f, 0.38f, 0.38f, 0.2f), 0, 0, 0, 0);
        style_minimap_camera->set_border_color(Color(0.38f, 0.38f, 0.38f, 0.45f)
        );
    ...
    Ref<Texture> minimap_resizer_icon =
        theme->get_icon("GuiResizer", "EditorIcons");
    Color minimap_resizer_color;
    if (dark_theme) {
        minimap_resizer_color = Color(1, 1, 1, 0.65f);
    } else {
        minimap_resizer_color = Color(0, 0, 0, 0.65f);
    }
    theme->set_icon(
        "resizer",
        "GraphEditMinimap",
        flip_icon(minimap_resizer_icon, true, true)
    );
    theme
    ...
    graphsbselected->set_border_color(
        Color(accent_color.r, accent_color.g, accent_color.b, 0.9f)
    );
    ...
    graphsbbreakpoint->set_shadow_color(warning_color * Color(1, 1, 1, 0.1f));
    ...
    graphsbposition->set_shadow_color(error_color * Color(1, 1, 1, 0.2f));
    Ref<StyleBoxFlat> smgraphsb = make_flat_stylebox(
        Color(mv, mv, mv, 0.7f),
        gn_margin_side,
        24,
        gn_margin_side,
    ...
    Ref<StyleBoxFlat> smgraphsbselected = make_flat_stylebox(
        Color(mv, mv, mv, 0.9f),
        gn_margin_side,
        24,
        gn_margin_side,
    ...
    smgraphsbselected->set_border_color(
        Color(accent_color.r, accent_color.g, accent_color.b, 0.9f)
    );
    ...
    theme->set_icon(
        "parent_folder",
        "FileDialog",
        theme->get_icon("ArrowUp", "EditorIcons")
    );
    theme->set_icon(
        "reload",
        "FileDialog",
    ...
    // Use a different color for folder icons to make them easier to distinguish
    // from files. On a light theme, the icon will be dark, so we need to
    // lighten it before blending it with the accent color.
    theme->set_color(
        "folder_icon_modulate",
        "FileDialog",
        (dark_theme ? Color(1, 1, 1) : Color(4.25, 4.25, 4.25))
            .linear_interpolate(accent_color, 0.7f)
    );
    ...
    theme->set_icon(
        ...
        "ColorPicker",
        theme->get_icon("OverbrightIndicator", "EditorIcons")
    );
    ...
    style_info_3d_viewport->set_bg_color(
        style_info_3d_viewport->get_bg_color() * Color(1, 1, 1, 0.5)
    );
    style_info_3d_viewport->set_border_width_all(0);
    theme->set_stylebox(
        "Information3dViewport",
        "EditorStyles",
    ...
    theme->set_stylebox(
        "preview_picker_overlay",
        "ThemeEditor",
        theme_preview_picker_sb
    );
    Color theme_preview_picker_label_bg_color = accent_color;
    theme_preview_picker_label_bg_color.set_hsv(
        theme_preview_picker_label_bg_color.get_h(),
        theme_preview_picker_label_bg_color.get_s(),
        0.5
    );
    Ref<StyleBoxFlat> theme_preview_picker_label_sb = make_flat_stylebox(
    ...
    theme->set_stylebox(
        "preview_picker_label",
        "ThemeEditor",
        theme_preview_picker_label_sb
    );
    ...
    const Color main_color =
        dark_theme ? Color(0.34f, 0.7f, 1) : Color(0.02f, 0.5f, 1);

    const Color symbol_color =
        Color(0.34f, 0.57f, 1)
            .linear_interpolate(mono_color, dark_theme ? 0.5 : 0.3);
    const Color keyword_color = Color(1, 0.44f, 0.52f);
    const Color control_flow_keyword_color =
        dark_theme ? Color(1, 0.55f, 0.8f) : Color(0.8f, 0.4f, 0.6f);
    const Color basetype_color =
        dark_theme ? Color(0.26f, 1, 0.76f) : Color(0, 0.76f, 0.38f);
    const Color type_color =
        basetype_color.linear_interpolate(mono_color, dark_theme ? 0.4f : 0.3f);
    const Color usertype_color =
        basetype_color.linear_interpolate(mono_color, dark_theme ? 0.7f : 0.5f);
    const Color comment_color = dim_color;
    const Color string_color =
        (dark_theme ? Color(1, 0.85f, 0.26f) : Color(1, 0.82f, 0.09f))
            .linear_interpolate(mono_color, dark_theme ? 0.5f : 0.3f);

    const Color te_background_color =
        dark_theme ? background_color : base_color;
    const Color completion_background_color =
    ...
    const Color number_color =
        basetype_color.linear_interpolate(mono_color, dark_theme ? 0.5f : 0.3f);
    const Color function_color = main_color;
    const Color member_variable_color =
        main_color.linear_interpolate(mono_color, 0.6f);
    const Color mark_color =
        Color(error_color.r, error_color.g, error_color.b, 0.3f);
...
Ref<Theme> create_custom_theme(const Ref<Theme> p_theme) {
    ...
    const String custom_theme_path =
