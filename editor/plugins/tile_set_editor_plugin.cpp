    ...
    for (int i = 0; i < p_node->get_child_count(); i++) {
        ...
        if (mi->is_region()) {
            ...
            p_library->tile_set_region(
                id,
                Rect2(Vector2(frame % hframes, frame / hframes) * s, s)
            );
        }
        ...
        for (int j = 0; j < mi->get_child_count(); j++) {
            ...
            if (Object::cast_to<NavigationPolygonInstance>(child2)) {
                nav_poly = Object::cast_to<NavigationPolygonInstance>(child2)
                               ->get_navigation_polygon();
            }

            if (Object::cast_to<LightOccluder2D>(child2)) {
                occluder = Object::cast_to<LightOccluder2D>(child2)
            ...
            for (List<uint32_t>::Element* E = shapes.front(); E;
                ...
                Transform2D shape_transform =
                    sb->get_transform()
                    * sb->shape_owner_get_transform(E->get());
                bool one_way =
                    sb->is_shape_owner_one_way_collision_enabled(E->get());

                shape_transform[2] -= phys_offset;

                for (int k = 0; k < sb->shape_owner_get_shape_count(E->get());
                     k++) {
...
void TileSetEditor::_import_scene(
    Node* p_scene,
...
Error TileSetEditor::update_library_file(
    Node* p_base_scene,
    Ref<TileSet> ml,
    bool p_merge
) {
...
Variant TileSetEditor::get_drag_data_fw(
...
bool TileSetEditor::can_drop_data_fw(
    const Point2& p_point,
...
void TileSetEditor::drop_data_fw(
    const Point2& p_point,
    const Variant& p_data,
    Control* p_from
) {
...
void TileSetEditor::_bind_methods() {
    ClassDB::bind_method(
        "_undo_redo_import_scene",
        &TileSetEditor::_undo_redo_import_scene
    );
    ClassDB::bind_method(
        "_on_tileset_toolbar_button_pressed",
        &TileSetEditor::_on_tileset_toolbar_button_pressed
    );
    ClassDB::bind_method(
        "_on_textures_added",
        &TileSetEditor::_on_textures_added
    );
    ClassDB::bind_method(
        "_on_tileset_toolbar_confirm",
        &TileSetEditor::_on_tileset_toolbar_confirm
    );
    ClassDB::bind_method(
        "_on_texture_list_selected",
        &TileSetEditor::_on_texture_list_selected
    );
    ClassDB::bind_method(
        "_on_edit_mode_changed",
        &TileSetEditor::_on_edit_mode_changed
    );
    ClassDB::bind_method(
        "_on_scroll_container_input",
        &TileSetEditor::_on_scroll_container_input
    );
    ClassDB::bind_method(
        "_on_workspace_mode_changed",
        &TileSetEditor::_on_workspace_mode_changed
    );
    ClassDB::bind_method(
        "_on_workspace_overlay_draw",
        &TileSetEditor::_on_workspace_overlay_draw
    );
    ClassDB::bind_method(
        "_on_workspace_process",
        &TileSetEditor::_on_workspace_process
    );
    ClassDB::bind_method(
        "_on_workspace_draw",
        &TileSetEditor::_on_workspace_draw
    );
    ClassDB::bind_method(
        "_on_workspace_input",
        &TileSetEditor::_on_workspace_input
    );
    ClassDB::bind_method("_on_tool_clicked", &TileSetEditor::_on_tool_clicked);
    ClassDB::bind_method(
        "_on_priority_changed",
        &TileSetEditor::_on_priority_changed
    );
    ClassDB::bind_method(
        "_on_z_index_changed",
        &TileSetEditor::_on_z_index_changed
    );
    ClassDB::bind_method(
        "_on_grid_snap_toggled",
        &TileSetEditor::_on_grid_snap_toggled
    );
    ...
    ClassDB::bind_method(
        "update_texture_list_icon",
        &TileSetEditor::update_texture_list_icon
    );
    ClassDB::bind_method(
    ...
    switch (p_what) {
        case NOTIFICATION_READY: {
            add_constant_override(
                "autohide",
                1
            ); // Fixes the dragger always showing up.
        } break;
        case NOTIFICATION_ENTER_TREE:
        case NOTIFICATION_THEME_CHANGED: {
            tileset_toolbar_buttons[TOOL_TILESET_ADD_TEXTURE]->set_icon(
                get_icon("ToolAddNode", "EditorIcons")
            );
            tileset_toolbar_buttons[TOOL_TILESET_REMOVE_TEXTURE]->set_icon(
                get_icon("Remove", "EditorIcons")
            );
            tileset_toolbar_tools->set_icon(get_icon("Tools", "EditorIcons"));

            tool_workspacemode[WORKSPACE_EDIT]->set_icon(
                get_icon("Edit", "EditorIcons")
            );
            tool_workspacemode[WORKSPACE_CREATE_SINGLE]->set_icon(
                get_icon("AddSingleTile", "EditorIcons")
            );
            tool_workspacemode[WORKSPACE_CREATE_AUTOTILE]->set_icon(
                get_icon("AddAutotile", "EditorIcons")
            );
            tool_workspacemode[WORKSPACE_CREATE_ATLAS]->set_icon(
                get_icon("AddAtlasTile", "EditorIcons")
            );
            ...
            tools[SHAPE_NEW_POLYGON]->set_icon(
                get_icon("CollisionPolygon2D", "EditorIcons")
            );
            tools[SHAPE_NEW_RECTANGLE]->set_icon(
                get_icon("CollisionShape2D", "EditorIcons")
            );
            tools[SELECT_PREVIOUS]->set_icon(
                get_icon("ArrowLeft", "EditorIcons")
            );
            tools[SELECT_NEXT]->set_icon(get_icon("ArrowRight", "EditorIcons"));
            tools[SHAPE_DELETE]->set_icon(get_icon("Remove", "EditorIcons"));
            tools[SHAPE_KEEP_INSIDE_TILE]->set_icon(
            ...
            tools[VISIBLE_INFO]->set_icon(
                get_icon("InformationSign", "EditorIcons")
            );
            _update_toggle_shape_button();

            tool_editmode[EDITMODE_REGION]->set_icon(
                get_icon("RegionEdit", "EditorIcons")
            );
            tool_editmode[EDITMODE_COLLISION]->set_icon(
                get_icon("StaticBody2D", "EditorIcons")
            );
            tool_editmode[EDITMODE_OCCLUSION]->set_icon(
                get_icon("LightOccluder2D", "EditorIcons")
            );
            tool_editmode[EDITMODE_NAVIGATION]->set_icon(
                get_icon("Navigation2D", "EditorIcons")
            );
            tool_editmode[EDITMODE_BITMASK]->set_icon(
                get_icon("PackedDataContainer", "EditorIcons")
            );
            tool_editmode[EDITMODE_PRIORITY]->set_icon(
                get_icon("MaterialPreviewLight1", "EditorIcons")
            );
            tool_editmode[EDITMODE_ICON]->set_icon(
                get_icon("LargeTexture", "EditorIcons")
            );
...
TileSetEditor::TileSetEditor(EditorNode* p_editor) {
    ...
    tileset_toolbar_buttons[TOOL_TILESET_ADD_TEXTURE]->connect(
        "pressed",
        this,
        "_on_tileset_toolbar_button_pressed",
        varray(TOOL_TILESET_ADD_TEXTURE)
    );
    tileset_toolbar_container->add_child(
    ...
    tileset_toolbar_buttons[TOOL_TILESET_REMOVE_TEXTURE]->connect(
        "pressed",
        this,
        "_on_tileset_toolbar_button_pressed",
        varray(TOOL_TILESET_REMOVE_TEXTURE)
    );
    tileset_toolbar_container->add_child(
        tileset_toolbar_buttons[TOOL_TILESET_REMOVE_TEXTURE]
    );
    tileset_toolbar_buttons[TOOL_TILESET_REMOVE_TEXTURE]->set_tooltip(
        TTR("Remove selected Texture from TileSet.")
    );
    ...
    tileset_toolbar_tools->get_popup()->add_item(
        TTR("Create from Scene"),
        TOOL_TILESET_CREATE_SCENE
    );
    tileset_toolbar_tools->get_popup()->add_item(
        TTR("Merge from Scene"),
        TOOL_TILESET_MERGE_SCENE
    );

    tileset_toolbar_tools->get_popup()
        ->connect("id_pressed", this, "_on_tileset_toolbar_button_pressed");
    ...
    for (int i = 0; i < (int)WORKSPACE_MODE_MAX; i++) {
        ...
        tool_workspacemode[i]
            ->connect("pressed", this, "_on_workspace_mode_changed", varray(i));
    ...
    tools[SELECT_NEXT]->set_shortcut(ED_SHORTCUT(
        "tileset_editor/next_shape",
        TTR("Next Coordinate"),
        KEY_PAGEDOWN
    ));
    tools[SELECT_NEXT]
        ->connect("pressed", this, "_on_tool_clicked", varray(SELECT_NEXT));
    tools[SELECT_NEXT]->set_tooltip(
        TTR("Select the next shape, subtile, or Tile.")
    );
    ...
    tools[SELECT_PREVIOUS]->set_shortcut(ED_SHORTCUT(
        "tileset_editor/previous_shape",
        TTR("Previous Coordinate"),
        KEY_PAGEUP
    ));
    tools[SELECT_PREVIOUS]->set_tooltip(
        TTR("Select the previous shape, subtile, or Tile.")
    );
    tools[SELECT_PREVIOUS]
        ->connect("pressed", this, "_on_tool_clicked", varray(SELECT_PREVIOUS));
    ...
    for (int i = 0; i < (int)EDITMODE_MAX; i++) {
        ...
        tool_editmode[i]
            ->connect("pressed", this, "_on_edit_mode_changed", varray(i));
        tool_hb->add_child(tool_editmode[i]);
    }
    tool_editmode[EDITMODE_COLLISION]->set_pressed(true);
    edit_mode = EDITMODE_COLLISION;

    tool_editmode[EDITMODE_REGION]->set_shortcut(
        ED_SHORTCUT("tileset_editor/editmode_region", TTR("Region Mode"), KEY_1)
    );
    tool_editmode[EDITMODE_COLLISION]->set_shortcut(ED_SHORTCUT(
        "tileset_editor/editmode_collision",
        TTR("Collision Mode"),
        KEY_2
    ));
    tool_editmode[EDITMODE_OCCLUSION]->set_shortcut(ED_SHORTCUT(
        "tileset_editor/editmode_occlusion",
        TTR("Occlusion Mode"),
        KEY_3
    ));
    tool_editmode[EDITMODE_NAVIGATION]->set_shortcut(ED_SHORTCUT(
        "tileset_editor/editmode_navigation",
        TTR("Navigation Mode"),
        KEY_4
    ));
    tool_editmode[EDITMODE_BITMASK]->set_shortcut(ED_SHORTCUT(
        "tileset_editor/editmode_bitmask",
        TTR("Bitmask Mode"),
        KEY_5
    ));
    tool_editmode[EDITMODE_PRIORITY]->set_shortcut(ED_SHORTCUT(
        "tileset_editor/editmode_priority",
        TTR("Priority Mode"),
        KEY_6
    ));
    tool_editmode[EDITMODE_ICON]->set_shortcut(
        ED_SHORTCUT("tileset_editor/editmode_icon", TTR("Icon Mode"), KEY_7)
    );
    tool_editmode[EDITMODE_Z_INDEX]->set_shortcut(ED_SHORTCUT(
    ...
    tools[TOOL_SELECT]
        ->connect("pressed", this, "_on_tool_clicked", varray(TOOL_SELECT));
    ...
    tools[BITMASK_COPY]
        ->connect("pressed", this, "_on_tool_clicked", varray(BITMASK_COPY));
    ...
    tools[BITMASK_PASTE]
        ->connect("pressed", this, "_on_tool_clicked", varray(BITMASK_PASTE));
    ...
    tools[BITMASK_CLEAR]
        ->connect("pressed", this, "_on_tool_clicked", varray(BITMASK_CLEAR));
    ...
    tools[SHAPE_NEW_RECTANGLE]->connect(
        "pressed",
        this,
        "_on_tool_clicked",
        varray(SHAPE_NEW_RECTANGLE)
    );
    tools[SHAPE_NEW_RECTANGLE]->set_shortcut(ED_SHORTCUT(
        "tileset_editor/shape_new_rectangle",
        TTR("New Rectangle"),
        KEY_MASK_SHIFT | KEY_R
    ));
    ...
    tools[SHAPE_NEW_POLYGON]->connect(
        "pressed",
        this,
        "_on_tool_clicked",
        varray(SHAPE_NEW_POLYGON)
    );
    tools[SHAPE_NEW_POLYGON]->set_shortcut(ED_SHORTCUT(
        "tileset_editor/shape_new_polygon",
        TTR("New Polygon"),
        KEY_MASK_SHIFT | KEY_P
    ));
    ...
    tools[SHAPE_TOGGLE_TYPE]->connect(
        "pressed",
        this,
        "_on_tool_clicked",
        varray(SHAPE_TOGGLE_TYPE)
    );
    ...
    tools[SHAPE_DELETE]
        ->connect("pressed", this, "_on_tool_clicked", varray(SHAPE_DELETE));
    tools[SHAPE_DELETE]->set_shortcut(ED_SHORTCUT(
        "tileset_editor/shape_delete",
        TTR("Delete Selected Shape"),
        KEY_MASK_SHIFT | KEY_BACKSPACE
    ));
    ...
    tools[SHAPE_KEEP_INSIDE_TILE]->set_tooltip(
        TTR("Keep polygon inside region Rect.")
    );
    ...
    tools[TOOL_GRID_SNAP]->set_tooltip(
        TTR("Enable snap and show grid (configurable via the Inspector).")
    );
    ...
    empty_message->set_text(
        TTR("Add or select a texture on the left panel to edit the tiles bound "
            "to it.")
    );
    ...
    switch (option) {
        ...
        case TOOL_TILESET_REMOVE_TEXTURE: {
            if (get_current_texture().is_valid()) {
                cd->set_text(
                    TTR("Remove selected texture? This will remove all tiles "
                        "which use it.")
                );
                cd->popup_centered(Size2(300, 60));
            } else {
                err_dialog->set_text(
                    TTR("You haven't selected a texture to remove.")
                );
        ...
        case TOOL_TILESET_CREATE_SCENE: {
            cd->set_text(
                TTR("Create from scene? This will overwrite all current tiles.")
            );
            cd->popup_centered(Size2(300, 60));
        } break;
    ...
    switch (option) {
        case TOOL_TILESET_REMOVE_TEXTURE: {
            ...
            for (List<int>::Element* E = ids.front(); E; E = E->next()) {
                if (tileset->tile_get_texture(E->get())->get_path()
                    == current_texture_path) {
                    undo_redo
                        ->add_do_method(tileset.ptr(), "remove_tile", E->get());
                    _undo_tile_removal(E->get());
                }
            }
            undo_redo
                ->add_do_method(this, "remove_texture", get_current_texture());
            undo_redo
                ->add_undo_method(this, "add_texture", get_current_texture());
        ...
        case TOOL_TILESET_CREATE_SCENE: {
            ...
            undo_redo->create_action(TTR(
                option == TOOL_TILESET_MERGE_SCENE ? "Merge Tileset from Scene"
                                                   : "Create Tileset from Scene"
            ));
            undo_redo->add_do_method(
                this,
                "_undo_redo_import_scene",
                scene,
                option == TOOL_TILESET_MERGE_SCENE
            );
...
void TileSetEditor::_on_textures_added(const PoolStringArray& p_paths) {
    ...
    for (int i = 0; i < p_paths.size(); i++) {
        ...
        ERR_CONTINUE_MSG(
            !t.is_valid(),
            "'" + p_paths[i] + "' is not a valid texture."
        );
...
void TileSetEditor::_on_edit_mode_changed(int p_edit_mode) {
    ...
    switch (edit_mode) {
        case EDITMODE_REGION: {
            ...
            tools[TOOL_SELECT]->set_pressed(true);
            tools[TOOL_SELECT]->set_tooltip(TTR(
        ...
        case EDITMODE_NAVIGATION: {
            ...
            tools[TOOL_SELECT]->set_tooltip(
                TTR("Select current edited sub-tile.\nClick on another Tile to "
                    "edit it.")
            );
        ...
        case EDITMODE_ICON: {
            ...
            if (edit_mode == EDITMODE_ICON) {
                tools[TOOL_SELECT]->set_tooltip(
                    TTR("Select sub-tile to use as icon, this will be also "
                        "used on invalid autotile bindings.\nClick on another "
                        "Tile to edit it.")
                );
                spin_priority->hide();
                spin_z_index->hide();
                ...
                tools[TOOL_SELECT]->set_tooltip(
                    TTR("Select sub-tile to change its priority.\nClick on "
                        "another Tile to edit it.")
                );
                spin_priority->show();
                spin_z_index->hide();
            } else {
                tools[TOOL_SELECT]->set_tooltip(
                    TTR("Select sub-tile to change its z index.\nClick on "
                        "another Tile to edit it.")
                );
                spin_priority->hide();
                spin_z_index->show();
...
void TileSetEditor::_on_workspace_draw() {
    ...
    if (get_current_tile() >= 0) {
        ...
        switch (edit_mode) {
            case EDITMODE_ICON: {
                Vector2 coord =
                    tileset->autotile_get_icon_coordinate(get_current_tile());
            ...
            case EDITMODE_BITMASK: {
                ...
                for (int x = 0; x < region.size.x / (spacing + size.x); x++) {
                    for (int y = 0; y < region.size.y / (spacing + size.y);
                        ...
                        if (tileset->autotile_get_bitmask_mode(get_current_tile(
                            ))
                            == TileSet::BITMASK_2X2) {
                            ...
                            if (mask & TileSet::BIND_IGNORE_BOTTOMRIGHT) {
                                workspace->draw_rect(
                                    Rect2(anchor + size / 2, size / 4),
                                    ci
                                );
                                workspace->draw_rect(
                                    Rect2(anchor + size * 3 / 4, size / 4),
                                    ci
                                );
                            } else if (mask & TileSet::BIND_BOTTOMRIGHT) {
                                workspace->draw_rect(
                                    Rect2(anchor + size / 2, size / 2),
                                    c
                                );
                            }
                        } else {
                            ...
                            if (mask & TileSet::BIND_IGNORE_CENTER) {
                                workspace->draw_rect(
                                    Rect2(anchor + size / 3, size / 6),
                                    ci
                                );
                                workspace->draw_rect(
                                    Rect2(anchor + size / 2, size / 6),
                                    ci
                                );
                            ...
                            if (mask & TileSet::BIND_IGNORE_BOTTOMRIGHT) {
                                workspace->draw_rect(
                                    Rect2(anchor + size * 4 / 6, size / 6),
                                    ci
                                );
                                workspace->draw_rect(
                                    Rect2(anchor + size * 5 / 6, size / 6),
                                    ci
                                );
                            } else if (mask & TileSet::BIND_BOTTOMRIGHT) {
                                workspace->draw_rect(
                                    Rect2(anchor + (size / 3) * 2, size / 3),
                                    c
                                );
                            }
                        }
            ...
            case EDITMODE_PRIORITY: {
                spin_priority->set_value(tileset->autotile_get_subtile_priority(
                    get_current_tile(),
                    edited_shape_coord
                ));
                uint32_t mask = tileset->autotile_get_bitmask(
                ...
                for (Map<Vector2, uint32_t>::Element* E =
                         tileset->autotile_get_bitmask_map(get_current_tile())
                             .front();
                     E;
                     E = E->next()) {
                    if (E->value() == mask) {
                        total += tileset->autotile_get_subtile_priority(
                            get_current_tile(),
                            E->key()
                        );
            ...
            case EDITMODE_Z_INDEX: {
                spin_z_index->set_value(tileset->autotile_get_z_index(
                    get_current_tile(),
                    edited_shape_coord
                ));
                draw_highlight_subtile(edited_shape_coord);
    ...
    for (List<int>::Element* E = tiles.front(); E; E = E->next()) {
        ...
        if (tileset->tile_get_texture(t_id)->get_path() == current_texture_path
            ...
            if (tileset->tile_get_tile_mode(t_id) == TileSet::SINGLE_TILE) {
                c = COLOR_SINGLE;
            } else if (tileset->tile_get_tile_mode(t_id)
                       == TileSet::AUTO_TILE) {
                c = COLOR_AUTOTILE;
            } else if (tileset->tile_get_tile_mode(t_id)
                       == TileSet::ATLAS_TILE) {
...
void TileSetEditor::_on_workspace_overlay_draw() {
    ...
    if (tile_names_visible) {
        ...
        for (List<int>::Element* E = tiles.front(); E; E = E->next()) {
            ...
            String tile_id_name =
                String::num(t_id, 0) + ": " + tileset->tile_get_name(t_id);
    ...
    if (draw_handles) {
        for (int i = 0; i < current_shape.size(); i++) {
            workspace_overlay->draw_texture(
                handle,
                current_shape[i] * workspace->get_scale().x
                    - handle->get_size() * 0.5
            );
        }
    }
...
int TileSetEditor::get_grabbed_point(
...
bool TileSetEditor::is_within_grabbing_distance_of_first_point(
    const Vector2& p_pos,
    real_t p_grab_threshold
) {
    Transform2D xform = workspace->get_transform();
    ...
    const real_t distance =
        xform.xform(current_shape[0]).distance_to(xform.xform(p_pos));
...
void TileSetEditor::_on_scroll_container_input(const Ref<InputEvent>& p_event) {
    ...
    if (mb.is_valid()) {
        // Zoom in/out using Ctrl + mouse wheel. This is done on the
        // ScrollContainer to allow performing this action anywhere, even if the
        // cursor isn't hovering the texture in the workspace.
...
void TileSetEditor::_on_workspace_input(const Ref<InputEvent>& p_ie) {
    ...
    if (mm.is_valid()) {
        if (mm->get_button_mask() & BUTTON_MASK_MIDDLE) {
            ...
            scroll->set_h_scroll(
                scroll->get_h_scroll() - dragged.x * workspace->get_scale().x
            );
            scroll->set_v_scroll(
                scroll->get_v_scroll() - dragged.y * workspace->get_scale().x
            );
        }
    }
    ...
    if (edit_mode == EDITMODE_REGION) {
        if (mb.is_valid()) {
            if (mb->is_pressed() && mb->get_button_index() == BUTTON_LEFT) {
            ...
            } else if (dragging && mb->is_pressed()
                ...
                if (!edited_region.has_no_area()) {
                    if (get_current_tile() >= 0
                        ...
                        Size2 tile_workspace_size = edited_region.position
                                                  + edited_region.size
                                                  + WORKSPACE_MARGIN * 2;
                        Size2 workspace_minsize =
                            workspace->get_custom_minimum_size();
                        // If the new region is bigger, just directly change the
                        // workspace size to avoid checking all other tiles.
                        if (tile_workspace_size.x > workspace_minsize.x
                            || tile_workspace_size.y > workspace_minsize.y) {
                            Size2 max_workspace_size = Size2(
                                MAX(tile_workspace_size.x, workspace_minsize.x),
                                MAX(tile_workspace_size.y, workspace_minsize.y)
                            );
                            undo_redo->add_do_method(
                                workspace,
                        ...
                        } else if (workspace_minsize.x
                                       > get_current_texture()->get_size().x
                                             + WORKSPACE_MARGIN.x * 2
                                   || workspace_minsize.y
                                          > get_current_texture()->get_size().y
                                                + WORKSPACE_MARGIN.y * 2) {
                            undo_redo->add_do_method(
                                this,
                                "update_workspace_minsize"
                            );
                        ...
                        undo_redo
                            ->add_do_method(tileset.ptr(), "create_tile", t_id);
                        undo_redo->add_undo_method(
                            tileset.ptr(),
                            "remove_tile",
                            t_id
                        ...
                        undo_redo->add_do_method(
                            ...
                            "tile_set_name",
                            t_id,
                            get_current_texture()->get_path().get_file() + " "
                                + String::num(t_id, 0)
                        );
                        if (workspace_mode != WORKSPACE_CREATE_SINGLE) {
                            undo_redo->add_do_method(
                            ...
                            undo_redo->add_do_method(
                                ...
                                "tile_set_tile_mode",
                                t_id,
                                workspace_mode == WORKSPACE_CREATE_AUTOTILE
                                    ? TileSet::AUTO_TILE
                                    : TileSet::ATLAS_TILE
                            );
                        }
                        ...
                        if (tile_workspace_size.x > workspace_minsize.x
                            || tile_workspace_size.y > workspace_minsize.y) {
                            Size2 new_workspace_minsize = Size2(
                                MAX(tile_workspace_size.x, workspace_minsize.x),
                                MAX(tile_workspace_size.y, workspace_minsize.y)
                            );
                            undo_redo->add_do_method(
            ...
            switch (edit_mode) {
                case EDITMODE_ICON: {
                    if (mb.is_valid()) {
                        if (mb->is_pressed()
                            && mb->get_button_index() == BUTTON_LEFT
                            && current_tile_region.has_point(mb->get_position()
                            )) {
                            Vector2 coord(
                                (int)((mb->get_position().x
                                       - current_tile_region.position.x)
                                      / (spacing + size.x)),
                                (int)((mb->get_position().y
                                       - current_tile_region.position.y)
                                      / (spacing + size.y))
                            );
                ...
                case EDITMODE_BITMASK: {
                    if (mb.is_valid()) {
                        if (mb->is_pressed()) {
                            ...
                            if ((mb->get_button_index() == BUTTON_RIGHT
                                ...
                                pos = mb->get_position()
                                    - (pos + current_tile_region.position);
                                uint32_t bit = 0;
                                if (tileset->autotile_get_bitmask_mode(
                                        get_current_tile()
                                    )
                                    == TileSet::BITMASK_2X2) {
                                    if (pos.x < size.x / 2) {
                    ...
                    if (mm.is_valid()) {
                        if (dragging
                            && current_tile_region.has_point(mm->get_position()
                            )) {
                            Vector2 coord(
                                (int)((mm->get_position().x
                                       - current_tile_region.position.x)
                                      / (spacing + size.x)),
                                (int)((mm->get_position().y
                                       - current_tile_region.position.y)
                                      / (spacing + size.y))
                            );
                            Vector2 pos(
                            ...
                            uint32_t old_mask = tileset->autotile_get_bitmask(
                                get_current_tile(),
                                coord
                            );
                            uint32_t new_mask = old_mask;
                ...
                case EDITMODE_Z_INDEX: {
                    ...
                    if (tools[TOOL_SELECT]->is_pressed()) {
                        if (mb.is_valid()) {
                            if (mb->is_pressed()
                                && mb->get_button_index() == BUTTON_LEFT) {
                                if (edit_mode != EDITMODE_PRIORITY
                                    && current_shape.size() > 0) {
                                    int grabbed_point = get_grabbed_point(
                                        mb->get_position(),
                                        grab_threshold
                                    );
                                ...
                                if (edit_mode == EDITMODE_COLLISION) {
                                    if (dragging_point >= 0) {
                                        ...
                                        for (int i = 0;
                                             i < current_shape.size();
                                             i++) {
                                            Vector2 p = current_shape[i];
                                            if (tools[TOOL_GRID_SNAP]
                                                    ->is_pressed()
                                    ...
                                    if (dragging_point >= 0) {
                                        ...
                                        for (int i = 0;
                                             ...
                                             i++) {
                                            w[i] =
                                                current_shape[i] - shape_anchor;
                                        }

                                        w.release();

                                        undo_redo->create_action(
                                            TTR("Edit Occlusion Polygon")
                                        );
                                        undo_redo->add_do_method(
                                            edited_occlusion_shape.ptr(),
                                            "set_polygon",
                                            polygon
                                        );
                                        undo_redo->add_undo_method(
                                            edited_occlusion_shape.ptr(),
                                            "set_polygon",
                                            edited_occlusion_shape->get_polygon(
                                            )
                                        );
                                        undo_redo->add_do_method(
                                            this,
                                    ...
                                    if (dragging_point >= 0) {
                                        ...
                                        undo_redo->create_action(
                                            TTR("Edit Navigation Polygon")
                                        );
                                        undo_redo->add_do_method(
                                            edited_navigation_shape.ptr(),
                                            "set_vertices",
                                            polygon
                                        );
                                        undo_redo->add_undo_method(
                                ...
                                if (creating_shape) {
                                    ...
                                    workspace->update();
                                       ...
                                       && mb->get_button_index()
                                              == BUTTON_LEFT) {
                                if (creating_shape) {
                                    // if the first two corners are within
                                    // grabbing distance of one another, expand
                                    // the rect to fill the tile
                                    if (is_within_grabbing_distance_of_first_point(
                                            current_shape[1],
                                            grab_threshold
                                        )) {
...
void TileSetEditor::_on_tool_clicked(int p_tool) {
    if (p_tool == BITMASK_COPY) {
        ...
        for (Map<Vector2, uint32_t>::Element* E =
                 tileset->autotile_get_bitmask_map(get_current_tile()).front();
             E;
             E = E->next()) {
            undo_redo->add_undo_method(
                tileset.ptr(),
        ...
        if (edited_collision_shape.is_valid()) {
            ...
            if (convex.is_valid()) {
                ...
                Ref<ConcavePolygonShape2D> _concave =
                    memnew(ConcavePolygonShape2D);
                ...
                Ref<ConvexPolygonShape2D> _convex =
                    memnew(ConvexPolygonShape2D);
            ...
            switch (edit_mode) {
                case EDITMODE_REGION: {
                    ...
                    if (workspace_mode == WORKSPACE_EDIT && t_id >= 0) {
                        ...
                        undo_redo
                            ->add_do_method(tileset.ptr(), "remove_tile", t_id);
                        ...
                        Rect2 tile_region =
                            tileset->tile_get_region(get_current_tile());
                        Size2 tile_workspace_size =
                            tile_region.position + tile_region.size;
                        if (tile_workspace_size.x
                                > get_current_texture()->get_size().x
                            || tile_workspace_size.y
                                   > get_current_texture()->get_size().y) {
                            undo_redo->add_do_method(
                                this,
                                "update_workspace_minsize"
                ...
                case EDITMODE_COLLISION: {
                    if (!edited_collision_shape.is_null()) {
                        // Necessary to get the version that returns a Array
                        // instead of a Vector.
                        Array sd = tileset->call(
                            "tile_get_shapes",
                            get_current_tile()
                        );
                        for (int i = 0; i < sd.size(); i++) {
    ...
    } else if (p_tool == TOOL_SELECT || p_tool == SHAPE_NEW_POLYGON
               || p_tool == SHAPE_NEW_RECTANGLE) {
        if (creating_shape) {
            // Cancel Creation
            creating_shape = false;
            current_shape.resize(0);
...
void TileSetEditor::_on_z_index_changed(float val) {
    if ((int)val
        == tileset
               ->autotile_get_z_index(get_current_tile(), edited_shape_coord)) {
        return;
    }
    ...
    undo_redo->add_undo_method(
        ...
        get_current_tile(),
        edited_shape_coord,
        tileset->autotile_get_z_index(get_current_tile(), edited_shape_coord)
    );
    undo_redo->add_do_method(workspace, "update");
    undo_redo->add_undo_method(workspace, "update");
    undo_redo->commit_action();
...
void TileSetEditor::_update_tile_data() {
    ...
    Vector<TileSet::ShapeData> sd =
        tileset->tile_get_shapes(get_current_tile());
    if (tileset->tile_get_tile_mode(get_current_tile())
        == TileSet::SINGLE_TILE) {
        SubtileData data;
        for (int i = 0; i < sd.size(); i++) {
        ...
        data.navigation_shape =
            tileset->tile_get_navigation_polygon(get_current_tile());
        data.occlusion_shape =
            tileset->tile_get_light_occluder(get_current_tile());
...
void TileSetEditor::_update_toggle_shape_button() {
    ...
    if (edit_mode != EDITMODE_COLLISION || !edited_collision_shape.is_valid()) {
        ...
        tools[SHAPE_TOGGLE_TYPE]->set_icon(
            get_icon("ConvexPolygonShape2D", "EditorIcons")
        );
        tools[SHAPE_TOGGLE_TYPE]->set_text(TTR("Make Convex"));
    } else if (convex.is_valid()) {
        tools[SHAPE_TOGGLE_TYPE]->set_icon(
            get_icon("ConcavePolygonShape2D", "EditorIcons")
        );
...
void TileSetEditor::_select_next_tile() {
    ...
    if (get_current_tile() == -1) {
        return;
    } else if (tileset->tile_get_tile_mode(get_current_tile())
               == TileSet::SINGLE_TILE) {
        return;
    } else {
        switch (edit_mode) {
...
void TileSetEditor::_select_previous_tile() {
    ...
    if (get_current_tile() == -1) {
        ...
        switch (edit_mode) {
            ...
            case EDITMODE_Z_INDEX: {
                edited_shape_coord =
                    _get_subtiles_count(get_current_tile()) - Vector2(1, 1);
                _select_edited_shape_coord();
            } break;
            default: {
            }
...
void TileSetEditor::_select_next_subtile() {
    ...
    if (tileset->tile_get_tile_mode(get_current_tile())
        == TileSet::SINGLE_TILE) {
        _select_next_tile();
    } else if (edit_mode == EDITMODE_REGION || edit_mode == EDITMODE_BITMASK
               || edit_mode == EDITMODE_ICON) {
        _select_next_tile();
    } else {
        Vector2 cell_count = _get_subtiles_count(get_current_tile());
        if (edited_shape_coord.x >= cell_count.x - 1
            && edited_shape_coord.y >= cell_count.y - 1) {
            _select_next_tile();
        } else {
        ...
        if (edited_collision_shape.is_valid()) {
            for (int i = 0; i < _get_edited_shape_points().size(); i++) {
                current_shape.push_back(
                    _get_edited_shape_points()[i] + current_tile_region.position
                );
            }
        }
        workspace->update();
    ...
    if (get_current_tile() == -1) {
        ...
        if (get_current_tile() != -1 && edit_mode == EDITMODE_COLLISION) {
            ...
            if (data.collisions.size() > 1) {
                _set_edited_collision_shape(
                    data.collisions[data.collisions.size() - 1]
                );
            }
        } else {
            return;
...
void TileSetEditor::_undo_tile_removal(int p_id) {
    ...
    undo_redo->add_undo_method(
        ...
        "tile_set_name",
        p_id,
        tileset->tile_get_name(p_id)
    );
    undo_redo->add_undo_method(
        tileset.ptr(),
        "tile_set_normal_map",
        p_id,
        tileset->tile_get_normal_map(p_id)
    );
    undo_redo->add_undo_method(
        tileset.ptr(),
        "tile_set_texture_offset",
        p_id,
        tileset->tile_get_texture_offset(p_id)
    );
    undo_redo->add_undo_method(
        tileset.ptr(),
        "tile_set_material",
        p_id,
        tileset->tile_get_material(p_id)
    );
    undo_redo->add_undo_method(
        tileset.ptr(),
        "tile_set_modulate",
        p_id,
        tileset->tile_get_modulate(p_id)
    );
    undo_redo->add_undo_method(
        tileset.ptr(),
        "tile_set_occluder_offset",
        p_id,
        tileset->tile_get_occluder_offset(p_id)
    );
    undo_redo->add_undo_method(
        tileset.ptr(),
        "tile_set_navigation_polygon_offset",
        p_id,
        tileset->tile_get_navigation_polygon_offset(p_id)
    );
    undo_redo->add_undo_method(
        tileset.ptr(),
        ...
        p_id,
        0,
        tileset->tile_get_shape_offset(p_id, 0)
    );
    undo_redo->add_undo_method(
        tileset.ptr(),
        ...
        p_id,
        0,
        tileset->tile_get_shape_transform(p_id, 0)
    );
    undo_redo->add_undo_method(
        tileset.ptr(),
        "tile_set_z_index",
        p_id,
        tileset->tile_get_z_index(p_id)
    );
    undo_redo->add_undo_method(
        tileset.ptr(),
        "tile_set_texture",
        p_id,
        tileset->tile_get_texture(p_id)
    );
    undo_redo->add_undo_method(
        tileset.ptr(),
        "tile_set_region",
        p_id,
        tileset->tile_get_region(p_id)
    );
    ...
    undo_redo->add_undo_method(
        ...
        "tile_set_shapes",
        p_id,
        tileset->call("tile_get_shapes", p_id)
    );
    if (tileset->tile_get_tile_mode(p_id) == TileSet::SINGLE_TILE) {
        undo_redo->add_undo_method(
            ...
            "tile_set_light_occluder",
            p_id,
            tileset->tile_get_light_occluder(p_id)
        );
        undo_redo->add_undo_method(
            tileset.ptr(),
            "tile_set_navigation_polygon",
            p_id,
            tileset->tile_get_navigation_polygon(p_id)
        );
    } else {
        Map<Vector2, Ref<OccluderPolygon2D>> oclusion_map =
            tileset->autotile_get_light_oclusion_map(p_id);
        ...
        Map<Vector2, Ref<NavigationPolygon>> navigation_map =
            tileset->autotile_get_navigation_map(p_id);
        for (Map<Vector2, Ref<NavigationPolygon>>::Element* E =
                 navigation_map.front();
             E;
             E = E->next()) {
            undo_redo->add_undo_method(
                tileset.ptr(),
        ...
        undo_redo->add_undo_method(
            ...
            "autotile_set_icon_coordinate",
            p_id,
            tileset->autotile_get_icon_coordinate(p_id)
        );
        ...
        undo_redo->add_undo_method(
            ...
            "tile_set_tile_mode",
            p_id,
            tileset->tile_get_tile_mode(p_id)
        );
        undo_redo->add_undo_method(
            tileset.ptr(),
            "autotile_set_size",
            p_id,
            tileset->autotile_get_size(p_id)
        );
        undo_redo->add_undo_method(
            tileset.ptr(),
            "autotile_set_spacing",
            p_id,
            tileset->autotile_get_spacing(p_id)
        );
        undo_redo->add_undo_method(
            tileset.ptr(),
            "autotile_set_bitmask_mode",
            p_id,
            tileset->autotile_get_bitmask_mode(p_id)
        );
    }
}
...
void TileSetEditor::_zoom_in() {
    ...
    if (scale < max_scale) {
        ...
        workspace_container->set_custom_minimum_size(
            workspace->get_rect().size * scale
        );
        workspace_overlay->set_custom_minimum_size(
            workspace->get_rect().size * scale
        );
    }
}
...
void TileSetEditor::draw_highlight_current_tile() {
    ...
    if ((workspace_mode == WORKSPACE_EDIT && get_current_tile() >= 0)
        ...
        if (region.position.y >= 0) {
            workspace->draw_rect(
                Rect2(0, 0, workspace->get_rect().size.x, region.position.y),
                shadow_color
            );
        }
        if (region.position.x >= 0) {
            workspace->draw_rect(
                Rect2(
                    0,
                    MAX(0, region.position.y),
                    region.position.x,
                    MIN(workspace->get_rect().size.y - region.position.y,
                        MIN(region.size.y, region.position.y + region.size.y))
                ),
                shadow_color
            );
        }
        if (region.position.x + region.size.x <= workspace->get_rect().size.x) {
            workspace->draw_rect(
                Rect2(
                    ...
                    MAX(0, region.position.y),
                    workspace->get_rect().size.x - region.position.x
                        - region.size.x,
                    MIN(workspace->get_rect().size.y - region.position.y,
                        MIN(region.size.y, region.position.y + region.size.y))
                ),
        ...
        if (region.position.y + region.size.y <= workspace->get_rect().size.y) {
            workspace->draw_rect(
                Rect2(
                    ...
                    workspace->get_rect().size.x,
                    workspace->get_rect().size.y - region.size.y
                        - region.position.y
                ),
                shadow_color
            );
        ...
        workspace->draw_rect(
            Rect2(Point2(0, 0), workspace->get_rect().size),
            shadow_color
        );
    }
}
...
void TileSetEditor::draw_highlight_subtile(
    ...
    if (coord.y >= 0) {
        workspace->draw_rect(
            Rect2(0, 0, workspace->get_rect().size.x, coord.y),
            shadow_color
        );
    }
    if (coord.x >= 0) {
        workspace->draw_rect(
            Rect2(
                ...
                MAX(0, coord.y),
                coord.x,
                MIN(workspace->get_rect().size.y - coord.y,
                    MIN(size.y, coord.y + size.y))
            ),
            shadow_color
        );
    }
    if (coord.x + size.x <= workspace->get_rect().size.x) {
        workspace->draw_rect(
            Rect2(
                coord.x + size.x,
                MAX(0, coord.y),
                workspace->get_rect().size.x - coord.x - size.x,
                MIN(workspace->get_rect().size.y - coord.y,
                    MIN(size.y, coord.y + size.y))
            ),
            shadow_color
    ...
    if (coord.y + size.y <= workspace->get_rect().size.y) {
        workspace->draw_rect(
            Rect2(
                ...
                coord.y + size.y,
                workspace->get_rect().size.x,
                workspace->get_rect().size.y - size.y - coord.y
            ),
            shadow_color
        );
    }
...
void TileSetEditor::draw_tile_subdivision(int p_id, Color p_color) const {
    ...
    if (tileset->tile_get_tile_mode(p_id) == TileSet::AUTO_TILE
        ...
        while (j < region.size.x) {
            if (spacing <= 0) {
                workspace->draw_line(
                    region.position + WORKSPACE_MARGIN + Point2(j, 0),
                    region.position + WORKSPACE_MARGIN
                        + Point2(j, region.size.y),
                    c
                );
            } else {
                workspace->draw_rect(
        ...
        while (j < region.size.y) {
            if (spacing <= 0) {
                workspace->draw_line(
                    region.position + WORKSPACE_MARGIN + Point2(0, j),
                    region.position + WORKSPACE_MARGIN
                        + Point2(region.size.x, j),
                    c
                );
            } else {
                workspace->draw_rect(
...
void TileSetEditor::draw_edited_region_subdivision() const {
    ...
    if (draw) {
        ...
        while (j < region.size.x) {
            if (spacing <= 0) {
                workspace->draw_line(
                    region.position + Point2(j, 0),
                    region.position + Point2(j, region.size.y),
                    c
                );
            } else {
                workspace->draw_rect(
        ...
        while (j < region.size.y) {
            if (spacing <= 0) {
                workspace->draw_line(
                    region.position + Point2(0, j),
                    region.position + Point2(region.size.x, j),
                    c
                );
            } else {
                workspace->draw_rect(
...
void TileSetEditor::draw_grid_snap() {
    if (tools[TOOL_GRID_SNAP]->is_pressed()) {
        ...
        int height_count = Math::floor(
            (s.height - WORKSPACE_MARGIN.y) / (snap_step.y + snap_separation.y)
        );
        ...
        if (snap_step.x != 0) {
            for (int i = 0; i <= width_count; i++) {
                ...
                if (snap_separation.x != 0) {
                    if (i != 0) {
                        workspace->draw_rect(
                            Rect2(last_p, 0, snap_separation.x, s.height),
                            grid_color
                        );
        ...
        if (snap_step.y != 0) {
            for (int i = 0; i <= height_count; i++) {
                ...
                if (snap_separation.y != 0) {
                    if (i != 0) {
                        workspace->draw_rect(
                            Rect2(0, last_p, s.width, snap_separation.y),
                            grid_color
                        );
...
void TileSetEditor::draw_polygon_shapes() {
        ...
        case EDITMODE_COLLISION: {
            ...
            for (int i = 0; i < sd.size(); i++) {
                ...
                if (shape.is_valid()) {
                    ...
                    Vector<Color> colors;
                    if (!creating_shape && shape == edited_collision_shape
                        && current_shape.size() > 2) {
                        ...
                        for (int j = 0;
                             j < _get_collision_shape_points(shape).size();
                             j++) {
                            polygon.push_back(
                                _get_collision_shape_points(shape)[j] + anchor
                            );
                            colors.push_back(c_bg);
                        }
                    }
        ...
        case EDITMODE_OCCLUSION: {
            if (tileset->tile_get_tile_mode(get_current_tile())
                ...
                if (shape.is_valid()) {
                    ...
                    if (!creating_shape && shape == edited_occlusion_shape
                        && current_shape.size() > 2) {
                        for (int j = 0; j < current_shape.size(); j++) {
                            polygon.push_back(current_shape[j]);
                            colors.push_back(c_bg);
                ...
                Map<Vector2, Ref<OccluderPolygon2D>> map =
                    tileset->autotile_get_light_oclusion_map(t_id);
                for (Map<Vector2, Ref<OccluderPolygon2D>>::Element* E =
                         map.front();
                     E;
                     E = E->next()) {
                    ...
                    if (shape.is_valid()) {
                        ...
                        if (!creating_shape && shape == edited_occlusion_shape
                            ...
                            for (int j = 0; j < shape->get_polygon().size();
                                 j++) {
                                polygon.push_back(
                                    shape->get_polygon()[j] + anchor
                                );
                                colors.push_back(c_bg);
                            }
                        }
        ...
        case EDITMODE_NAVIGATION: {
            if (tileset->tile_get_tile_mode(get_current_tile())
                ...
                if (shape.is_valid()) {
                    ...
                    anchor +=
                        tileset->tile_get_region(get_current_tile()).position;
                    if (!creating_shape && shape == edited_navigation_shape
                        && current_shape.size() > 2) {
                        for (int j = 0; j < current_shape.size(); j++) {
                            polygon.push_back(current_shape[j]);
                            colors.push_back(c_bg);
                        ...
                        for (int j = 0; j < shape->get_polygon(0).size(); j++) {
                            polygon.push_back(
                                vertices[shape->get_polygon(0)[j]] + anchor
                            );
                            colors.push_back(c_bg);
                        }
                    }
                ...
                Map<Vector2, Ref<NavigationPolygon>> map =
                    tileset->autotile_get_navigation_map(t_id);
                for (Map<Vector2, Ref<NavigationPolygon>>::Element* E =
                         map.front();
                     E;
                     E = E->next()) {
                    ...
                    if (shape.is_valid()) {
                        ...
                        if (!creating_shape && shape == edited_navigation_shape
                            ...
                            PoolVector<Vector2> vertices =
                                shape->get_vertices();
                            for (int j = 0; j < shape->get_polygon(0).size();
                                 j++) {
                                polygon.push_back(
                                    vertices[shape->get_polygon(0)[j]] + anchor
                                );
    ...
    if (creating_shape && current_shape.size() > 1) {
        ...
        workspace->draw_line(
            current_shape[current_shape.size() - 1],
            snap_point(workspace->get_local_mouse_position()),
            Color(0, 1, 1),
            1,
            true
        );
        ...
        if (current_shape.size() >= 3) {
            ...
            for (int i = 0; i < current_shape.size(); i++) {
                ...
                if (i != current_shape.size() - 1) {
                    p_total +=
                        ((current_shape[i + 1].x - current_shape[i].x)
                         * (-current_shape[i + 1].y + (-current_shape[i].y)));
                } else {
                    p_total +=
                        ((current_shape[0].x - current_shape[i].x)
                         * (-current_shape[0].y + (-current_shape[i].y)));
                }
            }
            ...
            // Necessary to get the version that returns a Array instead of a
            // Vector.
            Array sd = tileset->call("tile_get_shapes", get_current_tile());
            undo_redo->add_undo_method(
                tileset.ptr(),
...
void TileSetEditor::select_coord(const Vector2& coord) {
    ...
    if (tileset->tile_get_tile_mode(get_current_tile())
        ...
        if (edited_collision_shape
            != tileset->tile_get_shape(get_current_tile(), 0)) {
            _set_edited_collision_shape(
                tileset->tile_get_shape(get_current_tile(), 0)
            );
        }
        ...
        if (edit_mode == EDITMODE_COLLISION) {
            ...
            if (edited_collision_shape.is_valid()) {
                for (int i = 0; i < _get_edited_shape_points().size(); i++) {
                    current_shape.push_back(
                        _get_edited_shape_points()[i]
                        + current_tile_region.position
                    );
                }
            }
        } else if (edit_mode == EDITMODE_OCCLUSION) {
            ...
            if (edited_occlusion_shape.is_valid()) {
                for (int i = 0;
                     i < edited_occlusion_shape->get_polygon().size();
                     i++) {
                    current_shape.push_back(
                        edited_occlusion_shape->get_polygon()[i]
                        + current_tile_region.position
                    );
                }
            }
            ...
            if (edited_navigation_shape.is_valid()) {
                if (edited_navigation_shape->get_polygon_count() > 0) {
                    PoolVector<Vector2> vertices =
                        edited_navigation_shape->get_vertices();
                    for (int i = 0;
                         i < edited_navigation_shape->get_polygon(0).size();
                         i++) {
                        current_shape.push_back(
                            vertices[edited_navigation_shape->get_polygon(0)[i]]
                            + current_tile_region.position
                        );
                    }
                }
        ...
        if (edited_occlusion_shape
            != tileset
                   ->autotile_get_light_occluder(get_current_tile(), coord)) {
            edited_occlusion_shape =
                tileset->autotile_get_light_occluder(get_current_tile(), coord);
        }
        if (edited_navigation_shape
            != tileset->autotile_get_navigation_polygon(
                get_current_tile(),
            ...
            edited_navigation_shape = tileset->autotile_get_navigation_polygon(
                get_current_tile(),
                coord
            );
        }
        ...
        if (edit_mode == EDITMODE_COLLISION) {
            ...
            if (edited_collision_shape.is_valid()) {
                for (int j = 0; j < _get_edited_shape_points().size(); j++) {
                    current_shape.push_back(
                        _get_edited_shape_points()[j] + shape_anchor
                    );
                }
            }
        } else if (edit_mode == EDITMODE_OCCLUSION) {
            ...
            if (edited_occlusion_shape.is_valid()) {
                for (int i = 0;
                     ...
                     i++) {
                    current_shape.push_back(
                        edited_occlusion_shape->get_polygon()[i] + shape_anchor
                    );
                }
            }
        } else if (edit_mode == EDITMODE_NAVIGATION) {
...
Vector2 TileSetEditor::snap_point(const Vector2& point) {
    ...
    Rect2 tile_region(
        tileset->tile_get_region(get_current_tile()).position
            + WORKSPACE_MARGIN,
        tileset->tile_get_region(get_current_tile()).size
    );
    if (tileset->tile_get_tile_mode(get_current_tile())
        == TileSet::SINGLE_TILE) {
        region.position = tileset->tile_get_region(get_current_tile()).position
                        + WORKSPACE_MARGIN;
        region.size = tileset->tile_get_region(get_current_tile()).size;
    }

    if (tools[TOOL_GRID_SNAP]->is_pressed()) {
        p.x = Math::snap_scalar_separation(
            snap_offset.x,
            snap_step.x,
            p.x,
            snap_separation.x
        );
        p.y = Math::snap_scalar_separation(
            snap_offset.y,
            snap_step.y,
            p.y,
            snap_separation.y
...
void TileSetEditor::add_texture(Ref<Texture> p_texture) {
    ...
    texture_list->set_item_metadata(
        texture_list->get_item_count() - 1,
        p_texture->get_path()
    );
}
...
void TileSetEditor::update_texture_list() {
    ...
    for (List<int>::Element* E = ids.front(); E; E = E->next()) {
        // Clear tiles referencing gone textures (user has been already given
        // the chance to fix broken deps)
    ...
    if (texture_list->get_item_count() > 0 && selected_texture.is_valid()) {
        texture_list->select(
            texture_list->find_metadata(selected_texture->get_path())
        );
        ...
        _on_texture_list_selected(
            texture_list->find_metadata(get_current_texture()->get_path())
        );
    } else {
        _validate_current_tile_id();
        _on_texture_list_selected(-1);
...
void TileSetEditor::update_texture_list_icon() {
    for (int current_idx = 0; current_idx < texture_list->get_item_count();
         current_idx++) {
        ...
        texture_list->set_item_icon_region(
            current_idx,
            Rect2(0, 0, MIN(texture_size.x, 150), MIN(texture_size.y, 100))
        );
    }
    texture_list->update();
}
...
void TileSetEditor::update_workspace_tile_mode() {
    ...
    if (tileset->tile_get_tile_mode(get_current_tile())
        == TileSet::SINGLE_TILE) {
        if (tool_editmode[EDITMODE_ICON]->is_pressed()
            || tool_editmode[EDITMODE_PRIORITY]->is_pressed()
            || tool_editmode[EDITMODE_BITMASK]->is_pressed()
            || tool_editmode[EDITMODE_Z_INDEX]->is_pressed()) {
            tool_editmode[EDITMODE_COLLISION]->set_pressed(true);
            edit_mode = EDITMODE_COLLISION;
        }
        ...
        if (edit_mode == EDITMODE_ICON) {
            ...
            _select_edited_shape_coord();
               ...
               == TileSet::ATLAS_TILE) {
        if (tool_editmode[EDITMODE_PRIORITY]->is_pressed()
            || tool_editmode[EDITMODE_BITMASK]->is_pressed()) {
            tool_editmode[EDITMODE_COLLISION]->set_pressed(true);
            edit_mode = EDITMODE_COLLISION;
        }
...
void TileSetEditor::update_workspace_minsize() {
    ...
    workspace_container->set_custom_minimum_size(
        workspace_min_size * workspace->get_scale() + WORKSPACE_MARGIN * 2
    );
    workspace_overlay->set_custom_minimum_size(
        workspace_min_size * workspace->get_scale() + WORKSPACE_MARGIN * 2
    );
    // Make sure workspace size is initialized last (otherwise it might be
    // incorrect).
    workspace->call_deferred(
        "set_custom_minimum_size",
        workspace_min_size + WORKSPACE_MARGIN * 2
    );
}
...
void TileSetEditor::update_edited_region(const Vector2& end_point) {
    ...
    if (tools[TOOL_GRID_SNAP]->is_pressed()) {
        Vector2 grid_coord;
        grid_coord =
            ((region_from - snap_offset) / (snap_step + snap_separation))
                .floor();
        ...
        grid_coord =
            ((end_point - snap_offset) / (snap_step + snap_separation)).floor();
        grid_coord *= (snap_step + snap_separation);
        grid_coord += snap_offset;
        edited_region.expand_to(grid_coord);
        grid_coord += snap_step;
...
Ref<Texture> TileSetEditor::get_current_texture() {
    if (texture_list->get_selected_items().size() == 0) {
        ...
        return texture_map[texture_list->get_item_metadata(
            texture_list->get_selected_items()[0]
        )];
    }
}
...
bool TilesetEditorContext::_set(
    ...
    if (name == "options_offset") {
        ...
        if (name2 == "autotile_bitmask_mode") {
            tileset->set(
                String::num(tileset_editor->get_current_tile(), 0)
                    + "/autotile/bitmask_mode",
                p_value,
                &v
            );
        ...
        for (int index = 0; index < sd.size(); index++) {
            if (sd[index].shape == tileset_editor->edited_collision_shape) {
                tileset->tile_set_shape_one_way(
                    tileset_editor->get_current_tile(),
                    index,
                    p_value
                );
                return true;
...
bool TilesetEditorContext::_get(const StringName& p_name, Variant& r_ret)
    ...
    if (name == "options_offset") {
        ...
        if (name == "autotile_bitmask_mode") {
            ...
            r_ret = tileset->get(
                String::num(tileset_editor->get_current_tile(), 0) + "/" + name,
                &v
            );
        }
        return v;
...
void TilesetEditorContext::_get_property_list(List<PropertyInfo>* p_list
) const {
    ...
    if (tileset_editor->get_current_tile() >= 0 && !tileset.is_null()) {
        ...
        p_list->push_back(PropertyInfo(
            ...
            "tile_tile_mode",
            PROPERTY_HINT_ENUM,
            "SINGLE_TILE,AUTO_TILE,ATLAS_TILE"
        ));
        if (tileset->tile_get_tile_mode(id) == TileSet::AUTO_TILE) {
            p_list->push_back(PropertyInfo(
                Variant::INT,
                "tile_autotile_bitmask_mode",
                PROPERTY_HINT_ENUM,
                "2x2,3x3 (minimal),3x3"
            ));
            p_list->push_back(
                PropertyInfo(Variant::VECTOR2, "tile_subtile_size")
            );
            p_list->push_back(PropertyInfo(
                Variant::INT,
                "tile_subtile_spacing",
                PROPERTY_HINT_RANGE,
                "0, 1024, 1"
            ));
        ...
        p_list->push_back(
            PropertyInfo(Variant::VECTOR2, "tile_navigation_offset")
        );
        p_list->push_back(PropertyInfo(
            Variant::VECTOR2,
            "tile_shape_offset",
        ...
        p_list->push_back(PropertyInfo(
            ...
            "tile_z_index",
            PROPERTY_HINT_RANGE,
            itos(VS::CANVAS_ITEM_Z_MIN) + "," + itos(VS::CANVAS_ITEM_Z_MAX)
                + ",1"
        ));
    }
    if (tileset_editor->edit_mode == TileSetEditor::EDITMODE_COLLISION
        && tileset_editor->edited_collision_shape.is_valid()) {
        p_list->push_back(PropertyInfo(
            Variant::OBJECT,
            "selected_collision",
            PROPERTY_HINT_RESOURCE_TYPE,
            tileset_editor->edited_collision_shape->get_class()
        ));
        if (tileset_editor->edited_collision_shape.is_valid()) {
            ...
            p_list->push_back(PropertyInfo(
                Variant::REAL,
                "selected_collision_one_way_margin",
                PROPERTY_HINT_NONE
            ));
        }
    }
    if (tileset_editor->edit_mode == TileSetEditor::EDITMODE_NAVIGATION
        && tileset_editor->edited_navigation_shape.is_valid()) {
        p_list->push_back(PropertyInfo(
            Variant::OBJECT,
            "selected_navigation",
            PROPERTY_HINT_RESOURCE_TYPE,
            tileset_editor->edited_navigation_shape->get_class()
        ));
    }
    if (tileset_editor->edit_mode == TileSetEditor::EDITMODE_OCCLUSION
        && tileset_editor->edited_occlusion_shape.is_valid()) {
        p_list->push_back(PropertyInfo(
            Variant::OBJECT,
            "selected_occlusion",
            PROPERTY_HINT_RESOURCE_TYPE,
            tileset_editor->edited_occlusion_shape->get_class()
        ));
    }
    if (!tileset.is_null()) {
        p_list->push_back(PropertyInfo(
...
void TilesetEditorContext::_bind_methods() {
    ClassDB::bind_method(
        ...
        &TilesetEditorContext::_hide_script_from_inspector
    );
}
...
Dictionary TileSetEditorPlugin::get_state() const {
    ...
    state["keep_inside_tile"] =
        tileset_editor->tools[TileSetEditor::SHAPE_KEEP_INSIDE_TILE]
            ->is_pressed();
    state["show_information"] =
        tileset_editor->tools[TileSetEditor::VISIBLE_INFO]->is_pressed();
    return state;
}
...
void TileSetEditorPlugin::set_state(const Dictionary& p_state) {
    ...
    if (state.has("snap_enabled")) {
        tileset_editor->tools[TileSetEditor::TOOL_GRID_SNAP]->set_pressed(
            state["snap_enabled"]
        );
    ...
    if (state.has("keep_inside_tile")) {
        tileset_editor->tools[TileSetEditor::SHAPE_KEEP_INSIDE_TILE]
            ->set_pressed(state["keep_inside_tile"]);
    }

    if (state.has("show_information")) {
        tileset_editor->tools[TileSetEditor::VISIBLE_INFO]->set_pressed(
            state["show_information"]
        );
    }
}
