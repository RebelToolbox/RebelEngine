        ...
        case CAPSULE_SHAPE: {
            if (idx < 2) {
                ...
                if (idx == 0) {
                    ...
                    capsule->set_height(
                        parameter * 2 - capsule->get_radius() * 2
                    );
                }
        ...
        case RECTANGLE_SHAPE: {
            if (idx < 8) {
                ...
                if (Input::get_singleton()->is_key_pressed(KEY_ALT)) {
                    ...
                    rect->set_extents(((Point2)original
                                       + (extents - (Point2)original) * 0.5)
                                          .abs());
                    Point2 pos = original_transform.affine_inverse().xform(
                        original_transform.get_origin()
                    );
                    pos +=
                        (extents - (Point2)original) * RECT_HANDLES[idx] * 0.5;
...
void CollisionShape2DEditor::commit_handle(int idx, Variant& p_org) {
    ...
    switch (shape_type) {
        ...
        case RAY_SHAPE: {
            ...
            undo_redo
                ->add_do_method(ray.ptr(), "set_length", ray->get_length());
            undo_redo->add_do_method(canvas_item_editor, "update_viewport");
        ...
        case RECTANGLE_SHAPE: {
            ...
            undo_redo
                ->add_do_method(rect.ptr(), "set_extents", rect->get_extents());
            undo_redo->add_do_method(
                node,
...
bool CollisionShape2DEditor::forward_canvas_gui_input(
    ...
    if (mm.is_valid()) {
        ...
        Vector2 cpoint = canvas_item_editor->snap_point(
    ...
    if (k.is_valid()) {
        ...
        if (shape_type == RECTANGLE_SHAPE && k->get_scancode() == KEY_ALT) {
            set_handle(
                edit_handle,
                last_point
            ); // Update handle when Alt key is toggled.
        }
    }
...
void CollisionShape2DEditor::forward_canvas_draw_over_viewport(
    Control* p_overlay
) {
    if (!node) {
        return;
    ...
    Transform2D gt = canvas_item_editor->get_canvas_transform()
                   * node->get_global_transform();
...
void CollisionShape2DEditor::_bind_methods() {
    ClassDB::bind_method(
        "_get_current_shape_type",
        &CollisionShape2DEditor::_get_current_shape_type
    );
    ClassDB::bind_method(
        D_METHOD("_node_removed"),
        &CollisionShape2DEditor::_node_removed
    );
}
