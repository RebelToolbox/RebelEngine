...
void RoomManagerEditorPlugin::_bind_methods() {
    ClassDB::bind_method(
        "_flip_portals",
        &RoomManagerEditorPlugin::_flip_portals
    );
}
...
RoomManagerEditorPlugin::RoomManagerEditorPlugin(EditorNode* p_node) {
    ...
    button_flip_portals->set_icon(
        editor->get_gui_base()->get_icon("Portal", "EditorIcons")
    );
    ...
    Ref<RoomGizmoPlugin> room_gizmo_plugin =
        Ref<RoomGizmoPlugin>(memnew(RoomGizmoPlugin));
    SpatialEditor::get_singleton()->add_gizmo_plugin(room_gizmo_plugin);

    Ref<PortalGizmoPlugin> portal_gizmo_plugin =
        Ref<PortalGizmoPlugin>(memnew(PortalGizmoPlugin));
    SpatialEditor::get_singleton()->add_gizmo_plugin(portal_gizmo_plugin);

    Ref<OccluderGizmoPlugin> occluder_gizmo_plugin =
        Ref<OccluderGizmoPlugin>(memnew(OccluderGizmoPlugin));
...
void RoomEditorPlugin::edit(Object* p_object) {
    ...
    if (SpatialEditor::get_singleton()->is_visible() && s->_planes.size()) {
        String string =
            String(s->get_name()) + " [" + itos(s->_planes.size()) + " planes]";
...
void RoomEditorPlugin::_bind_methods() {
    ClassDB::bind_method(
        "_generate_points",
        &RoomEditorPlugin::_generate_points
    );
}
...
RoomEditorPlugin::RoomEditorPlugin(EditorNode* p_node) {
    ...
    button_generate->set_icon(
        editor->get_gui_base()->get_icon("Room", "EditorIcons")
    );
    ...
    if (_occluder && _occluder->is_inside_tree()) {
        ...
        if (ref.is_valid()) {
            ...
            if (parent) {
                ...
                Transform new_local_xform = ref->center_node(
                    _occluder->get_global_transform(),
...
OccluderEditorPlugin::OccluderEditorPlugin(EditorNode* p_node) {
    ...
    button_center->set_icon(
        editor->get_gui_base()->get_icon("EditorPosition", "EditorIcons")
    );
