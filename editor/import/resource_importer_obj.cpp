...
static Error _parse_material_library(
    const String& p_path,
    Map<String, Ref<SpatialMaterial>>& material_map,
    ...
    ERR_FAIL_COND_V_MSG(
        ...
        ERR_CANT_OPEN,
        vformat(
            "Couldn't open MTL file '%s', it may not exist or not be readable.",
            p_path
        )
    );
    ...
    while (true) {
        ...
        if (l.begins_with("newmtl ")) {
            ...
            // uv
            WARN_PRINT(
                "OBJ: Ambient light for material '" + current_name
                + "' is ignored in PBR"
            );
            ...
            // uv
            WARN_PRINT(
                "OBJ: Ambient light texture for material '" + current_name
                + "' is ignored in PBR"
            );
            ...
            String p =
                l.replace("map_bump", "").replace("\\", "/").strip_edges();
...
static Error _parse_obj(
    const String& p_path,
    List<Ref<Mesh>>& r_meshes,
    bool p_single_mesh,
    bool p_generate_tangents,
    int p_compress_flags,
    Vector3 p_scale_mesh,
    Vector3 p_offset_mesh,
    List<String>* r_missing_deps
) {
    FileAccessRef f = FileAccess::open(p_path, FileAccess::READ);
    ERR_FAIL_COND_V_MSG(
        ...
        ERR_CANT_OPEN,
        vformat(
            "Couldn't open OBJ file '%s', it may not exist or not be readable.",
            p_path
        )
    );
        ...
        if (l.begins_with("v ")) {
            ...
            for (int i = 2; i < v.size() - 1; i++) {
                ...
                for (int j = 0; j < 3; j++) {
                    ...
                    // if (weld_vertices)
                    //	vertex.snap(Vector3(weld_tolerance, weld_tolerance,
                    // weld_tolerance));
            ...
            if (surf_tool->get_vertex_array().size()) {
                ...
                print_verbose(
                    "OBJ: Current material library " + current_material_library
                    + " has " + itos(material_map.has(current_material_library))
                );
                print_verbose(
                    "OBJ: Current material " + current_material + " has "
                    + itos(
                        material_map.has(current_material_library)
                        && material_map[current_material_library].has(
                            current_material
                        )
                    )
                );
                ...
                if (material_map.has(current_material_library)
                    ...
                    surf_tool->set_material(
                        material_map[current_material_library][current_material]
                    );
                }
                ...
                print_verbose(
                    "OBJ: Added surface :"
                    + mesh->surface_get_name(mesh->get_surface_count() - 1)
                );
                surf_tool->clear();
                surf_tool->begin(Mesh::PRIMITIVE_TRIANGLES);
            }
            ...
            if (!material_map.has(current_material_library)) {
                ...
                if (lib_path.is_rel_path()) {
                    lib_path =
                        p_path.get_base_dir().plus_file(current_material_library
                        );
                }
                Error err =
                    _parse_material_library(lib_path, lib, r_missing_deps);
                if (err == OK) {
...
Node* EditorOBJImporter::import_scene(
    const String& p_path,
    uint32_t p_flags,
    int p_bake_fps,
...
Ref<Animation> EditorOBJImporter::import_animation(
    const String& p_path,
    uint32_t p_flags,
    int p_bake_fps
) {
...
void ResourceImporterOBJ::get_recognized_extensions(List<String>* p_extensions
) const {
...
void ResourceImporterOBJ::get_import_options(
    ...
    r_options->push_back(
        ImportOption(PropertyInfo(Variant::BOOL, "generate_tangents"), true)
    );
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::VECTOR3, "scale_mesh"),
        Vector3(1, 1, 1)
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::VECTOR3, "offset_mesh"),
        Vector3(0, 0, 0)
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(Variant::BOOL, "octahedral_compression"),
        true
    ));
    r_options->push_back(ImportOption(
        PropertyInfo(
...
bool ResourceImporterOBJ::get_option_visibility(
...
Error ResourceImporterOBJ::import(
    ...
    uint32_t compress_flags = int(p_options["optimize_mesh_flags"])
    ...
    ERR_FAIL_COND_V_MSG(
        err != OK,
        err,
        "Cannot save Mesh to file '" + save_path + "'."
    );
