...
struct ColladaImport {
    ...
    Error _create_mesh_surfaces(
        ...
        const Map<String, Collada::NodeGeometry::Material>& p_material_map,
        const Collada::MeshData& meshdata,
        const Transform& p_local_xform,
        const Vector<int>& bone_remap,
        const Collada::SkinControllerData* p_skin_controller,
        const Collada::MorphControllerData* p_morph_data,
        Vector<Ref<ArrayMesh>> p_morph_meshes = Vector<Ref<ArrayMesh>>(),
        uint32_t p_use_compression            = 0,
        bool p_use_mesh_material              = false
    );
    Error load(
        const String& p_path,
        int p_flags,
        bool p_force_make_tangents = false,
        uint32_t p_use_compression = 0
    );
    void _fix_param_animation_tracks();
    void create_animation(
        int p_clip,
        bool p_make_tracks_in_all_bones,
        bool p_import_value_tracks
    );
...
Error ColladaImport::_populate_skeleton(
    ...
    if (collada.state.bone_rest_map.has(joint->sid)) {
        p_skeleton->set_bone_rest(
            r_bone,
            collada.fix_transform(collada.state.bone_rest_map[joint->sid])
        );
    ...
    for (int i = 0; i < p_node->children.size(); i++) {
        Error err =
            _populate_skeleton(p_skeleton, p_node->children[i], r_bone, id);
        if (err) {
            return err;
        }
    }
...
Error ColladaImport::_create_scene_skeletons(Collada::Node* p_node) {
    if (p_node->type == Collada::Node::TYPE_SKELETON) {
        ...
        sk->localize_rests(
        ); // after creating skeleton, rests must be localized...!
...
Error ColladaImport::_create_scene(Collada::Node* p_node, Spatial* p_parent) {
    ...
    switch (p_node->type) {
        ...
        case Collada::Node::TYPE_LIGHT: {
            // node = memnew( Light)
            Collada::NodeLight* light =
                static_cast<Collada::NodeLight*>(p_node);
            if (collada.state.light_data_map.has(light->light)) {
                Collada::LightData& ld =
                    collada.state.light_data_map[light->light];
        ...
        case Collada::Node::TYPE_CAMERA: {
            Collada::NodeCamera* cam =
                static_cast<Collada::NodeCamera*>(p_node);
            Camera* camera = memnew(Camera);

            if (collada.state.camera_data_map.has(cam->camera)) {
                const Collada::CameraData& cd =
                    collada.state.camera_data_map[cam->camera];
        ...
        case Collada::Node::TYPE_GEOMETRY: {
            Collada::NodeGeometry* ng =
                static_cast<Collada::NodeGeometry*>(p_node);
            ...
            if (collada.state.curve_data_map.has(ng->source)) {
                ...
                // Object::cast_to<MeshInstance>(node)->set_flag(GeometryInstance::FLAG_USE_BAKED_LIGHT,
                // true);
            }
        } break;
...
Error ColladaImport::_create_material(const String& p_target) {
    ...
    ERR_FAIL_COND_V(
        !collada.state.material_map.has(p_target),
        ERR_INVALID_PARAMETER
    );
    Collada::Material& src_mat = collada.state.material_map[p_target];
    ERR_FAIL_COND_V(
        !collada.state.effect_map.has(src_mat.instance_effect),
        ERR_INVALID_PARAMETER
    );
    ...
    if (effect.diffuse.texture != "") {
        String texfile =
            effect.get_texture_path(effect.diffuse.texture, collada);
        if (texfile != "") {
    ...
    if (effect.specular.texture != "") {
        String texfile =
            effect.get_texture_path(effect.specular.texture, collada);
        if (texfile != "") {
            if (texfile.begins_with("/")) {
                texfile = texfile.replace_first("/", "res://");
            }
    ...
    if (effect.emission.texture != "") {
        String texfile =
            effect.get_texture_path(effect.emission.texture, collada);
        if (texfile != "") {
            if (texfile.begins_with("/")) {
                texfile = texfile.replace_first("/", "res://");
            }
...
Error ColladaImport::_create_mesh_surfaces(
    ...
    const Collada::SkinControllerData* p_skin_controller,
    const Collada::MorphControllerData* p_morph_data,
    Vector<Ref<ArrayMesh>> p_morph_meshes,
    uint32_t p_use_compression,
    ...
    if (p_morph_data) {
        // add morphie target
        ERR_FAIL_COND_V(
            !p_morph_data->targets.has("MORPH_TARGET"),
            ERR_INVALID_DATA
        );
        ...
        for (int i = 0; i < morph_targets; i++) {
            ...
            ERR_FAIL_COND_V(
                !collada.state.mesh_data_map.has(target),
                ERR_INVALID_DATA
            );
    ...
    for (int p_i = 0; p_i < meshdata.primitives.size(); p_i++) {
        ...
        ERR_FAIL_COND_V(
            !meshdata.vertices.has(vertex_src_id),
            ERR_INVALID_DATA
        );

        ERR_FAIL_COND_V(
            !meshdata.vertices[vertex_src_id].sources.has("POSITION"),
            ERR_INVALID_DATA
        );
        String position_src_id =
            meshdata.vertices[vertex_src_id].sources["POSITION"];

        ERR_FAIL_COND_V(
            !meshdata.sources.has(position_src_id),
            ERR_INVALID_DATA
        );

        const Collada::MeshData::Source* vertex_src =
            &meshdata.sources[position_src_id];
...

        {
            ...
            if (p.sources.has("NORMAL")) {
                ...
                normal_source_id =
                    meshdata.vertices[vertex_src_id].sources["NORMAL"];
                normal_ofs = vertex_ofs;
            }

            if (normal_source_id != "") {
                ERR_FAIL_COND_V(
                    !meshdata.sources.has(normal_source_id),
                    ERR_INVALID_DATA
                );
...

        {
            ...
            } else if (meshdata.vertices[vertex_src_id].sources.has(
                           "TEXBINORMAL"
                       )) {
                binormal_source_id =
                    meshdata.vertices[vertex_src_id].sources["TEXBINORMAL"];
                binormal_ofs = vertex_ofs;
            }

            if (binormal_source_id != "") {
                ERR_FAIL_COND_V(
                    !meshdata.sources.has(binormal_source_id),
                    ERR_INVALID_DATA
                );
...

        {
            ...
            if (p.sources.has("TEXTANGENT")) {
                ...
                tangent_source_id =
                    meshdata.vertices[vertex_src_id].sources["TEXTANGENT"];
                tangent_ofs = vertex_ofs;
            }

            if (tangent_source_id != "") {
                ERR_FAIL_COND_V(
                    !meshdata.sources.has(tangent_source_id),
                    ERR_INVALID_DATA
                );
...

        {
            ...
            if (p.sources.has("TEXCOORD0")) {
                ...
                uv_source_id =
                    meshdata.vertices[vertex_src_id].sources["TEXCOORD0"];
                uv_ofs = vertex_ofs;
            }

            if (uv_source_id != "") {
                ERR_FAIL_COND_V(
                    !meshdata.sources.has(uv_source_id),
                    ERR_INVALID_DATA
                );
...

        {
            ...
            if (p.sources.has("TEXCOORD1")) {
                ...
                uv2_source_id =
                    meshdata.vertices[vertex_src_id].sources["TEXCOORD1"];
                uv2_ofs = vertex_ofs;
            }

            if (uv2_source_id != "") {
                ERR_FAIL_COND_V(
                    !meshdata.sources.has(uv2_source_id),
                    ERR_INVALID_DATA
                );
...

        {
            ...
            if (p.sources.has("COLOR")) {
                ...
                color_source_id =
                    meshdata.vertices[vertex_src_id].sources["COLOR"];
                color_ofs = vertex_ofs;
            }

            if (color_source_id != "") {
                ERR_FAIL_COND_V(
                    !meshdata.sources.has(color_source_id),
                    ERR_INVALID_DATA
                );
        ...
        if (p_skin_controller) {
            ...
            if (p_skin_controller->weights.sources.has("WEIGHT")) {
                String weight_id =
                    p_skin_controller->weights.sources["WEIGHT"].source;
                weight_ofs =
                    p_skin_controller->weights.sources["WEIGHT"].offset;
            ...
            for (int w_i = 0; w_i < p_skin_controller->weights.sets.size();
                 w_i++) {
                ...
                for (int a_i = 0; a_i < amount; a_i++) {
                    ...
                    ERR_FAIL_INDEX_V(
                        read_from + wstride - 1,
                        p_skin_controller->weights.indices.size(),
                        ERR_INVALID_DATA
                    );
                    int weight_index = p_skin_controller->weights
                                           .indices[read_from + weight_ofs];
                    ERR_FAIL_INDEX_V(
                        weight_index,
                        weight_src->array.size(),
                        ERR_INVALID_DATA
                    );

                    w.weight = weight_src->array[weight_index];

                    int bone_index = p_skin_controller->weights
                                         .indices[read_from + joint_ofs];
                    if (bone_index == -1) {
                ...
                if (weights.size() == 0
                    || total == 0) { // if nothing, add a weight to bone 0
                    // no weights assigned
                    Collada::Vertex::Weight w;
                    w.bone_idx = 0;
                    w.weight   = 1.0;
        ...
        // The way collada uses indices is more optimal, and friendlier with 3D
        // modelling software, because it can index everything, not only
        // vertices (similar to how the WII works). This is, however, more
        // incompatible with standard video cards, so arrays must be converted.
        ...
        for (int p_j = 0; p_j < p.count; p_j++) {
            ...
            for (int j = 0; j < amount; j++) {
                ...
                int vertex_index =
                    p.indices[src + vertex_ofs]; // used for index field (later
                                                 // used by controllers)
                int vertex_pos = (vertex_src->stride ? vertex_src->stride : 3)
                               * vertex_index;
                ERR_FAIL_INDEX_V(
                    vertex_pos + 0,
                    vertex_src->array.size(),
                    ERR_INVALID_DATA
                );
                ERR_FAIL_INDEX_V(
                    vertex_pos + 2,
                ...
                vertex.vertex = Vector3(
                    vertex_src->array[vertex_pos + 0],
                    vertex_src->array[vertex_pos + 1],
                ...
                if (normal_src) {
                    int normal_pos =
                        (normal_src->stride ? normal_src->stride : 3)
                        * p.indices[src + normal_ofs];
                    ERR_FAIL_INDEX_V(
                        normal_pos + 0,
                        normal_src->array.size(),
                        ERR_INVALID_DATA
                    );
                    ERR_FAIL_INDEX_V(
                        normal_pos + 2,
                    ...
                    vertex.normal = Vector3(
                        normal_src->array[normal_pos + 0],
                        normal_src->array[normal_pos + 1],
                        normal_src->array[normal_pos + 2]
                    );

                    if (tangent_src && binormal_src) {
                        int binormal_pos =
                            (binormal_src->stride ? binormal_src->stride : 3)
                            * p.indices[src + binormal_ofs];
                        ERR_FAIL_INDEX_V(
                            binormal_pos + 0,
                            binormal_src->array.size(),
                            ERR_INVALID_DATA
                        );
                        ERR_FAIL_INDEX_V(
                            binormal_pos + 2,
                        ...
                        Vector3 binormal = Vector3(
                            binormal_src->array[binormal_pos + 0],
                            binormal_src->array[binormal_pos + 1],
                            binormal_src->array[binormal_pos + 2]
                        );

                        int tangent_pos =
                            (tangent_src->stride ? tangent_src->stride : 3)
                            * p.indices[src + tangent_ofs];
                        ERR_FAIL_INDEX_V(
                            tangent_pos + 0,
                            tangent_src->array.size(),
                            ERR_INVALID_DATA
                        );
                        ERR_FAIL_INDEX_V(
                            tangent_pos + 2,
                        ...
                        Vector3 tangent = Vector3(
                            tangent_src->array[tangent_pos + 0],
                            tangent_src->array[tangent_pos + 1],
                            tangent_src->array[tangent_pos + 2]
                        );

                        vertex.tangent.normal = tangent;
                        vertex.tangent.d =
                            vertex.normal.cross(tangent).dot(binormal) > 0 ? 1
                                                                           : -1;
                    }
                }

                if (uv_src) {
                    int uv_pos = (uv_src->stride ? uv_src->stride : 2)
                               * p.indices[src + uv_ofs];
                    ERR_FAIL_INDEX_V(
                        uv_pos + 0,
                        uv_src->array.size(),
                        ERR_INVALID_DATA
                    ...
                    vertex.uv = Vector3(
                        uv_src->array[uv_pos + 0],
                        1.0 - uv_src->array[uv_pos + 1],
                        0
                    );
                }

                if (uv2_src) {
                    int uv2_pos = (uv2_src->stride ? uv2_src->stride : 2)
                                * p.indices[src + uv2_ofs];
                    ERR_FAIL_INDEX_V(
                        uv2_pos + 0,
                        uv2_src->array.size(),
                        ERR_INVALID_DATA
                    ...
                    vertex.uv2 = Vector3(
                        uv2_src->array[uv2_pos + 0],
                        1.0 - uv2_src->array[uv2_pos + 1],
                        0
                    );
                }
                ...
                if (color_src) {
                    int color_pos =
                        (color_src->stride ? color_src->stride : 3)
                        * p.indices[src + color_ofs]; // colors are RGB in
                                                      // collada..
                    ERR_FAIL_INDEX_V(
                        color_pos + 0,
                        color_src->array.size(),
                        ERR_INVALID_DATA
                    );
                    ERR_FAIL_INDEX_V(
                        color_pos + ((color_src->stride > 3) ? 3 : 2),
                        color_src->array.size(),
                        ERR_INVALID_DATA
                    );
                    vertex.color = Color(
                        color_src->array[color_pos + 0],
                        color_src->array[color_pos + 1],
                        color_src->array[color_pos + 2],
                        (color_src->stride > 3)
                            ? color_src->array[color_pos + 3]
                            : 1.0
                    );
                }
                ...
                if (collada.state.up_axis == Vector3::AXIS_Z) {
                    Vector3 bn = vertex.normal.cross(vertex.tangent.normal)
                               * vertex.tangent.d;
                    ...
                    vertex.tangent.d =
                        vertex.normal.cross(vertex.tangent.normal).dot(bn) > 0
                            ? 1
                            : -1;
                }
                ...
                if (j >= 2) {
                    // insert indices in reverse order (collada uses CCW as
                    // frontface)
        ...
        for (Set<Collada::Vertex>::Element* F = vertex_set.front(); F;
             F                                = F->next()) {
        ...
        if (has_weights) {
            ...
            for (int i = 0; i < vertex_array.size(); i++) {
                vertex_array.write[i].vertex =
                    local_xform.xform(vertex_array[i].vertex);
                vertex_array.write[i].normal =
                    local_xform.basis.xform(vertex_array[i].normal)
                        .normalized();
                vertex_array.write[i].tangent.normal =
                    local_xform.basis.xform(vertex_array[i].tangent.normal)
                        .normalized();
                if (local_xform_mirror) {
...

        {
...

            {
                if (p_material_map.has(p.material)) {
                    ...
                    WARN_PRINT(
                        "Collada: Unreferenced material in geometry instance: "
                        + p.material
                    );
                }
            }
            ...
            for (int k = 0; k < vertex_array.size(); k++) {
                ...
                if (uv_src) {
                    surftool->add_uv(
                        Vector2(vertex_array[k].uv.x, vertex_array[k].uv.y)
                    );
                }
                if (uv2_src) {
                    surftool->add_uv2(
                        Vector2(vertex_array[k].uv2.x, vertex_array[k].uv2.y)
                    );
                }
                if (color_src) {
                ...
                if (has_weights) {
                    ...
                    for (int l = 0; l < VS::ARRAY_WEIGHTS_SIZE; l++) {
                        if (l < vertex_array[k].weights.size()) {
                            weights.write[l] =
                                vertex_array[k].weights[l].weight;
                            bones.write[l] =
                                vertex_array[k].weights[l].bone_idx;
            ...
            for (List<int>::Element* E = indices_list.front(); E;
                 E                     = E->next()) {
            ...
            if ((!binormal_src || !tangent_src) && normal_src && uv_src
                && force_make_tangents) {
            ...
            for (int mi = 0; mi < p_morph_meshes.size(); mi++) {
                ...
                // add valid weight and bone arrays if they exist, TODO check if
                // they are unique to shape (generally not)
...
Error ColladaImport::_create_resources(
    Collada::Node* p_node,
    uint32_t p_use_compression
) {
    if (p_node->type == Collada::Node::TYPE_GEOMETRY
        && node_map.has(p_node->id)) {
        ...
        if (Object::cast_to<Path>(node)) {
            ...
            if (curve_cache.has(ng->source)) {
                ...
                const Collada::CurveData& cd =
                    collada.state.curve_data_map[ng->source];

                ERR_FAIL_COND_V(
                    !cd.control_vertices.has("POSITION"),
                    ERR_INVALID_DATA
                );
                ERR_FAIL_COND_V(
                    !cd.control_vertices.has("IN_TANGENT"),
                    ERR_INVALID_DATA
                );
                ERR_FAIL_COND_V(
                    !cd.control_vertices.has("OUT_TANGENT"),
                    ERR_INVALID_DATA
                );
                ERR_FAIL_COND_V(
                    !cd.control_vertices.has("INTERPOLATION"),
                    ERR_INVALID_DATA
                );

                ERR_FAIL_COND_V(
                    !cd.sources.has(cd.control_vertices["POSITION"]),
                    ERR_INVALID_DATA
                );
                const Collada::CurveData::Source& vertices =
                    cd.sources[cd.control_vertices["POSITION"]];
                ERR_FAIL_COND_V(vertices.stride != 3, ERR_INVALID_DATA);

                ERR_FAIL_COND_V(
                    !cd.sources.has(cd.control_vertices["IN_TANGENT"]),
                    ERR_INVALID_DATA
                );
                const Collada::CurveData::Source& in_tangents =
                    cd.sources[cd.control_vertices["IN_TANGENT"]];
                ERR_FAIL_COND_V(in_tangents.stride != 3, ERR_INVALID_DATA);

                ERR_FAIL_COND_V(
                    !cd.sources.has(cd.control_vertices["OUT_TANGENT"]),
                    ERR_INVALID_DATA
                );
                const Collada::CurveData::Source& out_tangents =
                    cd.sources[cd.control_vertices["OUT_TANGENT"]];
                ERR_FAIL_COND_V(out_tangents.stride != 3, ERR_INVALID_DATA);

                ERR_FAIL_COND_V(
                    !cd.sources.has(cd.control_vertices["INTERPOLATION"]),
                    ERR_INVALID_DATA
                );
                const Collada::CurveData::Source& interps =
                    cd.sources[cd.control_vertices["INTERPOLATION"]];
                ERR_FAIL_COND_V(interps.stride != 1, ERR_INVALID_DATA);

                const Collada::CurveData::Source* tilts = nullptr;
                if (cd.control_vertices.has("TILT")
                    && cd.sources.has(cd.control_vertices["TILT"])) {
                ...
                for (int i = 0; i < pc; i++) {
                    Vector3 pos(
                        vertices.array[i * 3 + 0],
                        vertices.array[i * 3 + 1],
                        vertices.array[i * 3 + 2]
                    );
                    Vector3 in(
                        in_tangents.array[i * 3 + 0],
                        in_tangents.array[i * 3 + 1],
                        in_tangents.array[i * 3 + 2]
                    );
                    Vector3 out(
                        out_tangents.array[i * 3 + 0],
                        out_tangents.array[i * 3 + 1],
        ...
        if (Object::cast_to<MeshInstance>(node)) {
            ...
            if (ng2->controller) {
                ...
                if (collada.state.skin_controller_data_map.has(ngsource)) {
                    ERR_FAIL_COND_V(
                        !collada.state.skin_controller_data_map.has(ngsource),
                        ERR_INVALID_DATA
                    );
                    ...
                    if (collada.state.morph_controller_data_map.has(meshid)) {
                        // it's a morph!!
                        morph =
                            &collada.state.morph_controller_data_map[meshid];
                        ngsource = meshid;
                        meshid   = morph->mesh;
                    } else {
                        ngsource = "";
                    ...
                    if (apply_mesh_xform_to_vertices) {
                        apply_xform =
                            collada.fix_transform(p_node->default_transform);
                    ...
                    ERR_FAIL_COND_V(
                        !skin->weights.sources.has("JOINT"),
                        ERR_INVALID_DATA
                    );

                    String joint_id = skin->weights.sources["JOINT"].source;
                    ERR_FAIL_COND_V(
                        !skin->sources.has(joint_id),
                        ERR_INVALID_DATA
                    );

                    Collada::SkinControllerData::Source* joint_src =
                        &skin->sources[joint_id];
                        ...
                        if (morph->sources.has(target)) {
                            ...
                            for (int i = 0; i < names.size(); i++) {
                                ...
                                if (collada.state.mesh_data_map.has(meshid2)) {
                                    ...
                                    const Collada::MeshData& meshdata =
                                        collada.state.mesh_data_map[meshid2];
                                    mesh->set_name(meshdata.name);
                                    Error err = _create_mesh_surfaces(
                                        false,
                                        mesh,
                                        ng2->material_map,
                                        meshdata,
                ...
                ERR_FAIL_COND_V_MSG(
                    ngsource != "",
                    ERR_INVALID_DATA,
                    "Controller instance source '" + ngsource
                        + "' is neither skin or morph!"
                );
            ...
            if (mesh_cache.has(meshid)) {
                ...
                if (collada.state.mesh_data_map.has(meshid)) {
                    ...
                    ERR_FAIL_COND_V(
                        !collada.state.mesh_data_map.has(meshid),
                        ERR_INVALID_DATA
                    );
                    mesh = Ref<ArrayMesh>(memnew(ArrayMesh));
                    const Collada::MeshData& meshdata =
                        collada.state.mesh_data_map[meshid];
                    mesh->set_name(meshdata.name);
                    Error err = _create_mesh_surfaces(
                        morphs.size() == 0,
                        mesh,
                ...
                if (!use_mesh_builtin_materials) {
                    ...
                    for (int i = 0; i < meshdata.primitives.size(); i++) {
                        ...
                        if (ng2->material_map.has(matname)) {
                            ...
                            WARN_PRINT(
                                "Collada: Unreferenced material in geometry "
                                "instance: "
                                + matname
                            );
                        }
...
Error ColladaImport::load(
    ...
    ERR_FAIL_COND_V(
        !collada.state.visual_scene_map.has(collada.state.root_visual_scene),
        ERR_INVALID_DATA
    );
    Collada::VisualScene& vs =
        collada.state.visual_scene_map[collada.state.root_visual_scene];
...
void ColladaImport::_fix_param_animation_tracks() {
    for (Map<String, Collada::Node*>::Element* E =
             collada.state.scene_map.front();
         E;
         E = E->next()) {
        ...
        switch (n->type) {
            ...
            case Collada::Node::TYPE_GEOMETRY: {
                Collada::NodeGeometry* ng =
                    static_cast<Collada::NodeGeometry*>(n);
                // test source(s)
                ...
                while (source != "") {
                    if (collada.state.skin_controller_data_map.has(source)) {
                        ...
                        source = skin.base;
                    } else if (collada.state.morph_controller_data_map.has(
                                   source
                               )) {
                        const Collada::MorphControllerData& morph =
                            collada.state.morph_controller_data_map[source];

                        if (morph.targets.has("MORPH_WEIGHT")
                            && morph.targets.has("MORPH_TARGET")) {
                            String weights = morph.targets["MORPH_WEIGHT"];
                            String targets = morph.targets["MORPH_TARGET"];
                            // fails here

                            if (morph.sources.has(targets)
                                && morph.sources.has(weights)) {
                                const Collada::MorphControllerData::Source&
                                    weight_src = morph.sources[weights];
                                const Collada::MorphControllerData::Source&
                                    target_src = morph.sources[targets];

                                ERR_FAIL_COND(
                                    weight_src.array.size()
                                    != target_src.sarray.size()
                                );

                                for (int i = 0; i < weight_src.array.size();
                                     i++) {
                                    String track_name =
                                        weights + "(" + itos(i) + ")";
                                    String mesh_name = target_src.sarray[i];
                                    if (collada.state.mesh_name_map.has(
                                            mesh_name
                                        )
                                        && collada.state.referenced_tracks.has(
                                            track_name
                                        )) {
                                        const Vector<int>& rt =
                                            collada.state
                                                .referenced_tracks[track_name];

                                        for (int rti = 0; rti < rt.size();
                                             rti++) {
                                            Collada::AnimationTrack* at =
                                                &collada.state.animation_tracks
                                                     .write[rt[rti]];
...
void ColladaImport::create_animations(
    ...
    for (int i = 0; i < collada.state.animation_clips.size(); i++) {
        for (int j = 0; j < collada.state.animation_clips[i].tracks.size();
             j++) {
...
void ColladaImport::create_animation(
    int p_clip,
    bool p_make_tracks_in_all_bones,
    bool p_import_value_tracks
) {
    ...
    for (Map<String, NodeMap>::Element* E = node_map.front(); E;
         E                                = E->next()) {
        if (E->get().bone < 0) {
            continue;
        }
    ...
    float snapshot_interval =
        1.0 / bake_fps; // should be customizable somewhere...
    ...
    for (Set<String>::Element* E = valid_animated_nodes.front(); E;
         E                       = E->next()) {
        // take snapshots
        ...
        if (nm.anim_tracks.size() == 1) {
            // use snapshot keys from anim track instead, because this was most
            // likely exported baked
            const Collada::AnimationTrack& at =
                collada.state.animation_tracks[nm.anim_tracks.front()->get()];
            snapshots.clear();
        ...
        for (int i = 0; i < snapshots.size(); i++) {
            for (List<int>::Element* ET = nm.anim_tracks.front(); ET;
                 ET                     = ET->next()) {
                // apply tracks
                ...
                const Collada::AnimationTrack& at =
                    collada.state.animation_tracks[ET->get()];
                ...
                if (xform_idx == -1) {
                    WARN_PRINT(
                        "Collada: Couldn't find matching node " + at.target
                        + " xform for track " + at.param + "."
                    );
                    continue;
                }
                ...
                if (at.component == "ANGLE") {
                    ...
                    ERR_CONTINUE_MSG(
                        data.size() != xf.data.size(),
                        "Component " + at.component + " has datasize "
            ...
            bool singular_matrix = Math::is_equal_approx(s.x, 0.0f)
                                || Math::is_equal_approx(s.y, 0.0f)
    ...
    if (p_make_tracks_in_all_bones) {
        // some bones may lack animation, but since we don't store pose as a
        // property, we must add keyframes!
        for (Map<String, bool>::Element* E = bones_with_animation.front(); E;
             E                             = E->next()) {
            if (E->get()) {
                continue;
            }
    ...
    if (p_import_value_tracks) {
        for (int i = 0; i < valid_animated_properties.size(); i++) {
            ...
            const Collada::AnimationTrack& at =
                collada.state.animation_tracks[ti];
            ...
            for (int j = 0; j < at.keys.size(); j++) {
                ...
                if (data.size() == 1) {
                    ...
                    WARN_PRINT(
                        "Collada: Unexpected amount of value keys: "
                        + itos(data.size())
                    );
                }
...
void EditorSceneImporterCollada::get_extensions(List<String>* r_extensions
) const {
...
Node* EditorSceneImporterCollada::import_scene(
    ...
    state.use_mesh_builtin_materials =
        !(p_flags & IMPORT_MATERIALS_IN_INSTANCES);
    state.bake_fps = p_bake_fps;

    Error err = state.load(
        p_path,
        flags,
        p_flags & EditorSceneImporter::IMPORT_GENERATE_TANGENT_ARRAYS,
        p_compress_flags
    );
    ...
    ERR_FAIL_COND_V_MSG(
        err != OK,
        nullptr,
        "Cannot load scene from file '" + p_path + "'."
    );

    if (state.missing_textures.size()) {
        /*
        for(int i=0;i<state.missing_textures.size();i++) {
            EditorNode::add_io_error("Texture Not Found:
        "+state.missing_textures[i]);
        }
        */
    ...
    if (p_flags & IMPORT_ANIMATION) {
        state.create_animations(
            ...
            p_flags & EditorSceneImporter::IMPORT_ANIMATION_KEEP_VALUE_TRACKS
        );
...
Ref<Animation> EditorSceneImporterCollada::import_animation(
    const String& p_path,
    uint32_t p_flags,
    int p_bake_fps
) {
    ...
    Error err = state.load(
        p_path,
        Collada::IMPORT_FLAG_ANIMATION,
        p_flags & EditorSceneImporter::IMPORT_GENERATE_TANGENT_ARRAYS
    );
    ERR_FAIL_COND_V_MSG(
        err != OK,
        RES(),
        "Cannot load animation from file '" + p_path + "'."
    );

    state.create_animations(
        p_flags
            & EditorSceneImporter::
                IMPORT_ANIMATION_FORCE_ALL_TRACKS_IN_ALL_CLIPS,
        p_flags & EditorSceneImporter::IMPORT_ANIMATION_KEEP_VALUE_TRACKS
    );
    if (state.scene) {
    ...
    if (p_flags & IMPORT_ANIMATION_DETECT_LOOP) {
        if (base.begins_with("loop") || base.ends_with("loop")
            || base.begins_with("cycle") || base.ends_with("cycle")) {
            anim->set_loop(true);
        }
    }
