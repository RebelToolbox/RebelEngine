...
const char* EditorFeatureProfile::feature_descriptions[FEATURE_MAX] = {
    ...
    TTRC("Allows to work with signals and groups of the node selected in the "
         "Scene dock."),
    TTRC("Allows to browse the local file system via a dedicated dock."),
    TTRC("Allows to configure import settings for individual assets. Requires "
         "the FileSystem dock to function."),
};
...
void EditorFeatureProfile::set_disable_class(
...
void EditorFeatureProfile::set_disable_class_editor(
    const StringName& p_class,
    bool p_disabled
) {
    if (p_disabled) {
...
bool EditorFeatureProfile::is_class_editor_disabled(const StringName& p_class
) const {
    if (p_class == StringName()) {
        return false;
    }
...
bool EditorFeatureProfile::has_class_properties_disabled(
    const StringName& p_class
) const {
    return disabled_properties.has(p_class);
}

void EditorFeatureProfile::set_item_collapsed(
    const StringName& p_class,
    bool p_collapsed
) {
    if (p_collapsed) {
...
Error EditorFeatureProfile::save_to_file(const String& p_path) {
    ...
    for (Map<StringName, Set<StringName>>::Element* E =
             disabled_properties.front();
         E;
         E = E->next()) {
...
Error EditorFeatureProfile::load_from_file(const String& p_path) {
    ...
    if (err != OK) {
        ERR_PRINT(
            "Error parsing '" + p_path + "' on line " + itos(err_line) + ": "
            + err_str
        );
    ...
    if (!json.has("type") || String(json["type"]) != "feature_profile") {
        ERR_PRINT(
            "Error parsing '" + p_path + "', it's not a feature profile."
        );
        return ERR_PARSE_ERROR;
    }
...
void EditorFeatureProfile::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_disable_class", "class_name", "disable"),
        &EditorFeatureProfile::set_disable_class
    );
    ClassDB::bind_method(
        D_METHOD("is_class_disabled", "class_name"),
        &EditorFeatureProfile::is_class_disabled
    );

    ClassDB::bind_method(
        D_METHOD("set_disable_class_editor", "class_name", "disable"),
        &EditorFeatureProfile::set_disable_class_editor
    );
    ClassDB::bind_method(
        D_METHOD("is_class_editor_disabled", "class_name"),
    ...
    ClassDB::bind_method(
        D_METHOD("is_class_property_disabled", "class_name", "property"),
        &EditorFeatureProfile::is_class_property_disabled
    );

    ClassDB::bind_method(
        D_METHOD("set_disable_feature", "feature", "disable"),
        &EditorFeatureProfile::set_disable_feature
    );
    ClassDB::bind_method(
        D_METHOD("is_feature_disabled", "feature"),
        &EditorFeatureProfile::is_feature_disabled
    );

    ClassDB::bind_method(
        D_METHOD("get_feature_name", "feature"),
        &EditorFeatureProfile::_get_feature_name
    );

    ClassDB::bind_method(
        D_METHOD("save_to_file", "path"),
        ...
        if (current_profile != String()) {
            ...
            Error err = current->load_from_file(
                EditorSettings::get_singleton()
                    ->get_feature_profiles_dir()
                    .plus_file(current_profile + ".profile")
            );
            if (err != OK) {
                ERR_PRINT(
                    "Error loading default feature profile: " + current_profile
                );
...
void EditorFeatureProfileManager::_update_profile_list(
    const String& p_select_profile
) {
    String selected_profile;
    if (p_select_profile == String()) { // default, keep
        if (profile_list->get_selected() >= 0) {
            selected_profile =
                profile_list->get_item_metadata(profile_list->get_selected());
            if (!FileAccess::exists(
                    EditorSettings::get_singleton()
                        ->get_feature_profiles_dir()
                        .plus_file(selected_profile + ".profile")
                )) {
    ...
    DirAccessRef d = DirAccess::open(
        EditorSettings::get_singleton()->get_feature_profiles_dir()
    );
    ERR_FAIL_COND_MSG(
        !d,
    ...
    current_profile_name->set_text(
        current_profile != String() ? current_profile : TTR("(none)")
    );
...
void EditorFeatureProfileManager::_profile_action(int p_action) {
    switch (p_action) {
        ...
        case PROFILE_EXPORT: {
            ...
            export_profile->set_current_file(
                _get_selected_profile() + ".profile"
            );
        } break;
        case PROFILE_NEW: {
        ...
        case PROFILE_ERASE: {
            ...
            erase_profile_dialog->set_text(vformat(
                TTR("Remove currently selected profile, '%s'? Cannot be undone."
                ),
                selected
            ));
...
void EditorFeatureProfileManager::_erase_selected_profile() {
    String selected = _get_selected_profile();
    ERR_FAIL_COND(selected == String());
    DirAccessRef da = DirAccess::open(
        EditorSettings::get_singleton()->get_feature_profiles_dir()
    );
    ERR_FAIL_COND_MSG(
        !da,
...
void EditorFeatureProfileManager::_create_new_profile() {
    ...
    String file =
        EditorSettings::get_singleton()->get_feature_profiles_dir().plus_file(
            name + ".profile"
        );
    if (FileAccess::exists(file)) {
        EditorNode::get_singleton()->show_warning(
            TTR("Profile with this name already exists.")
        );
        return;
    }
    ...
    // The newly created profile is the first one, make it the current profile
    // automatically.
...
void EditorFeatureProfileManager::_fill_classes_from(
    ...
    class_item->set_icon(
        0,
        EditorNode::get_singleton()->get_class_icon(p_class, "Node")
    );
    String text = p_class;
...
void EditorFeatureProfileManager::_class_list_item_selected() {
    ...
    if (md.get_type() == Variant::STRING) {
        ...
        String feature_description =
            EditorFeatureProfile::get_feature_description(
                EditorFeatureProfile::Feature(feature_id)
            );
    ...
    if (props.size() > 0) {
        ...
        for (List<PropertyInfo>::Element* E = props.front(); E; E = E->next()) {
            ...
            property->set_checked(
                0,
                !edited->is_class_property_disabled(class_name, name)
            );
            ...
            property->set_icon(
                0,
                EditorNode::get_singleton()->get_class_icon(icon_type)
            );
        }
    }
...
void EditorFeatureProfileManager::_class_list_item_edited() {
    ...
    if (md.get_type() == Variant::STRING) {
        ...
        edited->set_disable_feature(
            EditorFeatureProfile::Feature(feature_selected),
            !checked
        );
        _save_and_update();
    }
...
void EditorFeatureProfileManager::_property_item_edited() {
    ...
    if (md.get_type() == Variant::STRING) {
        ...
        edited->set_disable_class_property(
            class_name,
            property_selected,
            !checked
        );
...
void EditorFeatureProfileManager::_update_selected_profile() {
    ...
    if (profile == current_profile) {
        ...
        ); // nothing selected, current should never be null
    } else {
        // reload edited, if different from current
        edited.instance();
        Error err =
            edited->load_from_file(EditorSettings::get_singleton()
                                       ->get_feature_profiles_dir()
                                       .plus_file(profile + ".profile"));
        ERR_FAIL_COND_MSG(
            err != OK,
            "Error when loading EditorSettings from file '"
                + EditorSettings::get_singleton()
                      ->get_feature_profiles_dir()
                      .plus_file(profile + ".profile")
                + "'."
    ...
    for (int i = 0; i < EditorFeatureProfile::FEATURE_MAX; i++) {
        ...
        feature->set_text(
            0,
            TTRGET(EditorFeatureProfile::get_feature_name(
                EditorFeatureProfile::Feature(i)
            ))
        );
...
void EditorFeatureProfileManager::_import_profiles(const Vector<String>& p_paths
    ...
    for (int i = 0; i < p_paths.size(); i++) {
        ...
        if (err != OK) {
            EditorNode::get_singleton()->show_warning(vformat(
                TTR("File '%s' format is invalid, import aborted."),
                basefile
            ));
            return;
        }

        String dst_file = EditorSettings::get_singleton()
                              ->get_feature_profiles_dir()
                              .plus_file(basefile);

        if (FileAccess::exists(dst_file)) {
            EditorNode::get_singleton()->show_warning(vformat(
                TTR("Profile '%s' already exists. Remove it first before "
                    "importing, import aborted."),
    ...
    _update_profile_list();
    // The newly imported profile is the first one, make it the current profile
    // automatically.
    if (profile_list->get_item_count() == 1) {
        _profile_action(PROFILE_SET);
    }
...
void EditorFeatureProfileManager::_bind_methods() {
    ...
    ClassDB::bind_method(
        "_profile_action",
        &EditorFeatureProfileManager::_profile_action
    );
    ClassDB::bind_method(
        "_create_new_profile",
    ...
    ClassDB::bind_method(
        "_profile_selected",
        &EditorFeatureProfileManager::_profile_selected
    );
    ClassDB::bind_method(
        "_erase_selected_profile",
        &EditorFeatureProfileManager::_erase_selected_profile
    );
    ClassDB::bind_method(
        "_import_profiles",
        &EditorFeatureProfileManager::_import_profiles
    );
    ClassDB::bind_method(
        "_export_profile",
...
EditorFeatureProfileManager::EditorFeatureProfileManager() {
    ...
    profile_actions[PROFILE_CLEAR]
        ->connect("pressed", this, "_profile_action", varray(PROFILE_CLEAR));
    ...
    profile_actions[PROFILE_NEW]
        ->connect("pressed", this, "_profile_action", varray(PROFILE_NEW));
    ...
    profile_actions[PROFILE_ERASE]
        ->connect("pressed", this, "_profile_action", varray(PROFILE_ERASE));
    ...
    profile_actions[PROFILE_SET]
        ->connect("pressed", this, "_profile_action", varray(PROFILE_SET));
    ...
    profile_actions[PROFILE_IMPORT]
        ->connect("pressed", this, "_profile_action", varray(PROFILE_IMPORT));
    ...
    profile_actions[PROFILE_EXPORT]
        ->connect("pressed", this, "_profile_action", varray(PROFILE_EXPORT));
    ...
    class_list_vbc->add_margin_child(
        TTR("Configure Selected Profile:"),
        class_list,
        true
    );
    ...
    property_list_vbc
        ->add_margin_child(TTR("Description:"), description_bit, false);
    description_bit->set_custom_minimum_size(Size2(0, 80) * EDSCALE);

    property_list = memnew(Tree);
    property_list_vbc
        ->add_margin_child(TTR("Extra Options:"), property_list, true);
    ...
    no_profile_selected_help = memnew(Label(TTR(
