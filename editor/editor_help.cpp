...
void EditorHelp::_class_desc_select(const String& p_select) {
    if (p_select.begins_with("$")) { // enum
        ...
        emit_signal(
            "go_to_help",
            "class_name:" + p_select.substr(1, p_select.length())
        );
        return;
    } else if (p_select.begins_with("@")) {
        ...
        String link =
            p_select.substr(tag_end + 1, p_select.length()).lstrip(" ");
...
void EditorHelp::_class_desc_resized() {
    ...
    Ref<StyleBox> class_desc_stylebox =
        EditorNode::get_singleton()
            ->get_theme_base()
            ->get_stylebox("normal", "RichTextLabel")
            ->duplicate();
...
void EditorHelp::_add_method(
    ...
    method_line[p_method.name] =
        class_desc->get_line_count() - 2; // gets overridden if description
    ...
    for (int j = 0; j < p_method.arguments.size(); j++) {
        ...
        class_desc->add_text(": ");
        _add_type(
            p_method.arguments[j].type,
            p_method.arguments[j].enumeration
        );
...
void EditorHelp::_update_docs() {
    ...
    // Class description
    if (cd.description != "") {
        section_line.push_back(Pair<String, int>(
            TTR("Description"),
            class_desc->get_line_count() - 2
        ));
    ...
    if (cd.properties.size()) {
        ...
        for (int i = 0; i < cd.properties.size(); i++) {
            property_line[cd.properties[i].name] =
                class_desc->get_line_count()
                - 2; // gets overridden if description
            ...
            if (cd.properties[i].default_value != "") {
                class_desc->push_color(symbol_color);
                class_desc->add_text(
                    cd.properties[i].overridden ? " [" + TTR("override:") + " "
                                                : " [" + TTR("default:") + " "
                );
                class_desc->pop();
                class_desc->push_color(value_color);
    ...
    bool sort_methods = EditorSettings::get_singleton()->get(
    ...
    for (int i = 0; i < cd.methods.size(); i++) {
        if (skip_methods.has(cd.methods[i].name)) {
            if (cd.methods[i].arguments.size() == 0 /* getter */
                || (cd.methods[i].arguments.size() == 1
                    && cd.methods[i].return_type == "void" /* setter */)) {
                continue;
            }
        }
    ...
    if (methods.size()) {
        ...
        section_line.push_back(
            Pair<String, int>(TTR("Methods"), class_desc->get_line_count() - 2)
        );
        class_desc->push_color(title_color);
        class_desc->push_font(doc_title_font);
        ...
        for (int pass = 0; pass < 2; pass++) {
            ...
            for (int i = 0; i < methods.size(); i++) {
                ...
                if ((pass == 0 && q.find("virtual") != -1)
                    || (pass == 1 && q.find("virtual") == -1)) {
    ...
    if (cd.signals.size()) {
        ...
        section_line.push_back(
            Pair<String, int>(TTR("Signals"), class_desc->get_line_count() - 2)
        );
        class_desc->push_color(title_color);
        class_desc->push_font(doc_title_font);
    ...
    if (cd.constants.size()) {
        ...
        if (enums.size()) {
            ...
            for (Map<String, Vector<DocsData::ConstantDoc>>::Element* E =
                ...
                class_desc->pop();
                String e = E->key();
                if ((e.get_slice_count(".") > 1)
                    && (e.get_slice(".", 0) == edited_class)) {
                    e = e.get_slice(".", 1);
                }
                ...
                for (int i = 0; i < enum_list.size(); i++) {
                    if (cd.name == "@GlobalScope") {
                        enumValuesContainer[enum_list[i].name] =
                            enumStartingLine;
                    }

                    // Add the enum constant line to the constant_line map so we
                    // can locate it as a constant
                    constant_line[enum_list[i].name] =
                        class_desc->get_line_count() - 2;
        ...
        if (constants.size()) {
            ...
            for (int i = 0; i < constants.size(); i++) {
                constant_line[constants[i].name] =
                    class_desc->get_line_count() - 2;
                class_desc->push_font(doc_code_font);

                if (constants[i].value.begins_with("Color(")
                    && constants[i].value.ends_with(")")) {
                    String stripped = constants[i]
                                          .value.replace(" ", "")
                                          .replace("Color(", "")
                                          .replace(")", "");
                    Vector<float> color = stripped.split_floats(",");
                    if (color.size() >= 3) {
                        class_desc->push_color(
        ...
        for (int i = 0; i < cd.properties.size(); i++) {
            ...
            if (cd.properties[i].setter != "") {
                ...
                if (method_map[cd.properties[i].setter].arguments.size() > 1) {
                    // Setters with additional arguments are exposed in the
                    // method list, so we link them here for quick access.
                    class_desc->push_meta("@method " + cd.properties[i].setter);
                    class_desc->add_text(
                        cd.properties[i].setter + TTR("(value)")
                    );
                    class_desc->pop();
                } else {
                    class_desc->add_text(
                ...
                class_desc->pop(); // font
                class_desc->pop(); // cell
                method_line[cd.properties[i].setter] =
                    property_line[cd.properties[i].name];
            }
            ...
            if (cd.properties[i].getter != "") {
                ...
                if (method_map[cd.properties[i].getter].arguments.size() > 0) {
                    // Getters with additional arguments are exposed in the
                    // method list, so we link them here for quick access.
                ...
                class_desc->pop(); // font
                class_desc->pop(); // cell
                method_line[cd.properties[i].getter] =
                    property_line[cd.properties[i].name];
            }
            ...
            if (cd.properties[i].description.strip_edges() != String()) {
                ...
                class_desc->push_color(comment_color);
                class_desc->append_bbcode(
                    TTR("There is currently no description for this property. "
                        "Please help us by "
                        "[color=$color][url=$url]contributing "
                        "one[/url][/color]!")
                        .replace("$url", CONTRIBUTE_URL)
                        .replace("$color", link_color_text)
                );
                class_desc->pop();
            }
            class_desc->pop();
        ...
        for (int pass = 0; pass < 2; pass++) {
            ...
            for (int i = 0; i < methods_filtered.size(); i++) {
                ...
                if (methods_filtered[i].description.strip_edges() != String()) {
                    ...
                    class_desc->push_color(comment_color);
                    class_desc->append_bbcode(
                        TTR("There is currently no description for this "
                            "method. Please help us by "
                            "[color=$color][url=$url]contributing "
                            "one[/url][/color]!")
                            .replace("$url", CONTRIBUTE_URL)
                            .replace("$color", link_color_text)
...
void EditorHelp::_help_callback(const String& p_topic) {
    ...
    if (what == "class_desc") {
        ...
        if (constant_line.has(name)) {
            ...
            Map<String, Map<String, int>>::Element* iter =
                enum_values_line.front();
            while (true) {
...
static void _add_text_to_rt(const String& p_bbcode, RichTextLabel* p_rt) {
    ...
    while (pos < bbcode.length()) {
        ...
        if (tag.begins_with("/")) {
            ...
            String link_target =
                tag.substr(tag_end + 1, tag.length()).lstrip(" ");

            p_rt->push_color(link_color);
            p_rt->push_meta("@" + link_tag + " " + link_target);
            p_rt->add_text(
                link_target + (tag.begins_with("method ") ? "()" : "")
            );
            p_rt->pop();
            p_rt->pop();
            pos = brk_end + 1;
            ...
            Ref<Texture> texture =
                ResourceLoader::load(base_path.plus_file(image), "Texture");
            if (texture.is_valid()) {
            ...
            Ref<Font> font =
                ResourceLoader::load(base_path.plus_file(fnt), "Font");
            if (font.is_valid()) {
                p_rt->push_font(font);
            } else {
...
void EditorHelp::_bind_methods() {
    ...
    ClassDB::bind_method(
        "_class_desc_resized",
        &EditorHelp::_class_desc_resized
    );
    ClassDB::bind_method("_request_help", &EditorHelp::_request_help);
    ClassDB::bind_method(
        "_unhandled_key_input",
        &EditorHelp::_unhandled_key_input
    );
...
EditorHelp::EditorHelp() {
    ...
    // Added second so it opens at the bottom so it won't offset the entire
    // widget.
...
void FindBar::_bind_methods() {
    ...
    ClassDB::bind_method(
        ...
        &FindBar::_search_text_changed
    );
    ClassDB::bind_method(
        "_search_text_entered",
        &FindBar::_search_text_entered
    );
...
void FindBar::_unhandled_input(const Ref<InputEvent>& p_event) {
    ...
    if (k.is_valid()) {
        if (k->is_pressed()
            && (rich_text_label->has_focus()
                || is_a_parent_of(get_focus_owner()))) {
            bool accepted = true;
