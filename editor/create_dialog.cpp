...
void CreateDialog::popup_create(
    bool p_dont_clear,
    bool p_replace_mode,
            ...
            if ((ClassDB::class_exists(name)
                ...
                ti->set_icon(
                    ...
                    EditorNode::get_singleton()
                        ->get_class_icon(name, icon_fallback)
                );
            }
        }
...
void CreateDialog::add_type(
    ...
    if (!can_instance) {
        ...
        if (p_type.to_lower() == search_term) {
            *to_select = item;
            ...
            if (*to_select) {
                String name =
                    (*to_select)->get_text(0).split(" ")[0].to_lower();
                is_substring_of_selected = name.find(search_term) >= 0;
                is_subsequence_of_selected =
                    search_term.is_subsequence_of(name);
            ...
            if (is_subsequence_of_type && !is_selected_equal) {
                if (is_substring_of_type) {
                    ...
                    // substring results weigh more than subsequences, so let's
    ...
    item->set_icon(
        0,
        EditorNode::get_singleton()->get_class_icon(p_type, icon_fallback)
    );
...
void CreateDialog::_update_search() {
    ...
    help_bit->set_text(
        vformat(TTR("No results for \"%s\"."), search_box->get_text())
    );
    ...
    for (List<StringName>::Element* I = type_list.front(); I; I = I->next()) {
        ...
        if (cpp_type) {
            ...
            for (Set<StringName>::Element* E = type_blacklist.front();
                 E && !skip;
                 E = E->next()) {
        ...
        if (search_box->get_text() == "") {
            ...
            while (type2 != ""
                   && (cpp_type2 ? ClassDB::is_parent_class(type2, base_type)
                ...
                cpp_type2 = cpp_type2
                         || ClassDB::class_exists(type2
                            ...
                            // we can skip the check if it's already true.

                if (!cpp_type2 && !search_loaded_scripts.has(type2)) {
                    search_loaded_scripts[type2] =
                        ed.script_class_load_script(type2);
                }
            }
        ...
        if (EditorNode::get_editor_data().get_custom_types().has(type)
            ...
            for (int i = 0; i < ct.size(); i++) {
                bool show =
                    search_box->get_text().is_subsequence_ofi(ct[i].name);
                ...
                item->set_icon(
                    0,
                    ct[i].icon.is_valid() ? ct[i].icon
                                          : get_icon(base_icon, "EditorIcons")
                );
...
void CreateDialog::_item_selected() {
    ...
    if (!brief_desc.empty()) {
        ...
        // resized to be taller).
        help_bit->set_text(vformat("[b]%s[/b]: %s", name, brief_desc));
        help_bit->get_rich_text()->set_self_modulate(Color(1, 1, 1, 1));
    } else {
        // Use nested `vformat()` as translators shouldn't interfere with BBCode
        // tags.
        help_bit->set_text(vformat(
            TTR("No description available for %s."),
            vformat("[b]%s[/b]", name)
        ));
        help_bit->get_rich_text()->set_self_modulate(Color(1, 1, 1, 0.5));
    }
...
void CreateDialog::_update_favorite_list() {
    ...
    for (int i = 0; i < favorite_list.size(); i++) {
        ...
        ti->set_icon(
            0,
            EditorNode::get_singleton()->get_class_icon(name, icon_fallback)
        );
    }
...
void CreateDialog::drop_data_fw(
    const Point2& p_point,
    const Variant& p_data,
    Control* p_from
) {
...
void CreateDialog::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("_favorite_toggled"),
        &CreateDialog::_favorite_toggled
    );
    ClassDB::bind_method(
        D_METHOD("_history_selected"),
    ...
    ClassDB::bind_method(
        D_METHOD("_history_activated"),
        &CreateDialog::_history_activated
    );
    ClassDB::bind_method(
        D_METHOD("_favorite_activated"),
        &CreateDialog::_favorite_activated
    );
    ClassDB::bind_method(
        D_METHOD("_save_and_update_favorite_list"),
...
CreateDialog::CreateDialog() {
    ...
    ); // PluginScript must be initialized before use, which is not possible
       // here
    type_blacklist.insert("ScriptCreateDialog"
    ); // This is an exposed editor Node that doesn't have an Editor prefix.
}
