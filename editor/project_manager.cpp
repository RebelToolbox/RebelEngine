...
private:
    ...
    void _create_folder() {
        const String project_name_no_edges =
            project_name->get_text().strip_edges();
            ...
            if (mode == MODE_IMPORT) {
                ...
                if (mode == MODE_NEW) {
                    ...
                    if (rasterizer_button_group->get_pressed_button()->get_meta(
                        ...
                        initial_settings
                            ["rendering/vram_compression/import_etc2"] = false;
                        initial_settings
                            ["rendering/vram_compression/import_etc"] = true;
                    }
                    ...
                    if (!pkg) {
                        dialog_error->set_text(TTR(
                            "Error opening package file, not in ZIP format."
                        ));
                        dialog_error->popup_centered_minsize();
                        return;
                    }
                    ...
                    while (ret == UNZ_OK) {
                        ...
                        if (name.ends_with("project.rebel")) {
                            zip_root =
                                name.substr(0, name.rfind("project.rebel"));
                            break;
                        }
                    ...
                    if (failed_files.size()) {
                        ...
                        dialog_error->set_text(
                            TTR("Package installed successfully!")
                        );
                        dialog_error->popup_centered_minsize();
                    }
                }
...
public:
    ...
    ProjectDialog() {
        ...
        fdialog_install
            ->connect("dir_selected", this, "_install_path_selected");
        fdialog_install
            ->connect("file_selected", this, "_install_path_selected");
...
void ProjectManager::_update_project_buttons() {
    Vector<ProjectList::Item> selected_projects =
        _project_list->get_selected_projects();
...
void ProjectManager::_unhandled_input(const Ref<InputEvent>& p_ev) {
    ...
    if (k.is_valid()) {
        ...
        switch (k->get_scancode()) {
            ...
            case KEY_END: {
                if (_project_list->get_project_count() > 0) {
                    _project_list->select_project(
                        _project_list->get_project_count() - 1
                    );
                    _update_project_buttons();
                }
...
void ProjectManager::_open_selected_projects() {
    ...
    const Set<String>& selected_list =
        _project_list->get_selected_project_keys();
        ...
        if (!DirAccess::exists(path + "/" + project_data_dir_name)) {
            run_error_diag->set_text(
                TTR("Can't run project: Assets need to be imported.\nPlease "
                    "edit the project to trigger the initial import.")
            );
            run_error_diag->popup_centered();
            continue;
        }
...
ProjectManager::ProjectManager() {
    ...
    ); // just write settings as they came
