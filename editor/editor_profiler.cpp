...
void EditorProfiler::_make_metric_ptrs(Metric& m) {
    for (int i = 0; i < m.categories.size(); i++) {
        ...
        for (int j = 0; j < m.categories[i].items.size(); j++) {
            m.item_ptrs[m.categories[i].items[j].signature] =
                &m.categories.write[i].items.write[j];
        }
    }
}
...
void EditorProfiler::add_frame_metric(const Metric& p_metric, bool p_final) {
    ...
    cursor_metric_edit->set_min(
        MAX(frame_metrics[last_metric].frame_number - frame_metrics.size(), 0)
    );
...
void EditorProfiler::clear() {
    int metric_size = EditorSettings::get_singleton()->get(
...
String EditorProfiler::_get_time_as_text(
    const Metric& m,
    float p_time,
    int p_calls
) {
...
void EditorProfiler::_update_plot() {
    ...
    for (int i = 0; i < frame_metrics.size(); i++) {
        ...
        for (Set<StringName>::Element* E = plot_sigs.front(); E;
            ...
            const Map<StringName, Metric::Category::Item*>::Element* G =
                m.item_ptrs.find(E->get());
            if (G) {
                if (use_self) {
        ...
        for (int i = 0; i < w; i++) {
            ...
            for (Set<StringName>::Element* E = plot_sigs.front(); E;
                ...
                for (int j = current; j < next; j++) {
                    ...
                    plot_pos =
                        MAX(CLAMP(int(value * h / highest), 0, h - 1),
                            plot_pos);
                }
                ...
                for (int j = prev_plot; j <= plot_pos; j++) {
                    column[j * 4 + 0] +=
                        Math::fast_ftoi(CLAMP(col.r * 255, 0, 255));
                    column[j * 4 + 1] +=
                        Math::fast_ftoi(CLAMP(col.g * 255, 0, 255));
                    column[j * 4 + 2] +=
                        Math::fast_ftoi(CLAMP(col.b * 255, 0, 255));
                    column[j * 4 + 3] += 1;
                }
            }
            ...
            for (int j = 0; j < h * 4; j += 4) {
                ...
                wr[widx + 0] =
                    is_filled ? red : Math::fast_ftoi(background_color.r * 255);
                wr[widx + 1] = is_filled
                                 ? green
                                 : Math::fast_ftoi(background_color.g * 255);
                wr[widx + 2] = is_filled
                                 ? blue
                                 : Math::fast_ftoi(background_color.b * 255);
                wr[widx + 3] = 255;
            }
        }
    }
...
void EditorProfiler::_update_frame() {
    ...
    for (int i = 0; i < m.categories.size(); i++) {
        ...
        category->set_text(
            1,
            _get_time_as_text(m, m.categories[i].total_time, 1)
        );

        if (plot_sigs.has(m.categories[i].signature)) {
            category->set_checked(0, true);
            category->set_custom_color(
                0,
                _get_color_from_signature(m.categories[i].signature)
            );
        }
...
void EditorProfiler::_graph_tex_input(const Ref<InputEvent>& p_ev) {
    ...
    if ((mb.is_valid() && mb->get_button_index() == BUTTON_LEFT
        ...
        if (mb.is_valid() || mm->get_button_mask() & BUTTON_MASK_LEFT) {
            ...
            // metric may be invalid, so look for closest metric that is valid,
            // this makes snap feel better
            bool valid = false;
            for (int i = 0; i < frame_metrics.size(); i++) {
...
int EditorProfiler::_get_cursor_index() const {
    ...
    int diff =
        (frame_metrics[last_metric].frame_number
         - cursor_metric_edit->get_value());
...
void EditorProfiler::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("_graph_tex_draw"),
        &EditorProfiler::_graph_tex_draw
    );
    ClassDB::bind_method(
        D_METHOD("_graph_tex_input"),
        &EditorProfiler::_graph_tex_input
    );
    ClassDB::bind_method(
        D_METHOD("_graph_tex_mouse_exit"),
        &EditorProfiler::_graph_tex_mouse_exit
    );
    ClassDB::bind_method(
        D_METHOD("_cursor_metric_changed"),
        &EditorProfiler::_cursor_metric_changed
    );
    ClassDB::bind_method(
        D_METHOD("_combo_changed"),
        &EditorProfiler::_combo_changed
    );
...
Vector<Vector<String>> EditorProfiler::get_data_as_csv() const {
    ...
    for (int i = 0; i < frame_metrics.size(); i++) {
        ...
        for (Map<StringName, Metric::Category*>::Element* E =
                 m.category_ptrs.front();
             E;
             E = E->next()) {
            possible_signatures.insert(E->key());
        }
        for (Map<StringName, Metric::Category::Item*>::Element* E =
                 m.item_ptrs.front();
             E;
             E = E->next()) {
            possible_signatures.insert(E->key());
    ...
    for (int i = 0; i < frame_metrics.size(); i++) {
        ...
        for (Map<StringName, Metric::Category*>::Element* E =
             ...
             E;
             E = E->next()) {
            values.write[sig_map[E->key()]] =
                String::num_real(E->value()->total_time);
        }
        for (Map<StringName, Metric::Category::Item*>::Element* E =
                 m.item_ptrs.front();
             E;
...
EditorProfiler::EditorProfiler() {
    ...
    display_time->set_tooltip(TTR(
        "Inclusive: Includes time from other functions called by this "
        "function.\nUse this to spot bottlenecks.\n\nSelf: Only count the time "
        "spent in the function itself, not in other functions called by that "
    ...
    cursor_metric_edit
        ->connect("value_changed", this, "_cursor_metric_changed");
    ...
    int metric_size = CLAMP(
        int(EDITOR_DEF("debugger/profiler_frame_history_size", 600)),
        60,
        1024
    );
    frame_metrics.resize(metric_size);
