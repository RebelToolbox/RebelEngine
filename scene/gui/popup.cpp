...
void Popup::_popup(const Rect2& p_bounds, const bool p_centered) {
    ...
    if (!p_bounds.has_no_area()) {
        ...
        if (p_centered && p_bounds.size != get_size()) {
            set_position(
                p_bounds.position - ((get_size() - p_bounds.size) / 2.0).floor()
            );
        } else {
...
void Popup::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("popup_centered", "size"),
        &Popup::popup_centered,
        DEFVAL(Size2())
    );
    ClassDB::bind_method(
        D_METHOD("popup_centered_ratio", "ratio"),
        &Popup::popup_centered_ratio,
        DEFVAL(0.75)
    );
    ClassDB::bind_method(
        D_METHOD("popup_centered_minsize", "minsize"),
    ...
    ClassDB::bind_method(
        D_METHOD("popup", "bounds"),
        &Popup::popup,
        DEFVAL(Rect2())
    );
    ClassDB::bind_method(
        D_METHOD("set_exclusive", "enable"),
        &Popup::set_exclusive
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "popup_exclusive"),
        "set_exclusive",
        "is_exclusive"
    );
...
String Popup::get_configuration_warning() const {
    ...
    if (is_visible_in_tree()) {
        ...
        warning +=
            TTR("Popups will hide by default unless you call popup() or any of "
