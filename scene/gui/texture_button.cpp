...
bool TextureButton::has_point(const Point2& p_point) const {
    if (click_mask.is_valid()) {
        ...
        if (_position_rect.has_no_area()) {
            ...
            // if the stretch mode is tile we offset the point to keep it inside
            // the mask size
            rect.size = mask_size;
            ...
            // offset and scale the new point position to adjust it to the
            // bitmask size
            point -= ofs;
            point *= scale;

            // finally, we need to check if the point is inside a rectangle with
            // a position >= 0,0 and a size <= mask_size
            rect.position = Point2(
                MAX(0, _texture_region.position.x),
                MAX(0, _texture_region.position.y)
            );
            rect.size = Size2(
                MIN(mask_size.x, _texture_region.size.x),
...
void TextureButton::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_normal_texture", "texture"),
        &TextureButton::set_normal_texture
    );
    ClassDB::bind_method(
        D_METHOD("set_pressed_texture", "texture"),
        &TextureButton::set_pressed_texture
    );
    ClassDB::bind_method(
        D_METHOD("set_hover_texture", "texture"),
        &TextureButton::set_hover_texture
    );
    ClassDB::bind_method(
        D_METHOD("set_disabled_texture", "texture"),
        &TextureButton::set_disabled_texture
    );
    ClassDB::bind_method(
        D_METHOD("set_focused_texture", "texture"),
        &TextureButton::set_focused_texture
    );
    ClassDB::bind_method(
        D_METHOD("set_click_mask", "mask"),
        &TextureButton::set_click_mask
    );
    ClassDB::bind_method(
        D_METHOD("set_expand", "p_expand"),
        &TextureButton::set_expand
    );
    ClassDB::bind_method(
        D_METHOD("set_stretch_mode", "p_mode"),
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            "stretch_mode",
            PROPERTY_HINT_ENUM,
            "Scale,Tile,Keep,Keep Centered,Keep Aspect,Keep Aspect "
            "Centered,Keep Aspect Covered"
        ),
        "set_stretch_mode",
        "get_stretch_mode"
    );
