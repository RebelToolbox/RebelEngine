...
AnimationTreePlayer::AnimationProcessMode AnimationTreePlayer::
        ...
        switch (nt) {
            ...
            case NODE_ONESHOT: {
                ...
                oneshot_node_set_autorestart_delay(
                    id,
                    node.get_valid("autorestart_delay")
                );
                oneshot_node_set_autorestart_random_delay(
                    id,
                    node.get_valid("autorestart_random_delay")
                );
                Array filters = node.get_valid("filter");
                for (int j = 0; j < filters.size(); j++) {
...
bool AnimationTreePlayer::_get(const StringName& p_name, Variant& r_ret) const {
    ...
    for (Map<StringName, NodeBase*>::Element* E = node_map.front(); E;
         E                                      = E->next()) {
        ...
        switch (n->type) {
            ...
            case NODE_ANIMATION: {
                ...
                k.resize(keys.size());
                int i = 0;
                for (List<NodePath>::Element* F = keys.front(); F;
                     F                          = F->next()) {
                    k[i++] = F->get();
                }
                node["filter"] = k;
            ...
            case NODE_ONESHOT: {
                ...
                node["autorestart_random_delay"] =
                    osn->autorestart_random_delay;
...
void AnimationTreePlayer::_get_property_list(List<PropertyInfo>* p_list) const {
    p_list->push_back(PropertyInfo(
        ...
        PROPERTY_HINT_NONE,
        "",
        PROPERTY_USAGE_STORAGE | PROPERTY_USAGE_NETWORK
    ));
}
...
void AnimationTreePlayer::_notification(int p_what) {
    switch (p_what) {
        case NOTIFICATION_ENTER_TREE: {
            WARN_DEPRECATED_MSG(
                "AnimationTreePlayer has been deprecated. Use AnimationTree "
                "instead."
            );
...
void AnimationTreePlayer::_compute_weights(
    float* p_fallback_weight,
    HashMap<NodePath, float>* p_weights,
    float p_coeff,
    const HashMap<NodePath, bool>* p_filter,
    float p_filtered_coeff
) {
    if (p_filter != nullptr) {
        ...
        for (List<NodePath>::Element* E = key_list.front(); E; E = E->next()) {
            if ((*p_filter)[E->get()]) {
                if (p_weights->has(E->get())) {
                    ...
                    p_weights->set(
                        E->get(),
                        *p_fallback_weight * p_filtered_coeff
                    );
                }
...
float AnimationTreePlayer::_process_node(
    ...
    switch (nb->type) {
        ...
        case NODE_ANIMATION: {
            ...
            if (!an->animation.is_null()) {
                ...
                for (List<AnimationNode::TrackRef>::Element* E =
                         an->tref.front();
                     E;
                     E = E->next()) {
                    NodePath track_path =
                        an->animation->track_get_path(E->get().local_track);
        ...
        case NODE_TRANSITION: {
            ...
            if (tn->prev
                < 0) { // process current animation, check for transition

                float rem = _process_node(
                    tn->inputs[tn->current].node,
                    r_prev_anim,
                    p_time,
                    p_seek,
                    p_fallback_weight,
                ...
                if (tn->input_data[tn->current].auto_advance
                    && rem <= tn->xfade) {
                ...
                if (!p_seek && tn->switched) { // just switched, seek to start
                                               // of current
                ...
                if (p_seek) { // don't seek prev animation
                    _process_node(
                        tn->inputs[tn->prev].node,
                        r_prev_anim,
                        0,
                        false,
                        prev_fallback_weight,
                        &prev_weights
                    );
                    tn->time = p_time;
                } else {
...
void AnimationTreePlayer::_process_animation(float p_delta) {
    ...
    while (anim_list) {
        if (!anim_list->animation.is_null() && !anim_list->skip) {
            ...
            for (List<AnimationNode::TrackRef>::Element* E =
                     anim_list->tref.front();
                 E;
                 E = E->next()) {
                AnimationNode::TrackRef& tr = E->get();
                if (tr.track == nullptr || tr.local_track < 0
                    || tr.weight < CMP_EPSILON
                    || !a->track_is_enabled(tr.local_track)) {
                    continue;
                }

                switch (a->track_get_type(tr.local_track)) {
                    case Animation::TYPE_TRANSFORM: { ///< Transform a node or a
                                                      ///< bone.

                        Vector3 loc;
                        Quat rot;
                        Vector3 scale;
                        a->transform_track_interpolate(
                            tr.local_track,
                            anim_list->time,
                            &loc,
                            &rot,
                        ...
                        tr.track->rot =
                            tr.track->rot * empty_rot.slerp(rot, tr.weight);

                    } break;
                    case Animation::TYPE_VALUE: { ///< Set a value in a
                                                  ///< property, can be
                                                  ///< interpolated.

                        if (a->value_track_get_update_mode(tr.local_track)
                            == Animation::UPDATE_CONTINUOUS) {
                            Variant value = a->value_track_interpolate(
                                tr.local_track,
                                anim_list->time
                            );
                            Variant::blend(
                            ...
                            int index = a->track_find_key(
                                tr.local_track,
                                anim_list->time
                            );
                            tr.track->value =
                                a->track_get_key_value(tr.local_track, index);
                        }
                    } break;
                    case Animation::TYPE_METHOD: { ///< Call any method on a
                                                   ///< specific node.

                        List<int> indices;
                        a->method_track_get_key_indices(
                            tr.local_track,
                            anim_list->time,
                            anim_list->step,
                            &indices
                        ...
                        for (List<int>::Element* F = indices.front(); F;
                             F                     = F->next()) {
                            StringName method = a->method_track_get_name(
                                tr.local_track,
                                F->get()
                            );
                            Vector<Variant> args = a->method_track_get_params(
                                tr.local_track,
                                F->get()
                            );
                            args.resize(VARIANT_ARG_MAX);
                            tr.track->object->call(
                                method,
                                args[0],
                                args[1],
                                args[2],
...
StringName AnimationTreePlayer::node_get_input_source(
    const StringName& p_node,
    int p_input
) const {
...
#define GET_NODE(m_type, m_cast)                                               \
    ERR_FAIL_COND(!node_map.has(p_node));                                      \
    ERR_FAIL_COND_MSG(                                                         \
        node_map[p_node]->type != m_type,                                      \
        "Invalid parameter for node type."                                     \
    );                                                                         \
    m_cast* n = static_cast<m_cast*>(node_map[p_node]);

void AnimationTreePlayer::animation_node_set_animation(
    const StringName& p_node,
...
void AnimationTreePlayer::animation_node_set_master_animation(
    const StringName& p_node,
...
void AnimationTreePlayer::animation_node_set_get_filtered_paths(
    const StringName& p_node,
...
void AnimationTreePlayer::oneshot_node_set_fadein_time(
    const StringName& p_node,
    float p_time
) {
...
void AnimationTreePlayer::oneshot_node_set_fadeout_time(
    const StringName& p_node,
    float p_time
) {
    GET_NODE(NODE_ONESHOT, OneShotNode);
...
void AnimationTreePlayer::oneshot_node_set_mix_mode(
    const StringName& p_node,
    bool p_mix
) {
    GET_NODE(NODE_ONESHOT, OneShotNode);
...
void AnimationTreePlayer::oneshot_node_set_autorestart(
    const StringName& p_node,
    bool p_active
) {
    GET_NODE(NODE_ONESHOT, OneShotNode);
...
void AnimationTreePlayer::oneshot_node_set_autorestart_delay(
    const StringName& p_node,
    float p_time
) {
    GET_NODE(NODE_ONESHOT, OneShotNode);
...
void AnimationTreePlayer::oneshot_node_set_autorestart_random_delay(
    const StringName& p_node,
    float p_time
) {
    GET_NODE(NODE_ONESHOT, OneShotNode);
...
void AnimationTreePlayer::oneshot_node_set_filter_path(
    const StringName& p_node,
...
void AnimationTreePlayer::oneshot_node_set_get_filtered_paths(
    const StringName& p_node,
    List<NodePath>* r_paths
) const {
    GET_NODE(NODE_ONESHOT, OneShotNode);
...
void AnimationTreePlayer::mix_node_set_amount(
    const StringName& p_node,
    float p_amount
) {
...
void AnimationTreePlayer::blend2_node_set_amount(
    const StringName& p_node,
    float p_amount
) {
...
void AnimationTreePlayer::blend2_node_set_filter_path(
    const StringName& p_node,
    const NodePath& p_filter,
    bool p_enable
) {
...
void AnimationTreePlayer::blend2_node_set_get_filtered_paths(
    const StringName& p_node,
    List<NodePath>* r_paths
) const {
    GET_NODE(NODE_BLEND2, Blend2Node);
...
void AnimationTreePlayer::blend3_node_set_amount(
    const StringName& p_node,
    float p_amount
) {
...
void AnimationTreePlayer::blend4_node_set_amount(
    const StringName& p_node,
...
void AnimationTreePlayer::timescale_node_set_scale(
    const StringName& p_node,
    float p_scale
) {
...
void AnimationTreePlayer::transition_node_set_input_count(
    const StringName& p_node,
    int p_inputs
) {
...
void AnimationTreePlayer::transition_node_set_input_auto_advance(
    const StringName& p_node,
    int p_input,
    bool p_auto_advance
) {
...
void AnimationTreePlayer::transition_node_set_current(
    const StringName& p_node,
    int p_current
) {
    GET_NODE(NODE_TRANSITION, TransitionNode);
...
void AnimationTreePlayer::node_set_position(
    const StringName& p_node,
    const Vector2& p_pos
) {
    ERR_FAIL_COND(!node_map.has(p_node));
...
AnimationTreePlayer::NodeType AnimationTreePlayer::node_get_type(
    const StringName& p_node
) const {
...
Ref<Animation> AnimationTreePlayer::animation_node_get_animation(
    const StringName& p_node
) const {
...
String AnimationTreePlayer::animation_node_get_master_animation(
    const StringName& p_node
) const {
...
float AnimationTreePlayer::animation_node_get_position(const StringName& p_node
) const {
    GET_NODE_V(NODE_ANIMATION, AnimationNode, 0);
    return n->time;
}

bool AnimationTreePlayer::animation_node_is_path_filtered(
    const StringName& p_node,
...
float AnimationTreePlayer::oneshot_node_get_fadein_time(const StringName& p_node
) const {
    GET_NODE_V(NODE_ONESHOT, OneShotNode, 0);
    return n->fade_in;
}

float AnimationTreePlayer::oneshot_node_get_fadeout_time(
    const StringName& p_node
) const {
    GET_NODE_V(NODE_ONESHOT, OneShotNode, 0);
    return n->fade_out;
...
bool AnimationTreePlayer::oneshot_node_get_mix_mode(const StringName& p_node
) const {
    GET_NODE_V(NODE_ONESHOT, OneShotNode, 0);
    return n->mix;
}

bool AnimationTreePlayer::oneshot_node_has_autorestart(const StringName& p_node
) const {
    GET_NODE_V(NODE_ONESHOT, OneShotNode, 0);
    return n->autorestart;
}

float AnimationTreePlayer::oneshot_node_get_autorestart_delay(
    const StringName& p_node
) const {
    GET_NODE_V(NODE_ONESHOT, OneShotNode, 0);
...
float AnimationTreePlayer::oneshot_node_get_autorestart_random_delay(
    const StringName& p_node
) const {
    GET_NODE_V(NODE_ONESHOT, OneShotNode, 0);
...
bool AnimationTreePlayer::oneshot_node_is_active(const StringName& p_node
) const {
    GET_NODE_V(NODE_ONESHOT, OneShotNode, 0);
    return n->active;
}

bool AnimationTreePlayer::oneshot_node_is_path_filtered(
    const StringName& p_node,
    const NodePath& p_path
) const {
    GET_NODE_V(NODE_ONESHOT, OneShotNode, 0);
...
float AnimationTreePlayer::blend2_node_get_amount(const StringName& p_node
) const {
    GET_NODE_V(NODE_BLEND2, Blend2Node, 0);
    return n->value;
}

bool AnimationTreePlayer::blend2_node_is_path_filtered(
    const StringName& p_node,
    const NodePath& p_path
) const {
    GET_NODE_V(NODE_BLEND2, Blend2Node, 0);
...
float AnimationTreePlayer::blend3_node_get_amount(const StringName& p_node
) const {
    GET_NODE_V(NODE_BLEND3, Blend3Node, 0);
    return n->value;
}

Vector2 AnimationTreePlayer::blend4_node_get_amount(const StringName& p_node
) const {
    GET_NODE_V(NODE_BLEND4, Blend4Node, Vector2());
    return n->value;
}

float AnimationTreePlayer::timescale_node_get_scale(const StringName& p_node
) const {
    GET_NODE_V(NODE_TIMESCALE, TimeScaleNode, 0);
    return n->scale;
}

void AnimationTreePlayer::transition_node_delete_input(
    const StringName& p_node,
    int p_input
) {
    GET_NODE(NODE_TRANSITION, TransitionNode);
...
int AnimationTreePlayer::transition_node_get_input_count(
    const StringName& p_node
) const {
...
bool AnimationTreePlayer::transition_node_has_input_auto_advance(
    const StringName& p_node,
    int p_input
) const {
...
float AnimationTreePlayer::transition_node_get_xfade_time(
    const StringName& p_node
) const {
    GET_NODE_V(NODE_TRANSITION, TransitionNode, 0);
    return n->xfade;
...
int AnimationTreePlayer::transition_node_get_current(const StringName& p_node
) const {
...
AnimationTreePlayer::ConnectError AnimationTreePlayer::_cycle_test(
...
Error AnimationTreePlayer::connect_nodes(
...
bool AnimationTreePlayer::are_nodes_connected(
    const StringName& p_src_node,
    const StringName& p_dst_node,
    int p_dst_input
) const {
...
void AnimationTreePlayer::disconnect_nodes(
    const StringName& p_node,
    int p_input
) {
    ERR_FAIL_COND(!node_map.has(p_node));
...
void AnimationTreePlayer::get_connection_list(List<Connection>* p_connections
) const {
    for (Map<StringName, NodeBase*>::Element* E = node_map.front(); E;
         E                                      = E->next()) {
        NodeBase* nb = E->get();
...
AnimationTreePlayer::Track* AnimationTreePlayer::_find_track(
    const NodePath& p_path
) {
    ...
    Node* child =
        parent->get_node_and_resource(p_path, resource, leftover_path);
    if (!child) {
        String err = "Animation track references unknown Node: '"
                   + String(p_path) + "'.";
    ...
    if (p_path.get_subname_count()) {
        if (Object::cast_to<Skeleton>(child)) {
            bone_idx =
                Object::cast_to<Skeleton>(child)->find_bone(p_path.get_subname(0
                ));
        }
    }
...
Error AnimationTreePlayer::node_rename(
    const StringName& p_node,
...
void AnimationTreePlayer::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("transition_node_has_input_auto_advance", "id", "input_idx"),
        &AnimationTreePlayer::transition_node_has_input_auto_advance
    );

    ClassDB::bind_method(
        D_METHOD("transition_node_set_xfade_time", "id", "time_sec"),
        &AnimationTreePlayer::transition_node_set_xfade_time
    );
    ClassDB::bind_method(
        D_METHOD("transition_node_get_xfade_time", "id"),
        &AnimationTreePlayer::transition_node_get_xfade_time
    );

    ClassDB::bind_method(
        D_METHOD("transition_node_set_current", "id", "input_idx"),
        &AnimationTreePlayer::transition_node_set_current
    );
    ClassDB::bind_method(
        D_METHOD("transition_node_get_current", "id"),
        &AnimationTreePlayer::transition_node_get_current
    );

    ClassDB::bind_method(
        D_METHOD("node_set_position", "id", "screen_position"),
        &AnimationTreePlayer::node_set_position
    );
    ClassDB::bind_method(
        D_METHOD("node_get_position", "id"),
        &AnimationTreePlayer::node_get_position
    );

    ClassDB::bind_method(
        D_METHOD("remove_node", "id"),
        &AnimationTreePlayer::remove_node
    );
    ClassDB::bind_method(
        D_METHOD("connect_nodes", "id", "dst_id", "dst_input_idx"),
        &AnimationTreePlayer::connect_nodes
    );
    ClassDB::bind_method(
        D_METHOD("are_nodes_connected", "id", "dst_id", "dst_input_idx"),
        &AnimationTreePlayer::are_nodes_connected
    );
    ClassDB::bind_method(
        D_METHOD("disconnect_nodes", "id", "dst_input_idx"),
        &AnimationTreePlayer::disconnect_nodes
    );

    ClassDB::bind_method(
        D_METHOD("set_active", "enabled"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_master_player", "nodepath"),
        &AnimationTreePlayer::set_master_player
    );
    ClassDB::bind_method(
        D_METHOD("get_master_player"),
        &AnimationTreePlayer::get_master_player
    );

    ClassDB::bind_method(
        D_METHOD("get_node_list"),
        &AnimationTreePlayer::_get_node_list
    );

    ClassDB::bind_method(
        D_METHOD("set_animation_process_mode", "mode"),
        &AnimationTreePlayer::set_animation_process_mode
    );
    ClassDB::bind_method(
        D_METHOD("get_animation_process_mode"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::NODE_PATH, "base_path"),
        "set_base_path",
        "get_base_path"
    );
    ADD_PROPERTY(
