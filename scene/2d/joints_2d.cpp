...
void Joint2D::_update_joint(bool p_only_free) {
    if (joint.is_valid()) {
        if (ba.is_valid() && bb.is_valid() && exclude_from_collision) {
            Physics2DServer::get_singleton()
                ->joint_disable_collisions_between_bodies(joint, false);
        }
    ...
    Physics2DServer::get_singleton()->get_singleton()->joint_set_param(
        joint,
        Physics2DServer::JOINT_PARAM_BIAS,
        bias
    );
    ...
    Physics2DServer::get_singleton()->joint_disable_collisions_between_bodies(
        joint,
...
void Joint2D::_bind_methods() {
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "bias", PROPERTY_HINT_RANGE, "0,0.9,0.001"),
        "set_bias",
        "get_bias"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "disable_collision"),
...
RID PinJoint2D::_configure_joint(PhysicsBody2D* body_a, PhysicsBody2D* body_b) {
    RID pj = Physics2DServer::get_singleton()->pin_joint_create(
        get_global_transform().get_origin(),
        body_a->get_rid(),
        body_b ? body_b->get_rid() : RID()
    );
    Physics2DServer::get_singleton()->pin_joint_set_param(
        pj,
        Physics2DServer::PIN_JOINT_SOFTNESS,
        softness
    );
    return pj;
}
...
void PinJoint2D::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_softness", "softness"),
...
void GrooveJoint2D::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set_initial_offset", "offset"),
        &GrooveJoint2D::set_initial_offset
    );
    ClassDB::bind_method(
        D_METHOD("get_initial_offset"),
...
RID DampedSpringJoint2D::_configure_joint(
    ...
    RID dsj = Physics2DServer::get_singleton()->damped_spring_joint_create(
        anchor_A,
        anchor_B,
        body_a->get_rid(),
        body_b->get_rid()
    ...
    if (rest_length) {
        Physics2DServer::get_singleton()->damped_string_joint_set_param(
            dsj,
            Physics2DServer::DAMPED_STRING_REST_LENGTH,
            rest_length
        );
    }
    Physics2DServer::get_singleton()->damped_string_joint_set_param(
        dsj,
        Physics2DServer::DAMPED_STRING_STIFFNESS,
        stiffness
    );
    Physics2DServer::get_singleton()->damped_string_joint_set_param(
        dsj,
        Physics2DServer::DAMPED_STRING_DAMPING,
        damping
    );
...
void DampedSpringJoint2D::set_rest_length(real_t p_rest_length) {
    ...
    if (get_joint().is_valid()) {
        Physics2DServer::get_singleton()->damped_string_joint_set_param(
            get_joint(),
            Physics2DServer::DAMPED_STRING_REST_LENGTH,
            p_rest_length ? p_rest_length : length
        );
    }
}
