...
void TileMap::_notification(int p_what) {
    switch (p_what) {
        ...
        case NOTIFICATION_EXIT_TREE: {
            ...
            for (Map<PosKey, Quadrant>::Element* E = quadrant_map.front(); E;
                 E                                 = E->next()) {
                Quadrant& q = E->get();
                if (navigation) {
                    for (Map<PosKey, Quadrant::NavPoly>::Element* F =
                             q.navpoly_ids.front();
                         F;
                         F = F->next()) {
                ...
                for (Map<PosKey, Quadrant::Occluder>::Element* F =
                         q.occluder_instances.front();
                     F;
                     F = F->next()) {
        ...
        case NOTIFICATION_VISIBILITY_CHANGED: {
            for (Map<PosKey, Quadrant>::Element* E = quadrant_map.front(); E;
                 E                                 = E->next()) {
                for (Map<PosKey, Quadrant::Occluder>::Element* F =
                         E->get().occluder_instances.front();
                     F;
                     F = F->next()) {
    ...
    for (Map<PosKey, Quadrant>::Element* E = quadrant_map.front(); E;
        ...
        if (navigation) {
            for (Map<PosKey, Quadrant::NavPoly>::Element* F =
                 ...
                 F;
                 F = F->next()) {
                navigation->navpoly_set_transform(
                    F->get().id,
                    nav_rel * F->get().xform
                );
            }
        ...
        for (Map<PosKey, Quadrant::Occluder>::Element* F =
             ...
             F;
             F = F->next()) {
            VS::get_singleton()->canvas_light_occluder_set_transform(
                F->get().id,
                global_transform * F->get().xform
            );
        }
    }
}
...
void TileMap::_fix_cell_transform(
    Transform2D& xform,
    const Cell& p_cell,
    ...
    if (compatibility_mode && !centered_textures) {
        ...
        if (s.y > s.x) {
            if ((p_cell.flip_h && (p_cell.flip_v || p_cell.transpose))
                || (p_cell.flip_v && !p_cell.transpose)) {
                offset.y += s.y - s.x;
            }
        } else if (s.y < s.x) {
            if ((p_cell.flip_v && (p_cell.flip_h || p_cell.transpose))
                || (p_cell.flip_h && !p_cell.transpose)) {
                offset.x += s.x - s.y;
            }
        }
    }
    ...
    if (p_cell.flip_h) {
        ...
        if (compatibility_mode && !centered_textures) {
            if (tile_origin == TILE_ORIGIN_TOP_LEFT
                || tile_origin == TILE_ORIGIN_BOTTOM_LEFT) {
...
void TileMap::_add_shape(
    ...
    if (!use_parent) {
        ...
        ps->body_set_shape_as_one_way_collision(
            p_q.body,
            shape_idx,
            p_shape_data.one_way_collision,
            p_shape_data.one_way_collision_margin
        );
        ...
        int real_index = collision_parent->shape_owner_get_shape_index(
            p_q.shape_owner_id,
            shape_idx
        );
...
void TileMap::update_dirty_quadrants() {
    ...
    while (dirty_quadrant_list.first()) {
        ...
        if (navigation) {
            for (Map<PosKey, Quadrant::NavPoly>::Element* E =
                     q.navpoly_ids.front();
                 E;
                 E = E->next()) {
        ...
        for (Map<PosKey, Quadrant::Occluder>::Element* E =
                 q.occluder_instances.front();
             E;
             E = E->next()) {
        ...
        for (int i = 0; i < q.cells.size(); i++) {
            ...
            if (tile_set->tile_get_tile_mode(c.id) == TileSet::AUTO_TILE
                ...
                z_index += tile_set->autotile_get_z_index(
                    c.id,
                    Vector2(c.autotile_coord_x, c.autotile_coord_y)
                );
            }
            ...
            if (prev_canvas_item == RID() || prev_material != mat
                ...
                if (debug_shapes) {
                    ...
                    vs->canvas_item_set_z_as_relative_to_parent(
                        debug_canvas_item,
                        false
                    );
                    vs->canvas_item_set_z_index(
                        debug_canvas_item,
                        VS::CANVAS_ITEM_Z_MAX - 1
                    );
            ...
            if (tile_set->tile_get_tile_mode(c.id) == TileSet::AUTO_TILE
                ...
                r.position  += (r.size + Vector2(spacing, spacing))
                            * Vector2(c.autotile_coord_x, c.autotile_coord_y);
            }
            ...
            if (compatibility_mode && !centered_textures) {
                if (rect.size.y > rect.size.x) {
                    if ((c.flip_h && (c.flip_v || c.transpose))
                        || (c.flip_v && !c.transpose)) {
                        tile_ofs.y += rect.size.y - rect.size.x;
                    }
                } else if (rect.size.y < rect.size.x) {
                    if ((c.flip_v && (c.flip_h || c.transpose))
                        || (c.flip_h && !c.transpose)) {
            ...
            modulate                = Color(
                modulate.r * self_modulate.r,
                modulate.g * self_modulate.g,
                modulate.b * self_modulate.b,
            ...
            for (int j = 0; j < shapes.size(); j++) {
                ...
                if (shape.is_valid()) {
                    if (tile_set->tile_get_tile_mode(c.id)
                            == TileSet::SINGLE_TILE
                        || (shapes[j].autotile_coord.x == c.autotile_coord_x
                            && shapes[j].autotile_coord.y == c.autotile_coord_y
                        )) {
                        Transform2D xform;
                        xform.set_origin(offset.floor());

                        Vector2 shape_ofs =
                            shapes[j].shape_transform.get_origin();
                        ...
                        if (debug_canvas_item.is_valid()) {
                            vs->canvas_item_add_set_transform(
                                debug_canvas_item,
                                xform
                            );
                            shape->draw(
                            ...
                            for (int k = 0; k < _shapes.size(); k++) {
                                ...
                                if (convex.is_valid()) {
                                    _add_shape(
                                        ...
                                        shapes[j],
                                        xform,
                                        Vector2(E->key().x, E->key().y)
                                    );
#ifdef DEBUG_ENABLED
                                } else {
                                    print_error(
                                        "The TileSet assigned to the TileMap "
                                        + get_name()
                                        + " has an invalid convex shape."
                                    );
#endif
                                }
                            }
            ...
            if (navigation) {
                ...
                Vector2 npoly_ofs;
                if (tile_set->tile_get_tile_mode(c.id) == TileSet::AUTO_TILE
                    || tile_set->tile_get_tile_mode(c.id)
                           == TileSet::ATLAS_TILE) {
                    navpoly = tile_set->autotile_get_navigation_polygon(
                        c.id,
                        Vector2(c.autotile_coord_x, c.autotile_coord_y)
                    );
                    npoly_ofs = Vector2();
                    ...
                    npoly_ofs =
                        tile_set->tile_get_navigation_polygon_offset(c.id);
                }
                ...
                if (navpoly.is_valid()) {
                    ...
                    if (debug_navigation) {
                        ...
                        vs->canvas_item_set_parent(
                            debug_navigation_item,
                            canvas_item
                        );
                        vs->canvas_item_set_z_as_relative_to_parent(
                            debug_navigation_item,
                        ...
                        vs->canvas_item_set_z_index(
                            debug_navigation_item,
                            VS::CANVAS_ITEM_Z_MAX - 2
                        ); // Display one below collision debug

                        if (debug_navigation_item.is_valid()) {
                            PoolVector<Vector2> navigation_polygon_vertices =
                                navpoly->get_vertices();
                            ...
                            if (vsize > 2) {
                                ...
                                colors.resize(vsize);
                                {
                                    PoolVector<Vector2>::Read vr =
                                        navigation_polygon_vertices.read();
                                    for (int j = 0; j < vsize; j++) {
                                        vertices.write[j] = vr[j];
                                        colors.write[j] =
                                            debug_navigation_color;
                                    }
                                }
                                ...
                                for (int j = 0;
                                     j < navpoly->get_polygon_count();
                                     j++) {
                                    Vector<int> polygon =
                                        navpoly->get_polygon(j);
            ...
            if (tile_set->tile_get_tile_mode(c.id) == TileSet::AUTO_TILE
                || tile_set->tile_get_tile_mode(c.id) == TileSet::ATLAS_TILE) {
                occluder = tile_set->autotile_get_light_occluder(
                    c.id,
                    Vector2(c.autotile_coord_x, c.autotile_coord_y)
                );
            } else {
            ...
            if (occluder.is_valid()) {
                ...
                VS::get_singleton()->canvas_light_occluder_set_polygon(
                    orid,
                    occluder->get_rid()
                );
                VS::get_singleton()->canvas_light_occluder_attach_to_canvas(
                    orid,
                    get_canvas()
                );
                VS::get_singleton()->canvas_light_occluder_set_light_mask(
                    orid,
                    occluder_light_mask
                );
                VS::get_singleton()->canvas_light_occluder_set_enabled(
    ...
    if (quadrant_order_dirty) {
        ...
        for (Map<PosKey, Quadrant>::Element* E = quadrant_map.front(); E;
             E                                 = E->next()) {
            Quadrant& q = E->get();
            for (List<RID>::Element* F = q.canvas_items.front(); F;
                 F                     = F->next()) {
                VS::get_singleton()->canvas_item_set_draw_index(
                    F->get(),
                    index++
                );
            }
...
void TileMap::_recompute_rect_cache() {
    ...
    for (Map<PosKey, Quadrant>::Element* E = quadrant_map.front(); E;
         E                                 = E->next()) {
        Rect2 r;
        r.position = _map_to_world(
            E->key().x * _get_quadrant_size(),
            E->key().y * _get_quadrant_size()
        );
        r.expand_to(_map_to_world(
            E->key().x * _get_quadrant_size() + _get_quadrant_size(),
            E->key().y * _get_quadrant_size()
        ));
        r.expand_to(_map_to_world(
            E->key().x * _get_quadrant_size() + _get_quadrant_size(),
            E->key().y * _get_quadrant_size() + _get_quadrant_size()
        ));
        r.expand_to(_map_to_world(
            E->key().x * _get_quadrant_size(),
            E->key().y * _get_quadrant_size() + _get_quadrant_size()
...
Map<TileMap::PosKey, TileMap::Quadrant>::Element* TileMap::_create_quadrant(
    const PosKey& p_qk
) {
    Transform2D xform;
    ...
    if (!use_parent) {
        ...
        Physics2DServer::get_singleton()->body_set_mode(
            q.body,
            use_kinematic ? Physics2DServer::BODY_MODE_KINEMATIC
                          : Physics2DServer::BODY_MODE_STATIC
        );

        Physics2DServer::get_singleton()->body_attach_object_instance_id(
            q.body,
            get_instance_id()
        );
        Physics2DServer::get_singleton()->body_set_collision_layer(
            q.body,
            collision_layer
        );
        Physics2DServer::get_singleton()->body_set_collision_mask(
            q.body,
            collision_mask
        );
...
void TileMap::_erase_quadrant(Map<PosKey, Quadrant>::Element* Q) {
    ...
    if (navigation) {
        for (Map<PosKey, Quadrant::NavPoly>::Element* E = q.navpoly_ids.front();
             E;
             E = E->next()) {
            navigation->navpoly_remove(E->get().id);
        }
...
void TileMap::_make_quadrant_dirty(
...
void TileMap::set_cellv(
    const Vector2& p_pos,
    int p_tile,
    bool p_flip_x,
    bool p_flip_y,
    bool p_transpose,
    Vector2 p_autotile_coord
) {
    set_cell(
        p_pos.x,
        p_pos.y,
...
void TileMap::_set_celld(const Vector2& p_pos, const Dictionary& p_data) {
    Variant v_pos_x = p_pos.x, v_pos_y = p_pos.y, v_tile = p_data["id"],
            v_flip_h = p_data["flip_h"], v_flip_v = p_data["flip_y"],
            v_transpose      = p_data["transpose"],
            v_autotile_coord = p_data["auto_coord"];
    const Variant* args[7]   = {
        &v_pos_x,
        &v_pos_y,
        &v_tile,
        &v_flip_h,
...
void TileMap::set_cell(
    ...
    if (!E) {
        ...
        if (E->get().id == p_tile && E->get().flip_h == p_flip_x
            && E->get().flip_v == p_flip_y && E->get().transpose == p_transpose
            && E->get().autotile_coord_x == (uint16_t)p_autotile_coord.x
            && E->get().autotile_coord_y == (uint16_t)p_autotile_coord.y) {
...
void TileMap::update_bitmask_region(
    const Vector2& p_start,
    const Vector2& p_end
) {
    if ((p_end.x < p_start.x || p_end.y < p_start.y)
        || (p_end.x == p_start.x && p_end.y == p_start.y)) {
...
void TileMap::update_cell_bitmask(int p_x, int p_y) {
    ERR_FAIL_COND_MSG(
        tile_set.is_null(),
        "Cannot update cell bitmask if Tileset is not open."
    );
    PosKey p(p_x, p_y);
    ...
    if (E != nullptr) {
        ...
        if (tile_set->tile_get_tile_mode(id) == TileSet::AUTO_TILE) {
            uint16_t mask = 0;
            if (tile_set->autotile_get_bitmask_mode(id)
                == TileSet::BITMASK_2X2) {
                if (tile_set->is_tile_bound(id, get_cell(p_x - 1, p_y - 1))
                    && tile_set->is_tile_bound(id, get_cell(p_x, p_y - 1))
                    && tile_set->is_tile_bound(id, get_cell(p_x - 1, p_y))) {
                ...
                if (tile_set->autotile_get_bitmask_mode(id)
                    ...
                    if (tile_set->is_tile_bound(id, get_cell(p_x - 1, p_y - 1))
                        && tile_set->is_tile_bound(id, get_cell(p_x, p_y - 1))
                        && tile_set
                               ->is_tile_bound(id, get_cell(p_x - 1, p_y))) {
                        mask |= TileSet::BIND_TOPLEFT;
                    }
                    if (tile_set->is_tile_bound(id, get_cell(p_x + 1, p_y - 1))
                        && tile_set->is_tile_bound(id, get_cell(p_x, p_y - 1))
                    ...
                    if (tile_set
                            ->is_tile_bound(id, get_cell(p_x - 1, p_y - 1))) {
                        mask |= TileSet::BIND_TOPLEFT;
                    }
                    if (tile_set
                            ->is_tile_bound(id, get_cell(p_x + 1, p_y - 1))) {
            ...
            Vector2 coord = tile_set->autotile_get_subtile_for_bitmask(
                id,
                mask,
                this,
                Vector2(p_x, p_y)
            ...
            if (tile_set->autotile_get_bitmask(id, Vector2(p_x, p_y))
                == TileSet::BIND_CENTER) {
                Vector2 coord = tile_set->atlastile_get_subtile_by_priority(
                    id,
                    this,
                    Vector2(p_x, p_y)
                );
...
void TileMap::fix_invalid_tiles() {
    ERR_FAIL_COND_MSG(
        tile_set.is_null(),
        "Cannot fix invalid tiles if Tileset is not open."
    );

    Map<PosKey, Cell> temp_tile_map = tile_map;
    for (Map<PosKey, Cell>::Element* E = temp_tile_map.front(); E;
         E                             = E->next()) {
...
void TileMap::set_cell_autotile_coord(
    int p_x,
    int p_y,
...
void TileMap::_recreate_quadrants() {
    ...
    for (Map<PosKey, Cell>::Element* E = tile_map.front(); E; E = E->next()) {
        PosKey qk =
            PosKey(E->key().x, E->key().y).to_quadrant(_get_quadrant_size());
...
void TileMap::_update_item_material_state(const RID& p_canvas_item) {
    VS::get_singleton()->canvas_item_set_use_parent_material(
        p_canvas_item,
        get_use_parent_material() || get_material().is_valid()
    );
}
...
PoolVector<int> TileMap::_get_tile_data() const {
    ...
    int idx = 0;
    for (const Map<PosKey, Cell>::Element* E = tile_map.front(); E;
         E                                   = E->next()) {
...
Transform2D TileMap::get_cell_transform() const {
    switch (mode) {
        ...
        case MODE_ISOMETRIC: {
            // isometric only makes sense when y is positive in both x and y
            // vectors, otherwise the drawing of tiles will overlap
            Transform2D m;
    ...
    if (!p_ignore_ofs) {
        switch (half_offset) {
            ...
            case HALF_OFFSET_NEGATIVE_X: {
                if (ABS(p_y) & 1) {
                    ret += get_cell_transform()[0]
                         * (half_offset == HALF_OFFSET_X ? 0.5 : -0.5);
                }
            } break;
            case HALF_OFFSET_Y:
            case HALF_OFFSET_NEGATIVE_Y: {
                if (ABS(p_x) & 1) {
                    ret += get_cell_transform()[1]
                         * (half_offset == HALF_OFFSET_Y ? 0.5 : -0.5);
                }
            } break;
...
void TileMap::_validate_property(PropertyInfo& property) const {
    if (use_parent && property.name != "collision_use_parent"
        && property.name.begins_with("collision_")) {
    ...
    if (used_size_cache_dirty) {
        if (tile_map.size() > 0) {
            used_size_cache = Rect2(
                tile_map.front()->key().x,
                tile_map.front()->key().y,
                0,
                0
            );

            for (Map<PosKey, Cell>::Element* E = tile_map.front(); E;
                 E                             = E->next()) {
...
void TileMap::set_occluder_light_mask(int p_mask) {
    ...
    for (Map<PosKey, Quadrant>::Element* E = quadrant_map.front(); E;
        ...
        for (Map<PosKey, Quadrant::Occluder>::Element* F =
             ...
             F;
             F = F->next()) {
            VisualServer::get_singleton()->canvas_light_occluder_set_light_mask(
                F->get().id,
                occluder_light_mask
            );
        }
...
void TileMap::set_light_mask(int p_light_mask) {
    ...
    for (Map<PosKey, Quadrant>::Element* E = quadrant_map.front(); E;
         E                                 = E->next()) {
        for (List<RID>::Element* F = E->get().canvas_items.front(); F;
             F                     = F->next()) {
            VisualServer::get_singleton()->canvas_item_set_light_mask(
                F->get(),
                get_light_mask()
            );
        }
...
String TileMap::get_configuration_warning() const {
    ...
    if (use_parent && !collision_parent) {
        ...
        return TTR(
            "TileMap with Use Parent on needs a parent CollisionObject2D to "
            "give shapes to. Please use it as a child of Area2D, StaticBody2D, "
...
void TileMap::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set_half_offset", "half_offset"),
        &TileMap::set_half_offset
    );
    ClassDB::bind_method(
        D_METHOD("get_half_offset"),
        &TileMap::get_half_offset
    );

    ClassDB::bind_method(
        D_METHOD("set_custom_transform", "custom_transform"),
        &TileMap::set_custom_transform
    );
    ClassDB::bind_method(
        D_METHOD("get_custom_transform"),
    ...
    ClassDB::bind_method(
        D_METHOD("_set_old_cell_size", "size"),
        &TileMap::_set_old_cell_size
    );
    ClassDB::bind_method(
        D_METHOD("_get_old_cell_size"),
        &TileMap::_get_old_cell_size
    );

    ClassDB::bind_method(
        D_METHOD("set_quadrant_size", "size"),
        &TileMap::set_quadrant_size
    );
    ClassDB::bind_method(
        D_METHOD("get_quadrant_size"),
        &TileMap::get_quadrant_size
    );

    ClassDB::bind_method(
        D_METHOD("set_tile_origin", "origin"),
        &TileMap::set_tile_origin
    );
    ClassDB::bind_method(
        D_METHOD("get_tile_origin"),
        &TileMap::get_tile_origin
    );

    ClassDB::bind_method(
        D_METHOD("set_clip_uv", "enable"),
        &TileMap::set_clip_uv
    );
    ClassDB::bind_method(D_METHOD("get_clip_uv"), &TileMap::get_clip_uv);

    ClassDB::bind_method(
        D_METHOD("set_y_sort_mode", "enable"),
        &TileMap::set_y_sort_mode
    );
    ClassDB::bind_method(
        D_METHOD("is_y_sort_mode_enabled"),
        &TileMap::is_y_sort_mode_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_compatibility_mode", "enable"),
        &TileMap::set_compatibility_mode
    );
    ClassDB::bind_method(
        D_METHOD("is_compatibility_mode_enabled"),
        &TileMap::is_compatibility_mode_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_show_collision", "enable"),
        &TileMap::set_show_collision
    );
    ClassDB::bind_method(
        D_METHOD("is_show_collision_enabled"),
        &TileMap::is_show_collision_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_centered_textures", "enable"),
        &TileMap::set_centered_textures
    );
    ClassDB::bind_method(
        D_METHOD("is_centered_textures_enabled"),
        &TileMap::is_centered_textures_enabled
    );

    ClassDB::bind_method(
        D_METHOD("set_collision_use_kinematic", "use_kinematic"),
        &TileMap::set_collision_use_kinematic
    );
    ClassDB::bind_method(
        D_METHOD("get_collision_use_kinematic"),
        &TileMap::get_collision_use_kinematic
    );

    ClassDB::bind_method(
        D_METHOD("set_collision_use_parent", "use_parent"),
        &TileMap::set_collision_use_parent
    );
    ClassDB::bind_method(
        D_METHOD("get_collision_use_parent"),
        &TileMap::get_collision_use_parent
    );
    ...
    ClassDB::bind_method(
        D_METHOD("set_collision_friction", "value"),
        &TileMap::set_collision_friction
    );
    ClassDB::bind_method(
        D_METHOD("get_collision_friction"),
        &TileMap::get_collision_friction
    );

    ClassDB::bind_method(
        D_METHOD("set_collision_bounce", "value"),
        &TileMap::set_collision_bounce
    );
    ClassDB::bind_method(
        D_METHOD("get_collision_bounce"),
        &TileMap::get_collision_bounce
    );

    ClassDB::bind_method(
        D_METHOD("set_occluder_light_mask", "mask"),
        &TileMap::set_occluder_light_mask
    );
    ClassDB::bind_method(
        D_METHOD("get_occluder_light_mask"),
    ...
    ClassDB::bind_method(
        D_METHOD("_set_celld", "position", "data"),
        &TileMap::_set_celld
    );
    ClassDB::bind_method(D_METHOD("get_cell", "x", "y"), &TileMap::get_cell);
    ClassDB::bind_method(
        D_METHOD("get_cellv", "position"),
        &TileMap::get_cellv
    );
    ClassDB::bind_method(
        D_METHOD("is_cell_x_flipped", "x", "y"),
        &TileMap::is_cell_x_flipped
    );
    ClassDB::bind_method(
        D_METHOD("is_cell_y_flipped", "x", "y"),
        &TileMap::is_cell_y_flipped
    );
    ClassDB::bind_method(
        D_METHOD("is_cell_transposed", "x", "y"),
        &TileMap::is_cell_transposed
    );

    ClassDB::bind_method(
        D_METHOD("get_cell_autotile_coord", "x", "y"),
        &TileMap::get_cell_autotile_coord
    );

    ClassDB::bind_method(
        D_METHOD("fix_invalid_tiles"),
    ...
    ClassDB::bind_method(
        D_METHOD("get_used_cells_by_id", "id"),
        &TileMap::get_used_cells_by_id
    );
    ClassDB::bind_method(D_METHOD("get_used_rect"), &TileMap::get_used_rect);

    ClassDB::bind_method(
        D_METHOD("map_to_world", "map_position", "ignore_half_ofs"),
        &TileMap::map_to_world,
        DEFVAL(false)
    );
    ClassDB::bind_method(
        D_METHOD("world_to_map", "world_position"),
        &TileMap::world_to_map
    );

    ClassDB::bind_method(
        D_METHOD("_clear_quadrants"),
        &TileMap::_clear_quadrants
    );
    ClassDB::bind_method(
        D_METHOD("_recreate_quadrants"),
        &TileMap::_recreate_quadrants
    );
    ClassDB::bind_method(
        D_METHOD("update_dirty_quadrants"),
        &TileMap::update_dirty_quadrants
    );

    ClassDB::bind_method(
        D_METHOD("update_bitmask_area", "position"),
        &TileMap::update_bitmask_area
    );
    ClassDB::bind_method(
        D_METHOD("update_bitmask_region", "start", "end"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::TRANSFORM2D, "cell_custom_transform"),
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            "cell_half_offset",
            PROPERTY_HINT_ENUM,
            "Offset X,Offset Y,Disabled,Offset Negative X,Offset Negative Y"
        ),
        "set_half_offset",
        "get_half_offset"
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "cell_y_sort"),
        "set_y_sort_mode",
        "is_y_sort_mode_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "show_collision"),
        "set_show_collision",
        "is_show_collision_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "compatibility_mode"),
        "set_compatibility_mode",
        "is_compatibility_mode_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "centered_textures"),
        "set_centered_textures",
        "is_centered_textures_enabled"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "cell_clip_uv"),
        "set_clip_uv",
        "get_clip_uv"
    );
