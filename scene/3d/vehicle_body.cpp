...
public:
    ...
    btVehicleJacobianEntry(
        ...
        m_Adiag =
            massInvA + m_0MinvJt.dot(m_aJ) + massInvB + m_1MinvJt.dot(m_bJ);

        // btAssert(m_Adiag > real_t(0.0));
    }

    real_t getRelativeVelocity(
...
void VehicleWheel::_notification(int p_what) {
    if (p_what == NOTIFICATION_ENTER_TREE) {
        ...
        m_wheelDirectionCS =
            -get_transform().basis.get_axis(Vector3::AXIS_Y).normalized();
        m_wheelAxleCS =
            get_transform().basis.get_axis(Vector3::AXIS_X).normalized();
    }
...
String VehicleWheel::get_configuration_warning() const {
    ...
    if (!Object::cast_to<VehicleBody>(get_parent())) {
        ...
        warning +=
            TTR("VehicleWheel serves to provide a wheel system to a "
                "VehicleBody. Please use it as a child of a VehicleBody.");
    }
...
void VehicleWheel::_update(PhysicsDirectBodyState* s) {
...

    {
        ...
        Vector3 relpos =
            m_raycastInfo.m_contactPointWS - s->get_transform().origin;
...
void VehicleWheel::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set_engine_force", "engine_force"),
        &VehicleWheel::set_engine_force
    );
    ClassDB::bind_method(
        D_METHOD("get_engine_force"),
        &VehicleWheel::get_engine_force
    );

    ClassDB::bind_method(
        D_METHOD("set_brake", "brake"),
        &VehicleWheel::set_brake
    );
    ClassDB::bind_method(D_METHOD("get_brake"), &VehicleWheel::get_brake);

    ClassDB::bind_method(
        D_METHOD("set_steering", "steering"),
        &VehicleWheel::set_steering
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "use_as_traction"),
        "set_use_as_traction",
        "is_used_as_traction"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "use_as_steering"),
        "set_use_as_steering",
        "is_used_as_steering"
    );
    ADD_GROUP("Wheel", "wheel_");
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "wheel_roll_influence"),
        "set_roll_influence",
        "get_roll_influence"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "wheel_radius"),
        "set_radius",
        "get_radius"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "wheel_rest_length"),
        "set_suspension_rest_length",
        "get_suspension_rest_length"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "wheel_friction_slip"),
        "set_friction_slip",
        "get_friction_slip"
    );
    ADD_GROUP("Suspension", "suspension_");
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "suspension_travel"),
        "set_suspension_travel",
        "get_suspension_travel"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "suspension_stiffness"),
        "set_suspension_stiffness",
        "get_suspension_stiffness"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "suspension_max_force"),
        "set_suspension_max_force",
        "get_suspension_max_force"
    );
    ADD_GROUP("Damping", "damping_");
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "damping_compression"),
        "set_damping_compression",
        "get_damping_compression"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::REAL, "damping_relaxation"),
...
void VehicleBody::_update_wheel_transform(
    VehicleWheel& wheel,
    ...
    */
    ...
    wheel.m_raycastInfo.m_hardPointWS =
        chassisTrans.xform(wheel.m_chassisConnectionPointCS);
    // wheel.m_raycastInfo.m_hardPointWS+=s->get_linear_velocity()*s->get_step();
    wheel.m_raycastInfo.m_wheelDirectionWS =
        chassisTrans.get_basis().xform(wheel.m_wheelDirectionCS).normalized();
    wheel.m_raycastInfo.m_wheelAxleWS =
        chassisTrans.get_basis().xform(wheel.m_wheelAxleCS).normalized();
}
...
real_t VehicleBody::_ray_cast(int p_idx, PhysicsDirectBodyState* s) {
    ...
    bool col =
        ss->intersect_ray(source, target, rr, exclude, get_collision_mask());
    ...
    if (col) {
        ...
        if (rr.collider) {
            wheel.m_raycastInfo.m_groundObject =
                Object::cast_to<PhysicsBody>(rr.collider);
        }

        real_t hitDistance = param * raylen;
        wheel.m_raycastInfo.m_suspensionLength =
            hitDistance - wheel.m_wheelRadius;
        // clamp on max suspension travel

        real_t minSuspensionLength =
            wheel.m_suspensionRestLength
            - wheel.m_maxSuspensionTravelCm * real_t(0.01);
        real_t maxSuspensionLength =
            wheel.m_suspensionRestLength
            + wheel.m_maxSuspensionTravelCm * real_t(0.01);
        ...
        real_t denominator = wheel.m_raycastInfo.m_contactNormalWS.dot(
            wheel.m_raycastInfo.m_wheelDirectionWS
        );
        ...
        real_t projVel = wheel.m_raycastInfo.m_contactNormalWS.dot(
            chassis_velocity_at_contactPoint
        );
...
void VehicleBody::_resolve_single_bilateral(
    PhysicsDirectBodyState* s,
    const Vector3& pos1,
    PhysicsBody* body2,
    const Vector3& pos2,
    ...
    if (body2) {
        vel2 = body2->get_linear_velocity()
             + body2->get_angular_velocity().cross(rel_pos2);
    }
    ...
    btVehicleJacobianEntry jac(
        s->get_transform().basis.transposed(),
        b2trans,
        rel_pos1,
        rel_pos2,
        normal,
    ...
    // FIXME: rel_vel assignment here is overwritten by the following
    // assignment. What seems to be intended in the next next assignment is:
    // rel_vel = normal.dot(rel_vel); Investigate why.
    ...
    if (p_rollInfluence > 0.0) {
        // !BAS! But seeing we apply this frame by frame, makes more sense to me
        // to make this time based keeping in mind our anti roll factor if it is
        // set
...
real_t VehicleBody::_calc_rolling_friction(
    btVehicleWheelContactPoint& contactPoint
) {
    real_t j1 = 0.f;
    ...
    Vector3 rel_pos1 =
        contactPosWorld - contactPoint.m_s->get_transform().origin;
    Vector3 rel_pos2;
    if (contactPoint.m_body1) {
        rel_pos2 = contactPosWorld
                 - contactPoint.m_body1->get_global_transform().origin;
    }

    real_t maxImpulse = contactPoint.m_maxImpulse;

    Vector3 vel1 = contactPoint.m_s->get_linear_velocity()
                 + (contactPoint.m_s->get_angular_velocity())
                       .cross(rel_pos1); // * mPos);

    Vector3 vel2;
    if (contactPoint.m_body1) {
        vel2 = contactPoint.m_body1->get_linear_velocity()
             + contactPoint.m_body1->get_angular_velocity().cross(rel_pos2);
    }
...
void VehicleBody::_update_friction(PhysicsDirectBodyState* s) {
...

    {
        for (int i = 0; i < wheels.size(); i++) {
            ...
            if (wheelInfo.m_raycastInfo.m_isInContact) {
                ...
                Basis wheelBasis0 = wheelInfo.m_worldTransform
    ...
    bool sliding = false;
    {
        for (int wheel = 0; wheel < wheels.size(); wheel++) {
            ...
            // class btRigidBody* groundObject = (class btRigidBody*)
            // wheelInfo.m_raycastInfo.m_groundObject;
            ...
            if (wheelInfo.m_raycastInfo.m_isInContact) {
                ...
                real_t maximp = wheelInfo.m_wheelsSuspensionForce
                              * s->get_step() * wheelInfo.m_frictionSlip;
                real_t maximpSide = maximp;

                real_t maximpSquared = maximp * maximpSide;

                m_forwardImpulse.write[wheel] =
    ...
    // apply the impulses
    {
        for (int wheel = 0; wheel < wheels.size(); wheel++) {
            ...
            if (m_sideImpulse[wheel] != real_t(0.)) {
                PhysicsBody* groundObject =
                    wheelInfo.m_raycastInfo.m_groundObject;

                Vector3 rel_pos2;
                if (groundObject) {
                    rel_pos2 = wheelInfo.m_raycastInfo.m_contactPointWS
                             - groundObject->get_global_transform().origin;
                }
...
void VehicleBody::_direct_state_changed(Object* p_state) {
    ...
    for (int i = 0; i < wheels.size(); i++) {
        _ray_cast(i, state);
        wheels[i]->set_transform(
            state->get_transform().inverse() * wheels[i]->m_worldTransform
        );
    }
    ...
    for (int i = 0; i < wheels.size(); i++) {
        ...
        Vector3 relpos = wheel.m_raycastInfo.m_contactPointWS
                       - state->get_transform().origin;
    ...
    for (int i = 0; i < wheels.size(); i++) {
        VehicleWheel& wheel = *wheels[i];
        Vector3 relpos =
            wheel.m_raycastInfo.m_hardPointWS - state->get_transform().origin;
        Vector3 vel = state->get_linear_velocity()
                    + (state->get_angular_velocity()).cross(relpos); // * mPos);
