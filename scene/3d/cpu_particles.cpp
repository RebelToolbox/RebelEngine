...
void CPUParticles::set_emitting(bool p_emitting) {
    ...
    if (emitting) {
        ...
        // first update before rendering to avoid one frame delay after emitting
        // starts
        if (time == 0) {
            _update_internal();
        }
...
void CPUParticles::set_amount(int p_amount) {
    ERR_FAIL_COND_MSG(
        p_amount < 1,
        "Amount of particles must be greater than 0."
    );
...
void CPUParticles::set_lifetime(float p_lifetime) {
    ERR_FAIL_COND_MSG(
        p_lifetime <= 0,
        "Particles lifetime must be greater than 0."
    );
    lifetime = p_lifetime;
}
...
String CPUParticles::get_configuration_warning() const {
    ...
    if (get_mesh().is_valid()) {
        ...
        for (int j = 0; j < get_mesh()->get_surface_count(); j++) {
            anim_material_found = Object::cast_to<ShaderMaterial>(
                                      get_mesh()->surface_get_material(j).ptr()
                                  )
                               != nullptr;
    ...
    anim_material_found =
        ...
        || Object::cast_to<ShaderMaterial>(get_material_override().ptr())
               != nullptr;
    SpatialMaterial* spat =
        Object::cast_to<SpatialMaterial>(get_material_override().ptr());
    ...
    if (!mesh_found) {
        ...
        warnings +=
            "- " + TTR("Nothing is visible because no mesh has been assigned.");
    }

    if (!anim_material_found
        && (get_param(PARAM_ANIM_SPEED) != 0.0
            || get_param(PARAM_ANIM_OFFSET) != 0.0
            || get_param_curve(PARAM_ANIM_SPEED).is_valid()
            || get_param_curve(PARAM_ANIM_OFFSET).is_valid())) {
        if (warnings != String()) {
            warnings += "\n";
        }
        warnings +=
            "- "
            + TTR(
                "CPUParticles animation requires the usage of a "
                "SpatialMaterial whose Billboard Mode is set to \"Particle "
                "Billboard\"."
            );
    }
...
void CPUParticles::set_param_curve(
    Parameter p_param,
    const Ref<Curve>& p_curve
) {
    ERR_FAIL_INDEX(p_param, PARAM_MAX);
...
void CPUParticles::_validate_property(PropertyInfo& property) const {
    ...
    if ((property.name == "emission_points"
         || property.name == "emission_colors")
        && (emission_shape != EMISSION_SHAPE_POINTS)
        && (emission_shape != EMISSION_SHAPE_DIRECTED_POINTS)) {
        property.usage = 0;
    }

    if (property.name == "emission_normals"
        && emission_shape != EMISSION_SHAPE_DIRECTED_POINTS) {
        property.usage = 0;
    }

    if (property.name.begins_with("emission_ring")
...
void CPUParticles::_particles_process(float p_delta) {
    ...
    for (int i = 0; i < pcount; i++) {
        ...
        // The phase is a ratio between 0 (birth) and 1 (end of life) for each
        // particle. While we use time in tests later on, for randomness we use
        // the phase as done in the original shader code, and we later multiply
        // by lifetime to get the time.
        ...
        if (restart) {
            ...
            if (curve_parameters[PARAM_INITIAL_LINEAR_VELOCITY].is_valid()) {
                tex_linear_velocity =
            curve_parameters[PARAM_INITIAL_LINEAR_VELOCITY]->interpolate(0);
            }*/
            ...
            if (curve_parameters[PARAM_ANGLE].is_valid()) {
                tex_anim_offset =
                    curve_parameters[PARAM_ANGLE]->interpolate(tv);
            }
            ...
            if (flags[FLAG_DISABLE_Z]) {
                ...
                Vector3 rot =
                    Vector3(Math::cos(angle1_rad), Math::sin(angle1_rad), 0.0);
                p.velocity = rot * parameters[PARAM_INITIAL_LINEAR_VELOCITY]
                           * Math::lerp(
                                 1.0f,
                                 float(Math::randf()),
                                 randomness[PARAM_INITIAL_LINEAR_VELOCITY]
                           );
            } else {
                // initiate velocity spread in 3D
                float angle1_rad =
                    (Math::randf() * 2.0 - 1.0) * Math_PI * spread / 180.0;
                float angle2_rad = (Math::randf() * 2.0 - 1.0)
                                 * (1.0 - flatness) * Math_PI * spread / 180.0;

                Vector3 direction_xz =
                    Vector3(Math::sin(angle1_rad), 0, Math::cos(angle1_rad));
                Vector3 direction_yz =
                    Vector3(0, Math::sin(angle2_rad), Math::cos(angle2_rad));
                Vector3 spread_direction = Vector3(
                ...
                p.velocity = spread_direction
                           * parameters[PARAM_INITIAL_LINEAR_VELOCITY]
                           * Math::lerp(
                                 1.0f,
                                 float(Math::randf()),
                                 randomness[PARAM_INITIAL_LINEAR_VELOCITY]
            ...
            float base_angle =
                (parameters[PARAM_ANGLE] + tex_angle)
                * Math::lerp(1.0f, p.angle_rand, randomness[PARAM_ANGLE]);
            ...
            switch (emission_shape) {
                ...
                case EMISSION_SHAPE_SPHERE: {
                    float s = 2.0 * Math::randf() - 1.0,
                          t = 2.0 * Math_PI * Math::randf();
                    float radius =
                        emission_sphere_radius * Math::sqrt(1.0 - s * s);
                    p.transform.origin = Vector3(
                        radius * Math::cos(t),
                        radius * Math::sin(t),
                        emission_sphere_radius * s
                    );
                } break;
            ...
            if (flags[FLAG_DISABLE_Z]) {
                if (curve_parameters[PARAM_ORBIT_VELOCITY].is_valid()) {
                    tex_orbit_velocity =
                        curve_parameters[PARAM_ORBIT_VELOCITY]->interpolate(tv);
                }
            }

            float tex_angular_velocity = 0.0;
            if (curve_parameters[PARAM_ANGULAR_VELOCITY].is_valid()) {
                tex_angular_velocity =
                    curve_parameters[PARAM_ANGULAR_VELOCITY]->interpolate(tv);
            }

            float tex_linear_accel = 0.0;
            if (curve_parameters[PARAM_LINEAR_ACCEL].is_valid()) {
                tex_linear_accel =
                    curve_parameters[PARAM_LINEAR_ACCEL]->interpolate(tv);
            }

            float tex_tangential_accel = 0.0;
            if (curve_parameters[PARAM_TANGENTIAL_ACCEL].is_valid()) {
                tex_tangential_accel =
                    curve_parameters[PARAM_TANGENTIAL_ACCEL]->interpolate(tv);
            }

            float tex_radial_accel = 0.0;
            if (curve_parameters[PARAM_RADIAL_ACCEL].is_valid()) {
                tex_radial_accel =
                    curve_parameters[PARAM_RADIAL_ACCEL]->interpolate(tv);
            }
            ...
            if (curve_parameters[PARAM_ANIM_SPEED].is_valid()) {
                tex_anim_speed =
                    curve_parameters[PARAM_ANIM_SPEED]->interpolate(tv);
            }
            ...
            force +=
                p.velocity.length() > 0.0
                    ? p.velocity.normalized()
                          * (parameters[PARAM_LINEAR_ACCEL] + tex_linear_accel)
                          * Math::lerp(
                              1.0f,
                              rand_from_seed(alt_seed),
                              randomness[PARAM_LINEAR_ACCEL]
                          )
                    : Vector3();
            ...
            force +=
                diff.length() > 0.0
                    ? diff.normalized()
                          * (parameters[PARAM_RADIAL_ACCEL] + tex_radial_accel)
                          * Math::lerp(
                              1.0f,
                              rand_from_seed(alt_seed),
                              randomness[PARAM_RADIAL_ACCEL]
                          )
                    : Vector3();
            ...
            if (flags[FLAG_DISABLE_Z]) {
                ...
                Vector3 crossDiff =
                    diff.normalized().cross(gravity.normalized());
                force += crossDiff.length() > 0.0
                           ? crossDiff.normalized()
                                 * ((parameters[PARAM_TANGENTIAL_ACCEL]
                                     + tex_tangential_accel)
                                    * Math::lerp(
                                        1.0f,
            ...
            base_angle +=
                p.custom[1] * lifetime
                * (parameters[PARAM_ANGULAR_VELOCITY] + tex_angular_velocity)
                * Math::lerp(
                    1.0f,
                    rand_from_seed(alt_seed) * 2.0f - 1.0f,
                    randomness[PARAM_ANGULAR_VELOCITY]
                );
            p.custom[0] = Math::deg2rad(base_angle); // angle
            p.custom[2] = (parameters[PARAM_ANIM_OFFSET] + tex_anim_offset)
                            * Math::lerp(
                        ...
                        + p.custom[1]
                              * (parameters[PARAM_ANIM_SPEED] + tex_anim_speed)
                              * Math::lerp(
                                  1.0f,
                                  rand_from_seed(alt_seed),
                                  randomness[PARAM_ANIM_SPEED]
                              ); // angle
        }
        // apply color
        // apply hue rotation
        ...
        if (curve_parameters[PARAM_HUE_VARIATION].is_valid()) {
            tex_hue_variation =
                curve_parameters[PARAM_HUE_VARIATION]->interpolate(tv);
        }

        float hue_rot_angle =
            (parameters[PARAM_HUE_VARIATION] + tex_hue_variation) * Math_PI
            * 2.0
            * Math::lerp(
                1.0f,
                p.hue_rot_rand * 2.0f - 1.0f,
                randomness[PARAM_HUE_VARIATION]
            );
        ...
        Basis hue_rot_mat;
        {
            ...
            for (int j = 0; j < 3; j++) {
                hue_rot_mat[j] =
                    mat1[j] + mat2[j] * hue_rot_c + mat3[j] * hue_rot_s;
            }
        }
        ...
        Vector3 color_rgb =
            hue_rot_mat.xform_inv(Vector3(p.color.r, p.color.g, p.color.b));
        ...
        if (flags[FLAG_DISABLE_Z]) {
            if (flags[FLAG_ALIGN_Y_TO_VELOCITY]) {
                if (p.velocity.length() > 0.0) {
                    ...
                    p.transform.basis.set_axis(
                        1,
                        p.transform.basis.get_axis(1)
                    );
                }
                p.transform.basis.set_axis(
                    0,
                    p.transform.basis.get_axis(1)
                        .cross(p.transform.basis.get_axis(2))
                        .normalized()
                );
                ...
                p.transform.basis.set_axis(
                    1,
                    Vector3(Math::sin(p.custom[0]), Math::cos(p.custom[0]), 0.0)
                );
                p.transform.basis.set_axis(2, Vector3(0, 0, 1));
            }
            ...
            if (flags[FLAG_ALIGN_Y_TO_VELOCITY]) {
                if (p.velocity.length() > 0.0) {
                    ...
                    p.transform.basis.set_axis(
                        1,
                        p.transform.basis.get_axis(1).normalized()
                    );
                }
                if (p.transform.basis.get_axis(1)
                    == p.transform.basis.get_axis(0)) {
                    p.transform.basis.set_axis(
                        0,
                        p.transform.basis.get_axis(1)
                            .cross(p.transform.basis.get_axis(2))
                    ...
                    p.transform.basis.set_axis(
                        2,
                        p.transform.basis.get_axis(0)
                            .cross(p.transform.basis.get_axis(1))
                            .normalized()
                    );
                } else {
                    p.transform.basis.set_axis(
...

    {
        ...
        if (draw_order != DRAW_ORDER_INDEX) {
            ...
            if (draw_order == DRAW_ORDER_LIFETIME) {
                ...
                if (c) {
                    Vector3 dir = c->get_global_transform().basis.get_axis(2
                    ); // far away to close

                    if (local_coords) {
                        // will look different from Particles in editor as this
                        // is based on the camera in the scenetree and not the
                        // editor camera
...
void CPUParticles::_set_redraw(bool p_redraw) {
    ...
    if (redraw) {
        VS::get_singleton()
            ->connect("frame_pre_draw", this, "_update_render_thread");
        VS::get_singleton()->instance_geometry_set_flag(
            get_instance(),
        ...
        if (VS::get_singleton()->is_connected(
                "frame_pre_draw",
                this,
                "_update_render_thread"
            )) {
            VS::get_singleton()
                ->disconnect("frame_pre_draw", this, "_update_render_thread");
        }
        VS::get_singleton()->instance_geometry_set_flag(
            get_instance(),
            VS::INSTANCE_FLAG_DRAW_NEXT_FRAME_IF_VISIBLE,
...
void CPUParticles::_update_render_thread() {
    ...
    if (can_update.is_set()) {
        VS::get_singleton()->multimesh_set_as_bulk_array(
            multimesh,
            particle_data
        );
...
void CPUParticles::convert_from_particles(Node* p_particles) {
    ...
    ERR_FAIL_COND_MSG(
        !particles,
        "Only Particles nodes can be converted to CPUParticles."
    );
    ...
    set_particle_flag(
        ...
        material->get_flag(ParticlesMaterial::FLAG_ALIGN_Y_TO_VELOCITY)
    );
    set_particle_flag(
        FLAG_ROTATE_Y,
        material->get_flag(ParticlesMaterial::FLAG_ROTATE_Y)
    );
    set_particle_flag(
        FLAG_DISABLE_Z,
        material->get_flag(ParticlesMaterial::FLAG_DISABLE_Z)
    );
...
#define CONVERT_PARAM(m_param)                                                 \
    ...
    {                                                                          \
        Ref<CurveTexture> ctex =                                               \
            material->get_param_texture(ParticlesMaterial::m_param);           \
        if (ctex.is_valid()) set_param_curve(m_param, ctex->get_curve());      \
    }                                                                          \
    set_param_randomness(                                                      \
        m_param,                                                               \
        material->get_param_randomness(ParticlesMaterial::m_param)             \
    );
...
void CPUParticles::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_emitting", "emitting"),
        &CPUParticles::set_emitting
    );
    ClassDB::bind_method(
        D_METHOD("set_amount", "amount"),
        &CPUParticles::set_amount
    );
    ClassDB::bind_method(
        D_METHOD("set_lifetime", "secs"),
        &CPUParticles::set_lifetime
    );
    ClassDB::bind_method(
        D_METHOD("set_one_shot", "enable"),
        &CPUParticles::set_one_shot
    );
    ClassDB::bind_method(
        D_METHOD("set_pre_process_time", "secs"),
        &CPUParticles::set_pre_process_time
    );
    ClassDB::bind_method(
        D_METHOD("set_explosiveness_ratio", "ratio"),
        &CPUParticles::set_explosiveness_ratio
    );
    ClassDB::bind_method(
        D_METHOD("set_randomness_ratio", "ratio"),
        &CPUParticles::set_randomness_ratio
    );
    ClassDB::bind_method(
        D_METHOD("set_lifetime_randomness", "random"),
        &CPUParticles::set_lifetime_randomness
    );
    ClassDB::bind_method(
        D_METHOD("set_use_local_coordinates", "enable"),
        &CPUParticles::set_use_local_coordinates
    );
    ClassDB::bind_method(
        D_METHOD("set_fixed_fps", "fps"),
        &CPUParticles::set_fixed_fps
    );
    ClassDB::bind_method(
        D_METHOD("set_fractional_delta", "enable"),
        &CPUParticles::set_fractional_delta
    );
    ClassDB::bind_method(
        D_METHOD("set_speed_scale", "scale"),
        &CPUParticles::set_speed_scale
    );
    ...
    ClassDB::bind_method(
        D_METHOD("get_pre_process_time"),
        &CPUParticles::get_pre_process_time
    );
    ClassDB::bind_method(
        D_METHOD("get_explosiveness_ratio"),
        &CPUParticles::get_explosiveness_ratio
    );
    ClassDB::bind_method(
        D_METHOD("get_randomness_ratio"),
        &CPUParticles::get_randomness_ratio
    );
    ClassDB::bind_method(
        D_METHOD("get_lifetime_randomness"),
        &CPUParticles::get_lifetime_randomness
    );
    ClassDB::bind_method(
        D_METHOD("get_use_local_coordinates"),
        &CPUParticles::get_use_local_coordinates
    );
    ClassDB::bind_method(
        D_METHOD("get_fixed_fps"),
        &CPUParticles::get_fixed_fps
    );
    ClassDB::bind_method(
        D_METHOD("get_fractional_delta"),
        &CPUParticles::get_fractional_delta
    );
    ClassDB::bind_method(
        D_METHOD("get_speed_scale"),
        &CPUParticles::get_speed_scale
    );

    ClassDB::bind_method(
        D_METHOD("set_draw_order", "order"),
        &CPUParticles::set_draw_order
    );

    ClassDB::bind_method(
        D_METHOD("get_draw_order"),
        &CPUParticles::get_draw_order
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "emitting"),
        "set_emitting",
        "is_emitting"
    );
    ADD_PROPERTY(
    ...
    ADD_PROPERTY(
        PropertyInfo(
            ...
            "lifetime",
            PROPERTY_HINT_EXP_RANGE,
            "0.01,600.0,0.01,or_greater"
        ),
        "set_lifetime",
        "get_lifetime"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "one_shot"),
        "set_one_shot",
        "get_one_shot"
    );
    ADD_PROPERTY(
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "fract_delta"),
        "set_fractional_delta",
        "get_fractional_delta"
    );
    ADD_GROUP("Drawing", "");
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "local_coords"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_param_curve", "param", "curve"),
        &CPUParticles::set_param_curve
    );
    ClassDB::bind_method(
        D_METHOD("get_param_curve", "param"),
        &CPUParticles::get_param_curve
    );

    ClassDB::bind_method(
        D_METHOD("set_color", "color"),
        &CPUParticles::set_color
    );
    ClassDB::bind_method(D_METHOD("get_color"), &CPUParticles::get_color);

    ClassDB::bind_method(
        D_METHOD("set_color_ramp", "ramp"),
        &CPUParticles::set_color_ramp
    );
    ClassDB::bind_method(
        D_METHOD("get_color_ramp"),
        &CPUParticles::get_color_ramp
    );

    ClassDB::bind_method(
        D_METHOD("set_particle_flag", "flag", "enable"),
        &CPUParticles::set_particle_flag
    );
    ClassDB::bind_method(
        D_METHOD("get_particle_flag", "flag"),
        &CPUParticles::get_particle_flag
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_shape", "shape"),
        &CPUParticles::set_emission_shape
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_shape"),
        &CPUParticles::get_emission_shape
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_sphere_radius", "radius"),
        &CPUParticles::set_emission_sphere_radius
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_sphere_radius"),
        &CPUParticles::get_emission_sphere_radius
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_box_extents", "extents"),
        &CPUParticles::set_emission_box_extents
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_box_extents"),
        &CPUParticles::get_emission_box_extents
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_points", "array"),
        &CPUParticles::set_emission_points
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_points"),
        &CPUParticles::get_emission_points
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_normals", "array"),
        &CPUParticles::set_emission_normals
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_normals"),
        &CPUParticles::get_emission_normals
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_colors", "array"),
        &CPUParticles::set_emission_colors
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_colors"),
        &CPUParticles::get_emission_colors
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_ring_radius", "radius"),
        &CPUParticles::set_emission_ring_radius
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_ring_radius"),
        &CPUParticles::get_emission_ring_radius
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_ring_inner_radius", "offset"),
        &CPUParticles::set_emission_ring_inner_radius
    );
    ClassDB::bind_method(
    ...
    ClassDB::bind_method(
        ...
        &CPUParticles::set_emission_ring_height
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_ring_height"),
        &CPUParticles::get_emission_ring_height
    );

    ClassDB::bind_method(
        D_METHOD("set_emission_ring_axis", "axis"),
        &CPUParticles::set_emission_ring_axis
    );
    ClassDB::bind_method(
        D_METHOD("get_emission_ring_axis"),
        &CPUParticles::get_emission_ring_axis
    );

    ClassDB::bind_method(D_METHOD("get_gravity"), &CPUParticles::get_gravity);
    ClassDB::bind_method(
        D_METHOD("set_gravity", "accel_vec"),
        &CPUParticles::set_gravity
    );

    ClassDB::bind_method(
        D_METHOD("convert_from_particles", "particles"),
        &CPUParticles::convert_from_particles
    );

    ClassDB::bind_method(
        D_METHOD("_update_render_thread"),
        &CPUParticles::_update_render_thread
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::POOL_VECTOR3_ARRAY, "emission_points"),
        "set_emission_points",
        "get_emission_points"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::POOL_VECTOR3_ARRAY, "emission_normals"),
        "set_emission_normals",
        "get_emission_normals"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::POOL_COLOR_ARRAY, "emission_colors"),
        "set_emission_colors",
        "get_emission_colors"
    );
    ADD_PROPERTY(
