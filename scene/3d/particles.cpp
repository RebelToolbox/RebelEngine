...
void Particles::set_amount(int p_amount) {
    ERR_FAIL_COND_MSG(
        p_amount < 1,
        "Amount of particles cannot be smaller than 1."
    );
    amount = p_amount;
...
void Particles::set_pre_process_time(float p_time) {
    ...
    VS::get_singleton()->particles_set_pre_process_time(
        particles,
        pre_process_time
    );
}
...
void Particles::set_explosiveness_ratio(float p_ratio) {
    ...
    VS::get_singleton()->particles_set_explosiveness_ratio(
        particles,
        explosiveness_ratio
    );
}
...
void Particles::set_randomness_ratio(float p_ratio) {
    ...
    VS::get_singleton()->particles_set_randomness_ratio(
        particles,
        randomness_ratio
    );
}
...
void Particles::set_use_local_coordinates(bool p_enable) {
    ...
    VS::get_singleton()->particles_set_use_local_coordinates(
        particles,
        local_coords
    );
}
...
void Particles::set_process_material(const Ref<Material>& p_material) {
    ...
    VS::get_singleton()->particles_set_process_material(
        particles,
        material_rid
    );
...
void Particles::set_draw_order(DrawOrder p_order) {
    draw_order = p_order;
    VS::get_singleton()->particles_set_draw_order(
        particles,
...
void Particles::set_draw_pass_mesh(int p_pass, const Ref<Mesh>& p_mesh) {
    ...
    VS::get_singleton()
        ->particles_set_draw_pass_mesh(particles, p_pass, mesh_rid);
...
String Particles::get_configuration_warning() const {
    ...
    if (OS::get_singleton()->get_current_video_driver()
        ...
        warnings +=
            "- "
            + TTR(
                "GPU-based particles are not supported by the GLES2 video "
                "driver.\nUse the CPUParticles node instead. You can use the "
                "\"Convert to CPUParticles\" toolbar option for this purpose."
            );
        return warnings;
    }
    ...
    warnings +=
        ...
        + TTR(
            ...
            "macOS.\nYou can use the \"Convert to CPUParticles\" toolbar "
    ...
    if (!meshes_found) {
        ...
        warnings +=
            "- "
            + TTR(
                "Nothing is visible because meshes have not been assigned to "
                "draw passes."
            );
    }
    ...
    if (process_material.is_null()) {
        ...
        warnings +=
            "- "
            + TTR(
                "A material to process the particles is not assigned, so no "
                "behavior is imprinted."
            );
    } else {
        const ParticlesMaterial* process =
            Object::cast_to<ParticlesMaterial>(process_material.ptr());
        if (!anim_material_found && process
            && (process->get_param(ParticlesMaterial::PARAM_ANIM_SPEED) != 0.0
                || process->get_param(ParticlesMaterial::PARAM_ANIM_OFFSET)
                       != 0.0
                || process
                       ->get_param_texture(ParticlesMaterial::PARAM_ANIM_SPEED)
                       .is_valid()
                || process
                       ->get_param_texture(ParticlesMaterial::PARAM_ANIM_OFFSET)
                       .is_valid())) {
            if (warnings != String()) {
                warnings += "\n";
            }
            warnings += "- "
                      + TTR("Particles animation requires the usage of a "
                            "SpatialMaterial whose Billboard Mode is set to "
                            "\"Particle Billboard\".");
        }
    }
...
void Particles::_notification(int p_what) {
    if (p_what == NOTIFICATION_PAUSED || p_what == NOTIFICATION_UNPAUSED) {
        if (can_process()) {
            VS::get_singleton()->particles_set_speed_scale(
                particles,
                speed_scale
            );
        } else {
    ...
    if (p_what == NOTIFICATION_VISIBILITY_CHANGED) {
        // make sure particles are updated before rendering occurs if they were
        // active before
        if (is_visible_in_tree()
            && !VS::get_singleton()->particles_is_inactive(particles)) {
...
void Particles::_bind_methods() {
    ...
    ClassDB::bind_method(
        ...
        &Particles::set_one_shot
    );
    ClassDB::bind_method(
        D_METHOD("set_pre_process_time", "secs"),
        &Particles::set_pre_process_time
    );
    ClassDB::bind_method(
        D_METHOD("set_explosiveness_ratio", "ratio"),
        &Particles::set_explosiveness_ratio
    );
    ClassDB::bind_method(
        D_METHOD("set_randomness_ratio", "ratio"),
        &Particles::set_randomness_ratio
    );
    ClassDB::bind_method(
        D_METHOD("set_visibility_aabb", "aabb"),
    ...
    ClassDB::bind_method(
        ...
        &Particles::set_fixed_fps
    );
    ClassDB::bind_method(
        D_METHOD("set_fractional_delta", "enable"),
        &Particles::set_fractional_delta
    );
    ClassDB::bind_method(
        D_METHOD("set_process_material", "material"),
        &Particles::set_process_material
    );
    ClassDB::bind_method(
        D_METHOD("set_speed_scale", "scale"),
        &Particles::set_speed_scale
    );
    ...
    ClassDB::bind_method(
        ...
        &Particles::get_pre_process_time
    );
    ClassDB::bind_method(
        D_METHOD("get_explosiveness_ratio"),
        &Particles::get_explosiveness_ratio
    );
    ClassDB::bind_method(
    ...
    ClassDB::bind_method(
        D_METHOD("get_visibility_aabb"),
        &Particles::get_visibility_aabb
    );
    ClassDB::bind_method(
        D_METHOD("get_use_local_coordinates"),
        &Particles::get_use_local_coordinates
    );
    ClassDB::bind_method(D_METHOD("get_fixed_fps"), &Particles::get_fixed_fps);
    ClassDB::bind_method(
        D_METHOD("get_fractional_delta"),
        &Particles::get_fractional_delta
    );
    ClassDB::bind_method(
        D_METHOD("get_process_material"),
        &Particles::get_process_material
    );
    ClassDB::bind_method(
        D_METHOD("get_speed_scale"),
        &Particles::get_speed_scale
    );

    ClassDB::bind_method(
        D_METHOD("set_draw_order", "order"),
        &Particles::set_draw_order
    );

    ClassDB::bind_method(
        D_METHOD("get_draw_order"),
        &Particles::get_draw_order
    );

    ClassDB::bind_method(
        D_METHOD("set_draw_passes", "passes"),
        &Particles::set_draw_passes
    );
    ClassDB::bind_method(
        D_METHOD("set_draw_pass_mesh", "pass", "mesh"),
        &Particles::set_draw_pass_mesh
    );

    ClassDB::bind_method(
        D_METHOD("get_draw_passes"),
        &Particles::get_draw_passes
    );
    ClassDB::bind_method(
        D_METHOD("get_draw_pass_mesh", "pass"),
    ...
    ADD_GROUP("Drawing", "");
    ADD_PROPERTY(
        PropertyInfo(Variant::AABB, "visibility_aabb"),
        "set_visibility_aabb",
        "get_visibility_aabb"
    );
    ADD_PROPERTY(
...
Particles::Particles() {
    ...
    set_base(particles);
    one_shot = false; // Needed so that set_emitting doesn't access
