...
String Portal::get_configuration_warning() const {
    ...
    if (Room::detect_nodes_using_lambda(this, lambda)) {
        if (Room::detect_nodes_of_type<RoomManager>(this)) {
            ...
            warning +=
                TTR("The RoomManager should not be a child or grandchild of a "
                    "Portal.");
        }
        if (Room::detect_nodes_of_type<Room>(this)) {
            ...
            warning +=
                TTR("A Room should not be a child or grandchild of a Portal.");
        }
        if (Room::detect_nodes_of_type<RoomGroup>(this)) {
            ...
            warning += TTR(
                "A RoomGroup should not be a child or grandchild of a Portal."
            );
        }
    }
...
void Portal::_notification(int p_what) {
    switch (p_what) {
        case NOTIFICATION_ENTER_WORLD: {
            ...
            // defer full creation of the visual server portal to when the
            // editor portal is in the scene tree
            VisualServer::get_singleton()->portal_set_scenario(
                _portal_rid,
        ...
        case NOTIFICATION_EXIT_WORLD: {
            // partially destroy  the visual server portal when the editor
            // portal exits the scene tree
            VisualServer::get_singleton()->portal_set_scenario(
                _portal_rid,
                RID()
            );
        ...
        case NOTIFICATION_TRANSFORM_CHANGED: {
            ...
            // In theory we shouldn't need to update the gizmo when the
            // transform changes .. HOWEVER, the portal margin is displayed in
            // world space units, back transformed to model space. If the Z
            // scale is changed by the user, the portal margin length can become
            // incorrect and needs 'resyncing' to the global scale of the portal
            // node. We really only need to do this when Z scale is changed, but
            // it is easier codewise to always change it, unless we have
            // evidence this is a performance problem.
            update_gizmo();
        } break;
    }
}
...
void Portal::resolve_links(
    ...
    if (has_node(_settings_path_linkedroom)) {
        ...
        // (already recognised).
        // If we don't check this, it will start trying to link to Room nodes
        // that are invalid, and crash.
        if (linkedroom && (p_rooms.find(linkedroom) == -1)) {
            // invalid room
            WARN_PRINT(
                "Portal attempting to link to Room outside the roomlist : "
                + linkedroom->get_name()
            );
            linkedroom = nullptr;
        }
        ...
        if (linkedroom && (linkedroom->_room_ID >= p_rooms.size())) {
            WARN_PRINT(
                "Portal attempting to link to invalid Room : "
                + linkedroom->get_name()
            );
            linkedroom = nullptr;
        }
...
bool Portal::create_from_mesh_instance(const MeshInstance* p_mi) {
    ...
    // get the verts sorted with winding, assume that the triangle initial
    // winding tells us the normal and hence which way the world space portal
    // should be facing
    _sort_verts_clockwise(normal, pts_world);

    // back calculate the plane from *all* the portal points, this will give us
    // a nice average plane (in case of wonky portals where artwork isn't bang
    // on)
    _plane = _plane_from_points_newell(pts_world);

    // change the portal transform to match our plane and the center of the
    // portal
    Transform tr_global;
    ...
    // We can't directly set this global transform on the portal, because the
    // parent node may already have a transform applied, so we need to account
    // for this and give a corrected local transform for the portal, such that
    // the end result global transform will be correct.

    // find the difference between this new global transform and the transform
    // of the parent then use this for the new local transform of the portal
    Spatial* parent = Object::cast_to<Spatial>(get_parent());
    ERR_FAIL_COND_V(!parent, false);

    Transform tr_inverse_parent =
        parent->get_global_transform().affine_inverse();
    Transform new_local_transform = tr_inverse_parent * tr_global;
    set_transform(new_local_transform);

    // now back calculate the local space coords of the portal from the world
    // space coords. The local space will be used in future for editing and as a
    // 'master' store of the verts.
...
void Portal::_sanitize_points() {
    ...
    // some peculiarity of convex_hull_2d function, it duplicates the last point
    // for some reason
    ...
    // a bit of a bodge, but a small epsilon pulling in the portal edges towards
    // the center can hide walls in the opposite room that abutt the portal (due
    // to floating point error) find 2d center
    Vector2 center;
...
void Portal::_sort_verts_clockwise(
    ...
    for (int n = 0; n < num_points - 2; n++) {
        ...
        Plane p = Plane(
            r_verts[n],
            _pt_center_world,
            _pt_center_world + p_portal_normal
        );
    ...
    // the vertices are now sorted, but may be in the opposite order to that
    // wanted. we detect this by calculating the normal of the poly, then
    // flipping the order if the normal is pointing the wrong way.
...
void Portal::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_portal_active", "p_active"),
        &Portal::set_portal_active
    );
    ClassDB::bind_method(
        D_METHOD("get_portal_active"),
        &Portal::get_portal_active
    );

    ClassDB::bind_method(
        D_METHOD("set_two_way", "p_two_way"),
        &Portal::set_two_way
    );
    ClassDB::bind_method(D_METHOD("is_two_way"), &Portal::is_two_way);

    ClassDB::bind_method(
        D_METHOD("set_use_default_margin", "p_use"),
        &Portal::set_use_default_margin
    );
    ClassDB::bind_method(
        D_METHOD("get_use_default_margin"),
        &Portal::get_use_default_margin
    );

    ClassDB::bind_method(
        D_METHOD("set_portal_margin", "p_margin"),
        &Portal::set_portal_margin
    );
    ClassDB::bind_method(
        D_METHOD("get_portal_margin"),
        &Portal::get_portal_margin
    );

    ClassDB::bind_method(
        D_METHOD("set_linked_room", "p_room"),
        &Portal::set_linked_room
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "portal_active"),
        "set_portal_active",
        "get_portal_active"
    );
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "two_way"),
        "set_two_way",
        "is_two_way"
    );
    ADD_PROPERTY(
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "use_default_margin"),
