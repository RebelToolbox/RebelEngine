...
void AudioStreamPlayer::_mix_to_bus(const AudioFrame* p_frames, int p_amount) {
    ...
    if (AudioServer::get_singleton()->get_speaker_mode()
        ...
        targets[0] = AudioServer::get_singleton()
                         ->thread_get_channel_mix_buffer(bus_index, 0);
    } else {
        switch (mix_target) {
            ...
            case MIX_TARGET_SURROUND: {
                for (int i = 0;
                     i < AudioServer::get_singleton()->get_channel_count();
                     i++) {
                    targets[i] =
                        AudioServer::get_singleton()
                            ->thread_get_channel_mix_buffer(bus_index, i);
                }
            } break;
            case MIX_TARGET_CENTER: {
                targets[0] = AudioServer::get_singleton()
                                 ->thread_get_channel_mix_buffer(bus_index, 1);
            } break;
        }
    }
...
void AudioStreamPlayer::_notification(int p_what) {
    ...
    if (p_what == NOTIFICATION_INTERNAL_PROCESS) {
        if (!active.is_set()
            || (setseek.get() < 0 && !stream_playback->is_playing())) {
            active.clear();
...
bool AudioStreamPlayer::is_playing() const {
    if (stream_playback.is_valid()) {
        return active.is_set()
            && !setstop.is_set(); //&& stream_playback->is_playing();
    }
...
void AudioStreamPlayer::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set_mix_target", "mix_target"),
        &AudioStreamPlayer::set_mix_target
    );
    ClassDB::bind_method(
        D_METHOD("get_mix_target"),
    ...
    ClassDB::bind_method(
        D_METHOD("_is_active"),
        &AudioStreamPlayer::_is_active
    );

    ClassDB::bind_method(
        D_METHOD("_bus_layout_changed"),
        &AudioStreamPlayer::_bus_layout_changed
    );
