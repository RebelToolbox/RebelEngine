    ...
    for (int i = 0; i < nc; i++) {
        ...
        if (i > 0) {
            ERR_FAIL_COND_V_MSG(
                ...
                nullptr,
                vformat(
                    "Invalid scene: node %s does not specify its parent node.",
                    snames[n.name]
                )
            );
            ...
            ERR_FAIL_COND_V_MSG(
                n.type == TYPE_INSTANCED && base_scene_idx < 0,
                nullptr,
                vformat(
                    "Invalid scene: root node %s in an instance, but there's "
                    "no base scene.",
                    snames[n.name]
                )
            );
        ...
        if (i == 0 && base_scene_idx >= 0) {
            ...
            // get the node from somewhere, it likely already exists from
            // another instance
            if (parent) {
                ...
                if (!node) {
                    WARN_PRINT(String(
                                   ...
                                   + "' was modified from inside an instance, "
                                     "but it has vanished."
                    )
                                   .ascii()
                                   .get_data());
            ...
            if (!Object::cast_to<Node>(obj)) {
                ...
                WARN_PRINT(vformat(
                               "Node %s of type %s cannot be created. A "
                               "placeholder will be created instead.",
                               snames[n.name],
                               snames[n.type]
                )
                               .ascii()
        ...
        if (node) {
            ...
            if (nprop_count) {
                ...
                for (int j = 0; j < nprop_count; j++) {
                    ...
                    if (snames[nprops[j].name]
                        ...
                        // work around to avoid old script variables from
                        // disappearing, should be the proper fix to:
                        // https://github.com/godotengine/godot/issues/2958

                        // store old state
                        List<Pair<StringName, Variant>> old_state;
                        if (node->get_script_instance()) {
                            node->get_script_instance()->get_property_state(
                                old_state
                            );
                        }

                        node->set(
                            snames[nprops[j].name],
                            props[nprops[j].value],
                            &valid
                        );
                        ...
                        if (value.get_type() == Variant::OBJECT) {
                            // handle resources that are local to scene by
                            // duplicating them if needed
                            Ref<Resource> res = value;
                            if (res.is_valid()) {
                                if (res->is_local_to_scene()) {
                                    Map<Ref<Resource>, Ref<Resource>>::Element*
                                        E = resources_local_to_scene.find(res);

                                    if (E) {
                                        value = E->get();
                                    } else {
                                        Node* base =
                                            i == 0 ? node : ret_nodes[0];

                                        if (p_edit_state
                                            == GEN_EDIT_STATE_MAIN) {
                                            // for the main scene, use the
                                            // resource as is
                                            res->configure_for_local_scene(
                                                base,
                                            ...
                                            // for instances, a copy must be
                                            // made
                                            Node* base2 =
                                                i == 0 ? node : ret_nodes[0];
                                            Ref<Resource> local_dupe =
                                                res->duplicate_for_local_scene(
                                                    base2,
                                                    resources_local_to_scene
                                                );
                                            resources_local_to_scene[res] =
                                                local_dupe;
                                            res   = local_dupe;
                                            value = local_dupe;
                                        }
                                    }
                                    // must make a copy, because this res is
                                    // local to scene
                                }
                            }
                        } else if (p_edit_state == GEN_EDIT_STATE_INSTANCE) {
                            value = value.duplicate(true
                            ); // Duplicate arrays and dictionaries for the
                               // editor
                        }
            ...
            if (n.instance >= 0 || n.type != TYPE_INSTANCED || i == 0) {
                // if node was not part of instance, must set its name,
                // parenthood and ownership
                if (i > 0) {
                    if (parent) {
                        parent->_add_child_nocheck(node, snames[n.name]);
                        if (n.index >= 0
                            && n.index < parent->get_child_count() - 1) {
                            parent->move_child(node, n.index);
                        }
                    } else {
                        // it may be possible that an instanced scene has
                        // changed and the node has nowhere to go anymore
                        stray_instances.push_back(node
                        ); // can't be added, go to stray list
                    }
                } else {
...
Error SceneState::_parse_node(
    ...
    // it instanced or inherited. given the complexity of this process, an
    // attempt will be made to properly document it. if you fail to understand
    ...
    // save the child instantiated scenes that are chosen as editable, so they
    // can be restored upon load back
    ...
    // saved when part of an inherited scene OR the node parent is from the
    // edited scene
        ...
        while (n) {
            if (n == p_owner) {
                ...
                if (n->get_filename() != String()) {
                    ...
                    if (state.is_valid()) {
                        int node =
                            state->find_node_by_path(n->get_path_to(p_node));
                        if (node >= 0) {
                            // this one has state for this node, save
                            PackState ps;
                            ps.node  = node;
    ...
    if (Engine::get_singleton()->is_editor_hint() && script.is_valid()) {
        ...
        // otherwise it can cause problems because of missing instance state
        // support.
    ...
    for (List<PropertyInfo>::Element* E = plist.front(); E; E = E->next()) {
        ...
        // placeholder or usage flag in the script, just the default value
        // function. if (E->get().usage & PROPERTY_USAGE_SCRIPT_DEFAULT_VALUE) {
        ...
        if (pack_state_stack.size()) {
            ...
            for (List<PackState>::Element* F = pack_state_stack.back(); F;
                 F                           = F->prev()) {
                // check all levels of pack to see if the property exists
                // somewhere
                const PackState& ps = F->get();

                original = ps.state->get_property_value(
                    ps.node,
                    E->get().name,
                    exists
                );
            ...
            if (exists) {
                ...
                if (value.get_type() == Variant::REAL
                    && original.get_type() == Variant::REAL) {
                    // this must be done because, as some scenes save as text,
                    // there might be a tiny difference in floats due to
                    // numerical error
                    float a = value;
                    float b = original;
    ...
    save_node = save_node
             || (p_node->get_owner() == p_owner && instanced_by_owner
             ); // part of scene and not instanced
...
Error SceneState::_parse_connections(
    ...
    for (List<MethodInfo>::Element* E = _signals.front(); E; E = E->next()) {
        ...
        for (List<Node::Connection>::Element* F = conns.front(); F;
             F                                  = F->next()) {
            const Node::Connection& c = F->get();

            if (!(c.flags & CONNECT_PERSIST
                )) { // only persistent connections get saved
                continue;
            }

            // only connections that originate or end into main saved scene are
            ...
            if (exists) { // already exists (comes from instance or
                          // inheritance), so don't save
                continue;
            }
                ...
                while (nl) {
                    if (nl == p_owner) {
                        ...
                        if (state.is_valid()) {
                            ...
                            int to_node =
                                state->find_node_by_path(nl->get_path_to(target)
                                );

                            if (from_node >= 0 && to_node >= 0) {
                                // this one has state for this node, save
                                if (state->is_connection(
                                        from_node,
                                        c.signal,
                                        to_node,
                                        c.method
                        ...
                        if (nl->get_filename() != String()) {
                            ...
                            Ref<SceneState> state =
                                nl->get_scene_instance_state();
                            if (state.is_valid()) {
                                int from_node = state->find_node_by_path(
                                    nl->get_path_to(p_node)
                                );
                                int to_node = state->find_node_by_path(
                                    nl->get_path_to(target)
                                );
...
Error SceneState::pack(Node* p_scene) {
    ...
    for (Map<StringName, int>::Element* E = name_map.front(); E;
         E                                = E->next()) {
...
Variant SceneState::get_property_value(
    int p_node,
...
bool SceneState::is_connection(
    ...
    if (p_node < nodes.size() && p_to_node < nodes.size()) {
        ...
        if (signal_idx >= 0 && method_idx >= 0) {
            ...
            for (int i = 0; i < connections.size(); i++) {
                if (connections[i].from == p_node
                    && connections[i].to == p_to_node
    ...
    if (base_scene_node_remap.has(p_node)
        ...
        return _get_base_scene_state()->is_connection(
            base_scene_node_remap[p_node],
            p_signal,
            base_scene_node_remap[p_to_node],
            p_to_method
        );
    }
...
Dictionary SceneState::get_bundled_scene() const {
    ...
    for (int i = 0; i < nodes.size(); i++) {
        ...
        if (nd.index < (1 << (32 - NAME_INDEX_BITS))
                           - 1) { // save if less than 16k children
            name_index |= uint32_t(nd.index + 1)
                       << NAME_INDEX_BITS; // for backwards compatibility, index
                                           // 0 is no index
        }
...
bool SceneState::has_connection(
...
void SceneState::add_connection(
    int p_from,
    int p_to,
    int p_signal,
    int p_method,
...
void SceneState::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("get_node_type", "idx"),
        &SceneState::get_node_type
    );
    ClassDB::bind_method(
        D_METHOD("get_node_name", "idx"),
        &SceneState::get_node_name
    );
    ClassDB::bind_method(
        D_METHOD("get_node_path", "idx", "for_parent"),
        &SceneState::get_node_path,
        DEFVAL(false)
    );
    ClassDB::bind_method(
        D_METHOD("get_node_owner_path", "idx"),
        &SceneState::get_node_owner_path
    );
    ClassDB::bind_method(
        D_METHOD("is_node_instance_placeholder", "idx"),
        &SceneState::is_node_instance_placeholder
    );
    ClassDB::bind_method(
        D_METHOD("get_node_instance_placeholder", "idx"),
        &SceneState::get_node_instance_placeholder
    );
    ClassDB::bind_method(
        D_METHOD("get_node_instance", "idx"),
        &SceneState::get_node_instance
    );
    ClassDB::bind_method(
        D_METHOD("get_node_groups", "idx"),
        &SceneState::_get_node_groups
    );
    ClassDB::bind_method(
        D_METHOD("get_node_index", "idx"),
        &SceneState::get_node_index
    );
    ClassDB::bind_method(
        D_METHOD("get_node_property_count", "idx"),
        &SceneState::get_node_property_count
    );
    ClassDB::bind_method(
        D_METHOD("get_node_property_name", "idx", "prop_idx"),
        &SceneState::get_node_property_name
    );
    ClassDB::bind_method(
        D_METHOD("get_node_property_value", "idx", "prop_idx"),
        &SceneState::get_node_property_value
    );
    ClassDB::bind_method(
        D_METHOD("get_connection_count"),
        &SceneState::get_connection_count
    );
    ClassDB::bind_method(
        D_METHOD("get_connection_source", "idx"),
        &SceneState::get_connection_source
    );
    ClassDB::bind_method(
        D_METHOD("get_connection_signal", "idx"),
        &SceneState::get_connection_signal
    );
    ClassDB::bind_method(
        D_METHOD("get_connection_target", "idx"),
        &SceneState::get_connection_target
    );
    ClassDB::bind_method(
        D_METHOD("get_connection_method", "idx"),
        &SceneState::get_connection_method
    );
    ClassDB::bind_method(
        D_METHOD("get_connection_flags", "idx"),
        &SceneState::get_connection_flags
    );
    ClassDB::bind_method(
        D_METHOD("get_connection_binds", "idx"),
