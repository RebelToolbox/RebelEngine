...
void Shader::set_default_texture_param(
    const StringName& p_param,
    const Ref<Texture>& p_texture
) {
    if (p_texture.is_valid()) {
        default_textures[p_param] = p_texture;
        VS::get_singleton()->shader_set_default_texture_param(
            shader,
            p_param,
            p_texture->get_rid()
        );
        ...
        VS::get_singleton()
            ->shader_set_default_texture_param(shader, p_param, RID());
    }

    emit_changed();
}

Ref<Texture> Shader::get_default_texture_param(const StringName& p_param
) const {
...
void Shader::get_default_texture_param_list(List<StringName>* r_textures
) const {
    for (const Map<StringName, Ref<Texture>>::Element* E =
...
void Shader::set_custom_defines(const String& p_defines) {
    ...
    if (!shader_custom_defines.empty()) {
        VS::get_singleton()->shader_remove_custom_define(
            shader,
            shader_custom_defines
        );
    }

    shader_custom_defines = p_defines;
    VS::get_singleton()->shader_add_custom_define(
        shader,
        shader_custom_defines
    );
}
...
void Shader::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("get_default_texture_param", "param"),
        &Shader::get_default_texture_param
    );

    ClassDB::bind_method(
        D_METHOD("set_custom_defines", "custom_defines"),
        &Shader::set_custom_defines
    );
    ClassDB::bind_method(
        D_METHOD("get_custom_defines"),
        &Shader::get_custom_defines
    );
...
RES ResourceFormatLoaderShader::load(
    const String& p_path,
...
String ResourceFormatLoaderShader::get_resource_type(const String& p_path
) const {
...
Error ResourceFormatSaverShader::save(
    const String& p_path,
...
bool ResourceFormatSaverShader::recognize(const RES& p_resource) const {
    return p_resource->get_class_name()
        == "Shader"; // only shader, not inherited
}
