...
Size2 StyleBox::get_minimum_size() const {
    return Size2(
        get_margin(MARGIN_LEFT) + get_margin(MARGIN_RIGHT),
...
void StyleBox::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("test_mask", "point", "rect"),
        &StyleBox::test_mask
    );

    ClassDB::bind_method(
        D_METHOD("set_default_margin", "margin", "offset"),
        &StyleBox::set_default_margin
    );
    ClassDB::bind_method(
        D_METHOD("get_default_margin", "margin"),
...
Rect2 StyleBoxTexture::get_draw_rect(const Rect2& p_rect) const {
    return p_rect.grow_individual(
        expand_margin[MARGIN_LEFT],
        expand_margin[MARGIN_TOP],
        expand_margin[MARGIN_RIGHT],
        expand_margin[MARGIN_BOTTOM]
    );
}
...
void StyleBoxTexture::set_expand_margin_size(
    Margin p_expand_margin,
    float p_size
) {
...
void StyleBoxTexture::set_expand_margin_size_individual(
    float p_left,
    float p_top,
    float p_right,
    float p_bottom
...
void StyleBoxTexture::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set_margin_size", "margin", "size"),
        &StyleBoxTexture::set_margin_size
    );
    ClassDB::bind_method(
        D_METHOD("get_margin_size", "margin"),
        &StyleBoxTexture::get_margin_size
    );

    ClassDB::bind_method(
        D_METHOD("set_expand_margin_size", "margin", "size"),
        &StyleBoxTexture::set_expand_margin_size
    );
    ClassDB::bind_method(
        D_METHOD("set_expand_margin_all", "size"),
    ...
    ClassDB::bind_method(
        D_METHOD("get_expand_margin_size", "margin"),
        &StyleBoxTexture::get_expand_margin_size
    );

    ClassDB::bind_method(
        D_METHOD("set_region_rect", "region"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_draw_center", "enable"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_modulate", "color"),
        &StyleBoxTexture::set_modulate
    );
    ClassDB::bind_method(
        D_METHOD("get_modulate"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::COLOR, "modulate_color"),
        "set_modulate",
        "get_modulate"
    );
    ADD_PROPERTY(
...
int StyleBoxFlat::get_border_width_min() const {
    return MIN(
        MIN(border_width[0], border_width[1]),
...
void StyleBoxFlat::set_corner_radius_individual(
...
void StyleBoxFlat::set_expand_margin_size(
    Margin p_expand_margin,
    float p_size
) {
    ERR_FAIL_INDEX((int)p_expand_margin, 4);
...
void StyleBoxFlat::set_expand_margin_size_individual(
    float p_left,
    float p_top,
    float p_right,
    float p_bottom
...
inline void set_inner_corner_radius(
    const Rect2 style_rect,
    const Rect2 inner_rect,
    const real_t corner_radius[4],
    real_t* inner_corner_radius
) {
...
inline void draw_ring(
    ...
    int adapted_corner_detail =
        (corner_radius[0] == 0 && corner_radius[1] == 0 && corner_radius[2] == 0
         && corner_radius[3] == 0)
            ? 1
            : corner_detail;

    real_t ring_corner_radius[4];
    set_inner_corner_radius(
        style_rect,
        ring_rect,
        corner_radius,
        ring_corner_radius
    ...
    outer_points.push_back(
        ring_rect.position
        + Vector2(ring_corner_radius[0], ring_corner_radius[0])
    ); // tl
    outer_points.push_back(Point2(
        ring_rect.position.x + ring_rect.size.x - ring_corner_radius[1],
        ring_rect.position.y + ring_corner_radius[1]
    )); // tr
    outer_points.push_back(
        ring_rect.position + ring_rect.size
        - Vector2(ring_corner_radius[2], ring_corner_radius[2])
    ); // br
    outer_points.push_back(Point2(
        ring_rect.position.x + ring_corner_radius[3],
        ring_rect.position.y + ring_rect.size.y - ring_corner_radius[3]
    )); // bl
    ...
    inner_points.push_back(
        inner_rect.position
        + Vector2(inner_corner_radius[0], inner_corner_radius[0])
    ); // tl
    inner_points.push_back(Point2(
        inner_rect.position.x + inner_rect.size.x - inner_corner_radius[1],
        inner_rect.position.y + inner_corner_radius[1]
    )); // tr
    inner_points.push_back(
        inner_rect.position + inner_rect.size
        - Vector2(inner_corner_radius[2], inner_corner_radius[2])
    ); // br
    inner_points.push_back(Point2(
        inner_rect.position.x + inner_corner_radius[3],
        inner_rect.position.y + inner_rect.size.y - inner_corner_radius[3]
    )); // bl
    ...
    for (int corner_index = 0; corner_index < 4; corner_index++) {
        for (int detail = 0; detail <= adapted_corner_detail; detail++) {
            for (int inner_outer = 0; inner_outer < 2; inner_outer++) {
                ...
                float x = radius
                            * (float)cos(
                                (double)corner_index * Math_PI / 2.0
...
inline void adapt_values(
    int p_index_a,
    int p_index_b,
    real_t* adapted_values,
    const real_t* p_values,
    const real_t p_width,
    const real_t p_max_a,
    const real_t p_max_b
) {
    if (p_values[p_index_a] + p_values[p_index_b] > p_width) {
        real_t factor;
        real_t new_value;

        factor = (real_t)p_width
               / (real_t)(p_values[p_index_a] + p_values[p_index_b]);
...
void StyleBoxFlat::draw(RID p_canvas_item, const Rect2& p_rect) const {
    ...
    adapt_values(
        ...
        corner_radius,
        height,
        height - adapted_border[MARGIN_BOTTOM],
    ...
    adapt_values(
        ...
        corner_radius,
        width,
        width - adapted_border[MARGIN_RIGHT],
    ...
    Rect2 infill_rect = style_rect.grow_individual(
        -adapted_border[MARGIN_LEFT],
        -adapted_border[MARGIN_TOP],
        -adapted_border[MARGIN_RIGHT],
        -adapted_border[MARGIN_BOTTOM]
    );
    ...
    if (aa_on) {
        for (int i = 0; i < 4; i++) {
            if (border_width[i] > 0) {
                border_style_rect =
                    border_style_rect.grow_margin((Margin)i, -aa_size);
            }
        }
    }
    ...
    if (aa_on) {
        ...
        Rect2 infill_inner_rect = infill_rect.grow_individual(
            -aa_border_width[MARGIN_LEFT],
            -aa_border_width[MARGIN_TOP],
            -aa_border_width[MARGIN_RIGHT],
            -aa_border_width[MARGIN_BOTTOM]
        );

        if (draw_center) {
            if (!blend_on && draw_border) {
                Rect2 infill_inner_rect_aa = infill_inner_rect.grow_individual(
                    aa_border_width[MARGIN_LEFT],
                    aa_border_width[MARGIN_TOP],
                    aa_border_width[MARGIN_RIGHT],
                    aa_border_width[MARGIN_BOTTOM]
            ...
            if (!blend_on || !draw_border) {
                Rect2 infill_rect_aa = infill_rect.grow_individual(
                    aa_fill_width[MARGIN_LEFT],
                    aa_fill_width[MARGIN_TOP],
                    aa_fill_width[MARGIN_RIGHT],
                    aa_fill_width[MARGIN_BOTTOM]
                );
        ...
        if (draw_border) {
            ...
            Rect2 style_rect_aa = style_rect.grow_individual(
                aa_border_width[MARGIN_LEFT],
                aa_border_width[MARGIN_TOP],
                aa_border_width[MARGIN_RIGHT],
                aa_border_width[MARGIN_BOTTOM]
            );
            Rect2 border_style_rect_aa = border_style_rect.grow_individual(
                aa_border_width[MARGIN_LEFT],
                aa_border_width[MARGIN_TOP],
                aa_border_width[MARGIN_RIGHT],
                aa_border_width[MARGIN_BOTTOM]
            ...
            draw_ring(
                ...
                border_style_rect,
                adapted_corner,
                border_style_rect_aa,
                ((blend_on) ? infill_rect : infill_rect_aa),
                border_color_inner,
                border_color,
                corner_detail
            );
...
void StyleBoxFlat::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("set_border_width_all", "width"),
        &StyleBoxFlat::set_border_width_all
    );
    ClassDB::bind_method(
        D_METHOD("get_border_width_min"),
        &StyleBoxFlat::get_border_width_min
    );

    ClassDB::bind_method(
        D_METHOD("set_border_width", "margin", "width"),
        &StyleBoxFlat::set_border_width
    );
    ClassDB::bind_method(
        D_METHOD("get_border_width", "margin"),
        &StyleBoxFlat::get_border_width
    );

    ClassDB::bind_method(
        D_METHOD("set_border_blend", "blend"),
        &StyleBoxFlat::set_border_blend
    );
    ClassDB::bind_method(
        D_METHOD("get_border_blend"),
    ...
    ClassDB::bind_method(
        D_METHOD("set_corner_radius_all", "radius"),
        &StyleBoxFlat::set_corner_radius_all
    );

    ClassDB::bind_method(
        D_METHOD("set_corner_radius", "corner", "radius"),
        &StyleBoxFlat::set_corner_radius
    );
    ClassDB::bind_method(
        D_METHOD("get_corner_radius", "corner"),
        &StyleBoxFlat::get_corner_radius
    );

    ClassDB::bind_method(
        D_METHOD("set_expand_margin", "margin", "size"),
    ...
    ClassDB::bind_method(
        D_METHOD("get_expand_margin", "margin"),
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "border_blend"),
        "set_border_blend",
        "get_border_blend"
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::COLOR, "shadow_color"),
        "set_shadow_color",
        "get_shadow_color"
    );
    ADD_PROPERTY(
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::VECTOR2, "shadow_offset"),
        "set_shadow_offset",
        "get_shadow_offset"
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "anti_aliasing"),
        "set_anti_aliased",
        "is_anti_aliased"
    );
    ADD_PROPERTY(
...
void StyleBoxLine::_bind_methods() {
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "vertical"),
        "set_vertical",
        "is_vertical"
    );
}
