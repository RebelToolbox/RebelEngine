...
void Theme::set_icon(
    ...
    bool existing = false;
    if (icon_map[p_node_type].has(p_name)
        && icon_map[p_node_type][p_name].is_valid()) {
        existing = true;
        icon_map[p_node_type][p_name]
            ->disconnect("changed", this, "_emit_theme_changed");
    }

    icon_map[p_node_type][p_name] = p_icon;

    if (p_icon.is_valid()) {
        icon_map[p_node_type][p_name]->connect(
            "changed",
            this,
            "_emit_theme_changed",
            varray(),
...
Ref<Texture> Theme::get_icon(
    const StringName& p_name,
    const StringName& p_node_type
) const {
    if (icon_map.has(p_node_type) && icon_map[p_node_type].has(p_name)
        && icon_map[p_node_type][p_name].is_valid()) {
...
bool Theme::has_icon(const StringName& p_name, const StringName& p_node_type)
    const {
    return (
        icon_map.has(p_node_type) && icon_map[p_node_type].has(p_name)
        && icon_map[p_node_type][p_name].is_valid()
    );
}

bool Theme::has_icon_nocheck(
    const StringName& p_name,
    const StringName& p_node_type
) const {
...
void Theme::rename_icon(
    ...
    ERR_FAIL_COND_MSG(
        !icon_map.has(p_node_type),
        "Cannot rename the icon '" + String(p_old_name)
            + "' because the node type '" + String(p_node_type)
            + "' does not exist."
    );
    ERR_FAIL_COND_MSG(
        icon_map[p_node_type].has(p_name),
        "Cannot rename the icon '" + String(p_old_name)
            + "' because the new name '" + String(p_name) + "' already exists."
    );
    ERR_FAIL_COND_MSG(
        !icon_map[p_node_type].has(p_old_name),
        "Cannot rename the icon '" + String(p_old_name)
...
void Theme::clear_icon(
    const StringName& p_name,
    const StringName& p_node_type
) {
    ERR_FAIL_COND_MSG(
        !icon_map.has(p_node_type),
        "Cannot clear the icon '" + String(p_name) + "' because the node type '"
            + String(p_node_type) + "' does not exist."
    );
    ERR_FAIL_COND_MSG(
        !icon_map[p_node_type].has(p_name),
        "Cannot clear the icon '" + String(p_name)
            + "' because it does not exist."
    );
...
void Theme::get_icon_list(StringName p_node_type, List<StringName>* p_list)
    const {
...
// Shaders.
void Theme::set_shader(
    const StringName& p_name,
    const StringName& p_node_type,
    const Ref<Shader>& p_shader
) {
    bool existing =
        (shader_map.has(p_node_type) && shader_map[p_node_type].has(p_name));
    shader_map[p_node_type][p_name] = p_shader;

    _emit_theme_changed(!existing);
}

Ref<Shader> Theme::get_shader(
    const StringName& p_name,
    const StringName& p_node_type
) const {
    if (shader_map.has(p_node_type) && shader_map[p_node_type].has(p_name)
        && shader_map[p_node_type][p_name].is_valid()) {
...
bool Theme::has_shader(const StringName& p_name, const StringName& p_node_type)
    const {
    return (
        shader_map.has(p_node_type) && shader_map[p_node_type].has(p_name)
        && shader_map[p_node_type][p_name].is_valid()
...
void Theme::set_stylebox(
    ...
    bool existing = false;
    if (style_map[p_node_type].has(p_name)
        && style_map[p_node_type][p_name].is_valid()) {
        existing = true;
        style_map[p_node_type][p_name]
            ->disconnect("changed", this, "_emit_theme_changed");
    }
    ...
    if (p_style.is_valid()) {
        style_map[p_node_type][p_name]->connect(
            "changed",
            this,
            "_emit_theme_changed",
            varray(),
...
Ref<StyleBox> Theme::get_stylebox(
    const StringName& p_name,
    const StringName& p_node_type
) const {
    if (style_map.has(p_node_type) && style_map[p_node_type].has(p_name)
        && style_map[p_node_type][p_name].is_valid()) {
...
bool Theme::has_stylebox(
    ...
    return (
        style_map.has(p_node_type) && style_map[p_node_type].has(p_name)
        && style_map[p_node_type][p_name].is_valid()
    );
}

bool Theme::has_stylebox_nocheck(
    const StringName& p_name,
    const StringName& p_node_type
) const {
...
void Theme::rename_stylebox(
    ...
    ERR_FAIL_COND_MSG(
        !style_map.has(p_node_type),
        "Cannot rename the stylebox '" + String(p_old_name)
            + "' because the node type '" + String(p_node_type)
            + "' does not exist."
    );
    ERR_FAIL_COND_MSG(
        style_map[p_node_type].has(p_name),
        "Cannot rename the stylebox '" + String(p_old_name)
            + "' because the new name '" + String(p_name) + "' already exists."
    );
    ERR_FAIL_COND_MSG(
        !style_map[p_node_type].has(p_old_name),
        "Cannot rename the stylebox '" + String(p_old_name)
            + "' because it does not exist."
    );
...
void Theme::clear_stylebox(
    const StringName& p_name,
    const StringName& p_node_type
) {
    ERR_FAIL_COND_MSG(
        !style_map.has(p_node_type),
        "Cannot clear the stylebox '" + String(p_name)
            + "' because the node type '" + String(p_node_type)
            + "' does not exist."
    );
    ERR_FAIL_COND_MSG(
...
void Theme::get_stylebox_list(StringName p_node_type, List<StringName>* p_list)
    const {
    ERR_FAIL_NULL(p_list);
...
void Theme::set_font(
    ...
    if (p_font.is_valid()) {
        font_map[p_node_type][p_name]->connect(
            "changed",
            this,
            "_emit_theme_changed",
            varray(),
...
Ref<Font> Theme::get_font(
    const StringName& p_name,
    const StringName& p_node_type
) const {
    if (font_map.has(p_node_type) && font_map[p_node_type].has(p_name)
        && font_map[p_node_type][p_name].is_valid()) {
...
bool Theme::has_font(const StringName& p_name, const StringName& p_node_type)
    const {
    return (
        font_map.has(p_node_type) && font_map[p_node_type].has(p_name)
        && font_map[p_node_type][p_name].is_valid()
    );
}

bool Theme::has_font_nocheck(
    const StringName& p_name,
    const StringName& p_node_type
) const {
...
void Theme::rename_font(
    ...
    ERR_FAIL_COND_MSG(
        !font_map.has(p_node_type),
        "Cannot rename the font '" + String(p_old_name)
            + "' because the node type '" + String(p_node_type)
            + "' does not exist."
    );
    ERR_FAIL_COND_MSG(
        font_map[p_node_type].has(p_name),
        "Cannot rename the font '" + String(p_old_name)
            + "' because the new name '" + String(p_name) + "' already exists."
    );
    ERR_FAIL_COND_MSG(
        !font_map[p_node_type].has(p_old_name),
        "Cannot rename the font '" + String(p_old_name)
            + "' because it does not exist."
    );
...
void Theme::clear_font(
    const StringName& p_name,
    const StringName& p_node_type
) {
    ERR_FAIL_COND_MSG(
        !font_map.has(p_node_type),
        "Cannot clear the font '" + String(p_name) + "' because the node type '"
            + String(p_node_type) + "' does not exist."
    );
    ERR_FAIL_COND_MSG(
        !font_map[p_node_type].has(p_name),
        "Cannot clear the font '" + String(p_name)
            + "' because it does not exist."
    );
...
void Theme::get_font_list(StringName p_node_type, List<StringName>* p_list)
    const {
    ERR_FAIL_NULL(p_list);
...
Color Theme::get_color(const StringName& p_name, const StringName& p_node_type)
    const {
...
bool Theme::has_color(const StringName& p_name, const StringName& p_node_type)
    const {
    return (color_map.has(p_node_type) && color_map[p_node_type].has(p_name));
}

bool Theme::has_color_nocheck(
    const StringName& p_name,
    const StringName& p_node_type
) const {
    return (color_map.has(p_node_type) && color_map[p_node_type].has(p_name));
...
void Theme::rename_color(
    ...
    ERR_FAIL_COND_MSG(
        !color_map.has(p_node_type),
        "Cannot rename the color '" + String(p_old_name)
            + "' because the node type '" + String(p_node_type)
            + "' does not exist."
    );
    ERR_FAIL_COND_MSG(
        color_map[p_node_type].has(p_name),
        "Cannot rename the color '" + String(p_old_name)
            + "' because the new name '" + String(p_name) + "' already exists."
    );
    ERR_FAIL_COND_MSG(
        !color_map[p_node_type].has(p_old_name),
        "Cannot rename the color '" + String(p_old_name)
            + "' because it does not exist."
    );
...
void Theme::clear_color(
    const StringName& p_name,
    const StringName& p_node_type
) {
    ERR_FAIL_COND_MSG(
        !color_map.has(p_node_type),
        "Cannot clear the color '" + String(p_name)
            + "' because the node type '" + String(p_node_type)
            + "' does not exist."
    );
    ERR_FAIL_COND_MSG(
...
void Theme::get_color_list(StringName p_node_type, List<StringName>* p_list)
    const {
    ERR_FAIL_NULL(p_list);
...
int Theme::get_constant(const StringName& p_name, const StringName& p_node_type)
    const {
    if (constant_map.has(p_node_type)
        && constant_map[p_node_type].has(p_name)) {
...
bool Theme::has_constant(
    const StringName& p_name,
    const StringName& p_node_type
) const {
    return (
        constant_map.has(p_node_type) && constant_map[p_node_type].has(p_name)
    );
}

bool Theme::has_constant_nocheck(
    const StringName& p_name,
    const StringName& p_node_type
) const {
    return (
...
void Theme::rename_constant(
    ...
    ERR_FAIL_COND_MSG(
        !constant_map.has(p_node_type),
        "Cannot rename the constant '" + String(p_old_name)
            + "' because the node type '" + String(p_node_type)
            + "' does not exist."
    );
    ERR_FAIL_COND_MSG(
    ...
    ERR_FAIL_COND_MSG(
        !constant_map[p_node_type].has(p_old_name),
        "Cannot rename the constant '" + String(p_old_name)
            + "' because it does not exist."
    );
...
void Theme::clear_constant(
    const StringName& p_name,
    const StringName& p_node_type
) {
    ERR_FAIL_COND_MSG(
        !constant_map.has(p_node_type),
        "Cannot clear the constant '" + String(p_name)
            + "' because the node type '" + String(p_node_type)
            + "' does not exist."
    );
    ERR_FAIL_COND_MSG(
...
void Theme::get_constant_list(StringName p_node_type, List<StringName>* p_list)
    const {
    ERR_FAIL_NULL(p_list);
...
void Theme::set_theme_item(
    ...
    switch (p_data_type) {
        case DATA_TYPE_COLOR: {
            ERR_FAIL_COND_MSG(
                p_value.get_type() != Variant::COLOR,
        ...
        case DATA_TYPE_FONT: {
            ERR_FAIL_COND_MSG(
                p_value.get_type() != Variant::OBJECT,
...
void Theme::add_theme_item_type(
    DataType p_data_type,
    const StringName& p_node_type
) {
    switch (p_data_type) {
...
void Theme::get_theme_item_types(DataType p_data_type, List<StringName>* p_list)
    const {
    switch (p_data_type) {
        case DATA_TYPE_COLOR:
...
// Internal methods for getting lists as a Vector of String (compatible with
// public API).
...
PoolVector<String> Theme::_get_theme_item_list(
    DataType p_data_type,
...
void Theme::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_icon", "name", "node_type", "texture"),
        &Theme::set_icon
    );
    ClassDB::bind_method(
        D_METHOD("get_icon", "name", "node_type"),
        &Theme::get_icon
    );
    ClassDB::bind_method(
        D_METHOD("has_icon", "name", "node_type"),
        &Theme::has_icon
    );
    ClassDB::bind_method(
        D_METHOD("rename_icon", "old_name", "name", "node_type"),
        &Theme::rename_icon
    );
    ClassDB::bind_method(
        D_METHOD("clear_icon", "name", "node_type"),
        &Theme::clear_icon
    );
    ClassDB::bind_method(
        D_METHOD("get_icon_list", "node_type"),
        &Theme::_get_icon_list
    );
    ClassDB::bind_method(D_METHOD("get_icon_types"), &Theme::_get_icon_types);

    ClassDB::bind_method(
        D_METHOD("set_stylebox", "name", "node_type", "texture"),
        &Theme::set_stylebox
    );
    ClassDB::bind_method(
        D_METHOD("get_stylebox", "name", "node_type"),
        &Theme::get_stylebox
    );
    ClassDB::bind_method(
        D_METHOD("has_stylebox", "name", "node_type"),
        &Theme::has_stylebox
    );
    ClassDB::bind_method(
        D_METHOD("rename_stylebox", "old_name", "name", "node_type"),
        &Theme::rename_stylebox
    );
    ClassDB::bind_method(
        D_METHOD("clear_stylebox", "name", "node_type"),
        &Theme::clear_stylebox
    );
    ClassDB::bind_method(
        D_METHOD("get_stylebox_list", "node_type"),
        &Theme::_get_stylebox_list
    );
    ClassDB::bind_method(
        D_METHOD("get_stylebox_types"),
        &Theme::_get_stylebox_types
    );

    ClassDB::bind_method(
        D_METHOD("set_font", "name", "node_type", "font"),
        &Theme::set_font
    );
    ClassDB::bind_method(
        D_METHOD("get_font", "name", "node_type"),
        &Theme::get_font
    );
    ClassDB::bind_method(
        D_METHOD("has_font", "name", "node_type"),
        &Theme::has_font
    );
    ClassDB::bind_method(
        D_METHOD("rename_font", "old_name", "name", "node_type"),
        &Theme::rename_font
    );
    ClassDB::bind_method(
        D_METHOD("clear_font", "name", "node_type"),
        &Theme::clear_font
    );
    ClassDB::bind_method(
        D_METHOD("get_font_list", "node_type"),
        &Theme::_get_font_list
    );
    ClassDB::bind_method(D_METHOD("get_font_types"), &Theme::_get_font_types);

    ClassDB::bind_method(
        D_METHOD("set_color", "name", "node_type", "color"),
        &Theme::set_color
    );
    ClassDB::bind_method(
        D_METHOD("get_color", "name", "node_type"),
        &Theme::get_color
    );
    ClassDB::bind_method(
        D_METHOD("has_color", "name", "node_type"),
        &Theme::has_color
    );
    ClassDB::bind_method(
        D_METHOD("rename_color", "old_name", "name", "node_type"),
        &Theme::rename_color
    );
    ClassDB::bind_method(
        D_METHOD("clear_color", "name", "node_type"),
        &Theme::clear_color
    );
    ClassDB::bind_method(
        D_METHOD("get_color_list", "node_type"),
        &Theme::_get_color_list
    );
    ClassDB::bind_method(D_METHOD("get_color_types"), &Theme::_get_color_types);

    ClassDB::bind_method(
        D_METHOD("set_constant", "name", "node_type", "constant"),
        &Theme::set_constant
    );
    ClassDB::bind_method(
        D_METHOD("get_constant", "name", "node_type"),
        &Theme::get_constant
    );
    ClassDB::bind_method(
        D_METHOD("has_constant", "name", "node_type"),
        &Theme::has_constant
    );
    ClassDB::bind_method(
        D_METHOD("rename_constant", "old_name", "name", "node_type"),
        &Theme::rename_constant
    );
    ClassDB::bind_method(
        D_METHOD("clear_constant", "name", "node_type"),
        &Theme::clear_constant
    );
    ClassDB::bind_method(
        D_METHOD("get_constant_list", "node_type"),
        &Theme::_get_constant_list
    );
    ClassDB::bind_method(
        D_METHOD("get_constant_types"),
        &Theme::_get_constant_types
    );

    ClassDB::bind_method(
        D_METHOD("set_default_font", "font"),
        &Theme::set_default_theme_font
    );
    ClassDB::bind_method(
        D_METHOD("get_default_font"),
        &Theme::get_default_theme_font
    );
    ClassDB::bind_method(
        D_METHOD("has_default_font"),
        &Theme::has_default_theme_font
    );

    ClassDB::bind_method(
        D_METHOD("set_theme_item", "data_type", "name", "node_type", "value"),
        &Theme::set_theme_item
    );
    ClassDB::bind_method(
        D_METHOD("get_theme_item", "data_type", "name", "node_type"),
        &Theme::get_theme_item
    );
    ClassDB::bind_method(
        D_METHOD("has_theme_item", "data_type", "name", "node_type"),
    ...
    ClassDB::bind_method(
        D_METHOD("clear_theme_item", "data_type", "name", "node_type"),
        &Theme::clear_theme_item
    );
    ClassDB::bind_method(
        D_METHOD("get_theme_item_list", "data_type", "node_type"),
        &Theme::_get_theme_item_list
    );
    ClassDB::bind_method(
        D_METHOD("get_theme_item_types", "data_type"),
        &Theme::_get_theme_item_types
    );

    ClassDB::bind_method(
        D_METHOD("get_type_list", "node_type"),
        &Theme::_get_type_list
    );

    ClassDB::bind_method(
        D_METHOD("_emit_theme_changed", "notify_list_changed"),
