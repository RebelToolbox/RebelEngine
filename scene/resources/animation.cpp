    ...
    if (name.begins_with("tracks/")) {
        ...
        if (what == "path") {
            ...
            if (track_get_type(track) == TYPE_TRANSFORM) {
                TransformTrack* tt =
                    static_cast<TransformTrack*>(tracks[track]);
                ...
                AnimationTrack* an =
                    static_cast<AnimationTrack*>(tracks[track]);
                Dictionary d = p_value;
                ERR_FAIL_COND_V(!d.has("times"), false);
                ERR_FAIL_COND_V(!d.has("clips"), false);
    ...
    if (name == "length") {
        ...
        if (what == "type") {
            ...
            if (track_get_type(track) == TYPE_TRANSFORM) {
                ...
                const ValueTrack* vt =
                    static_cast<const ValueTrack*>(tracks[track]);
                ...
                const BezierTrack* bt =
                    static_cast<const BezierTrack*>(tracks[track]);
                ...
                const AudioTrack* ad =
                    static_cast<const AudioTrack*>(tracks[track]);
...
void Animation::track_set_interpolation_type(
    int p_track,
...
Error Animation::transform_track_get_key(
    int p_track,
    int p_key,
    Vector3* r_loc,
    Quat* r_rot,
...
int Animation::transform_track_insert_key(
    int p_track,
    float p_time,
    const Vector3& p_loc,
    const Quat& p_rot,
...
void Animation::track_insert_key(
    int p_track,
    float p_time,
    const Variant& p_key,
    float p_transition
    ...
    switch (t->type) {
        case TYPE_TRANSFORM: {
            ...
            int idx =
                transform_track_insert_key(p_track, p_time, loc, rot, scale);
        ...
        case TYPE_METHOD: {
            ...
            Dictionary d = p_key;
            ERR_FAIL_COND(
                !d.has("method") || d["method"].get_type() != Variant::STRING
            );
...
void Animation::track_set_key_value(
    int p_track,
    int p_key_idx,
...
Animation::TransformKey Animation::_interpolate(
...
Quat Animation::_interpolate(const Quat& p_a, const Quat& p_b, float p_c)
    const {
    return p_a.slerp(p_b, p_c);
}

Variant Animation::_interpolate(
    const Variant& p_a,
    const Variant& p_b,
    float p_c
) const {
...
float Animation::_interpolate(const float& p_a, const float& p_b, float p_c)
    const {
...
Animation::TransformKey Animation::_cubic_interpolate(
    ...
    tk.scale =
        p_a.scale
            .cubic_interpolate(p_b.scale, p_pre_a.scale, p_post_b.scale, p_c);
    tk.rot = p_a.rot.cubic_slerp(p_b.rot, p_pre_a.rot, p_post_b.rot, p_c);

    return tk;
}

Vector3 Animation::_cubic_interpolate(
...
Quat Animation::_cubic_interpolate(
    const Quat& p_pre_a,
    const Quat& p_a,
    const Quat& p_b,
    const Quat& p_post_b,
...
Variant Animation::_cubic_interpolate(
    ...
    if (vformat == ((1 << Variant::INT) | (1 << Variant::REAL))
        || vformat == (1 << Variant::REAL)) {
        // mix of real and int
...
float Animation::_cubic_interpolate(
    const float& p_pre_a,
    const float& p_a,
    const float& p_b,
...
Error Animation::transform_track_interpolate(
    int p_track,
    float p_time,
    Vector3* r_loc,
    Quat* r_rot,
...
void Animation::_value_track_get_key_indices_in_range(
    ...
    // if event>=len then it will probably never be requested by the anim
    // player.
...
void Animation::value_track_get_key_indices(
    int p_track,
    float p_time,
    float p_delta,
    List<int>* p_indices
    ...
    if (loop) {
        ...
        if (from_time > to_time) {
            ...
            _value_track_get_key_indices_in_range(
                vt,
                from_time,
                length,
                p_indices
...
template <class T>
void Animation::_track_get_key_indices_in_range(
...
void Animation::track_get_key_indices_in_range(
    int p_track,
    float p_time,
    float p_delta,
    List<int>* p_indices
    ...
    if (loop) {
        ...
        if (from_time > to_time) {
            ...
            switch (t->type) {
                case TYPE_TRANSFORM: {
                    const TransformTrack* tt =
                        static_cast<const TransformTrack*>(t);
                ...
                case TYPE_ANIMATION: {
                    const AnimationTrack* an =
                        static_cast<const AnimationTrack*>(t);
                    _track_get_key_indices_in_range(
                        an->values,
                        from_time,
                        length,
...
void Animation::method_track_get_key_indices(
    int p_track,
    float p_time,
    float p_delta,
    List<int>* p_indices
    ...
    if (loop) {
        ...
        if (from_time > to_time) {
            // handle loop by splitting
            _method_track_get_key_indices_in_range(
                mt,
                from_time,
                length,
                p_indices
...
Vector<Variant> Animation::method_track_get_params(int p_track, int p_key_idx)
    const {
...
int Animation::bezier_track_insert_key(
    int p_track,
    float p_time,
    float p_value,
...
void Animation::bezier_track_set_key_value(
    int p_track,
    int p_index,
    float p_value
) {
...
Vector2 Animation::bezier_track_get_key_in_handle(int p_track, int p_index)
    const {
...
Vector2 Animation::bezier_track_get_key_out_handle(int p_track, int p_index)
    const {
    ERR_FAIL_INDEX_V(p_track, tracks.size(), Vector2());
    Track* t = tracks[p_track];
    ERR_FAIL_COND_V(t->type != TYPE_BEZIER, Vector2());
...
void Animation::audio_track_set_key_stream(
    int p_track,
    int p_key,
    const RES& p_stream
) {
...
void Animation::audio_track_set_key_start_offset(
    int p_track,
    int p_key,
    float p_offset
) {
...
float Animation::audio_track_get_key_start_offset(int p_track, int p_key)
    const {
    ERR_FAIL_INDEX_V(p_track, tracks.size(), 0);
    const Track* t = tracks[p_track];
...
int Animation::animation_track_insert_key(
    int p_track,
    float p_time,
...
StringName Animation::animation_track_get_key_animation(int p_track, int p_key)
    const {
    ERR_FAIL_INDEX_V(p_track, tracks.size(), StringName());
    const Track* t = tracks[p_track];
...
void Animation::copy_track(int p_track, Ref<Animation> p_to_animation) {
    ...
    for (int i = 0; i < track_get_key_count(p_track); i++) {
        p_to_animation->track_insert_key(
            dst_track,
            track_get_key_time(p_track, i),
...
void Animation::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("add_track", "type", "at_position"),
        &Animation::add_track,
        DEFVAL(-1)
    );
    ClassDB::bind_method(
        D_METHOD("remove_track", "track_idx"),
        &Animation::remove_track
    );
    ClassDB::bind_method(
        D_METHOD("get_track_count"),
        &Animation::get_track_count
    );
    ClassDB::bind_method(
        D_METHOD("track_get_type", "track_idx"),
        &Animation::track_get_type
    );
    ClassDB::bind_method(
        D_METHOD("track_get_path", "track_idx"),
        &Animation::track_get_path
    );
    ClassDB::bind_method(
        D_METHOD("track_set_path", "track_idx", "path"),
        &Animation::track_set_path
    );
    ClassDB::bind_method(
        D_METHOD("find_track", "path"),
        &Animation::find_track
    );

    ClassDB::bind_method(
        D_METHOD("track_move_up", "track_idx"),
        &Animation::track_move_up
    );
    ClassDB::bind_method(
        D_METHOD("track_move_down", "track_idx"),
        &Animation::track_move_down
    );
    ClassDB::bind_method(
        D_METHOD("track_move_to", "track_idx", "to_idx"),
        &Animation::track_move_to
    );
    ClassDB::bind_method(
        D_METHOD("track_swap", "track_idx", "with_idx"),
        &Animation::track_swap
    );

    ClassDB::bind_method(
        D_METHOD("track_set_imported", "track_idx", "imported"),
        &Animation::track_set_imported
    );
    ClassDB::bind_method(
        D_METHOD("track_is_imported", "track_idx"),
        &Animation::track_is_imported
    );

    ClassDB::bind_method(
        D_METHOD("track_set_enabled", "track_idx", "enabled"),
        &Animation::track_set_enabled
    );
    ClassDB::bind_method(
        D_METHOD("track_is_enabled", "track_idx"),
    ...
    ClassDB::bind_method(
        D_METHOD("track_remove_key", "track_idx", "key_idx"),
    ...
    ClassDB::bind_method(
        D_METHOD("track_get_key_count", "track_idx"),
        &Animation::track_get_key_count
    );
    ClassDB::bind_method(
        D_METHOD("track_get_key_value", "track_idx", "key_idx"),
        &Animation::track_get_key_value
    );
    ClassDB::bind_method(
        D_METHOD("track_get_key_time", "track_idx", "key_idx"),
        &Animation::track_get_key_time
    );
    ClassDB::bind_method(
        D_METHOD("track_find_key", "track_idx", "time", "exact"),
    ...
    ClassDB::bind_method(
        D_METHOD("track_get_interpolation_type", "track_idx"),
    ...
    ClassDB::bind_method(
        D_METHOD("value_track_get_update_mode", "track_idx"),
    ...
    ClassDB::bind_method(
        D_METHOD("value_track_interpolate", "track_idx", "time_sec"),
    ...
    ClassDB::bind_method(
        D_METHOD("copy_track", "track_idx", "to_animation"),
...
bool Animation::_transform_track_optimize_key(
    ...
    { // translation
        ...
        if (v0.is_equal_approx(v2)) {
            ...
            Vector3 s[2] = {v0, v2};
            real_t d =
                Geometry::get_closest_point_to_segment(v1, s).distance_to(v1);
            ...
            if (p_norm != Vector3()
                && Math::acos(pd.normalized().dot(p_norm))
...
void Animation::_transform_track_optimize(
    int p_idx,
    float p_allowed_linear_err,
