...
Vector<VisualShader::DefaultTextureParam> VisualShaderNode::
    get_default_texture_parameters(VisualShader::Type p_type, int p_id) const {
    return Vector<VisualShader::DefaultTextureParam>();
}

String VisualShaderNode::generate_global(
    Shader::Mode p_mode,
...
String VisualShaderNode::generate_global_per_node(
    Shader::Mode p_mode,
    VisualShader::Type p_type,
    int p_id
) const {
...
String VisualShaderNode::generate_global_per_func(
    Shader::Mode p_mode,
    VisualShader::Type p_type,
    int p_id
) const {
...
String VisualShaderNode::get_warning(
    Shader::Mode p_mode,
    VisualShader::Type p_type
) const {
    return String();
...
void VisualShaderNode::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_output_port_for_preview", "port"),
        &VisualShaderNode::set_output_port_for_preview
    );
    ClassDB::bind_method(
        D_METHOD("get_output_port_for_preview"),
    ...
    ClassDB::bind_method(
        D_METHOD("get_input_port_default_value", "port"),
        &VisualShaderNode::get_input_port_default_value
    );

    ClassDB::bind_method(
        D_METHOD("set_default_input_values", "values"),
        &VisualShaderNode::set_default_input_values
    );
    ClassDB::bind_method(
        D_METHOD("get_default_input_values"),
        &VisualShaderNode::get_default_input_values
    );

    ADD_PROPERTY(
        PropertyInfo(Variant::INT, "output_port_for_preview"),
...
void VisualShaderNodeCustom::update_ports() {
    ...
    if (get_script_instance()->has_method("_get_input_port_count")) {
        ...
        bool has_name =
            get_script_instance()->has_method("_get_input_port_name");
        bool has_type =
            get_script_instance()->has_method("_get_input_port_type");
        for (int i = 0; i < input_port_count; i++) {
            Port port;
            if (has_name) {
                port.name = (String)get_script_instance()->call(
            ...
            if (has_type) {
                port.type =
                    (int)get_script_instance()->call("_get_input_port_type", i);
            } else {
    ...
    if (get_script_instance()->has_method("_get_output_port_count")) {
        ...
        bool has_name =
            get_script_instance()->has_method("_get_output_port_name");
        bool has_type =
            get_script_instance()->has_method("_get_output_port_type");
        for (int i = 0; i < output_port_count; i++) {
            ...
            if (has_type) {
                port.type = (int)get_script_instance()->call(
...
VisualShaderNodeCustom::PortType VisualShaderNodeCustom::get_input_port_type(
    int p_port
) const {
...
VisualShaderNodeCustom::PortType VisualShaderNodeCustom::get_output_port_type(
    int p_port
) const {
...
String VisualShaderNodeCustom::generate_code(
    Shader::Mode p_mode,
    VisualShader::Type p_type,
    int p_id,
...
String VisualShaderNodeCustom::generate_global_per_node(
    ...
    if (get_script_instance()->has_method("_get_global_code")) {
        ...
        code        += (String)get_script_instance()->call(
            "_get_global_code",
            (int)p_mode
        );
        code += "\n";
...
void VisualShaderNodeCustom::set_input_port_default_value(
    int p_port,
    const Variant& p_value
) {
    if (!is_initialized) {
...
void VisualShaderNodeCustom::_bind_methods() {
    ...
    BIND_VMETHOD(MethodInfo(
        Variant::STRING,
        "_get_code",
        PropertyInfo(Variant::ARRAY, "input_vars"),
    ...
    ClassDB::bind_method(
        D_METHOD("_set_initialized", "enabled"),
        &VisualShaderNodeCustom::_set_initialized
    );
    ClassDB::bind_method(
        D_METHOD("_is_initialized"),
        &VisualShaderNodeCustom::_is_initialized
    );
    ClassDB::bind_method(
        D_METHOD("_set_input_port_default_value", "port", "value"),
...
void VisualShader::set_node_position(
    Type p_type,
    int p_id,
    const Vector2& p_position
) {
...
int VisualShader::find_node_id(Type p_type, const Ref<VisualShaderNode>& p_node)
    const {
    ...
    for (List<Connection>::Element* E = g->connections.front(); E;) {
        ...
        if (E->get().from_node == p_id || E->get().to_node == p_id) {
            ...
            if (E->get().from_node == p_id) {
                ...
                g->nodes[E->get().to_node].node->set_input_port_connected(
                    E->get().to_port,
                    false
                );
            }
...
bool VisualShader::is_node_connection(
    Type p_type,
    int p_from_node,
    int p_from_port,
    int p_to_node,
    ...
    for (const List<Connection>::Element* E = g->connections.front(); E;
        ...
        if (E->get().from_node == p_from_node
            && E->get().from_port == p_from_port
            && E->get().to_node == p_to_node && E->get().to_port == p_to_port) {
            return true;
        }
    }
...
bool VisualShader::is_nodes_connected_relatively(
    const Graph* p_graph,
    int p_node,
    int p_target
) const {
...
bool VisualShader::can_connect_nodes(
    Type p_type,
    int p_from_node,
    int p_from_port,
    int p_to_node,
...
void VisualShader::connect_nodes_forced(
    Type p_type,
    int p_from_node,
    int p_from_port,
    int p_to_node,
...
Error VisualShader::connect_nodes(
    Type p_type,
    int p_from_node,
    int p_from_port,
    int p_to_node,
    ...
    ERR_FAIL_INDEX_V(
        p_to_port,
        g->nodes[p_to_node].node->get_input_port_count(),
        ERR_INVALID_PARAMETER
    );
...
void VisualShader::disconnect_nodes(
    Type p_type,
    int p_from_node,
    int p_from_port,
    int p_to_node,
    ...
    for (List<Connection>::Element* E = g->connections.front(); E;
        ...
        if (E->get().from_node == p_from_node
            ...
            g->nodes[p_from_node].node->set_output_port_connected(
                p_from_port,
                false
            );
            g->nodes[p_to_node].node->set_input_port_connected(
                p_to_port,
                false
            );
            _queue_update();
...
void VisualShader::get_node_connections(
    Type p_type,
    List<Connection>* r_connections
) const {
    ERR_FAIL_INDEX(p_type, TYPE_MAX);
    const Graph* g = &graph[p_type];
...
void VisualShader::set_mode(Mode p_mode) {
    ERR_FAIL_INDEX_MSG(
        p_mode,
        Mode::MODE_MAX,
        vformat("Invalid shader mode: %d.", p_mode)
    );
    ...
    for (int i = 0; i < TYPE_MAX; i++) {
        ...
        Ref<VisualShaderNodeOutput> output =
            graph[i].nodes[NODE_ID_OUTPUT].node;
...
String VisualShader::generate_preview_shader(
    ...
    ERR_FAIL_COND_V(
        p_port < 0 || p_port >= node->get_output_port_count(),
        String()
    );
    ERR_FAIL_COND_V(
    ...
    for (int i = 0, index = 0; i < TYPE_MAX; i++) {
        for (Map<int, Node>::Element* E = graph[i].nodes.front(); E;
            ...
            if (global_expression.is_valid()) {
                String expr  = "";
                expr        += "// " + global_expression->get_caption() + ":"
                      + itos(index++) + "\n";
                expr +=
                    global_expression->generate_global(get_mode(), Type(i), -1);
                expr                = expr.replace("\n", "\n\t");
                expr               += "\n";
    ...
    if (node->get_output_port_type(p_port)
        ...
        code +=
            "\tCOLOR.rgb = n_out" + itos(p_node) + "p" + itos(p_port) + ";\n";
    }
    code += "}\n";
...
String VisualShader::validate_port_name(
    const String& p_name,
...
bool VisualShader::_get(const StringName& p_name, Variant& r_ret) const {
    ...
    if (name == "mode") {
        ...
        if (index == "connections") {
            Vector<int> conns;
            for (const List<Connection>::Element* E =
                     graph[type].connections.front();
                 E;
                 E = E->next()) {
...
void VisualShader::_get_property_list(List<PropertyInfo>* p_list) const {
    ...
    for (int i = 0; i < ShaderTypes::get_singleton()
        ...
        String mode = ShaderTypes::get_singleton()->get_modes(
            VisualServer::ShaderMode(shader_mode)
        )[i];
        int idx      = 0;
        bool in_enum = false;
...
Error VisualShader::_write_node(
    ...
    if (!vsnode->is_code_generated(
        )) { // just generate globals and ignore locals
    ...
    for (int i = 0; i < input_count; i++) {
        ...
        if (input_connections.has(ck)) {
            ...
            VisualShaderNode::PortType out_type =
                graph[type].nodes[from_node].node->get_output_port_type(
                    from_port
                );
            ...
            if (defval.get_type() == Variant::REAL
                || defval.get_type() == Variant::INT) {
                float val = defval;
                ...
                code +=
                    "\tmat4 " + inputs[i] + " = "
                    + String(
                          "mat4(vec4(%.5f, %.5f, %.5f, 0.0), vec4(%.5f, %.5f, "
                          "%.5f, 0.0), vec4(%.5f, %.5f, %.5f, 0.0), vec4(%.5f, "
                          "%.5f, %.5f, 1.0));\n"
                    )
                          .sprintf(values, &err);
            } else {
                // will go empty, node is expected to know what it is doing at
                // this point and handle it
            }
        }
    }
    ...
    if (vsnode->is_simple_decl(
        )) { // less code to generate for some simple_decl nodes
...
void VisualShader::_update_shader() const {
    ...
    for (int i = 0, index = 0; i < TYPE_MAX; i++) {
        for (Map<int, Node>::Element* E = graph[i].nodes.front(); E;
            ...
            Ref<VisualShaderNodeUniformRef> uniform_ref =
                Object::cast_to<VisualShaderNodeUniformRef>(E->get().node.ptr()
                );
            if (uniform_ref.is_valid()) {
                used_uniform_names.insert(uniform_ref->get_uniform_name());
            }
            Ref<VisualShaderNodeUniform> uniform =
                Object::cast_to<VisualShaderNodeUniform>(E->get().node.ptr());
            if (uniform.is_valid()) {
    ...
    for (int i = 0; i < uniforms.size(); i++) {
        ...
        if (used_uniform_names.has(uniform->get_uniform_name())) {
            ...
            const_cast<VisualShaderNodeUniform*>(uniform)
                ->set_global_code_generated(true);
        } else {
            const_cast<VisualShaderNodeUniform*>(uniform)
                ->set_global_code_generated(false);
        }
    }
    ...
    for (int i = 0; i < TYPE_MAX; i++) {
        // make it faster to go around through shader
        VMap<ConnectionKey, const List<Connection>::Element*> input_connections;
        VMap<ConnectionKey, const List<Connection>::Element*>
            output_connections;

        for (const List<Connection>::Element* E = graph[i].connections.front();
             E;
             E = E->next()) {
            ConnectionKey from_key;
            from_key.node = E->get().from_node;
    ...
    for (int i = 0; i < default_tex_params.size(); i++) {
        const_cast<VisualShader*>(this)->set_default_texture_param(
            default_tex_params[i].name,
            default_tex_params[i].param
        );
    }
...
void VisualShader::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("add_node", "type", "node", "position", "id"),
        &VisualShader::add_node
    );
    ClassDB::bind_method(
        D_METHOD("get_node", "type", "id"),
        &VisualShader::get_node
    );

    ClassDB::bind_method(
        D_METHOD("set_node_position", "type", "id", "position"),
        &VisualShader::set_node_position
    );
    ClassDB::bind_method(
        D_METHOD("get_node_position", "type", "id"),
        &VisualShader::get_node_position
    );

    ClassDB::bind_method(
        D_METHOD("get_node_list", "type"),
        &VisualShader::get_node_list
    );
    ClassDB::bind_method(
        D_METHOD("get_valid_node_id", "type"),
        &VisualShader::get_valid_node_id
    );

    ClassDB::bind_method(
        D_METHOD("remove_node", "type", "id"),
    ...
    ClassDB::bind_method(
        D_METHOD("get_node_connections", "type"),
        &VisualShader::_get_node_connections
    );

    ClassDB::bind_method(
        D_METHOD("set_graph_offset", "offset"),
        &VisualShader::set_graph_offset
    );
    ClassDB::bind_method(
        D_METHOD("get_graph_offset"),
        &VisualShader::get_graph_offset
    );

    ClassDB::bind_method(
        D_METHOD("_queue_update"),
        &VisualShader::_queue_update
    );
    ClassDB::bind_method(
        D_METHOD("_update_shader"),
        &VisualShader::_update_shader
    );

    ClassDB::bind_method(
        D_METHOD("_input_type_changed"),
        &VisualShader::_input_type_changed
    );
    ...
    ADD_PROPERTY_DEFAULT("code", ""); // Inherited from Shader, prevents showing
                                      // default code as override in docs.
...
const VisualShaderNodeInput::Port VisualShaderNodeInput::ports[] = {
    ...
    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_VERTEX,
     VisualShaderNode::PORT_TYPE_TRANSFORM,
     "inv_projection",     "INV_PROJECTION_MATRIX"           },
    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_VERTEX,
     VisualShaderNode::PORT_TYPE_SCALAR,
     "time",               "TIME"                            },
    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_VERTEX,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "viewport_size",      "vec3(VIEWPORT_SIZE, 0)"          },
    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_VERTEX,
     VisualShaderNode::PORT_TYPE_BOOLEAN,
    ...
    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_FRAGMENT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "point_coord",        "vec3(POINT_COORD, 0.0)"          },

    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_FRAGMENT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "screen_uv",          "vec3(SCREEN_UV, 0.0)"            },
    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_FRAGMENT,
     VisualShaderNode::PORT_TYPE_SCALAR,
     "side",               "float(FRONT_FACING ? 1.0 : 0.0)" },
    ...
    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_FRAGMENT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "viewport_size",      "vec3(VIEWPORT_SIZE, 0.0)"        },
    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_FRAGMENT,
     VisualShaderNode::PORT_TYPE_BOOLEAN,
     "output_is_srgb",     "OUTPUT_IS_SRGB"                  },
    ...
    {Shader::MODE_CANVAS_ITEM,
     VisualShader::TYPE_VERTEX,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "texture_pixel_size", "vec3(TEXTURE_PIXEL_SIZE, 1.0)"   },
    ...
    {Shader::MODE_CANVAS_ITEM,
     VisualShader::TYPE_FRAGMENT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "screen_pixel_size",  "vec3(SCREEN_PIXEL_SIZE, 1.0)"    },
    {Shader::MODE_CANVAS_ITEM,
     VisualShader::TYPE_FRAGMENT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "point_coord",        "vec3(POINT_COORD, 0.0)"          },
    ...
    {Shader::MODE_CANVAS_ITEM,
     VisualShader::TYPE_LIGHT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "light_color",        "LIGHT_COLOR.rgb"                 },
    {Shader::MODE_CANVAS_ITEM,
     VisualShader::TYPE_LIGHT,
     VisualShaderNode::PORT_TYPE_SCALAR,
     "light_alpha",        "LIGHT_COLOR.a"                   },
    {Shader::MODE_CANVAS_ITEM,
     VisualShader::TYPE_LIGHT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "light_uv",           "vec3(LIGHT_UV, 0.0)"             },
    {Shader::MODE_CANVAS_ITEM,
     VisualShader::TYPE_LIGHT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "shadow_color",       "SHADOW_COLOR.rgb"                },
    {Shader::MODE_CANVAS_ITEM,
     VisualShader::TYPE_LIGHT,
     VisualShaderNode::PORT_TYPE_SCALAR,
    ...
    {Shader::MODE_CANVAS_ITEM,
     VisualShader::TYPE_LIGHT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "shadow_vec",         "vec3(SHADOW_VEC, 0.0)"           },
    {Shader::MODE_CANVAS_ITEM,
     VisualShader::TYPE_LIGHT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "screen_uv",          "vec3(SCREEN_UV, 0.0)"            },
    ...
    {Shader::MODE_PARTICLES,
     VisualShader::TYPE_VERTEX,
     VisualShaderNode::PORT_TYPE_SCALAR,
     "restart",            "float(RESTART ? 1.0 : 0.0)"      },
    {Shader::MODE_PARTICLES,
     VisualShader::TYPE_VERTEX,
     VisualShaderNode::PORT_TYPE_SCALAR,
     "active",             "float(ACTIVE ? 1.0 : 0.0)"       },
    {Shader::MODE_PARTICLES,
     VisualShader::TYPE_VERTEX,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "custom",             "CUSTOM.rgb"                      },
...
VisualShaderNodeInput::PortType VisualShaderNodeInput::get_input_port_type(
    int p_port
) const {
...
VisualShaderNodeInput::PortType VisualShaderNodeInput::get_output_port_type(
    int p_port
) const {
    ...
    if (p_for_preview) {
        ...
        while (ports[idx].mode != Shader::MODE_MAX) {
            if (ports[idx].mode == shader_mode
                ...
                code =
                    "\t" + p_output_vars[0] + " = " + ports[idx].string + ";\n";
                break;
            }
            idx++;
        }
...
VisualShaderNodeInput::PortType VisualShaderNodeInput::get_input_type_by_name(
    String p_name
) const {
    int idx = 0;
...
VisualShaderNodeInput::PortType VisualShaderNodeInput::get_input_index_type(
    int p_index
) const {
    int idx   = 0;
    int count = 0;
...
void VisualShaderNodeInput::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_input_name", "name"),
        &VisualShaderNodeInput::set_input_name
    );
    ClassDB::bind_method(
        D_METHOD("get_input_name"),
        &VisualShaderNodeInput::get_input_name
    );
    ClassDB::bind_method(
        D_METHOD("get_input_real_name"),
        &VisualShaderNodeInput::get_input_real_name
    );

    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "input_name", PROPERTY_HINT_ENUM, ""),
        "set_input_name",
        "get_input_name"
    );
...
void VisualShaderNodeUniformRef::add_uniform(
    const String& p_name,
    UniformType p_type
) {
...
VisualShaderNodeUniformRef::PortType VisualShaderNodeUniformRef::
...
VisualShaderNodeUniformRef::UniformType VisualShaderNodeUniformRef::
    get_uniform_type_by_name(const String& p_name) const {
    for (int i = 0; i < uniforms.size(); i++) {
...
VisualShaderNodeUniformRef::UniformType VisualShaderNodeUniformRef::
    get_uniform_type_by_index(int p_idx) const {
    if (p_idx >= 0 && p_idx < uniforms.size()) {
...
String VisualShaderNodeUniformRef::generate_code(
    Shader::Mode p_mode,
    VisualShader::Type p_type,
    int p_id,
    const String* p_input_vars,
    ...
    switch (uniform_type) {
        ...
        case UniformType::UNIFORM_TYPE_TRANSFORM:
            return "\t" + p_output_vars[0] + " = " + get_uniform_name() + ";\n";
        case UniformType::UNIFORM_TYPE_COLOR: {
            String code = "\t" + p_output_vars[0] + " = " + get_uniform_name()
                        + ".rgb;\n";
            code +=
                "\t" + p_output_vars[1] + " = " + get_uniform_name() + ".a;\n";
            return code;
        } break;
        case UniformType::UNIFORM_TYPE_SAMPLER:
            break;
...
void VisualShaderNodeUniformRef::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_uniform_name", "name"),
        &VisualShaderNodeUniformRef::set_uniform_name
    );
    ClassDB::bind_method(
        D_METHOD("get_uniform_name"),
        &VisualShaderNodeUniformRef::get_uniform_name
    );

    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "uniform_name", PROPERTY_HINT_ENUM, ""),
        "set_uniform_name",
        "get_uniform_name"
    );
}
...
const VisualShaderNodeOutput::Port VisualShaderNodeOutput::ports[] = {
    ...
    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_FRAGMENT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "anisotropy_flow", "ANISOTROPY_FLOW:xy"},
    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_FRAGMENT,
     VisualShaderNode::PORT_TYPE_SCALAR,
     "subsurf_scatter", "SSS_STRENGTH"      },
    {Shader::MODE_SPATIAL,
     VisualShader::TYPE_FRAGMENT,
     VisualShaderNode::PORT_TYPE_VECTOR,
     "transmission",    "TRANSMISSION"      },
...
VisualShaderNodeOutput::PortType VisualShaderNodeOutput::get_input_port_type(
    int p_port
) const {
    int idx   = 0;
    int count = 0;
...
VisualShaderNodeOutput::PortType VisualShaderNodeOutput::get_output_port_type(
    int p_port
) const {
    return PORT_TYPE_SCALAR;
}
    ...
    while (ports[idx].mode != Shader::MODE_MAX) {
        if (ports[idx].mode == shader_mode
            ...
            if (p_input_vars[count] != String()) {
                ...
                if (s.find(":") != -1) {
                    code += "\t" + s.get_slicec(':', 0) + " = "
                          + p_input_vars[count] + "." + s.get_slicec(':', 1)
                          + ";\n";
                } else {
...
void VisualShaderNodeUniform::_bind_methods() {
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::STRING, "uniform_name"),
        "set_uniform_name",
        "get_uniform_name"
    );
}
...
void VisualShaderNodeGroupBase::add_input_port(
    int p_id,
    int p_type,
    const String& p_name
) {
...
void VisualShaderNodeGroupBase::add_output_port(
    int p_id,
    int p_type,
    const String& p_name
) {
...
void VisualShaderNodeGroupBase::remove_output_port(int p_id) {
    ...
    for (int i = p_id; i < outputs_strings.size(); i++) {
        outputs =
            outputs.replace_first(outputs_strings[i].split(",")[0], itos(i));
    }
...
VisualShaderNodeGroupBase::PortType VisualShaderNodeGroupBase::
    get_input_port_type(int p_id) const {
    ERR_FAIL_COND_V(!input_ports.has(p_id), (PortType)0);
    return input_ports[p_id].type;
}

void VisualShaderNodeGroupBase::set_input_port_name(
    int p_id,
    const String& p_name
) {
    ERR_FAIL_COND(!has_input_port(p_id));
...
VisualShaderNodeGroupBase::PortType VisualShaderNodeGroupBase::
    get_output_port_type(int p_id) const {
    ERR_FAIL_COND_V(!output_ports.has(p_id), (PortType)0);
    return output_ports[p_id].type;
}

void VisualShaderNodeGroupBase::set_output_port_name(
    int p_id,
    const String& p_name
) {
    ERR_FAIL_COND(!has_output_port(p_id));
...
String VisualShaderNodeGroupBase::generate_code(
    Shader::Mode p_mode,
    VisualShader::Type p_type,
    int p_id,
    const String* p_input_vars,
...
String VisualShaderNodeExpression::generate_code(
    Shader::Mode p_mode,
    VisualShader::Type p_type,
    int p_id,
    const String* p_input_vars,
...
void VisualShaderNodeExpression::_bind_methods() {
    ClassDB::bind_method(
        D_METHOD("set_expression", "expression"),
        &VisualShaderNodeExpression::set_expression
    );
    ClassDB::bind_method(
        D_METHOD("get_expression"),
