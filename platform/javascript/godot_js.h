...
// Input
extern void godot_js_input_mouse_button_cb(int (*p_callback)(
    int p_pressed,
    int p_button,
    double p_x,
    double p_y,
...
extern void godot_js_input_mouse_move_cb(void (*p_callback)(
    double p_x,
    double p_y,
    double p_rel_x,
    double p_rel_y,
...
extern void godot_js_input_mouse_wheel_cb(
    int (*p_callback)(double p_delta_x, double p_delta_y)
);
extern void godot_js_input_touch_cb(
    void (*p_callback)(int p_type, int p_count),
    uint32_t* r_identifiers,
    double* r_coords
);
extern void godot_js_input_key_cb(
...
// Input gamepad
extern void godot_js_input_gamepad_cb(void (*p_on_change)(
    int p_index,
    int p_connected,
    const char* p_id,
    const char* p_guid
...
extern int godot_js_input_gamepad_sample_get(
    int p_idx,
    float r_btns[16],
    int32_t* r_btns_num,
    float r_axes[10],
...
extern void godot_js_input_drop_files_cb(
...
extern void godot_js_display_cursor_set_custom_shape(
    const char* p_shape,
    const uint8_t* p_ptr,
    int p_len,
    int p_hotspot_x,
...
extern void godot_js_display_notification_cb(
...
extern void godot_js_display_vk_cb(
    void (*p_input)(const char* p_text, int p_cursor)
);
extern void godot_js_display_vk_show(
    const char* p_text,
    int p_multiline,
    int p_start,
    int p_end
