...
DWORD WINAPI
_xinput_set_state(DWORD dwUserIndex, XINPUT_VIBRATION* pVibration) {
    return ERROR_DEVICE_NOT_CONNECTED;
}

WindowsJoypad::WindowsJoypad() {}

WindowsJoypad::WindowsJoypad(InputDefault* _input, HWND* hwnd) {
    input            = _input;
    hWnd             = hwnd;
    joypad_count     = 0;
    dinput           = NULL;
    ...
    if (result == DI_OK) {
        ...
        if (result == DIERR_OUTOFMEMORY) {
            ERR_PRINT(
                "The Windows DirectInput subsystem could not allocate "
                "sufficient memory."
            );
...
bool WindowsJoypad::have_device(const GUID& p_guid) {
    for (int i = 0; i < JOYPADS_MAX; i++) {
...
bool WindowsJoypad::is_xinput_device(const GUID* p_guid) {
    ...
    if (GetRawInputDeviceList(NULL, &dev_list_count, sizeof(RAWINPUTDEVICELIST))
        == (UINT)-1) {
        return false;
    }
    dev_list = (PRAWINPUTDEVICELIST
    )memalloc(sizeof(RAWINPUTDEVICELIST) * dev_list_count);
...
bool WindowsJoypad::setup_dinput_joypad(const DIDEVICEINSTANCE* instance) {
    ERR_FAIL_NULL_V_MSG(
        dinput,
        false,
        "DirectInput not initialized. Rebooting your PC may solve this issue."
    );
    HRESULT hr;
    ...
    ERR_FAIL_COND_V_MSG(
        memcmp(&guid.Data4[2], "PIDVID", 6),
        false,
        "DirectInput device not recognised."
    );
...
void WindowsJoypad::setup_joypad_object(
    const DIDEVICEOBJECTINSTANCE* ob,
    int p_joy_id
) {
    if (ob->dwType & DIDFT_AXIS) {
        ...
        res = IDirectInputDevice8_SetProperty(
            joy.di_joy,
            DIPROP_RANGE,
            &prop_range.diph
        );
...
BOOL CALLBACK WindowsJoypad::enumCallback(
    const DIDEVICEINSTANCE* p_instance,
    void* p_context
) {
    WindowsJoypad* self = (WindowsJoypad*)p_context;
...
BOOL CALLBACK WindowsJoypad::objectsCallback(
    const DIDEVICEOBJECTINSTANCE* instance,
    void* context
) {
    WindowsJoypad* self = (WindowsJoypad*)context;
    self->setup_joypad_object(instance, self->id_to_change);

    return DIENUM_CONTINUE;
}

void WindowsJoypad::close_joypad(int id) {
    if (id == -1) {
        for (int i = 0; i < JOYPADS_MAX; i++) {
            close_joypad(i);
        }
    ...
    d_joypads[id].guid.Data1           = d_joypads[id].guid.Data2 =
        d_joypads[id].guid.Data3       = 0;
    input->joy_connection_changed(d_joypads[id].id, false, "");
    joypad_count--;
}

void WindowsJoypad::probe_joypads() {
    ERR_FAIL_NULL_MSG(
        dinput,
        "DirectInput not initialized. Rebooting your PC may solve this issue."
    );
...
void WindowsJoypad::process_joypads() {
    HRESULT hr;
        ...
        if (joy.state.dwPacketNumber != joy.last_packet) {
            ...
            for (int j = 0; j <= 16; j++) {
                input->joy_button(
                    joy.id,
                    j,
                    joy.state.Gamepad.wButtons & button_mask
                );
            ...
            input->joy_axis(
                joy.id,
                JOY_AXIS_0,
                axis_correct(joy.state.Gamepad.sThumbLX, true)
            );
            input->joy_axis(
                joy.id,
                JOY_AXIS_1,
                axis_correct(joy.state.Gamepad.sThumbLY, true, false, true)
            );
            input->joy_axis(
                joy.id,
                JOY_AXIS_2,
                axis_correct(joy.state.Gamepad.sThumbRX, true)
            );
            input->joy_axis(
                joy.id,
                JOY_AXIS_3,
                axis_correct(joy.state.Gamepad.sThumbRY, true, false, true)
            );
            input->joy_axis(
                joy.id,
                JOY_AXIS_4,
                axis_correct(joy.state.Gamepad.bLeftTrigger, true, true)
            );
            input->joy_axis(
                joy.id,
                JOY_AXIS_5,
                axis_correct(joy.state.Gamepad.bRightTrigger, true, true)
            );
    ...
    for (int i = 0; i < JOYPADS_MAX; i++) {
        ...
        LONG axes[] = {
            ...
            DIJOFS_RY,
            DIJOFS_RZ,
            (LONG)DIJOFS_SLIDER(0),
            (LONG)DIJOFS_SLIDER(1)
        };
        int values[] = {
            js.lX,
            js.lY,
...
void WindowsJoypad::post_hat(int p_device, DWORD p_dpad) {
    int dpad_val = 0;

    // Should be -1 when centered, but according to docs:
    // "Some drivers report the centered position of the POV indicator as
    // 65,535. Determine whether the indicator is centered as follows:
...
InputDefault::JoyAxis WindowsJoypad::axis_correct(
    int p_val,
    bool p_xinput,
    bool p_trigger,
    bool p_negate
...
void WindowsJoypad::joypad_vibration_start_xinput(
    int p_device,
...
void WindowsJoypad::joypad_vibration_stop_xinput(
    int p_device,
    uint64_t p_timestamp
) {
    xinput_gamepad& joy = x_joypads[p_device];
...
void WindowsJoypad::load_xinput() {
    ...
    XInputGetState_t func =
        (XInputGetState_t)GetProcAddress((HMODULE)xinput_dll, "XInputGetState");
    XInputSetState_t set_func =
        (XInputSetState_t)GetProcAddress((HMODULE)xinput_dll, "XInputSetState");
    if (!func || !set_func) {
        unload_xinput();
        return;
    }
...
void WindowsJoypad::unload_xinput() {
    if (xinput_dll) {
