...
static inline uint32_t hash_djb2_buffer(
    const uint8_t* p_buff,
    int p_len,
    uint32_t p_prev = 5381
) {
...
static inline uint32_t hash_djb2_one_float(
    double p_in,
    uint32_t p_prev = 5381
) {
    union {
...
struct HashMapHasherDefault {
    ...
    // static _FORCE_INLINE_ uint32_t hash(const void* p_ptr)  { return
    // uint32_t(uint64_t(p_ptr))*(0x9e3779b1L); }
};
