...
void Resource::set_path(const String& p_path, bool p_take_over) {
    ...
    if (has_path) {
        if (p_take_over) {
            ...
            ERR_FAIL_COND_MSG(
                exists,
                "Another resource is loaded from path '" + p_path
            ...
            if (sr.is_valid()) {
                if (sr->is_local_to_scene()) {
                    if (remap_cache.has(sr)) {
                        ...
                        RES dupe = sr->duplicate_for_local_scene(
                            p_for_scene,
                            remap_cache
                        );
                        p               = dupe;
...
Ref<Resource> Resource::duplicate(bool p_subresources) const {
    ...
    for (List<PropertyInfo>::Element* E = plist.front(); E; E = E->next()) {
        ...
        if ((p.get_type() == Variant::DICTIONARY
             || p.get_type() == Variant::ARRAY)) {
            r->set(E->get().name, p.duplicate(p_subresources));
        } else if (p.get_type() == Variant::OBJECT
                   && (p_subresources
                       || (E->get().usage
...
void Resource::notify_change_to_owners() {
    for (Set<ObjectID>::Element* E = owners.front(); E; E = E->next()) {
        ...
        ERR_CONTINUE_MSG(
            !obj,
            "Object was deleted, while still owning a resource."
        ); // wtf
        // TODO store string
...
#ifdef TOOLS_ENABLED
// helps keep IDs same number when loading/saving scenes. -1 clears ID and it
// Returns -1 when no id stored
...
void Resource::_bind_methods() {
    ...
    ClassDB::bind_method(
        D_METHOD("take_over_path", "path"),
        &Resource::_take_over_path
    );
    ...
    ClassDB::bind_method(
        D_METHOD("set_local_to_scene", "enable"),
        &Resource::set_local_to_scene
    );
    ClassDB::bind_method(
        D_METHOD("is_local_to_scene"),
        &Resource::is_local_to_scene
    );
    ClassDB::bind_method(
        D_METHOD("get_local_scene"),
        &Resource::get_local_scene
    );
    ClassDB::bind_method(
        D_METHOD("setup_local_to_scene"),
        &Resource::setup_local_to_scene
    );
    ClassDB::bind_method(D_METHOD("emit_changed"), &Resource::emit_changed);

    ClassDB::bind_method(
        D_METHOD("duplicate", "subresources"),
        &Resource::duplicate,
        DEFVAL(false)
    );
    ...
    ADD_PROPERTY(
        PropertyInfo(Variant::BOOL, "resource_local_to_scene"),
        "set_local_to_scene",
        "is_local_to_scene"
    );
    ADD_PROPERTY(
...
void ResourceCache::dump(const char* p_file, bool p_short) {
    ...
    if (p_file) {
        ...
        ERR_FAIL_COND_MSG(
            !f,
            "Cannot create file at path '" + String::utf8(p_file) + "'."
        );
    }
