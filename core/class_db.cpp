...
void ClassDB::get_inheriters_from_class(
    const StringName& p_class,
...
void ClassDB::get_direct_inheriters_from_class(
    const StringName& p_class,
    List<StringName>* p_classes
) {
    OBJTYPE_RLOCK;
...
StringName ClassDB::_get_parent_class(const StringName& p_class) {
    ...
    ERR_FAIL_COND_V_MSG(
        !ti,
        StringName(),
        "Cannot get class '" + String(p_class) + "'."
    );
    return ti->inherits;
}
...
void ClassDB::add_compatibility_class(
    const StringName& p_class,
...
Object* ClassDB::instance(const StringName& p_class) {
    ClassInfo* ti;
    {
        ...
        ERR_FAIL_COND_V_MSG(
            ti->disabled,
            nullptr,
            "Class '" + String(p_class) + "' is disabled."
        );
        ERR_FAIL_COND_V_MSG(
            !ti->creation_func,
            nullptr,
            "Class '" + String(p_class)
                + "' or its base class cannot be instantiated."
        );
    }
#ifdef TOOLS_ENABLED
    if (ti->api == API_EDITOR && !Engine::get_singleton()->is_editor_hint()) {
        ERR_PRINT(
            "Class '" + String(p_class)
            + "' can only be instantiated by editor."
        );
        return nullptr;
    }
#endif
...
void ClassDB::_add_class2(
    ...
    ERR_FAIL_COND_MSG(
        classes.has(name),
        "Class '" + String(p_class) + "' already exists."
    );
        ...
        for (List<StringName>::Element* E = type->method_order.front(); E;
            ...
            for (int i = 0; i < method->get_argument_count(); i++) {
                if (method->has_default_argument(i)) {
                    minfo.default_arguments.push_back(
                        method->get_default_argument(i)
                    );
                }
            }
...
void ClassDB::get_integer_constant_list(
    const StringName& p_class,
...
int ClassDB::get_integer_constant(
    const StringName& p_class,
    const StringName& p_name,
    bool* p_success
) {
...
StringName ClassDB::get_integer_constant_enum(
    ...
    while (type) {
        ...
        while ((k = type->enum_map.next(k))) {
            ...
            const List<StringName>::Element* found =
                constants_list.find(p_name);
            if (found) {
                return *k;
            }
        }
...
void ClassDB::add_signal(StringName p_class, const MethodInfo& p_signal) {
    ...
    while (check) {
        ERR_FAIL_COND_MSG(
            check->signal_map.has(sname),
            "Class '" + String(p_class) + "' already has signal '"
                + String(sname) + "'."
        );
...
void ClassDB::add_property_group(
    StringName p_class,
    const String& p_name,
    ...
    type->property_list.push_back(PropertyInfo(
        Variant::NIL,
        p_name,
        PROPERTY_HINT_NONE,
        p_prefix,
...
void ClassDB::add_property(
    ...
    if (p_setter) {
        ...
        ERR_FAIL_COND_MSG(
            !mb_set,
            "Invalid setter '" + p_class + "::" + p_setter + "' for property '"
                + p_pinfo.name + "'."
        );

        int exp_args = 1 + (p_index >= 0 ? 1 : 0);
        ERR_FAIL_COND_MSG(
            mb_set->get_argument_count() != exp_args,
            "Invalid function for setter '" + p_class + "::" + p_setter
                + " for property '" + p_pinfo.name + "'."
        );
#endif
    }
    ...
    if (p_getter) {
        ...
        ERR_FAIL_COND_MSG(
            !mb_get,
            "Invalid getter '" + p_class + "::" + p_getter + "' for property '"
                + p_pinfo.name + "'."
        );

        int exp_args = 0 + (p_index >= 0 ? 1 : 0);
        ERR_FAIL_COND_MSG(
            mb_get->get_argument_count() != exp_args,
            "Invalid function for getter '" + p_class + "::" + p_getter
                + "' for property: '" + p_pinfo.name + "'."
        );
#endif
    }
...
#ifdef DEBUG_METHODS_ENABLED
    ERR_FAIL_COND_MSG(
        type->property_setget.has(p_pinfo.name),
...
void ClassDB::set_property_default_value(
    StringName p_class,
    const StringName& p_name,
...
int ClassDB::get_property_index(
    const StringName& p_class,
    const StringName& p_property,
    bool* r_is_valid
) {
...
Variant::Type ClassDB::get_property_type(
    const StringName& p_class,
    const StringName& p_property,
    bool* r_is_valid
) {
...
StringName ClassDB::get_property_setter(
    StringName p_class,
    const StringName& p_property
) {
    ClassInfo* type  = classes.getptr(p_class);
...
StringName ClassDB::get_property_getter(
    StringName p_class,
    const StringName& p_property
) {
    ClassInfo* type  = classes.getptr(p_class);
...
MethodBind* ClassDB::bind_methodfi(
    ...
    ERR_FAIL_COND_V_MSG(
        has_method(instance_type, mdname),
        nullptr,
        "Class " + String(instance_type) + " already has a method "
            + String(mdname) + "."
    );
#endif
    ...
    if (!type) {
        ...
        ERR_FAIL_V_MSG(
            nullptr,
            "Couldn't bind method '" + mdname + "' for instance '"
                + instance_type + "'."
        );
    }
    ...
    if (type->method_map.has(mdname)) {
        ...
        ERR_FAIL_V_MSG(
            nullptr,
            "Method already bound '" + instance_type + "::" + mdname + "'."
        );
    }
    ...
    if (method_name.args.size() > p_bind->get_argument_count()) {
        ...
        ERR_FAIL_V_MSG(
            nullptr,
            "Method definition provides more arguments than the method "
            "actually has '"
...
void ClassDB::add_virtual_method(
    const StringName& p_class,
    ...
    ERR_FAIL_COND_MSG(
        !classes.has(p_class),
        "Request for nonexistent class '" + p_class + "'."
    );
...
void ClassDB::get_virtual_methods(
    const StringName& p_class,
    List<MethodInfo>* p_methods,
    bool p_no_inheritance
) {
...
void ClassDB::add_resource_base_extension(
...
void ClassDB::get_extensions_for_type(
    const StringName& p_class,
