...
class StringBuffer {
    ...
    _FORCE_INLINE_ CharType* current_buffer_ptr() {
        return static_cast<String&>(buffer).empty() ? short_buffer
                                                    : buffer.ptrw();
    }
...
StringBuffer<SHORT_BUFFER_SIZE>& StringBuffer<SHORT_BUFFER_SIZE>::append(
    CharType p_char
) {
...
template <int SHORT_BUFFER_SIZE>
StringBuffer<SHORT_BUFFER_SIZE>& StringBuffer<SHORT_BUFFER_SIZE>::reserve(
    int p_size
) {
