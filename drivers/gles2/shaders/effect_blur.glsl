...
void main() {
    ...
    gl_Position.xy =
        (blur_section.xy + (gl_Position.xy * 0.5 + 0.5) * blur_section.zw) * 2.0
        - 1.0;
#endif
}
...
const float dof_kernel[5] =
    float[](0.153388, 0.221461, 0.250301, 0.221461, 0.153388);
#endif
...
void main() {
    ...
    // This is significantly slower, but looks better and is more stable for
    // moving objects.
    vec4 color =
        texture2DLod(source_color, uv_interp + vec2(0.0, 0.0) * pix_size, lod)
        * 0.152781;
    color +=
        texture2DLod(source_color, uv_interp + vec2(1.0, 0.0) * pix_size, lod)
        * 0.144599;
    color +=
        texture2DLod(source_color, uv_interp + vec2(2.0, 0.0) * pix_size, lod)
        * 0.122589;
    color +=
        texture2DLod(source_color, uv_interp + vec2(3.0, 0.0) * pix_size, lod)
        * 0.093095;
    color +=
        texture2DLod(source_color, uv_interp + vec2(4.0, 0.0) * pix_size, lod)
        * 0.063327;
    color +=
        texture2DLod(source_color, uv_interp + vec2(-1.0, 0.0) * pix_size, lod)
        * 0.144599;
    ...
    color +=
        texture2DLod(source_color, uv_interp + vec2(0.0, 1.0) * pix_size, lod)
        * 0.152781;
    color +=
        texture2DLod(source_color, uv_interp + vec2(1.0, 1.0) * pix_size, lod)
        * 0.144599;
    color +=
        texture2DLod(source_color, uv_interp + vec2(2.0, 1.0) * pix_size, lod)
        * 0.122589;
    color +=
        texture2DLod(source_color, uv_interp + vec2(3.0, 1.0) * pix_size, lod)
        * 0.093095;
    color +=
        texture2DLod(source_color, uv_interp + vec2(4.0, 1.0) * pix_size, lod)
        * 0.063327;
    color +=
        texture2DLod(source_color, uv_interp + vec2(-1.0, 1.0) * pix_size, lod)
        * 0.144599;
    ...
    vec4 color =
        texture2DLod(source_color, uv_interp + vec2(0.0, 0.0) * pixel_size, lod)
        * 0.288713;
    color +=
        texture2DLod(source_color, uv_interp + vec2(0.0, 1.0) * pixel_size, lod)
        * 0.233062;
    color +=
        texture2DLod(source_color, uv_interp + vec2(0.0, 2.0) * pixel_size, lod)
        * 0.122581;
    color += texture2DLod(
                 source_color,
                 uv_interp + vec2(0.0, -1.0) * pixel_size,
                 lod
             )
           * 0.233062;
    color += texture2DLod(
                 source_color,
                 uv_interp + vec2(0.0, -2.0) * pixel_size,
                 lod
             )
           * 0.122581;
    color *= glow_strength;
    ...
    for (int i = 0; i < dof_kernel_size; i++) {
        ...
        vec2 tap_uv =
            uv_interp + dof_dir * float(int_ofs) * amount * dof_radius;
        ...
        tap_depth =
            ...
            / 2.0;
#else
        tap_depth = 2.0 * camera_z_near * camera_z_far
                  / (camera_z_far + camera_z_near
                     - tap_depth * (camera_z_far - camera_z_near));
#endif
        float tap_amount =
            int_ofs == 0 ? 1.0 : smoothstep(dof_begin, dof_end, tap_depth);
    ...
    for (int i = 0; i < dof_kernel_size; i++) {
        ...
        float ofs_influence =
            max(0.0, 1.0 - abs(float(int_ofs)) / float(dof_kernel_from));
    ...
    float feedback =
        max(smoothstep(
                glow_hdr_threshold,
                glow_hdr_threshold + glow_hdr_scale,
                luminance
            ),
            glow_bloom);
