...
#else
#define GLOW_TEXTURE_SAMPLE(m_tex, m_uv, m_lod)                                \
    textureLod(m_tex, m_uv, float(m_lod))
#endif

vec3 tonemap_filmic(vec3 color, float white) {
    // exposure bias: input scale (color *= bias, white *= bias) to make the
    // brightness consistent with other tonemappers also useful to scale the
    // input to the range that the tonemapper is designed for (some require very
    // high input values) has no effect on the curve's general shape or visual
    // properties
    const float exposure_bias = 2.0f;
    const float A             = 0.22f * exposure_bias
                  * exposure_bias; // bias baked into constants for performance
    ...
    vec3 color_tonemapped = ((color * (A * color + C * B) + D * E)
                             / (color * (A * color + B) + D * F))
                          - E / F;
    float white_tonemapped = ((white * (A * white + C * B) + D * E)
                              / (white * (A * white + B) + D * F))
                           - E / F;
...
vec3 tonemap_aces(vec3 color, float white) {
    ...
    float white_tonemapped =
        (white * (A * white + B)) / (white * (C * white + D) + E);

    return clamp(color_tonemapped / white_tonemapped, vec3(0.0f), vec3(1.0f));
}

// Adapted from
// https://github.com/TheRealMJP/BakingLab/blob/master/BakingLab/ACES.hlsl (MIT
// License).
vec3 tonemap_aces_fitted(vec3 color, float white) {
    ...
    // Exposure bias baked into transform to save shader instructions.
    // Equivalent to `color *= exposure_bias`
    const mat3 rgb_to_rrt = mat3(
        vec3(
            0.59719f * exposure_bias,
            0.35458f * exposure_bias,
            0.04823f * exposure_bias
        ),
        vec3(
            0.07600f * exposure_bias,
            0.90834f * exposure_bias,
            0.01566f * exposure_bias
        ),
        vec3(
            0.02840f * exposure_bias,
            0.13383f * exposure_bias,
            0.83777f * exposure_bias
        )
    );
    ...
    color *= rgb_to_rrt;
    vec3 color_tonemapped =
        (color * (color + A) - B) / (color * (C * color + D) + E);
    color_tonemapped *= odt_to_rgb;

    white *= exposure_bias;
    float white_tonemapped =
        (white * (white + A) - B) / (white * (C * white + D) + E);
...
// inputs are LINEAR, If Linear tonemapping is selected no transform is
// performed else outputs are clamped [0, 1] color
vec3 apply_tonemapping(vec3 color, float white) {
    // Ensure color values are positive.
    // They can be negative in the case of negative lights, which leads to
    // undesired behavior.
#if defined(USE_REINHARD_TONEMAPPER) || defined(USE_FILMIC_TONEMAPPER)         \
    || defined(USE_ACES_TONEMAPPER) || defined(USE_ACES_FITTED_TONEMAPPER)
...
vec3 apply_glow(
    ...
    color.r =
        (glow.r <= 0.5f)
            ...
            : (((glow.r > 0.5f) && (color.r <= 0.25f))
                   ? (color.r
                      + (2.0f * glow.r - 1.0f)
                            * (4.0f * color.r * (4.0f * color.r + 1.0f)
                                   * (color.r - 1.0f)
                               + 7.0f * color.r))
                   : (color.r
                      + (2.0f * glow.r - 1.0f) * (sqrt(color.r) - color.r)));
    color.g =
        (glow.g <= 0.5f)
            ? (color.g - (1.0f - 2.0f * glow.g) * color.g * (1.0f - color.g))
            : (((glow.g > 0.5f) && (color.g <= 0.25f))
                   ? (color.g
                      + (2.0f * glow.g - 1.0f)
                            * (4.0f * color.g * (4.0f * color.g + 1.0f)
                                   * (color.g - 1.0f)
                               + 7.0f * color.g))
                   : (color.g
                      + (2.0f * glow.g - 1.0f) * (sqrt(color.g) - color.g)));
    color.b =
        (glow.b <= 0.5f)
            ? (color.b - (1.0f - 2.0f * glow.b) * color.b * (1.0f - color.b))
            : (((glow.b > 0.5f) && (color.b <= 0.25f))
                   ? (color.b
                      + (2.0f * glow.b - 1.0f)
                            * (4.0f * color.b * (4.0f * color.b + 1.0f)
                                   * (color.b - 1.0f)
                               + 7.0f * color.b))
                   : (color.b
                      + (2.0f * glow.b - 1.0f) * (sqrt(color.b) - color.b)));
#endif
...
vec3 apply_fxaa(vec3 color, float exposure, vec2 uv_interp, vec2 pixel_size) {
    ...
    vec3 rgbNW =
        textureLod(source, uv_interp + vec2(-1.0, -1.0) * pixel_size, 0.0).xyz
        * exposure;
    vec3 rgbNE =
        textureLod(source, uv_interp + vec2(1.0, -1.0) * pixel_size, 0.0).xyz
        * exposure;
    ...
    vec3 rgbA =
        0.5 * exposure
        * (textureLod(source, uv_interp + dir * (1.0 / 3.0 - 0.5), 0.0).xyz
           + textureLod(source, uv_interp + dir * (2.0 / 3.0 - 0.5), 0.0).xyz);
    vec3 rgbB = rgbA * 0.5
              + 0.25 * exposure
...
vec3 screen_space_dither(vec2 frag_coord) {
    // Iestyn's RGB dither (7 asm instructions) from Portal 2 X360, slightly
    // modified for VR.
...
// Adapted from
// https://github.com/DadSchoorse/vkBasalt/blob/b929505ba71dea21d6c32a5a59f2d241592b30c4/src/shader/cas.frag.glsl
// (MIT license).
vec3 apply_cas(
    vec3 color,
    float exposure,
    ...
    //  g h i
    vec3 a =
        textureLodOffset(source, uv_interp, 0.0, ivec2(-1, -1)).rgb * exposure;
    vec3 b =
        textureLodOffset(source, uv_interp, 0.0, ivec2(0, -1)).rgb * exposure;
    vec3 c =
        textureLodOffset(source, uv_interp, 0.0, ivec2(1, -1)).rgb * exposure;
    vec3 d =
        textureLodOffset(source, uv_interp, 0.0, ivec2(-1, 0)).rgb * exposure;
    vec3 e = color.rgb;
    vec3 f =
        textureLodOffset(source, uv_interp, 0.0, ivec2(1, 0)).rgb * exposure;
    vec3 g =
...
void main() {
...
#ifdef USE_AUTO_EXPOSURE
    full_exposure /=
        texelFetch(source_auto_exposure, ivec2(0, 0), 0).r / auto_exposure_grey;
#endif
...
#ifdef USE_SHARPENING
    // CAS gives best results when applied after tonemapping, but `source` isn't
    // tonemapped. As a workaround, apply CAS before tonemapping so that the
    // image still has a correct appearance when tonemapped.
    ...
    // Early Tonemap & SRGB Conversion; note that Linear tonemapping does not
    // clamp to [0, 1]; some operations below expect a [0, 1] range and will
    // clamp
    ...
    ); // regular linear -> SRGB conversion (needs clamped values)
#endif
