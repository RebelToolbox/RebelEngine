...
#ifdef USE_ORTHOGONAL_PROJECTION
    P.z =
        ((P.z + (camera_z_far + camera_z_near) / (camera_z_far - camera_z_near))
         * (camera_z_far - camera_z_near))
        / 2.0;
#else
    P.z = 2.0 * camera_z_near * camera_z_far
        / (camera_z_far + camera_z_near - P.z * (camera_z_far - camera_z_near));
#endif
    P.z = -P.z;
 ...
 * unitOffset * ssR.  Assumes length(unitOffset) == 1 */
vec3 getOffsetPosition(ivec2 ssC, vec2 unitOffset, float ssR) {
    // Derivation:
    //  mipLevel = floor(log(ssR / MAX_OFFSET));
    int mipLevel =
        clamp(int(floor(log2(ssR))) - LOG_MAX_OFFSET, 0, MAX_MIP_LEVEL);

    ivec2 ssP = ivec2(ssR * unitOffset) + ssC;

    vec3 P;

    // We need to divide by 2^mipLevel to read the appropriately scaled
    // coordinate from a MIP-map. Manually clamp to the texture size because
    // texelFetch bypasses the texture unit
    ivec2 mipP =
        clamp(ssP >> mipLevel, ivec2(0), (screen_size >> mipLevel) - ivec2(1));
    ...
    if (mipLevel < 1) {
        ...
        P.z = 2.0 * camera_z_near * camera_z_far
            / (camera_z_far + camera_z_near
               - P.z * (camera_z_far - camera_z_near));
#endif
        P.z = -P.z;
...
float sampleAO(
    ...
    // that the contribution still falls off with radius^2, but we've adjusted
    // the rate in a way that is more computationally efficient and happens to
    // be aesthetically pleasing. return 4.0 * max(1.0 - vv * invRadius2, 0.0) *
    // max(vn - bias, 0.0);
...
void main() {
    ...
    */
    ...
    float randomPatternRotationAngle =
        mod(float((3 * ssC.x ^ ssC.y + ssC.x * ssC.y) * 10), TWO_PI);

    // Reconstruct normals from positions. These will lead to 1-pixel black
    // lines at depth discontinuities, however the blur will wipe those out so
    // they are not visible in the final image.
    ...
    float A =
        max(0.0, 1.0 - sum * intensity_div_r6 * (5.0 / float(NUM_SAMPLES)));

#ifdef ENABLE_RADIUS2

    // go again for radius2
    randomPatternRotationAngle =
        mod(float((5 * ssC.x ^ ssC.y + ssC.x * ssC.y) * 11), TWO_PI);
    ...
    A =
        min(A,
            max(0.0, 1.0 - sum * intensity_div_r62 * (5.0 / float(NUM_SAMPLES)))
        );
#endif
