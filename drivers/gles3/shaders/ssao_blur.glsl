...
/** Step in 2-pixel intervals since we already blurred against neighbors in the
    ...
    unobjectionable after shading was applied but eliminated most temporal
   incoherence from using small numbers of sample taps.
    */
...
const float gaussian[R + 1] =
    ...
    // float[](0.398943, 0.241971, 0.053991, 0.004432, 0.000134);  // stddev
    // = 1.0
    float[](0.153170, 0.144893, 0.122649, 0.092902, 0.062970); // stddev = 2.0
// float[](0.111220, 0.107798, 0.098151, 0.083953, 0.067458, 0.050920,
// 0.036108); // stddev = 3.0
...
void main() {
    ...
    */
    ...
    for (int r = -R; r <= R; ++r) {
        // We already handled the zero case above.  This loop should be unrolled
        // and the static branch optimized out, so the IF statement has no
        // runtime cost
        if (r != 0) {
            ...
            temp_depth = 2.0 * camera_z_near * camera_z_far
                       / (camera_z_far + camera_z_near
                          - temp_depth * (camera_z_far - camera_z_near));
            ...
            // range domain (the "bilateral" weight). As depth difference
