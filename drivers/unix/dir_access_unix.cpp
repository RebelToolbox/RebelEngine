        ...
        if (fd) {
            ...
            while (fgets(string, 1024, fd)) {
                ...
                if (strncmp(string, "file://", 7) == 0) {
                    // Strip any unwanted edges on the strings and push_back if
                    // it's not a duplicate
                    String fpath = String(string + 7)
                                       .strip_edges()
                                       .split_spaces()[0]
                                       .percent_decode();
                    if (!list->find(fpath)) {
...
Error DirAccessUnix::make_dir(String p_dir) {
    ...
    bool success =
        (mkdir(p_dir.utf8().get_data(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH)
         == 0);
    int err = errno;
...
Error DirAccessUnix::change_dir(String p_dir) {
    ...
    bool worked =
        (chdir(try_dir.utf8().get_data()) == 0); // we can only give this utf8
    if (!worked) {
    ...
    if (base != String() && !try_dir.begins_with(base)) {
        ERR_FAIL_COND_V(
            getcwd(real_current_dir_name, 2048) == nullptr,
            ERR_BUG
        );
        String new_dir;
...
Error DirAccessUnix::rename(String p_path, String p_new_path) {
    ...
    return ::rename(p_path.utf8().get_data(), p_new_path.utf8().get_data()) == 0
             ? OK
             : FAILED;
}
